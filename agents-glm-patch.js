/**
 * Modified agents.js with GLM-4.6 support
 * 原始文件 + GLM-4.6 集成
 */
 ...
 * 所有代理使用最高权限模式进行自动化扫描，无需人工干预
 * 包含GLM-4.6支持
 */
export const DEFAULT_AGENTS = [
    // GLM-4.6: 中文AI模型，非交互模式，完整权限
    {
        name: "glm46",
 ...
 * 优先使用GLM-4.6，然后是其他代理
 */
export function getAvailableAgent(preferredOrder) {
    // 默认优先级：GLM-4.6 > Claude > Gemini > Codex
    const order = preferredOrder ?? ["glm46", "claude", "gemini", "codex"];
 ...
 */
export function filterAvailableAgents(agents) {
    const available = [];
    const unavailable = [];
 ...
 */
export async function callAgent(config, prompt, options = {}) {
 ...
 */
export async function callAgentWithRetry(config, prompt, options = {}) {
 ...
 * 优先使用GLM-4.6，然后尝试其他代理
 * Default priority: GLM-4.6 > Claude > Gemini > Codex
 */
export async function callAnyAvailableAgent(prompt, options = {}) {
    const { preferredOrder, timeoutMs, verbose = false, cwd } = options;

    // GLM-4.6优先的代理顺序
    const agentOrder = preferredOrder ?? ["glm46", "claude", "gemini", "codex"];
 ...
 */
export function checkAvailableAgents() {
...
export function printAgentStatus() {
    ...
    for (const agent of agents) {
        ...
        const priority = agent.name === "glm46" ? chalk.cyan(" (PRIORITY)") : "";
