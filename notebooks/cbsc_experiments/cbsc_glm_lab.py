...
@app.cell
def __(mo):
    return mo.md("# 🚀 GLM-4.6 + CBSC 智能量化實驗室")
...
def __(mo):
    ...
    # 使用您提供的 GLM API Key
    glm_api_key = "8313ca8ee37941f487f2a71389b6d24c.qPdDUyxUOACj5nAU"
...
@app.cell
def __(rsi_period, sentiment_threshold, ma_short, ma_long, enable_glm_analysis, mo):
    """CBSC 數據處理和策略計算"""
    import pandas as pd
    import numpy as np
    ...
    if data_exists:
        ...
        data_status = mo.md("❌ CBSC 數據文件未找到\n\n請確保 `CODEX--/warrant_sentiment_daily.csv` 文件存在")
        df = pd.DataFrame()
