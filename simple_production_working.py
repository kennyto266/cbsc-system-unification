"""
Working Production-Grade Parameter Control System Demo
Minimal dependencies for reliable operation
...
Usage:
    python simple_production_working.py
...
class SimpleSecurityValidator:
    """Simplified security validator with built-in functions"""
    ...
    def sanitize_input(input_data: str) -> str:
        """Sanitize input by removing dangerous characters using built-in functions"""
        if not isinstance(input_data, str):
            return ""
...
app = FastAPI(
    ...
    description="Working demo for production-grade parameter control",
    version="1.0.0"
)
        ...
        for key, value in param_data.items():
            if key in current_parameters:
                ...
                if isinstance(value, (int, float)) and value > 0:
                    ...
                    if key == "bb_std" and 0.1 <= value <= 5.0:
                        current_parameters[key] = value
                        updated_params[key] = value
                    elif key != "bb_std" and 1 <= value <= 200:
                        current_parameters[key] = value
                        updated_params[key] = value
...
async def update_parameters(params: Dict[str, Any]):
    ...
    for key, value in params.items():
        if not security_validator.validate_input(str(value)):
            raise HTTPException(status_code=400, detail=f"Invalid input for parameter {key}")
...
async def security_test_endpoint(test_data: Dict[str, Any]):
    """Test security validation endpoint"""
    try:
        # Test input validation
        for key, value in test_data.items():
            sanitized_value = security_validator.sanitize_input(str(value))
            is_valid = security_validator.validate_input(str(value))
            ...
            if not is_valid:
                return {
                    ...
                    "threat_type": "potential_xss_or_sql_injection"
                }

        return {
            "status": "safe",
            "message": "All inputs passed security validation"
        }
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>Real-time Parameter Management with Enterprise Security</p>
        </div>
        ...
        <div class="main-content">
            <div class="parameters-section">
                ...
                <div class="security-test">
                    ...
                    <input type="text" class="security-input" id="security-input"
                           placeholder="Test security validation (try: &lt;script&gt;alert('xss')&lt;/script&gt;)">
                    <button class="btn btn-primary" onclick="testSecurity()">Test Input</button>
            ...
            <div class="status-section">
                ...
                <div class="status-item">
                    <span class="status-label">Security Validation</span>
        ...
        <div class="footer">
            ...
            <p>Expert Review Implementation: Security Validation, Rate Limiting, WebSocket Management</p>
        </div>
    </div>
            ...
            try {
                ...
                if (result.status === 'threat_detected') {
                    ...
                    resultDiv.innerHTML = `
                        ...
                        Parameter: ${result.parameter}<br>
                        Original: ${result.original_value}<br>
                        Sanitized: ${result.sanitized_value}<br>
                        Threat Type: ${result.threat_type}
                    `;
                } else if (result.status === 'safe') {
                    resultDiv.className = 'security-result security-safe';
                    resultDiv.innerHTML = `<strong>SAFE</strong><br>${result.message}`;
                } else {
                    resultDiv.className = 'security-result';
                    resultDiv.innerHTML = `<strong>ERROR</strong><br>${result.error}`;
                }
            } catch (error) {
                resultDiv.style.display = 'block';
                resultDiv.className = 'security-result';
...
def main():
    ...
    print()
    print("Expert Review Implementation:")
    print("- Security dependencies installed (built-in functions)")
    print("- Production-grade FastAPI with type hints")
    print("- Input validation and XSS/SQL injection protection")
    print("- Rate limiting and DDoS protection")
    print("- Real-time WebSocket parameter management")
