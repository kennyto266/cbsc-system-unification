...
class CBSCSystemManager:
    ...
    def setup_database(self):
        ...
        # 插入示例數據
        cursor.execute("""
            INSERT OR IGNORE INTO strategies (name, type, description, parameters, status)
            VALUES
            ('MA交叉策略', '技術指標', '移動平均線交叉策略', '{"fast_period": 10, "slow_period": 20}', 'active'),
            ('RSI策略', '技術指標', '相對強弱指數策略', '{"period": 14, "oversold": 30, "overbought": 70}', 'active'),
            ('動量策略', '動量分析', '價格動量追蹤策略', '{"lookback": 20, "threshold": 0.02}', 'paused')
        """)
...
'''
    ...
    def run(self):
        ...
        try:
            ...
            print("  健康檢查: http://localhost:3004/health")
            print("=" * 50)
