...
simplified_src_path = os.path.join(
    os.path.dirname(__file__), "..", "simplified_system", "src"
)
...
class TestGPUConfigManager(unittest.TestCase):
    ...
    def test_gpu_usage_decision(self):
        ...
        for data_size, complexity_factor in test_cases:
            should_use_gpu = self.config_manager.should_use_gpu(
                data_size, complexity_factor
            )
