...
from tests.factories.stock_data_factory import (
    create_test_hibor_data,
...
class TestHiborDataIntegration:
    ...
    async def test_hibor_historical_data(self, api_client, api_base_url):
        """測試HIBOR歷史數據"""
        try:
            response = await api_client.get(f"{api_base_url}/api / hibor / history?days = 30")
...
class TestVectorBTIntegration:
    ...
    async def test_backtest_endpoint(self, api_client, api_base_url):
        ...
        try:
            response = await api_client.post(
                f"{api_base_url}/api / backtest", json = backtest_request
            )
    ...
    async def test_multi_strategy_backtest(self, api_client, api_base_url):
        ...
        try:
            response = await api_client.post(
                f"{api_base_url}/api / backtest / multi", json = backtest_request
            )
...
class TestAPIPerformanceIntegration:
    ...
    async def test_large_data_request(self, api_client, api_base_url):
        ...
        try:
            ...
            start_time = time.time()
            response = await api_client.get(
                f"{api_base_url}/api / stock / 0700.HK?duration = 1095"
            )
    ...
    async def test_rate_limiting(self, api_client, api_base_url):
        ...
        try:
            ...
            if len(rate_limited_responses) > 0:
                assert any(
                    "X - RateLimit" in str(r.headers) for r in rate_limited_responses
                )
        ...
        for path, expected_status in test_cases:
            try:
                ...
                if response.status_code != 404:
                    ...
                    ), f"Path {path} should return {expected_status}"
    ...
    async def test_complete_trading_workflow(self, api_client, api_base_url):
        ...
        try:
            ...
            stock_response = await api_client.get(
                f"{api_base_url}/api / stock / 0700.HK?duration = 252"
            )
            ...
            # 2. 獲取HIBOR數據（用於無風險利率）
            await api_client.get(f"{api_base_url}/api / hibor / latest")
            # HIBOR端點可能未實現，這是可接受的
