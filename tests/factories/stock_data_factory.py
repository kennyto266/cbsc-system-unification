...
class StockDataGenerator:
    ...
    def generate_price_series(
        ...
        dates = pd.date_range(
            datetime.now() - timedelta(days = days), periods = days, freq="D"
        )
    ...
    def generate_ohlcv_data(
        ...
        for date, close in close_prices.items():
            ...
            if len(data) > 0:
                ...
                open_price = max(
                    close + random.uniform(-open_variation, open_variation), close * 0.5
                )
            else:
                open_price = close
        ...
        for i, symbol in enumerate(symbols):
            ...
            for date, close in close_series.items():
                if len(df_data) > 0:
                    open_variation = close * 0.02
                    open_price = max(
                        close + random.uniform(-open_variation, open_variation),
                        close * 0.5,
                    )
                else:
                    open_price = close
    ...
    def generate_market_regime_data(
        self, symbol: str, days: int = 504, regimes: List[Dict[str, Any]] = None
    ) -> pd.DataFrame:
        """
        生成包含不同市場階段的數據
        ...
        for regime in regimes:
            ...
            regime_data = self.generate_ohlcv_data(
                symbol, regime_days, current_price, regime["trend"]
            )
