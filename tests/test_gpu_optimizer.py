...
class TestGPUOptimizer(unittest.TestCase):
    ...
    def test_gpu_optimization_config(self):
        ...
        try:
            ...
            macd_config = self.optimizer.create_optimization_config(
                strategy_type="macd", use_gpu = True, enable_parallel = False
            )
    ...
    def test_single_source_optimization(self):
        ...
        try:
            ...
            self.assertIsInstance(
                report, self.optimizer.__class__.__module__ + ".OptimizationReport"
            )
        ...
        try:
            ...
            if self.gpu_engine.gpu_available:
                results = self.gpu_engine.optimize_rsi_parameters_gpu(
                    self.test_data, param_range=(10, 50), step = 5
                )
                method_used = "GPU"
            else:
            ...
            print(
                f"Best Sharpe ratio: {max(r.metrics['sharpe_ratio'] for r in results):.3f}"
            )
            ...
            for i, result in enumerate(sorted_results[:3]):
                print(
                    f"  {i + 1}. Period {result.parameters['period']}: Sharpe {result.metrics['sharpe_ratio']:.3f}"
                )
    ...
    def test_performance_monitoring(self):
        ...
        try:
            ...
            sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", "src"))
            from gpu.performance_monitor import (
                OptimizationMetrics,
            ...
            print(
                f"Monitoring samples: GPU={summary.get('gpu_samples', 0)}, CPU={summary.get('cpu_samples', 0)}"
            )
    ...
    def test_comprehensive_optimization(self):
        ...
        try:
            ...
            comprehensive_results = self.optimizer.run_comprehensive_optimization(
                data_dict, strategy_types=["rsi"]  # 只測試RSI以節省時間
            )
            ...
            for strategy_type, report in comprehensive_results.items():
                print(
                    f"  {strategy_type}: {len(report.results)} strategies, Best Sharpe: {report.best_strategy.metrics['sharpe_ratio']:.3f}"
                )
    ...
    def test_gpu_fallback_mechanism(self):
        ...
        try:
            ...
            cpu_results = self.gpu_engine.optimize_rsi_parameters_cpu(
                self.test_data, param_range=(10, 30), step = 5
            )
