...
# -*- coding: utf-8 -*-
"""
Multi-Indicator VectorBT Optimizer - 32 Core Version
多技术指标优化器 - 32核心版本

Modified to utilize all 32 CPU cores for maximum performance
修改版本，充分利用32个CPU核心以达到最大性能
...
class MultiIndicatorOptimizer32Cores:
    """32核心多技术指标参数优化器"""
    ...
    def __init__(self):
        ...
        # 指标类型配置
        self.indicator_types = ['RSI', 'MACD', 'BB', 'COMBO']
        ...
        print("=" * 80)
        print("MULTI-INDICATOR VECTORBT OPTIMIZER - 32 CORES")
        print("=" * 80)
    ...
    def run_parallel_optimization(self, stock_data, macro_data):
        """并行优化所有策略 - 32核心版本"""
        print(f"Starting 32-core parallel optimization...")
        ...
        max_workers = min(mp.cpu_count(), 32)  # 使用32进程
        print(f"Using {max_workers} processes for MAXIMUM CPU utilization!")
        ...
        if cpu_readings:
            ...
            print(f"   Theoretical Max: {theoretical_max:.1f}% ({max_workers}/{mp.cpu_count()} cores)")
    ...
    def generate_comprehensive_report(self, results):
        ...
        report = {
            'optimization_summary': {
                ...
                'max_workers_used': min(mp.cpu_count(), 32)
            },
    ...
    def print_comprehensive_results(self, report):
        """打印综合结果"""
        print("\n" + "="*80)
        print("32-CORE MULTI-INDICATOR OPTIMIZATION RESULTS")
        print("="*80)
        ...
        print(f"\n32-CORE PERFORMANCE SUMMARY:")
        speedup_8vs32 = summary['max_workers_used'] / 8
        print(f"   Process Count: {summary['max_workers_used']} (vs 8 before)")
        print(f"   Theoretical Speedup: {speedup_8vs32:.1f}x")
        print(f"   Expected CPU Usage: 60-90% (vs 20-30% before)")

    def save_results(self, report, filename='multi_indicator_optimization_32cores_results.json'):
        """保存结果到文件"""
        try:
...
def main():
    """主函数 - 32核心版多指标优化"""
    print("STARTING 32-CORE MULTI-INDICATOR VECTORBT OPTIMIZER...")
    print("WARNING: This will use ALL 32 CPU cores and maximize CPU usage!")

    optimizer = MultiIndicatorOptimizer32Cores()
    ...
    try:
        ...
        # 3. 运行32核心并行优化
        print("\n[STEP 3] Running 32-core parallel optimization...")
        print("Expect MAXIMUM CPU usage across all cores!")
        ...
        print(f"\n[DONE] 32-core optimization completed! Tested {len(results):,} strategies")
        print("Check your Task Manager - you should see near 100% CPU usage!")
