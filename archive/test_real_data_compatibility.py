#!/usr/bin/env python3
"""
Real Data Integration Compatibility Test
真實數據源兼容性集成測試
...
class RealDataCompatibilityTest:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if all(key in data for key in required_keys):
                    ...
                    self.data_sources_status['central_api'] = {'status': '❌ Invalid Structure'}
                    return False
            else:
                print(f"❌ API Error: {response.status_code}")
                self.data_sources_status['central_api'] = {'status': f'❌ HTTP {response.status_code}'}
                return False

        except Exception as e:
            print(f"❌ Central API test failed: {e}")
            self.data_sources_status['central_api'] = {'status': f'❌ Error: {str(e)}'}
            return False
                ...
                try:
                    if path.endswith('.json'):
                        ...
                        if isinstance(data, list) and len(data) > 0:
                            sample = data[0]
                            print(f"✅ {Path(path).name}: {len(data)} records")
                        ...
                        import pandas as pd
                        df = pd.read_csv(path)
                        print(f"✅ {Path(path).name}: {len(df)} rows, {len(df.columns)} columns")
                        print(f"   Sample columns: {list(df.columns)[:5]}")
                        compatible_sources.append(path)

                except Exception as e:
                    print(f"❌ {Path(path).name}: Error reading - {e}")
            else:
                print(f"⚠️  {Path(path).name}: File not found")

        self.data_sources_status['government_data'] = {
            'status': f'✅ {len(compatible_sources)}/{len(gov_data_paths)} compatible',
    ...
    def test_refactored_system_integration(self):
        ...
        try:
            ...
            try:
                ...
                print(f"✅ DataRepository stock data: {len(stock_data)} records")
                stock_data_success = True
            except:
                print("⚠️  Using fallback stock data")
                stock_data_success = False

            # Test government data
            try:
                gov_data = repo.get_government_data('HB')
                print(f"✅ DataRepository government data: {len(gov_data)} records")
                gov_data_success = True
            except:
                print("⚠️  Using fallback government data")
            ...
            print(f"✅ IndicatorFactory: {indicators_created}/{len(test_combinations)} created")
            ...
            if stock_data_success and indicators_created > 0:
                ...
                if len(test_price) > 50:
                    ...
                    print(f"✅ BacktestEngine: Test successful")
                ...
                print("⚠️  Skipping BacktestEngine test (insufficient data)")
                backtest_success = False
            ...
            if integration_success:
                print("✅ REFACTORED SYSTEM INTEGRATION: SUCCESS")
            else:
                print("⚠️  REFACTORED SYSTEM INTEGRATION: PARTIAL")

            return integration_success

        except Exception as e:
            print(f"❌ Refactored system integration failed: {e}")
            import traceback
            traceback.print_exc()
            return False
                ...
                try:
                    ...
                    if isinstance(data, list):
                        ...
                        if all(field in sample_result for field in required_fields):
                            print(f"✅ {result_file}: {len(data)} results, compatible format")
                            compatible_files.append(result_file)
                        else:
                            print(f"⚠️  {result_file}: Missing required fields")
                    else:
                        print(f"⚠️  {result_file}: Invalid format")

                except Exception as e:
                    print(f"❌ {result_file}: Error reading - {e}")
            else:
                print(f"⚠️  {result_file}: File not found")

        self.data_sources_status['backward_compatibility'] = {
            'status': f'✅ {len(compatible_files)}/{len(existing_results)} compatible',
    ...
    def generate_compatibility_report(self, results):
        ...
        if passed == total:
            print("  🎉 EXCELLENT: All compatibility tests passed!")
            print("  ✅ Ready for production deployment")
        elif passed >= total * 0.75:
            print("  ✅ GOOD: Most compatibility tests passed")
            print("  ⚠️  Review failed tests before deployment")
        else:
            print("  ⚠️  NEEDS ATTENTION: Multiple compatibility issues")
        ...
        if '❌' in central_api_status:
            print("  🔧 Fix Central API connectivity issues")
        ...
        if 'compatible_sources' in self.data_sources_status.get('government_data', {}):
            ...
            if compatible_count < 3:
                print("  📁 Ensure all government data files are available")
...
def main():
    ...
    if passed == total:
        print(f"\n🎉 COMPATIBILITY VERIFICATION SUCCESSFUL!")
        print("✅ Refactored system is fully compatible with existing data sources")
        print("✅ Ready for production deployment")
        return True
    elif passed >= total * 0.75:
        print(f"\n✅ COMPATIBILITY VERIFICATION MOSTLY SUCCESSFUL!")
        print(f"⚠️  {total-passed} minor issues to address")
        print("✅ Generally ready for production with minor fixes")
        return True
    else:
        print(f"\n❌ COMPATIBILITY VERIFICATION FAILED!")
        print(f"❌ {total-passed} critical issues need to be resolved")
