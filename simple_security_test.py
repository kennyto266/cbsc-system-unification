...
def test_syntax_fix():
    ...
    try:
        ...
        logger.error(f"❌ Error testing syntax fix: {e}")
        return False


def test_security_validator():
    """Test the SecurityValidator class"""
    logger.info("🛡️ Testing SecurityValidator")
    ...
    try:
        ...
        try:
            ...
            logger.error("❌ SQL injection protection not working")
            return False
        except ValueError:
            pass  # Expected
        ...
        try:
            ...
            logger.error("❌ Path traversal protection not working")
            return False
        except ValueError:
            pass  # Expected
        ...
        try:
            ...
            logger.error("❌ Numeric range validation not working")
            return False
        except ValueError:
            pass  # Expected

        logger.info("✅ SecurityValidator tests passed")
        return True

    except ImportError as e:
        logger.error(f"❌ Failed to import SecurityValidator: {e}")
        return False
    except Exception as e:
        logger.error(f"❌ Error testing SecurityValidator: {e}")
        return False
...
def test_sharpe_calculator():
    ...
    try:
        ...
        if not isinstance(result, dict) or 'sharpe_ratio' not in result:
            logger.error("❌ Sharpe calculator returning invalid format")
            return False

        sharpe = result['sharpe_ratio']
        if abs(sharpe) > 10:  # Unrealistic Sharpe ratio
            logger.error(f"❌ Unrealistic Sharpe ratio: {sharpe}")
            return False
        ...
        logger.info(f"✅ Sharpe Calculator tests passed (Sharpe: {sharpe:.3f})")
        return True

    except ImportError as e:
        logger.error(f"❌ Failed to import Sharpe calculator: {e}")
        return False
    except Exception as e:
        logger.error(f"❌ Error testing Sharpe calculator: {e}")
        return False
...
def main():
    """Main test function"""
    print("Simple Security Test for Quantitative Trading System")
    print("=" * 60)
