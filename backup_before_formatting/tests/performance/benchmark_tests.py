...
from src.models.base import MarketData, Portfolio, RiskMetrics, TradingSignal
...
class TestSystemPerformance:
    ...
    @pytest.mark.asyncio
    async def test_message_queue_throughput(
        self, message_queue, performance_metrics, resource_monitor
    ):
        """测试消息队列吞吐量"""
        ...
        assert (
            resource_usage["memory_delta_mb"] < 500
        ), f"内存使用过多: {resource_usage['memory_delta_mb']:.2f} MB"
    ...
    @pytest.mark.asyncio
    async def test_agent_message_processing_latency(
        self, message_queue, performance_metrics
    ):
        """测试Agent消息处理延迟"""
    ...
    async def test_concurrent_agent_performance(
        ...
        try:
            ...
            performance_metrics.record_metric(
                "concurrent_throughput_messages_per_second", throughput
            )

            # 记录资源使用
            resource_usage = resource_monitor.get_current_usage()
            performance_metrics.record_metric(
                "concurrent_cpu_usage_percent", resource_usage["cpu_percent"]
            )
            performance_metrics.record_metric(
                "concurrent_memory_usage_mb", resource_usage["memory_mb"]
            )
            ...
            ), f"CPU使用率过高: {resource_usage['cpu_percent']:.2f}%"
    ...
    async def test_memory_usage_under_load(self, message_queue, performance_metrics):
        ...
        # 创建Agent
        config = AgentConfig(
            agent_id="memory_test_agent", agent_type="DataScientist", status="active"
        )
        ...
        try:
            ...
            performance_metrics.record_metric(
                "memory_per_message_kb", (total_memory_delta * 1024) / message_count
            )

            # 内存使用断言
            assert (
                total_memory_delta < 1000
            ), f"内存使用过多: {total_memory_delta:.2f} MB"
            assert (
                total_memory_delta * 1024
            ) / message_count < 200, "每条消息内存使用过多"
    ...
    async def test_sustained_performance(self, message_queue, performance_metrics):
        ...
        try:
            ...
            throughput_stability = (
                1 - (statistics.stdev(throughput_samples) / avg_throughput)
                if avg_throughput > 0
                else 0
            )
            ...
            performance_metrics.record_metric(
                "avg_throughput_sustained", avg_throughput
            )
            performance_metrics.record_metric(
                "min_throughput_sustained", min_throughput
            )
            performance_metrics.record_metric(
                "max_throughput_sustained", max_throughput
            )
            performance_metrics.record_metric(
            ...
            assert (
                avg_throughput > 100
            ), f"持续平均吞吐量不足: {avg_throughput:.2f} msg / s"
            assert (
                min_throughput > 50
            ), f"持续最小吞吐量不足: {min_throughput:.2f} msg / s"
            assert (
                throughput_stability > 0.8
            ), f"吞吐量稳定性不足: {throughput_stability:.3f}"
    ...
    async def test_system_scalability(self, message_queue, performance_metrics):
        ...
        for agent_count in agent_counts:
            ...
            try:
                ...
                scalability_results[agent_count] = {
                    ...
                    "duration": duration,
                    "messages_per_second_per_agent": (
                        throughput / agent_count if agent_count > 0 else 0
                    ),
                }
        ...
        per_agent_throughputs = [
            results["messages_per_second_per_agent"]
    ...
    async def test_error_recovery_performance(self, message_queue, performance_metrics):
        ...
        # 创建Agent
        config = AgentConfig(
            agent_id="error_recovery_agent", agent_type="RiskAnalyst", status="active"
        )
        ...
        try:
            ...
            recovery_throughput = (
                100 / recovery_duration if recovery_duration > 0 else 0
            )

            performance_degradation = (
                (normal_throughput - recovery_throughput) / normal_throughput
            ...
            ), f"错误恢复后性能下降过多: {performance_degradation * 100:.2f}%"
...
class TestLoadTesting:
    ...
    async def test_stress_testing(self, message_queue):
        ...
        # 创建Agent
        config = AgentConfig(
            agent_id="stress_test_agent", agent_type="DataScientist", status="active"
        )
        ...
        try:
            ...
            for i in range(100):
                ...
                large_data = {
                    ...
                    "nested_data": {
                        "level1": {
                            "level2": {"level3": [f"data_{j}" for j in range(1000)]}
                        }
                    },
                    "sequence": i,
                    "timestamp": datetime.now().isoformat(),
