...
class TestTechnicalIndicatorsPerformance:
    ...
    def test_sma_performance(self, indicators, test_data):
        ...
        result = indicators.calculate_sma(prices, period)
        ...
        performance_metrics = {
            ...
            "points_per_second": test_data["size"] / execution_time if execution_time > 0 else float('inf')
        }

        print(f"Performance metrics for {test_data['name']} data: {performance_metrics}")
    ...
    def test_ema_performance(self, indicators, test_data):
        ...
        start_time = time.time()
        result = indicators.calculate_ema(prices, period)
        end_time = time.time()
        ...
        assert execution_time < expected_max_time, (
            f"EMA計算過慢: {execution_time:.4f}s "
            f"for {test_data['size']} points"
        )
    ...
    def test_macd_performance(self, indicators, test_data):
        ...
        assert execution_time < expected_max_time, (
            f"MACD計算過慢: {execution_time:.4f}s "
            f"for {test_data['size']} points"
        )
    ...
    def test_bollinger_bands_performance(self, indicators, test_data):
        ...
        assert execution_time < expected_max_time, (
            f"布林帶計算過慢: {execution_time:.4f}s "
            f"for {test_data['size']} points"
        )

        # 驗證布林帶邏輯
        valid_data = {
            k: v.dropna() for k, v in result.items()
        }
    ...
    def test_concurrent_calculations(self, indicators, test_data):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
            futures = [
                executor.submit(calculate_indicator_safe, indicators.calculate_sma, prices, 20),
                executor.submit(calculate_indicator_safe, indicators.calculate_ema, prices, 26),
                executor.submit(calculate_indicator_safe, indicators.calculate_rsi, prices, 14),
                executor.submit(calculate_indicator_safe, indicators.calculate_macd, prices, 12, 26, 9)
            ]
...
class TestIndicatorsBenchmarking:
    ...
    def benchmark_indicator_calculation(self, indicators, prices, indicator_func, *args):
        """基準測試輔助函數"""
        # 預熱
        ...
        for _ in range(5):
            ...
            result = indicator_func(prices, *args)
...
class TestMemoryProfiling:
    ...
    def test_memory_leak_detection(self):
        ...
        assert memory_increase < 100 * 1024 * 1024, f"可能存在內存洩漏，增長: {memory_increase / 1024 / 1024:.2f}MB"
    ...
    def test_large_dataset_memory_efficiency(self):
        ...
        assert memory_per_point < 1024, f"內存效率不足: {memory_per_point:.2f} bytes/point"
