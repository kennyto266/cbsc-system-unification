...
class TestGPUVectorBTIntegration(unittest.TestCase):
    ...
    def _create_test_stock_data(self):
        ...
        # 模擬高開低收
        volatility = 0.02
        data['high'] = data[['open', 'close']].max(axis=1) * (1 + np.random.uniform(0, volatility, len(dates)))
        data['low'] = data[['open', 'close']].min(axis=1) * (1 - np.random.uniform(0, volatility, len(dates)))
    ...
    def test_moving_average_strategy_optimization(self):
        ...
        try:
            ...
            self.assertLess(result.parameters['short_period'], result.parameters['long_period'])
    ...
    def test_batch_optimization(self):
        ...
        try:
            ...
            for strategy_type, result in results.items():
                print(f"  {strategy_type}: Sharpe {result.sharpe_ratio:.3f}, Return {result.total_return:.2%}")
    ...
    def test_vectorbt_portfolio_creation(self):
        ...
        try:
            result = self.integration.optimize_nonprice_strategy(
                self.test_data,
                'rsi',
                param_ranges={'period': (14, 14)}  # 固定參數
            )
            ...
            print(f"  Total trades: {len(result.trades) if result.trades is not None else 0}")
    ...
    def test_data_validation_and_error_handling(self):
        ...
        try:
            ...
            invalid_price_data.loc[invalid_price_data.index[10], 'high'] = invalid_price_data.loc[invalid_price_data.index[10], 'low'] - 1
    ...
    def test_configuration_and_initialization(self):
        ...
        try:
            from gpu.vectorbt_integration import GPUVectorBTConfig, GPUVectorBTIntegration
            ...
            self.assertEqual(custom_integration.config.optimization_metric, 'total_return')
        ...
        try:
            ...
            for i in range(3):
                result = self.integration.optimize_nonprice_strategy(
                    self.test_data,
                    'rsi',
                    param_ranges={'period': (10, 15)}
                )
                print(f"  Optimization {i+1} completed: Sharpe {result.sharpe_ratio:.3f}")
