...
class TestAgentIntegration:
    ...
    async def all_agents(self, message_queue):
        ...
        agent_configs = {
            ...
            "risk_analyst": AgentConfig(
                agent_id="risk_analyst_001", agent_type="RiskAnalyst", status="active"
            ),
    ...
    async def test_system_monitoring_flow(self, all_agents):
        ...
        metrics_message = Message(
            ...
            message_type="CONTROL",
            payload={"command": "collect_metrics", "parameters": {}},
            timestamp=datetime.now(),
            priority="NORMAL",
        )
        ...
        health_check_message = Message(
            ...
            receiver_id="quant_engineer_001",
            message_type="CONTROL",
            payload={"command": "run_health_check", "parameters": {}},
            timestamp=datetime.now(),
            priority="NORMAL",
        )
    ...
    async def test_high_frequency_message_processing(
        self, all_agents, sample_market_data
    ):
        """测试高频消息处理"""
    ...
    @pytest.mark.asyncio
    async def test_agent_coordination_through_coordinator(
        self, coordinator, all_agents
    ):
        """测试通过协调器的Agent协调"""
        ...
        for agent_name, agent in all_agents.items():
            await coordinator.register_agent(
                agent.config.agent_id, agent.config.agent_type
            )
    ...
    async def test_end_to_end_trading_workflow(self, all_agents, sample_market_data):
        ...
        # 验证系统监控
        await all_agents["quantitative_engineer"].process_message(
            Message(
                id=str(uuid.uuid4()),
                sender_id="system_coordinator",
                receiver_id="quant_engineer_001",
...
class TestSystemIntegration:
    ...
    async def test_system_scalability(self):
        ...
        for i in range(10):
            config = AgentConfig(
                agent_id=f"test_agent_{i}", agent_type="TestAgent", status="active"
            )
