...
from src.integration.health_monitor import HealthStatus, SystemHealthMonitor
from src.integration.system_initializer import InitializationStep, SystemInitializer
...
class TestBusinessProcessIntegration(IntegrationTestBase):
    ...
    async def test_trading_workflow_integration(self):
        ...
        portfolio_optimization = await mock_portfolio_manager.optimize_portfolio(
            trading_signal
        )
    ...
    async def test_strategy_optimization_workflow(self):
        ...
        optimization_result = await mock_optimizer.optimize_parameters(
...
class TestPerformanceIntegration(IntegrationTestBase):
    ...
    async def test_memory_usage_under_load(self):
        ...
        )  # Final memory should not be too high

        self.logger.info("Memory usage test results:")
    ...
    async def test_concurrent_agent_processing(self):
        ...
        self.logger.info("Concurrent agent processing test results:")
        self.logger.info(f"  Number of agents: {len(agents)}")
        self.logger.info(f"  Total processing time: {total_time:.2f} seconds")
        self.logger.info(
            f"  Average processing time per agent: {total_time / len(agents):.2f} seconds"
        )
...
class TestConfigurationIntegration(IntegrationTestBase):
    ...
    async def test_configuration_loading_and_validation(self):
        ...
        test_config = {
            "environment": {"name": "test", "debug": True, "log_level": "DEBUG"},
        ...
        assert config_manager.get_config("trading", "symbols") == [
            "00700.HK",
            "2800.HK",
        ]
...
class TestUtils:
    ...
    def create_mock_trading_signal(
        symbol: str, action: str, confidence: float
    ) -> Dict[str, Any]:
    ...
    async def benchmark_concurrent_requests():
        ...
        return {
            ...
            "successful_requests": len(
                [r for r in results if r["status"] == "processed"]
            ),
        }
