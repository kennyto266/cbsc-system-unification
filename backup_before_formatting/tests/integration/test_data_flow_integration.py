...
class TestDataPipelineIntegration:
    ...
    async def test_raw_data_to_analysis_pipeline(self):
        ...
        mock_analyst = self.mock_factory.create_mock_ai_agent(
            agent_id="quantitative_analyst", signal_type="BUY", confidence=0.85
        )
    ...
    async def test_analysis_to_trading_signal_pipeline(self):
        ...
        mock_trader = self.mock_factory.create_mock_ai_agent(
            agent_id="quantitative_trader", signal_type="BUY", confidence=0.80
        )

        # Test data flow
        # Step 1: Analyze market data
        analysis_result = await mock_analyst.analyze_market_data(
            self.market_data.to_dict("records")
        )
    ...
    async def test_trading_signal_to_portfolio_management_pipeline(self):
        ...
        mock_portfolio_manager = self.mock_factory.create_mock_ai_agent(
            agent_id="portfolio_manager", signal_type="BUY", confidence=0.75
        )

        # Test data flow
        # Step 1: Generate trading signal
        trading_signal = await mock_trader.analyze_market_data(
            self.market_data.to_dict("records")
        )

        # Step 2: Process portfolio management
        portfolio_decision = await mock_portfolio_manager.analyze_market_data(
            trading_signal
        )
    ...
    async def test_portfolio_to_risk_management_pipeline(self):
        ...
        mock_risk_analyst = self.mock_factory.create_mock_ai_agent(
            agent_id="risk_analyst", signal_type="BUY", confidence=0.70
        )
...
class TestDataTransformationIntegration:
    ...
    async def test_technical_indicator_calculation(self):
        ...
        assert all(valid_rsi >= 0) and all(
            valid_rsi <= 100
    ...
    async def test_risk_metrics_calculation(self):
        ...
        raw_data["volatility"] = raw_data["returns"].rolling(window=20).std() * np.sqrt(
            252
        )
        ...
        assert (
            not raw_data["volatility"].isna().all()
        )  # Should have some valid volatility
...
class TestDataQualityIntegration:
    ...
    async def setup_test_environment(self):
        ...
        self.dirty_data.loc[30:35, "high"] = (
            self.dirty_data.loc[30:35, "low"] - 1
        )  # Invalid OHLC
    ...
    async def test_missing_data_detection(self):
        ...
        assert len(cleaned_data) < len(
            self.dirty_data
        ), "Should remove rows with missing close prices"
    ...
    async def test_data_cleaning_pipeline(self):
        ...
        assert len(cleaned_data) < len(
            self.dirty_data
        ), "Should reduce data size after cleaning"
...
class TestRealTimeDataIntegration:
    ...
    async def test_real_time_data_validation(self):
        ...
        for data_point in self.real_time_data:
            is_valid = (
                data_point["price"] > 0
                and data_point["volume"] > 0
                and data_point["bid"] > 0
                and data_point["ask"] > 0
                and data_point["ask"] > data_point["bid"]
            )
        ...
        is_invalid = (
            ...
            or invalid_data["volume"] <= 0
            or invalid_data["bid"] <= 0
            or invalid_data["ask"] <= 0
            or invalid_data["ask"] <= invalid_data["bid"]
        )
