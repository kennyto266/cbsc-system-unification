...
class TestDashboardAgentIntegration:
    ...
    async def test_agent_data_flow(self, dashboard_api, mock_coordinator):
        ...
        assert (
            summary.system_metrics.system_memory_usage == 56.67
        )  # 平均值 (55 + 70 + 45)/3
    ...
    async def test_strategy_data_flow(self, dashboard_api, mock_coordinator):
        """测试策略数据流"""
        # 获取单个Agent的策略
        strategy = await dashboard_api.strategy_data_service.get_agent_strategy(
            "quant_analyst_001"
        )
    ...
    async def test_performance_data_flow(self, dashboard_api, mock_coordinator):
        """测试绩效数据流"""
        # 获取单个Agent的绩效
        performance = await dashboard_api.performance_service.get_agent_performance(
            "quant_analyst_001"
        )
    ...
    async def test_agent_control_flow(self, dashboard_api, mock_coordinator):
        """测试Agent控制流"""
        # 测试启动Agent
        action_id = await dashboard_api.agent_control_service.start_agent(
        ...
        # 测试获取操作状态
        action_status = await dashboard_api.agent_control_service.get_action_status(
            action_id
        )
        ...
        # 测试获取控制历史
        history = await dashboard_api.agent_control_service.get_action_history(
            "test_agent"
        )
    ...
    def test_html_endpoints(self, client):
        ...
        # 测试策略展示HTML
        response = client.get(
            "/api / dashboard / agents / quant_analyst_001 / strategy / display"
        )
        assert response.status_code == 200
        assert "text / html" in response.headers["content - type"]
        ...
        # 测试绩效图表HTML
        response = client.get(
            "/api / dashboard / agents / quant_analyst_001 / performance / charts"
        )
        assert response.status_code == 200
        assert "text / html" in response.headers["content - type"]
...
class TestDashboardPerformanceIntegration:
    ...
    async def test_large_dataset_performance(
        self, mock_coordinator, mock_message_queue
    ):
        """测试大数据集性能"""
        dashboard_api = DashboardAPI(mock_coordinator, mock_message_queue)
        await dashboard_api.initialize()
...
class TestDashboardErrorHandling:
    ...
    async def mock_message_queue(self):
        ...
        message_queue = Mock(spec=MessageQueue)
        message_queue.subscribe = AsyncMock(
            side_effect=Exception("Message queue error")
        )
        message_queue.unsubscribe = AsyncMock()
        message_queue.publish_message = AsyncMock()
        return message_queue
    ...
    async def test_error_handling_in_services(
        self, mock_coordinator_with_errors, mock_message_queue
    ):
        """测试服务中的错误处理"""
    ...
    def test_api_error_responses(
        ...
        response = client.post(
            "/api / dashboard / agents / test_agent / control / invalid_action"
        )
