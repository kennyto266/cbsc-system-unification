...
class TestNonPriceAdapters(unittest.TestCase):
    ...
    def test_economic_adapter(self):
        ...
        try:
            ...
            for source_id, data in latest_values.items():
                print(f"  {source_id}: {data.get('value', 'N/A')} ({data.get('name', 'N/A')})")
    ...
    def test_adapter_manager(self):
        ...
        try:
            ...
            print(f"  Healthy sources: {stats.get('summary', {}).get('healthy_sources', 0)}")
            print(f"  Total records: {stats.get('summary', {}).get('total_records', 0)}")
    ...
    def test_data_quality_report(self):
        ...
        try:
            ...
            print(f"Overall completeness: {report.get('overall_quality', {}).get('completeness', 0):.2%}")
            ...
            for source_id, source_report in sources.items():
                if 'error' not in source_report:
                    ...
                    print(f"  {source_id}: {record_count} records, {completeness:.2%} complete")
                    if issues:
