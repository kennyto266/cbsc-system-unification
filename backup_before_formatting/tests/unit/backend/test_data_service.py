...
class TestDataService:
    ...
    def test_fetch_stock_data_success(self, mock_ticker_class, data_service, mock_ticker):
        """測試成功獲取股票數據"""
    ...
    def test_get_market_data_with_date_filter(self, data_service):
        ...
        # 測試日期範圍過濾
        result = data_service.get_market_data("TEST.HK", start_date="2024-01-02", end_date="2024-01-02")
        assert len(result) == 1  # 只有1月2日的數據

    @patch('yfinance.Ticker')
    def test_fetch_stock_info_success(self, mock_ticker_class, data_service, mock_ticker):
        """測試成功獲取股票信息"""
        mock_ticker_class.return_value = mock_ticker
