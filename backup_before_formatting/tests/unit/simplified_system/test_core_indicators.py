...
sys.path.insert(0, str(Path(__file__).parent.parent.parent.parent / "simplified_system"))
...
class TestCoreIndicators:
    ...
    def sample_prices(self):
        ...
        return pd.Series(prices, index=pd.date_range('2024-01-01', periods=100, freq='D'))

    @pytest.fixture
    def short_prices(self):
        """創建短價格序列fixture（用於邊界測試）"""
        return pd.Series([100, 101, 102, 103, 104], index=pd.date_range('2024-01-01', periods=5, freq='D'))
    ...
    def test_calculate_bollinger_bands_insufficient_data(self, indicators, short_prices):
        """測試布林帶計算數據不足"""
...
class TestCoreIndicatorsEdgeCases:
    ...
    def test_constant_prices(self, indicators):
        """測試常數價格序列"""
        constant_prices = pd.Series([100.0] * 100, index=pd.date_range('2024-01-01', periods=100, freq='D'))
    ...
    def test_nan_values(self, indicators):
        """測試包含NaN的序列"""
        nan_prices = pd.Series([100.0, np.nan, 102.0, 101.0, 103.0],
    ...
    def test_extreme_values(self, indicators):
        """測試極端值"""
        extreme_prices = pd.Series([0.0001, 1e6, 0.0001, 1e6, 100.0],
                                 index=pd.date_range('2024-01-01', periods=5, freq='D'))
