...
try:
    from src.backtest.vectorbt_engine import VectorBTEngine, BacktestConfig, BacktestResult
...
class TestBacktestResult:
    ...
    def sample_result_data(self):
        ...
        equity_curve = pd.Series([100000, 102000, 101000, 103000, 105000,
                                 104000, 106000, 108000, 107000, 109000],
                                index=dates)
...
class TestVectorBTEngine:
    ...
    def test_calculate_rsi_signals(self, engine, sample_data):
        ...
        assert all(signal in valid_signals for signal in unique_signals if not np.isnan(signal))
