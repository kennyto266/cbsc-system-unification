...
def test_core_functionality():
    ...
    try:
        ...
        print(f"   PASS - Mean: {basic_stats['mean']:.2f}, Std: {basic_stats['std']:.2f}")
    ...
    try:
        ...
        test_results['trend_analysis'] = {
            'status': 'PASS',
            'trend': trend_info
        }
        print(f"   PASS - Trend: {trend_info['trend_direction']}, R²: {trend_info['r_squared']:.3f}")
        ...
        for period in periods_to_test:
            if len(price_data) >= period * 2:
                ...
                for lag in range(1, min(period, len(price_data) // 4)):
                    corr = np.corrcoef(price_data.values[:-lag], price_data.values[lag:])[0, 1]
    ...
    try:
        ...
        print(f"   PASS - Z-score anomalies: {np.sum(detected_anomalies)}, IQR anomalies: {np.sum(iqr_anomalies)}")
    ...
    try:
        ...
        print(f"   PASS - Processed {len(stream_data)} samples, generated {alerts} alerts")
    ...
    print(f"\nKey Metrics:")
    if 'basic_stats' in test_results and test_results['basic_stats']['status'] == 'PASS':
    ...
    if 'trend_analysis' in test_results and test_results['trend_analysis']['status'] == 'PASS':
        trend = test_results['trend_analysis']['trend']
        print(f"   - Trend Direction: {trend['trend_direction']}")
        print(f"   - Trend Significance: {'Significant' if trend['is_significant'] else 'Not Significant'}")

    if 'anomaly_detection' in test_results and test_results['anomaly_detection']['status'] == 'PASS':
