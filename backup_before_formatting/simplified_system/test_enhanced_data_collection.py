    ...
    def collect_data(self, source_name: str) -> dict:
        ...
        try:
            ...
            for record in records:
                date_val = (record.get('end_of_day') or
                           record.get('date') or
                           record.get('as_at_date') or
            ...
            if date_fields:
                ...
                if date_objects:
                    try:
                        dates = [datetime.strptime(d[:10], '%Y-%m-%d') for d in date_objects]
                        min_date = min(dates)
                        max_date = max(dates)
            ...
            print(f"[SUCCESS] {source_name}: {len(records)} 條記錄 ({years_of_data:.1f}年)")
