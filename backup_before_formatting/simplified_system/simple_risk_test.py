...
def generate_test_data():
    ...
    portfolio_positions = pd.DataFrame([
        {'asset': 'AAPL', 'market_value': 200000, 'weight': 0.40, 'sector': 'Technology'},
        {'asset': 'MSFT', 'market_value': 150000, 'weight': 0.30, 'sector': 'Technology'},
        {'asset': 'GOOGL', 'market_value': 100000, 'weight': 0.20, 'sector': 'Technology'},
        {'asset': 'Gold', 'market_value': 50000, 'weight': 0.10, 'sector': 'Commodities'}
    ])
...
def test_stress_test_engine(returns, portfolio_positions):
    ...
    try:
        ...
        stress_report = engine.run_stress_tests(
            portfolio_data,
            scenario_ids=['2008_financial_crisis', '2020_covid_crash']
        )
...
def test_liquidity_risk_analyzer(portfolio_positions):
    ...
    try:
        ...
        for _, position in portfolio_positions.iterrows():
            ...
            prices = pd.Series([100 + np.random.normal(0, 10) for _ in range(100)], index=dates)
            volumes = pd.Series([1000000 + np.random.normal(0, 100000) for _ in range(100)], index=dates)
