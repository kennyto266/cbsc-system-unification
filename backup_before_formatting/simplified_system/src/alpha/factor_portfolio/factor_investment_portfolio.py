...
class FactorInvestmentPortfolio:
    ...
    def __init__(self, config: Optional[PortfolioConfig] = None):
        ...
        logger.info(f"Factor Investment Portfolio initialized with strategy: {self.config.strategy.value}")
    ...
    def select_stocks(self,
        ...
        if len(selected) > self.config.max_positions:
            if self.config.strategy in [PortfolioStrategy.TOP_QUANTILES, PortfolioStrategy.FACTOR_TILT]:
                ...
                if self.config.strategy == PortfolioStrategy.BOTTOM_QUANTILES:
                    selected = scores.nsmallest(self.config.max_positions).index.tolist()
                else:
    ...
    def allocate_weights(self,
        ...
        if self.config.strategy == PortfolioStrategy.LONG_SHORT:
            ...
            if long_stocks:
                ...
                weights.update({f"LONG_{stock}": weight for stock, weight in long_weights.items()})

            # 空頭權重（負權重）
            if short_stocks:
                short_weights = self._allocate_equal_weights(short_stocks, factor_scores)
                weights.update({f"SHORT_{stock}": -weight for stock, weight in short_weights.items()})
            ...
            # 風險平價權重分配
            weights = self._allocate_risk_parity_weights(selected_stocks, factor_scores, price_data)
        ...
        if total_weight > 0:
            if self.config.strategy == PortfolioStrategy.LONG_SHORT:
                # 多空組合保持淨值為0
                weights = {k: v * self.config.leverage_limit for k, v in weights.items()}
            else:
                # 單向組合標準化到1
    ...
    def _allocate_equal_weights(self,
                               stocks: List[str],
                               factor_scores: pd.DataFrame) -> Dict[str, float]:
        """等權重分配"""
    ...
    def _allocate_factor_tilt_weights(self,
                                    stocks: List[str],
                                    factor_scores: pd.DataFrame) -> Dict[str, float]:
        """因子傾斜權重分配"""
        available_stocks = [s for s in stocks if s in factor_scores.index]
        ...
        # 使用綜合因子分數進行傾斜加權
        composite_scores = self._calculate_composite_scores(factor_scores.loc[available_stocks])
    ...
    def _allocate_risk_parity_weights(self,
                                    stocks: List[str],
                                    factor_scores: pd.DataFrame,
                                    price_data: Optional[pd.DataFrame] = None) -> Dict[str, float]:
        """風險平價權重分配"""
    ...
    def calculate_portfolio_metrics(self,
        ...
        try:
            ...
            concentration_ratio = max(abs(w) for w in weights.values()) if weights else 0
            ...
            logger.info(f"Portfolio metrics calculated: Sharpe={sharpe_ratio:.3f}, Return={annualized_return:.2%}")
            return metrics
    ...
    def _calculate_portfolio_returns(self,
        ...
        # 對齊權重和收益率數據
        available_assets = [w[5:] if w.startswith(("LONG_", "SHORT_")) else w
        ...
        # 計算投資組合收益率
        portfolio_returns = (returns_data[available_assets] * aligned_weights[available_assets]).sum(axis=1)
    ...
    def _analyze_position_breakdown(self) -> Dict[str, Any]:
        ...
        short_positions = sum(abs(p.weight) for p in self.positions.values() if p.weight < 0)

        top_positions = sorted(self.positions.values(),
