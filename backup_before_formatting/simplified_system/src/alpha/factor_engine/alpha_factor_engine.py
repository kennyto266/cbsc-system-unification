...
class MomentumFactorCalculator(BaseFactorCalculator):
    ...
    def calculate(self, data: pd.DataFrame, lookback_period: int = 20, **kwargs) -> pd.DataFrame:
        """計算動量因子
...
class ReversalFactorCalculator(BaseFactorCalculator):
    ...
    def calculate(self, data: pd.DataFrame, lookback_period: int = 5, **kwargs) -> pd.DataFrame:
        """計算短期反轉因子
...
class TechnicalFactorCalculator(BaseFactorCalculator):
    ...
    def calculate(self, data: pd.DataFrame, indicator_name: str = "RSI", **kwargs) -> pd.DataFrame:
        """基於技術指標計算因子
        ...
        try:
            ...
            if indicator_name.upper() == "RSI":
                ...
                macd_line, signal_line, histogram = indicators.calculate_macd(data['Close'], fast, slow, signal)
                return macd_line.to_frame('macd_factor')

            elif indicator_name.upper() == "BOLLINGER":
                period = kwargs.get('period', 20)
                std = kwargs.get('std', 2)
                bb_upper, bb_middle, bb_lower = indicators.calculate_bollinger_bands(data['Close'], period, std)
                # 計算布林帶位置因子
            ...
            logger.warning("Could not import technical indicators, using fallback calculation")
            # 簡單的技術指標計算
            if indicator_name.upper() == "RSI":
                period = kwargs.get('period', 14)
...
class AlphaFactorEngine:
    ...
    def __init__(self, config: Optional[FactorConfig] = None):
        ...
        logger.info(f"Alpha Factor Engine initialized with {len(self.factor_calculators)} calculators")
    ...
    def calculate_factors(self,
                         ...
                         **factor_params) -> Dict[str, FactorMetrics]:
        """
        計算多種Alpha因子
        ...
        if factor_types is None:
            factor_types = [FactorTypes.MOMENTUM, FactorTypes.REVERSAL,
                          FactorTypes.VOLATILITY, FactorTypes.VOLUME, FactorTypes.TECHNICAL]
        ...
        for factor_type in factor_types:
            ...
            if factor_type.value not in self.factor_calculators:
                logger.warning(f"No calculator registered for factor type: {factor_type.value}")
                continue
                ...
                try:
                    ...
                    if factor_type == FactorTypes.TECHNICAL:
                        ...
                        for indicator in ['RSI', 'MACD', 'BOLLINGER']:
                            tech_factor_name = f"technical_{indicator.lower()}_{lookback}"
                        ...
                        # 其他類型因子的標準處理
                        factor_data = calculator.calculate(data, lookback_period=lookback, **factor_params)
    ...
    def get_available_factor_types(self) -> List[FactorTypes]:
        """獲取可用的因子類型"""
        return list(set(calculator.get_factor_type() for calculator in self.factor_calculators.values()))
