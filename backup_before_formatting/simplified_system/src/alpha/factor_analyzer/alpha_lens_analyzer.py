...
class AlphaLensAnalyzer:
    ...
    def _create_alphalens_tear_sheet(self,
                                    factor_data: pd.DataFrame,
                                    pricing_data: pd.DataFrame,
                                    sector_data: Optional[pd.DataFrame] = None,
                                    start_date: Optional[str] = None,
        ...
        try:
            # 準備數據
            factor_data_prepared = self._prepare_factor_data(factor_data, start_date, end_date)
            pricing_data_prepared = self._prepare_pricing_data(pricing_data, start_date, end_date)
    ...
    def _create_builtin_tear_sheet(self,
                                  factor_data: pd.DataFrame,
                                  pricing_data: pd.DataFrame,
                                  sector_data: Optional[pd.DataFrame] = None,
                                  start_date: Optional[str] = None,
        ...
        try:
            ...
            pricing_clean = self._prepare_pricing_data(pricing_data, start_date, end_date)

            # 基本統計分析
            results['basic_stats'] = self._analyze_basic_stats(factor_clean)

            # 收益率分析
            results['returns_analysis'] = self._analyze_returns(factor_clean, pricing_clean)

            # IC分析
            results['ic_analysis'] = self._analyze_information_coefficient(factor_clean, pricing_clean)

            # 換手率分析
            results['turnover_analysis'] = self._analyze_turnover(factor_clean)

            if sector_data is not None:
                results['sector_analysis'] = self._analyze_by_sector(factor_clean, sector_data)
    ...
    def _analyze_returns(self,
                        factor_data: pd.DataFrame,
                        pricing_data: pd.DataFrame) -> Dict[str, Any]:
        """分析收益率"""
        try:
            # 計算收益率
            ...
            quantile_groups = pd.cut(quantiles, bins=self.config.quantiles, labels=False)
            ...
            if len(group_returns) >= 2:
                ...
                long_short_sharpe = long_short_return / returns_aligned_clean.std() if returns_aligned_clean.std() > 0 else 0
            else:
    ...
    def _analyze_information_coefficient(self,
                                        factor_data: pd.DataFrame,
                                        pricing_data: pd.DataFrame) -> Dict[str, Any]:
        """分析信息係數"""
        try:
            ...
            for date in factor_aligned.index:
                if date in returns_aligned.index:
                    ...
                    return_vals = returns_aligned.loc[date].reindex(factor_vals.index).dropna()

                    if len(factor_vals) > 5 and len(return_vals) > 5:
                        common_idx = factor_vals.index.intersection(return_vals.index)
                        if len(common_idx) > 5:
                            ic = factor_vals.loc[common_idx].corr(return_vals.loc[common_idx])
                            if not np.isnan(ic):
                                ic_series.append(ic)
        ...
        try:
            ...
            for i in range(1, len(factor_ranks)):
                ...
                if len(common_assets) > 0:
                    ...
                    # 計算排名變化
                    rank_change = np.abs(curr_common - prev_common).sum() / len(common_assets)
    ...
    def _analyze_by_sector(self,
        ...
        try:
            ...
            for sector in sector_data.unique():
                ...
                sector_factor_data = factor_matrix[sector_assets.intersection(factor_matrix.columns)]
    ...
    def stratified_analysis(self,
                          ...
                          pricing_data: pd.DataFrame,
                          group_by: str,
                          group_data: pd.DataFrame) -> Dict[str, Any]:
        """
        分層因子分析
        ...
        if group_by == 'sector':
            ...
            # 通用分層分析
            return self._create_stratified_analysis(factor_data, pricing_data, group_by, group_data)

    def _create_stratified_analysis(self,
                                  factor_data: pd.DataFrame,
                                  pricing_data: pd.DataFrame,
                                  group_by: str,
                                  group_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            for group in group_data.unique():
                ...
                group_factor_data = factor_matrix[group_assets.intersection(factor_matrix.columns)]
                    ...
                    try:
                        ...
                        for date in group_factor_data.index:
                            if date in aligned_returns.index:
                                ...
                                return_vals = aligned_returns.loc[date].reindex(factor_vals.index).dropna()
                        ...
                        if ic_values:
                            ...
                            group_stats['ic_positive_rate'] = np.mean(np.array(ic_values) > 0)
