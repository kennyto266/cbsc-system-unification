...
class FactorValidator:
    ...
    def __init__(self, risk_free_rate: float = 0.03, confidence_level: float = 0.95):
        ...
        logger.info(f"Factor Validator initialized with risk-free rate: {risk_free_rate}")

    def validate_factor(self,
                       factor_metrics: FactorMetrics,
                       price_data: pd.DataFrame,
                       returns_data: Optional[pd.Series] = None,
                       holding_period: int = 1) -> FactorValidationResult:
        """
        驗證單個因子的有效性
        ...
        if returns_data is None:
            ...
            if 'Close' in price_data.columns:
                ...
                returns_data = price_data.iloc[:, 0].pct_change(holding_period)  # 假設第一列是價格

        # 對齊數據
        aligned_data = pd.concat([factor_data, returns_data], axis=1, join='inner')
        if len(aligned_data.columns) < 2:
            logger.warning(f"Insufficient aligned data for factor {factor_metrics.factor_name}")
        ...
        if len(factor_valid) < 30:  # 最少需要30個觀察值
            logger.warning(f"Insufficient data points for factor {factor_metrics.factor_name}: {len(factor_valid)}")
            return FactorValidationResult(factor_name=factor_metrics.factor_name)
        ...
        # 計算穩定性分析
        stability_analysis = self._calculate_stability_metrics(factor_valid, returns_valid)
    ...
    def _calculate_ic_analysis(self, factor_values: pd.Series, returns: pd.Series) -> Dict[str, float]:
        """
        ...
        results = {
            ...
            'positive_rate': (rolling_ic > 0).mean() if len(rolling_ic.dropna()) > 0 else 0.0,
    ...
    def _calculate_factor_sharpe(self, factor_values: pd.Series, returns: pd.Series) -> Dict[str, float]:
        """
        計算因子Sharpe比率相關指標
    ...
    def _calculate_stability_metrics(self, factor_values: pd.Series, returns: pd.Series) -> Dict[str, float]:
        """
        計算因子穩定性指標
        ...
        if len(decay_rates) > 1:
            decay_rate = (decay_rates[0] - decay_rates[-1]) / abs(decay_rates[0]) if decay_rates[0] != 0 else 0
        else:
            decay_rate = 0.0
        ...
        if len(rolling_ic.dropna()) > 10:
            stability_score = 1.0 - rolling_ic.std() / abs(rolling_ic.mean()) if rolling_ic.mean() != 0 else 0
    ...
    def _perform_statistical_test(self, factor_values: pd.Series, returns: pd.Series) -> Dict[str, Any]:
        """
        執行統計檢驗
    ...
    def validate_multiple_factors(self,
                                 factor_metrics_dict: Dict[str, FactorMetrics],
                                 price_data: pd.DataFrame,
                                 returns_data: Optional[pd.Series] = None) -> Dict[str, FactorValidationResult]:
        """
        批量驗證多個因子
    ...
    def generate_validation_report(self, validation_results: Dict[str, FactorValidationResult]) -> pd.DataFrame:
        """
        生成因子檢驗報告
