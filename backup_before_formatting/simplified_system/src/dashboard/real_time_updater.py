...
class RealTimeUpdater:
    ...
    def _update_government_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if hibor_data:
                    latest_hibor = hibor_data[0] if isinstance(hibor_data, list) else hibor_data
                    gov_data['hibor'] = {
                        ...
                        'timestamp': latest_hibor.get('date', datetime.now().isoformat())
                    }
            except Exception as e:
    ...
    def _calculate_indicators(self, data: pd.DataFrame) -> Dict[str, float]:
        ...
        try:
            ...
            if len(data) >= 20:
                ...
                indicators['sma_20'] = float(sma_20.iloc[-1]) if not sma_20.empty else 0.0

            if len(data) >= 50:
                sma_50 = indicators_engine.calculate_sma(data['close'], 50)
                indicators['sma_50'] = float(sma_50.iloc[-1]) if not sma_50.empty else 0.0

            # MACD
            if len(data) >= 26:
                macd_result = indicators_engine.calculate_macd(data['close'], 12, 26, 9)
                indicators['macd'] = float(macd_result['macd'].iloc[-1]) if not macd_result['macd'].empty else 0.0
                indicators['macd_signal'] = float(macd_result['signal'].iloc[-1]) if not macd_result['signal'].empty else 0.0
    ...
    def _update_stats(self, update_time: float, success: bool):
        ...
        # 更新平均更新时间
        total_time = self.stats['average_update_time'] * (self.stats['total_updates'] - 1) + update_time
    ...
    def _simulate_websocket_data(self):
        ...
        for symbol in self.data_sources['stock_api']['symbols']:
            if symbol in self.data_cache:
                ...
                # 模拟小幅价格波动
                price_change = np.random.normal(0, cached_data['price'] * 0.001)  # 0.1%标准差
                new_price = max(cached_data['price'] + price_change, 0.01)

                # 更新缓存
                cached_data['price'] = new_price
                cached_data['change'] += price_change
                cached_data['change_percent'] = (cached_data['change'] / cached_data['price']) * 100
