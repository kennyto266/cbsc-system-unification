...
class PerformanceCharts:
    ...
    def create_price_chart(
        ...
        try:
            ...
            subplot_titles = ['价格走势', '成交量', '技术指标'] if show_volume else ['价格走势', '技术指标']
            ...
            if show_indicators and 'RSI_14' in data.columns:
                ...
                # RSI超买超卖线
                fig.add_hline(
                    y=70, line_dash="dash", line_color=self.colors['danger'],
                    annotation_text="超买(70)", row=rows, col=1
                )
                fig.add_hline(
                    y=30, line_dash="dash", line_color=self.colors['success'],
                    annotation_text="超卖(30)", row=rows, col=1
                )

            # MACD指标
            if show_indicators and all(col in data.columns for col in ['MACD', 'MACD_Signal']):
                fig.add_trace(
                    go.Scatter(
                        x=data.index,
                        y=data['MACD'],
    ...
    def create_performance_radar(
        ...
        try:
            ...
            if results_json:
                ...
                # 雷达图指标
                metrics = [
                    '总回报率', 'Sharpe比率', '最大回撤(反向)',
                    '胜率', 'Calmar比率', 'Sortino比率'
                ]
                ...
                values = [
                    ...
                    min(1 - abs(performance.get('max_drawdown', 0)) / 20, 1),  # 最大回撤反向
    ...
    def create_correlation_heatmap(
        self,
        data: pd.DataFrame,
        additional_assets: Optional[pd.DataFrame] = None
    ) -> go.Figure:
        """
        创建相关性热力图
        ...
        try:
            ...
            if len(returns_data.columns) == 1:
                # 添加一些技术指标作为对比
                returns_data['SMA_20收益率'] = data.get('SMA_20', data['close']).pct_change()
    ...
    def create_risk_return_scatter(
        self,
        strategies_data: List[Dict[str, float]]
    ) -> go.Figure:
        """
        创建风险收益散点图
