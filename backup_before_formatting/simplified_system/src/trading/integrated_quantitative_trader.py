...
class IntegratedQuantitativeTrader:
    ...
    def __init__(self, initial_capital: float = 100000):
        ...
        logger.info(f"Integrated Quantitative Trader initialized with capital: ${initial_capital:,.2f}")

    async def analyze_symbol(self, symbol: str, extend_data: bool = True) -> Optional[TradingSignal]:
        """分析單個股票符號並生成交易信號"""
        try:
            ...
            if extend_data and len(stock_data) < 1000:
                ...
                extension_result = extend_data_records(stock_records, 1000, 'hybrid_approach')
            ...
            analysis_result = self.advanced_signals.generate_enhanced_signals(
                data=stock_records if len(stock_data) < 1000 else stock_data.to_dict('records'),
                data_type='price',
            ...
            expected_return = self._calculate_expected_return(composite_signals, risk_score)
            ...
            logger.info(f"Generated {signal_type.value} signal for {symbol} with confidence {confidence:.2f}")
            return signal
    ...
    def _determine_signal_type(self, composite_signals: Dict, signal_quality: float) -> Tuple[SignalType, float]:
        """確定信號類型和置信度"""
        try:
            buy_signals = 0
            sell_signals = 0
    ...
    def _calculate_expected_return(self, composite_signals: Dict, risk_score: float) -> float:
        """計算預期回報"""
        try:
            # 基於信號強度和風險調整預期回報
            signal_strength = 0.0
                    ...
                    if isinstance(timeframes, dict):
                        ...
                        for tf_type, tf_signals in timeframes.items():
                            if tf_type == 'short_term':
                                timeframe_weights['short'] += len(tf_signals) if isinstance(tf_signals, dict) else 0
                            elif tf_type == 'medium_term':
                                timeframe_weights['medium'] += len(tf_signals) if isinstance(tf_signals, dict) else 0
                            elif tf_type == 'long_term':
                                timeframe_weights['long'] += len(tf_signals) if isinstance(tf_signals, dict) else 0
    ...
    async def execute_trading_decision(self, signal: TradingSignal) -> Optional[Dict[str, Any]]:
        """執行交易決策"""
        try:
            ...
            # 檢查是否已達最大持倉數量
            if not current_position and len(self.positions) >= self.config['max_positions']:
    ...
    async def _execute_buy_order(self, signal: TradingSignal, position_value: float) -> Dict[str, Any]:
        """執行買入訂單"""
        try:
            ...
            logger.info(f"Executed BUY order for {quantity} shares of {signal.symbol} at ${signal.price:.2f}")
    ...
    async def _execute_sell_order(self, signal: TradingSignal) -> Dict[str, Any]:
        ...
        try:
            ...
            # 檢查止損止盈
            current_return = (signal.price - position.entry_price) / position.entry_price
            ...
            position.unrealized_pnl = sell_proceeds - (position.quantity * position.entry_price)
            ...
            trade_record = {
                ...
                'pnl_percentage': ((signal.price - position.entry_price) / position.entry_price) * 100,
            ...
            logger.info(f"Executed SELL order for {position.quantity} shares of {signal.symbol} at ${signal.price:.2f}, PnL: ${position.unrealized_pnl:.2f}")
    ...
    async def run_trading_cycle(self, symbols: List[str]) -> Dict[str, Any]:
        ...
        try:
                ...
                try:
                    ...
                    if signal:
                        ...
                        if trade_result:
                            ...
                            if trade_result['action'] == 'SELL' and trade_result.get('realized_pnl', 0) > 0:
                                cycle_results['successful_trades'] += 1
                                cycle_results['total_pnl'] += trade_result.get('realized_pnl', 0)
            ...
            for position in self.positions.values():
                position.current_price = position.entry_price * (1 + position.unrealized_pnl / (position.quantity * position.entry_price))
                total_position_value += position.quantity * position.current_price

            cycle_results['capital_utilization'] = total_position_value / (self.current_capital + total_position_value)
            cycle_results['end_time'] = datetime.now().isoformat()

            # 生成週期報告
            await self._generate_cycle_report(cycle_results)

            logger.info(f"Trading cycle completed: {cycle_results['trades_executed']} trades, ${cycle_results['total_pnl']:.2f} PnL")
        ...
        try:
            report = {
                ...
                'top_signals': [
                    ...
                    for signal in sorted(self.signals_history[-10:], key=lambda x: x.confidence, reverse=True)
                ]
            }

            # 保存報告
            report_file = f"trading_cycle_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
