...
class ParallelOptimizer:
    ...
    def __init__(self, config: Optional[ParallelConfig] = None):
        ...
        logger.info(f"Parallel optimizer initialized with {self.optimal_workers} workers")
    ...
    def _prepare_tasks(
        ...
        for i, task in enumerate(tasks):
            task_dict = {
                'func': func,
                'args': (task,) if task is not None else ()
            }
    ...
    def _execute_parallel_tasks(
        ...
        # 选择执行器类型
        executor_class = mp.ProcessPoolExecutor if self.config.use_processes else concurrent.futures.ThreadPoolExecutor
    ...
    def parallel_map(
        ...
        if len(iterable) > chunk_size * 10:
            return self._chunked_parallel_map(func, iterable, chunk_size, progress_callback)
        else:
            # 直接并行处理
            tasks = list(iterable)
            return self.parallel_execute(func, tasks, progress_callback=progress_callback)
    ...
    def parallel_dataframe_apply(
        ...
        if axis == 0:
            ...
            results = self.parallel_execute(func, rows, progress_callback=progress_callback)
            return pd.Series(results, index=df.index)
        else:
            # 按列并行处理
            columns = [df[col] for col in df.columns]
            results = self.parallel_execute(func, columns, progress_callback=progress_callback)
    ...
    def optimize_parameter_combinations(
        ...
        logger.info(f"Parameter optimization completed: {len(successful_results)}/{len(param_combinations)} successful")
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'task_statistics': {
                ...
                'tasks_per_second': total_tasks / self.task_stats['total_time'] if self.task_stats['total_time'] > 0 else 0
            },
            'configuration': {
                'chunk_size': self.config.chunk_size,
...
def parallel_optimize_parameters(param_combinations: List[Dict], objective_func: Callable, **kwargs) -> List[Dict]:
    """便利函数：并行参数优化"""
    return global_parallel_optimizer.optimize_parameter_combinations(param_combinations, objective_func, **kwargs)
