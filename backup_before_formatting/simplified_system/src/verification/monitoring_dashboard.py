...
class MetricsCollector:
    ...
    def __init__(self, collection_interval: int = 10):
        ...
        self.metrics_data = defaultdict(lambda: deque(maxlen=1000))  # 保留最近1000個數據點
    ...
    async def _collect_system_metrics(self):
        ...
        self._add_metric_point("system.memory_used_gb", current_time, memory.used / (1024**3))

        # 磁盤使用率
        disk = psutil.disk_usage('/')
        self._add_metric_point("system.disk_usage", current_time, disk.percent)
        self._add_metric_point("system.disk_used_gb", current_time, disk.used / (1024**3))

        # GPU使用率（如果可用）
        try:
            gpus = GPUtil.getGPUs()
            for i, gpu in enumerate(gpus):
                self._add_metric_point(f"system.gpu_{i}_usage", current_time, gpu.load * 100)
                self._add_metric_point(f"system.gpu_{i}_memory", current_time, gpu.memoryUtil * 100)
        except:
            pass

        # 網絡統計
        network = psutil.net_io_counters()
        self._add_metric_point("system.network_bytes_sent", current_time, network.bytes_sent)
        self._add_metric_point("system.network_bytes_recv", current_time, network.bytes_recv)
    ...
    async def _collect_verification_metrics(self):
        ...
        try:
            ...
            self._add_metric_point("verification.total_verifications", current_time, metrics.total_verifications)
            self._add_metric_point("verification.successful_verifications", current_time, metrics.successful_verifications)
            self._add_metric_point("verification.failed_verifications", current_time, metrics.failed_verifications)
            self._add_metric_point("verification.success_rate", current_time,
                                  metrics.successful_verifications / metrics.total_verifications
                                  if metrics.total_verifications > 0 else 0)
            self._add_metric_point("verification.avg_response_time_ms", current_time, metrics.avg_response_time_ms)
            self._add_metric_point("verification.cache_hit_rate", current_time, metrics.cache_hit_rate)
            self._add_metric_point("verification.error_rate", current_time, metrics.error_rate)
            self._add_metric_point("verification.throughput_per_second", current_time, metrics.throughput_per_second)
    ...
    async def _collect_cache_metrics(self):
        ...
        try:
            ...
            self._add_metric_point("cache.entries_count", current_time, stats.get('entries_count', 0))
            self._add_metric_point("cache.memory_usage_mb", current_time, stats.get('memory_usage_mb', 0))
            self._add_metric_point("cache.hit_rate", current_time, stats.get('hit_rate', 0))
            ...
            self._add_metric_point("cache.evictions", current_time, stats.get('evictions', 0))
            self._add_metric_point("cache.compressions", current_time, stats.get('compressions', 0))
    ...
    async def _collect_processor_metrics(self):
        ...
        try:
            ...
            self._add_metric_point("processor.total_tasks", current_time, perf_metrics.get('total_tasks', 0))
            self._add_metric_point("processor.successful_tasks", current_time, perf_metrics.get('successful_tasks', 0))
            self._add_metric_point("processor.failed_tasks", current_time, perf_metrics.get('failed_tasks', 0))
            self._add_metric_point("processor.avg_execution_time", current_time, perf_metrics.get('avg_execution_time', 0))
            self._add_metric_point("processor.throughput_per_second", current_time, perf_metrics.get('throughput_per_second', 0))

            scheduler_stats = metrics.get('scheduler_stats', {})
            self._add_metric_point("processor.running_tasks", current_time, scheduler_stats.get('running_tasks', 0))
            self._add_metric_point("processor.completed_tasks", current_time, scheduler_stats.get('completed_tasks', 0))
    ...
    def _add_metric_point(self, metric_name: str, timestamp: float, value: float, metadata: Dict[str, Any] = None):
        """添加指標數據點"""
    ...
    def get_metric_data(self, metric_name: str, start_time: Optional[float] = None,
...
class AlertManager:
    ...
    async def _trigger_alert(self, rule: AlertRule, current_value: float):
        ...
        logger.warning(f"Alert triggered: {rule.name} - {rule.metric_path} {rule.operator} {rule.threshold} (current: {current_value})")
...
class MonitoringDashboard:
    ...
    def _setup_routes(self):
        ...
        def get_metric(metric_name):
            ...
            data = self.metrics_collector.get_metric_data(metric_name, start_time, end_time)
            result = [
                {
                    'timestamp': point.timestamp,
                    'value': point.value,
            ...
            for metric_name in self.metrics_collector.get_all_metrics_names():
                ...
                if latest_point:
                    summary[metric_name] = {
                        ...
                        'data_points_count': len(self.metrics_collector.metrics_data[metric_name])
                    }
        ...
        def health_check():
            ...
            try:
                ...
                health_status = {
                    ...
                    'timestamp': time.time(),
                    'components': {
                        'verification_system': 'healthy' if system_metrics else 'unhealthy',
                        'cache_system': 'healthy' if cache_stats else 'unhealthy',
                        'processor_system': 'healthy' if processor_metrics else 'unhealthy'
                    },
                    'system_metrics': {
            ...
            try:
                ...
                return jsonify({
                    ...
                    'verification': {
                        ...
                        'success_rate': verification_metrics.successful_verifications / verification_metrics.total_verifications
                                    if verification_metrics.total_verifications > 0 else 0,
                    ...
                    'processor': {
                        'total_tasks': processor_metrics.get('performance_metrics', {}).get('total_tasks', 0),
                        'running_tasks': processor_metrics.get('scheduler_stats', {}).get('running_tasks', 0),
                        'throughput_per_second': processor_metrics.get('performance_metrics', {}).get('throughput_per_second', 0)
                    },
                    'alerts': {
    ...
    def _create_system_overview_chart(self):
        ...
        # CPU使用率
        cpu_data = self.metrics_collector.get_metric_data('system.cpu_usage', current_time - time_window)
        if cpu_data:
            fig.add_trace(
                go.Scatter(
        ...
        # 內存使用率
        memory_data = self.metrics_collector.get_metric_data('system.memory_usage', current_time - time_window)
        if memory_data:
            fig.add_trace(
                go.Scatter(
        ...
        # 磁盤使用率
        disk_data = self.metrics_collector.get_metric_data('system.disk_usage', current_time - time_window)
        if disk_data:
            fig.add_trace(
                go.Scatter(
    ...
    def _create_verification_performance_chart(self):
        ...
        fig = make_subplots(
            rows=2, cols=2,
            subplot_titles=('Response Time', 'Success Rate', 'Cache Hit Rate', 'Throughput'),
            specs=[[{"secondary_y": False}, {"secondary_y": False}],
        ...
        # 響應時間
        response_time_data = self.metrics_collector.get_metric_data('verification.avg_response_time_ms', current_time - time_window)
        ...
        # 成功率
        success_rate_data = self.metrics_collector.get_metric_data('verification.success_rate', current_time - time_window)
        ...
        # 緩存命中率
        cache_hit_data = self.metrics_collector.get_metric_data('verification.cache_hit_rate', current_time - time_window)
        if cache_hit_data:
            fig.add_trace(
                go.Scatter(
        ...
        # 吞吐量
        throughput_data = self.metrics_collector.get_metric_data('verification.throughput_per_second', current_time - time_window)
        if throughput_data:
            fig.add_trace(
                go.Scatter(
        ...
        fig.update_layout(
            title='Verification System Performance',
            showlegend=True,
            height=600
        )
    ...
    def _create_cache_performance_chart(self):
        ...
        fig = make_subplots(
            rows=2, cols=2,
            subplot_titles=('Cache Hit Rate', 'Memory Usage', 'Entries Count', 'Compression Ratio'),
            specs=[[{"secondary_y": False}, {"secondary_y": False}],
                   [{"secondary_y": False}, {"secondary_y": False}]]
        )
        ...
        # 緩存命中率
        hit_rate_data = self.metrics_collector.get_metric_data('cache.hit_rate', current_time - time_window)
        if hit_rate_data:
            fig.add_trace(
                go.Scatter(
        ...
        # 內存使用量
        memory_usage_data = self.metrics_collector.get_metric_data('cache.memory_usage_mb', current_time - time_window)
        ...
        # 條目數量
        entries_data = self.metrics_collector.get_metric_data('cache.entries_count', current_time - time_window)
        if entries_data:
            fig.add_trace(
                go.Scatter(
        ...
        fig.update_layout(
            title='Cache System Performance',
            showlegend=True,
            height=600
        )
...
if __name__ == "__main__":
    async def test_monitoring():
        ...
        try:
            ...
            print("\nMetrics summary:")
            summary = {}
            for metric_name in monitoring_dashboard.metrics_collector.get_all_metrics_names():
                latest_point = monitoring_dashboard.metrics_collector.get_latest_metric(metric_name)
                if latest_point:
            ...
            print(f"Dashboard available at: http://{monitoring_dashboard.host}:{monitoring_dashboard.port}")
