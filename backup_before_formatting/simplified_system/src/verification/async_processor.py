...
class ResourcePool:
    ...
    def __init__(self, max_workers: int = None, pool_type: str = "thread"):
        ...
        logger.info(f"ResourcePool initialized - Type: {pool_type}, Workers: {self.max_workers}")
    ...
    async def submit(self, task: AsyncTask) -> asyncio.Future:
        ...
        try:
            ...
            # 設置回調
            result_future.add_done_callback(lambda f: self._task_completed(future, task, f))
    ...
    def get_stats(self) -> Dict[str, Any]:
        """獲取資源池統計"""
        avg_time = (self.total_execution_time / self.completed_tasks
...
class TaskScheduler:
    ...
    async def submit_task(self, task: AsyncTask) -> str:
        ...
        # 生成任務ID
        if not task.task_id:
            task.task_id = f"task_{int(time.time() * 1000000)}_{hash(str(task.func)) % 10000}"
...
class AsyncProcessor:
        ...
        while True:
            try:
                ...
                if self.monitoring_data:
                    ...
                    recent_tasks = [d for d in self.monitoring_data
                                  if current_time - d['timestamp'] < 60]  # 最近1分鐘
    ...
    async def wait_for_completion(self, task_ids: List[str], timeout: Optional[float] = None) -> Dict[str, Any]:
        """等待任務完成"""
        start_time = time.time()
        results = {}
...
# 便捷函數
async def submit_async_task(func: Callable,
                          args: tuple = (),
                          kwargs: dict = None,
...
async def process_parallel(data_items: List[Any],
                          processor_func: Callable,
                          batch_size: int = 10,
                          max_concurrent: int = 50) -> List[Any]:
    """並行處理數據"""
    return await async_processor.process_parallel(data_items, processor_func, batch_size, max_concurrent)

async def map_parallel(func: Callable, items: List[Any], max_concurrent: int = None) -> List[Any]:
    """並行映射函數"""
