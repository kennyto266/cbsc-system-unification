...
class VerificationSystem:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # 初始化各個組件
        self.verification_manager = UnifiedVerificationManager(self.config.get('verification_config'))
        self.cache = IntelligentCache(
            max_memory_mb=self.config.get('cache', {}).get('max_memory_mb', 512),
            max_entries=self.config.get('cache', {}).get('max_entries', 10000),
            enable_compression=self.config.get('cache', {}).get('enable_compression', True),
            enable_disk_cache=self.config.get('cache', {}).get('enable_disk_cache', True)
        )
        self.async_processor = AsyncProcessor(
            ...
            process_pools=self.config.get('async_processor', {}).get('process_pools', 1)
        )
    ...
    async def verify_batch(self, data_batch: List[Tuple[Any, str, Optional[str]]],
    ...
    async def verify_government_data(self, data_type: str, days_back: int = 30) -> VerificationResult:
        ...
        return await self.verification_manager.verify_government_data(data_type, days_back)
    ...
    async def process_data_parallel(self, data_items: List[Any], processor_func: Callable,
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'dashboard_url': f"http://{self.monitoring_dashboard.host}:{self.monitoring_dashboard.port}" if self.is_started else None
        }
        ...
        try:
            ...
            if self.verification_manager:
                ...
                health_status['components']['verification_manager'] = {
                    'status': 'healthy' if metrics.total_verifications >= 0 else 'unhealthy',
            ...
            if self.cache:
                ...
                health_status['components']['cache'] = {
                    'status': 'healthy' if cache_stats.get('entries_count', 0) >= 0 else 'unhealthy',
                    'stats': cache_stats
                }
            ...
            health_status['components']['monitoring_dashboard'] = {
                ...
                'url': f"http://{self.monitoring_dashboard.host}:{self.monitoring_dashboard.port}" if self.monitoring_dashboard else None
            }
...
async def create_verification_system(config: Optional[Dict[str, Any]] = None) -> VerificationSystem:
    """創建驗證系統實例"""
...
async def start_verification_system(config: Optional[Dict[str, Any]] = None) -> VerificationSystem:
    """啟動驗證系統"""
...
# 便利函數
async def verify(data: Any, data_type: str, source_url: Optional[str] = None) -> VerificationResult:
    """便利函數：驗證數據"""
...
async def verify_batch(data_batch: List[Tuple[Any, str, Optional[str]]]) -> List[VerificationResult]:
    """便利函數：批量驗證數據"""
    system = await get_verification_system()
    if not system:
        raise RuntimeError("Verification system is not initialized")
...
if __name__ == "__main__":
    async def test_verification_system():
        ...
        try:
            ...
            result = await system.verify_data(test_data, 'test_data', 'http://example.com')
            ...
            for i, result in enumerate(batch_results):
                print(f"  Result {i+1}: score={result.composite_score:.3f}, time={result.verification_time_ms:.2f}ms")
