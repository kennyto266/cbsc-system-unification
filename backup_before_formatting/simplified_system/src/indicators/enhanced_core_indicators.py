...
class EnhancedCoreIndicators(CoreIndicators):
    ...
    def __init__(self):
        ...
        logger.info("Enhanced Core Technical Indicators Engine initialized with non-price data support")
    ...
    def adapt_parameters_for_data_type(self, data_type: str, base_params: Dict = None) -> Dict:
        """
        根據數據類型適配參數
    ...
    def calculate_sma_non_price(self, data: pd.Series, period: int = None,
                               column_name: str = None) -> Tuple[pd.Series, Dict]:
        """
        為非價格數據計算SMA
        ...
        if period is None:
            ...
            adapted_params = self.adapt_parameters_for_data_type(data_type, {'sma_period': period})
        ...
        adaptation_info = {
            ...
            'adaptation_reason': adapted_params.get('adaptation', 'Standard adaptation'),
    ...
    def calculate_ema_non_price(self, data: pd.Series, period: int = None,
                               column_name: str = None) -> Tuple[pd.Series, Dict]:
        """
        為非價格數據計算EMA
        ...
        if period is None:
            ...
            adapted_params = self.adapt_parameters_for_data_type(data_type, {'ema_period': period})
    ...
    def calculate_macd_non_price(self, data: pd.Series, fast: int = None, slow: int = None,
                                signal: int = None, column_name: str = None) -> Dict[str, Union[Dict, pd.Series]]:
        """
        為非價格數據計算MACD
    ...
    def calculate_rsi_non_price(self, data: pd.Series, period: int = None,
                                column_name: str = None) -> Tuple[pd.Series, Dict]:
        """
        為非價格數據計算RSI
        ...
        if period is None:
            ...
            adapted_params = self.adapt_parameters_for_data_type(data_type, {'rsi_period': period})
    ...
    def calculate_rate_spread(self, rate_data_short: pd.Series, rate_data_long: pd.Series,
                             short_name: str = 'short_term', long_name: str = 'long_term') -> Dict:
        """
        計算利率期限結構利差
        ...
        # 計算百分位數
        spread_percentile = spread.rolling(window=252, min_periods=1).rank(pct=True) * 100
    ...
    def calculate_yield_curve_indicator(self, yield_rates: Dict[str, pd.Series]) -> Dict:
        """
        計算收益率曲線指標
        ...
        aligned_rates = {tenor: rates.loc[common_index] for tenor, rates in yield_rates.items()}
        ...
        if '2y' in aligned_rates and '10y' in aligned_rates and '30y' in aligned_rates:
            curvature = 2 * aligned_rates['10y'] - aligned_rates['2y'] - aligned_rates['30y']
    ...
    def calculate_monetary_momentum(self, monetary_data: pd.Series, period: int = 20) -> Dict:
        """
        計算貨幣數據動量指標
        ...
        return {
            ...
            'metadata': {
                ...
                'data_trend': 'increasing' if monetary_data.iloc[-1] > monetary_data.iloc[0] else 'decreasing'
            }
        }
    ...
    def _assess_macd_signal_quality(self, macd_result: Dict) -> Dict:
        ...
        return {
            ...
            'assessment': 'Good' if signal_quality > 0.7 else 'Fair' if signal_quality > 0.5 else 'Poor'
        }
    ...
    def _analyze_rsi_characteristics(self, rsi: pd.Series) -> Dict:
        ...
        return {
            ...
            'assessment': self._classify_rsi_pattern(mean_rsi, std_rsi, overbought_ratio, oversold_ratio)
        }

    def _classify_rsi_pattern(self, mean_rsi: float, std_rsi: float,
                             overbought_ratio: float, oversold_ratio: float) -> str:
        """分類RSI模式"""
        if mean_rsi > 55:
    ...
    def calculate_comprehensive_indicators(self, df: pd.DataFrame,
                                          target_columns: List[str] = None) -> Dict[str, Dict]:
        """
        為DataFrame計算全面的技術指標
        ...
        for column in target_columns:
            ...
            try:
                ...
                column_results['indicators']['sma'] = {
                    'values': sma,
                    'info': sma_info
                }
                column_results['indicators']['ema'] = {
                    'values': ema,
                    'info': ema_info
                }
                ...
                if 'rate' in column.lower() and 'yield' in column.lower():
                    ...
                    if len(data) > 100:  # 數據足夠長
                        ...
                        column_results['indicators']['yield_analysis'] = {
                            ...
                            'volatility_analysis': self._calculate_volatility_analysis(data)
                        }
                ...
                if 'error' not in data:
                    ...
                    if 'indicators' in data:
                        ...
                        if 'rsi' in indicators and 'info' in indicators['rsi']:
                            ...
                            print(f"    RSI pattern: {rsi_info.get('assessment', 'Unknown')}")

                        # 顯示MACD信息
                        if 'macd' in indicators and 'adaptation_info' in indicators['macd']:
                            macd_info = indicators['macd']['adaptation_info']
                            print(f"    MACD signal quality: {macd_info.get('signal_quality', {}).get('assessment', 'Unknown')}")
