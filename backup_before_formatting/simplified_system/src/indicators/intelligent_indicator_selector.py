...
class DataTypeClassifier:
    ...
    def classify_data(self, df: pd.DataFrame, source_name: str = None) -> DataCharacteristics:
        """
        分類數據類型並分析特徵
        ...
        self.logger.info(f"Classified {source_name}: {inferred_type.value}, "
    ...
    def _select_primary_numeric_column(self, df: pd.DataFrame, numeric_cols: List[str]) -> str:
        """選擇主要的數值列"""
        # 優先選擇看起來像主要指標的列
    ...
    def _is_rate_based_data(self, column_name: str, source_name: str) -> bool:
        ...
        return (any(keyword in column_lower for keyword in rate_keywords) or
                any(keyword in source_lower for keyword in rate_keywords))
    ...
    def _is_flow_data(self, column_name: str, source_name: str) -> bool:
        ...
        return (any(keyword in column_lower for keyword in flow_keywords) or
                any(keyword in source_lower for keyword in flow_keywords))
...
class IndicatorSuitabilityAssessor:
        ...
        return {
            ...
            'SMA': {
                'suitable_types': [DataType.INTEREST_RATE, DataType.EXCHANGE_RATE,
                                 DataType.MONETARY_BASE, DataType.YIELD_RATE],
            ...
            'MACD': {
                ...
                'base_score': 0.75,
                'adaptations': {
                    DataType.INTEREST_RATE: {'fast_range': (5, 15), 'slow_range': (20, 40)},
                    DataType.EXCHANGE_RATE: {'fast_range': (8, 20), 'slow_range': (25, 50)}
                }
            },
            ...
            'Bollinger_Bands': {
                ...
                'base_score': 0.7,
                'adaptations': {
                    DataType.EXCHANGE_RATE: {'period_range': (15, 25), 'std_range': (1.5, 2.5)},
                    DataType.YIELD_RATE: {'period_range': (10, 20), 'std_range': (1.8, 2.2)}
                }
            },
            'ATR': {
            ...
            'Rate_Spread': {
                ...
                'base_score': 0.9,
                'adaptations': {
                    DataType.INTEREST_RATE: {'tenor_pairs': [('1m', '3m'), ('3m', '12m')]},
                    DataType.YIELD_RATE: {'tenor_pairs': [('2y', '10y'), ('5y', '15y')]}
                }
            },
            'Yield_Curve': {
    ...
    def assess_suitability(self, indicator_name: str, data_characteristics: DataCharacteristics,
                          data_length: int) -> IndicatorSuitability:
        """
        評估指標適用性
        ...
        if 'high_volatility' in unsuitable_patterns and data_characteristics.volatility_level == 'high':
            volatility_penalty = 0.3

        # 趨勢適合性
        trend_penalty = 0
        if 'trending_only' in unsuitable_patterns and data_characteristics.trend_strength == 'weak':
            trend_penalty = 0.2
        elif 'smooth_trend' in unsuitable_patterns and data_characteristics.volatility_level == 'high':
            trend_penalty = 0.2

        # 計算最終適用性分數
        suitability_score = max(0, min(1, base_score + type_bonus + length_bonus - volatility_penalty - trend_penalty))
        ...
        if data_characteristics.data_type in suitable_types:
            adaptation_reasons.append(f"Suitable for {data_characteristics.data_type.value} data")
        else:
            limitations.append(f"Not optimized for {data_characteristics.data_type.value} data")

        if data_length >= data_characteristics.minimum_length_for_analysis:
            adaptation_reasons.append(f"Sufficient data length ({data_length} records)")
        else:
            limitations.append(f"Insufficient data length (need {data_characteristics.minimum_length_for_analysis} records)")

        if data_characteristics.volatility_level == 'high' and 'high_volatility' in unsuitable_patterns:
            limitations.append("High volatility may reduce indicator effectiveness")

        if data_characteristics.trend_strength == 'strong' and 'choppy_market' in unsuitable_patterns:
...
class ParameterAdaptationEngine:
    ...
    def adapt_parameters(self, indicator_name: str, data_characteristics: DataCharacteristics,
                        data_length: int, base_params: Dict = None) -> Dict[str, Any]:
        """
        適配指標參數
        ...
        if data_characteristics.data_type == DataType.INTEREST_RATE:
            adapted_params.update(self._adapt_for_interest_rate(data_characteristics, adaptation_factor))
        elif data_characteristics.data_type == DataType.EXCHANGE_RATE:
            adapted_params.update(self._adapt_for_exchange_rate(data_characteristics, adaptation_factor))
        elif data_characteristics.data_type == DataType.MONETARY_BASE:
            adapted_params.update(self._adapt_for_monetary_base(data_characteristics, adaptation_factor))
        elif data_characteristics.data_type == DataType.YIELD_RATE:
            adapted_params.update(self._adapt_for_yield_rate(data_characteristics, adaptation_factor))

        # 根據波動率調整
        if data_characteristics.volatility_level == 'high':
            adapted_params = self._adjust_for_high_volatility(indicator_name, adapted_params)
        elif data_characteristics.volatility_level == 'low':
            adapted_params = self._adjust_for_low_volatility(indicator_name, adapted_params)

        # 根據趨勢強度調整
        if data_characteristics.trend_strength == 'strong':
            adapted_params = self._adjust_for_strong_trend(indicator_name, adapted_params)

        self.logger.info(f"Adapted parameters for {indicator_name}: {adapted_params}")

        return adapted_params

    def _adapt_for_interest_rate(self, data_characteristics: DataCharacteristics, factor: float) -> Dict:
        """適配利率數據參數"""
        return {
    ...
    def _adapt_for_exchange_rate(self, data_characteristics: DataCharacteristics, factor: float) -> Dict:
        """適配匯率數據參數"""
        return {
    ...
    def _adapt_for_monetary_base(self, data_characteristics: DataCharacteristics, factor: float) -> Dict:
        """適配貨幣基礎數據參數"""
        return {
    ...
    def _adapt_for_yield_rate(self, data_characteristics: DataCharacteristics, factor: float) -> Dict:
        """適配收益率數據參數"""
        return {
            'rsi_period': max(10, int(14 * factor)),
            'macd_fast': max(8, int(12 * factor)),
...
class IntelligentIndicatorSelector:
    ...
    def select_indicators_for_data(self, df: pd.DataFrame, source_name: str,
                                  min_suitability_score: float = 0.5) -> List[IndicatorSuitability]:
        """
        為特定數據選擇適合的技術指標
        ...
        self.logger.info(f"Analyzing {source_name}: {data_characteristics.data_type.value}, "
        ...
        self.logger.info(f"Selected {len(suitable_indicators)} suitable indicators for {source_name}")

        return suitable_indicators

    def generate_recommendation_report(self, df: pd.DataFrame, source_name: str) -> Dict:
        """
        生成指標推薦報告
        ...
        """
        # 獲取適合的指標
        suitable_indicators = self.select_indicators_for_data(df, source_name, min_suitability_score=0.3)
        ...
        report = {
            ...
            'data_characteristics': {
                ...
                'length': len(df),
                'has_date_column': any(col.lower().find('date') != -1 for col in df.columns),
        ...
        # 頂級推薦（適用性分數 > 0.7）
        top_indicators = [ind for ind in suitable_indicators if ind.suitability_score > 0.7]
        ...
        # 不推薦的指標
        not_recommended = [ind for ind in suitable_indicators if ind.suitability_score < 0.5]
        ...
        report['summary'] = {
            ...
            'moderately_suitable_count': len([ind for ind in suitable_indicators if 0.5 <= ind.suitability_score <= 0.7]),
            'not_suitable_count': len(not_recommended),
            'best_indicator': top_indicators[0].indicator_name if top_indicators else None,
            'best_score': round(top_indicators[0].suitability_score, 3) if top_indicators else 0
        }
...
def main():
    ...
    for source_name, df in data_dict.items():
        ...
        try:
            ...
            print(f"Best indicator: {summary['best_indicator']} (score: {summary['best_score']})")

            # 顯示頂級推薦
            if report['top_recommendations']:
                print("\nTop recommendations:")
                for i, rec in enumerate(report['top_recommendations'][:3], 1):
                    print(f"  {i}. {rec['name']} (score: {rec['suitability_score']}, category: {rec['category']})")
    ...
    for source_name, report in all_reports.items():
        summary = report['summary']
        total_indicators += summary['highly_suitable_count'] + summary['moderately_suitable_count']
