...
class GPUTechnicalIndicators:
    ...
    def __init__(self, use_gpu: bool = True, gpu_device: int = 0):
        ...
        if self.use_gpu:
            try:
                ...
                logger.info(f"GPU Technical Indicators initialized with new GPU core on device {gpu_device}")
            except Exception as e:
                logger.error(f"GPU core initialization failed: {e}, falling back to CPU")
                self.use_gpu = False
                self.backend = 'cpu'
                self.gpu_core = None
        else:
        ...
        logger.info(f"GPU Technical Indicators initialized with backend: {self.backend}")
    ...
    def rsi(self, prices: Union[np.ndarray, pd.Series], period: int = 14) -> np.ndarray:
        ...
        if self.use_gpu and self.gpu_core is not None:
            try:
                ...
                logger.error(f"GPU core RSI calculation failed: {e}, falling back to CPU")
    ...
    def macd(self, prices: Union[np.ndarray, pd.Series],
        ...
        if self.use_gpu and self.gpu_core is not None:
            try:
                macd_gpu, signal_gpu, hist_gpu = self.gpu_core.calculate_macd_gpu(prices, fast, slow, signal)
                return {
                    'MACD': macd_gpu.get() if hasattr(macd_gpu, 'get') else np.array(macd_gpu),
                    'SIGNAL': signal_gpu.get() if hasattr(signal_gpu, 'get') else np.array(signal_gpu),
                    'HIST': hist_gpu.get() if hasattr(hist_gpu, 'get') else np.array(hist_gpu)
                }
            except Exception as e:
                logger.error(f"GPU core MACD calculation failed: {e}, falling back to CPU")
    ...
    def _bollinger_bands_gpu(self, prices: np.ndarray, period: int, std_dev: float) -> Dict[str, np.ndarray]:
        """GPU版本的布林帶計算"""
        try:
            ...
            middle_band = self.cp.concatenate([
                self.cp.full(period - 1, self.cp.nan),
                middle_band
            ])
    ...
    def calculate_multiple_indicators(self, prices: Union[np.ndarray, pd.Series],
        ...
        if 'kdj' in indicators_config:
            ...
            if hasattr(prices, '__len__') and len(prices.shape) == 1:
                ...
                kdj_results = self.kdj(prices, prices * 1.01, prices * 0.99, k_period, d_period)
                results.update({
                    'KDJ_K': kdj_results['K'],
                    'KDJ_D': kdj_results['D'],
                    'KDJ_J': kdj_results['J']
    ...
    def moving_average(self, prices: Union[np.ndarray, pd.Series], period: int = 20, ma_type: str = 'sma') -> np.ndarray:
        """
        計算移動平均指標（GPU加速版本）
        ...
        if self.use_gpu and self.gpu_core is not None:
            try:
                ma_gpu = self.gpu_core.calculate_moving_average_gpu(prices, period, ma_type)
                return ma_gpu.get() if hasattr(ma_gpu, 'get') else np.array(ma_gpu)
            except Exception as e:
                logger.error(f"GPU core moving average calculation failed: {e}, falling back to CPU")
                return self._moving_average_cpu(prices, period, ma_type)
        else:
            return self._moving_average_cpu(prices, period, ma_type)

    def _moving_average_cpu(self, prices: np.ndarray, period: int, ma_type: str) -> np.ndarray:
        """CPU版本的移動平均計算"""
    ...
    def kdj(self, close: Union[np.ndarray, pd.Series], high: Union[np.ndarray, pd.Series],
            low: Union[np.ndarray, pd.Series], k_period: int = 9, d_period: int = 3) -> Dict[str, np.ndarray]:
        """
        計算KDJ指標（GPU加速版本）
        ...
        if self.use_gpu and self.gpu_core is not None:
            try:
                k_gpu, d_gpu, j_gpu = self.gpu_core.calculate_kdj_gpu(high, low, close, k_period, d_period)
                return {
                ...
                logger.error(f"GPU core KDJ calculation failed: {e}, falling back to CPU")
                return self._kdj_cpu(close, high, low, k_period, d_period)
        else:
            return self._kdj_cpu(close, high, low, k_period, d_period)

    def _kdj_cpu(self, close: np.ndarray, high: np.ndarray, low: np.ndarray, k_period: int, d_period: int) -> Dict[str, np.ndarray]:
        """CPU版本的KDJ計算"""
        ...
        rsv = np.where(highest_high == lowest_low, 50.0, 100 * (close - lowest_low) / (highest_high - lowest_low))
