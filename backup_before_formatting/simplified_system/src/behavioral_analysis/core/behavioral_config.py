...
class HistoricalPatternConfig:
    ...
    similarity_methods: List[str] = field(default_factory=lambda: [
        "cosine", "dtw", "pearson", "euclidean", "manhattan"
    ])
...
class HongKongMarketConfig:
    ...
    hsi_constituents: List[str] = field(default_factory=lambda: [
        "0700.HK",  # Tencent
...
class BehavioralAnalysisConfig:
    ...
    intraday_pattern: IntradayPatternConfig = field(default_factory=IntradayPatternConfig)
    ml_models: MLModelConfig = field(default_factory=MLModelConfig)
    ensemble: EnsembleConfig = field(default_factory=EnsembleConfig)
    historical_pattern: HistoricalPatternConfig = field(default_factory=HistoricalPatternConfig)
    realtime_monitoring: RealTimeMonitoringConfig = field(default_factory=RealTimeMonitoringConfig)
    ...
    def __post_init__(self):
        """初始化后处理"""
        # 确保路径存在
        for path in [self.model_storage_path, self.baseline_storage_path,
                     self.pattern_storage_path, self.alert_storage_path]:
            path.mkdir(parents=True, exist_ok=True)

        # 验证权重总和
        total_weight = (self.ensemble.isolation_weight + self.ensemble.svm_weight +
                       self.ensemble.lof_weight + self.ensemble.rf_weight +
                       self.ensemble.gb_weight + self.ensemble.nn_weight)
...
if __name__ == "__main__":
    ...
    # 验证配置
    print(f"Total ensemble weights: {sum([custom_config.ensemble.isolation_weight, custom_config.ensemble.svm_weight, custom_config.ensemble.lof_weight, custom_config.ensemble.rf_weight, custom_config.ensemble.gb_weight, custom_config.ensemble.nn_weight]):.3f}")
