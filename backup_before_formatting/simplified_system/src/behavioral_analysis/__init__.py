...
__description__ = "Advanced behavioral analysis layer for financial market anomaly detection"
...
class BehavioralAnalysisPipeline:
    ...
    def __init__(self, config: Optional[BehavioralAnalysisConfig] = None):
        ...
        self.anomaly_detector = MLAnomalyDetector(self.config.ensemble, self.config.ml_models)
        self.realtime_monitor = RealTimeBehaviorMonitor(self.config.realtime_monitoring, self.anomaly_detector)
    ...
    def analyze_historical_patterns(self,
        ...
        try:
            ...
            # 时间序列模式分析
            results['time_series_patterns'] = self.ts_analyzer.analyze_patterns(price_data, symbol)

            # 盘中模式分析（如果有日内数据）
            if isinstance(price_data.index, pd.DatetimeIndex):
                results['intraday_patterns'] = self.intraday_analyzer.recognize_intraday_patterns(price_data, symbol)
            ...
            # 香港市场特定分析
            results['hk_market_insights'] = self._analyze_hk_market_specific_patterns(price_data)
    ...
    def _prepare_features_for_anomaly_detection(self, price_data: pd.Series) -> Optional[np.ndarray]:
        """为异常检测准备特征数据"""
        try:
            ...
            for window in windows:
                if len(price_data) >= window:
                    ...
                    features.extend([
                        rolling_mean.dropna(),
                        rolling_std.dropna(),
                        (price_data - rolling_min) / (rolling_max - rolling_min).dropna(),  # Relative position
                    ])
            ...
            aligned_features = [f.iloc[-min_length:].values for f in features if len(f) >= min_length]
    ...
    def _analyze_hk_market_specific_patterns(self, price_data: pd.Series) -> Dict[str, Any]:
        """分析香港市场特定模式"""
        try:
            hk_insights = {}
        ...
        try:
            ...
            if 'time_series_patterns' in results:
                ...
                if 'pattern_summary' in ts_patterns:
                    ...
                    if trend_strength in ['strong', 'moderate']:
                        ...
                        summary['key_insights'].append(f"Clear {direction} trend detected")
            ...
            if 'anomaly_detection' in results:
                ...
                if 'predictions' in anomaly_results:
                    ...
                    summary['overall_health_score'] = max(0, 1.0 - anomaly_rate * 2)  # 简单的健康评分

                    if anomaly_rate > 0.1:  # 10%以上异常率
                        summary['risk_level'] = 'high'
                        summary['recommendations'].append("High anomaly rate detected, investigate further")
            ...
            if 'intraday_patterns' in results:
                ...
                if 'pattern_summary' in intraday:
                    momentum = intraday['pattern_summary'].get('momentum_signal', 'neutral')
                    if momentum != 'neutral':
                        summary['key_insights'].append(f"{momentum.capitalize()} momentum detected")
            ...
            if 'hk_market_insights' in results:
                ...
                if 'month_end_effect' in hk_insights:
                    ...
                    if abs(month_end.get('avg_return', 0)) > 0.02:
                        summary['key_insights'].append("Significant month-end effect detected")
...
def create_behavioral_analyzer(market_regime: str = 'normal',
                              volatility_level: str = 'normal',
                              analysis_focus: str = 'comprehensive') -> BehavioralAnalysisPipeline:
    """
    创建行为分析器的便捷函数
...
def quick_pattern_analysis(price_data: pd.Series,
                          symbol: str = "UNKNOWN",
                          include_realtime: bool = False) -> Dict[str, Any]:
    """
    快速模式分析的便捷函数
