...
class TestPhase2SourceAuthentication:
    ...
    async def test_health_check(self, phase2_auth):
        ...
        assert health['phase2_authentication']['status'] in ['healthy', 'degraded', 'unhealthy']
    ...
    async def test_hkma_data_authentication(self, phase2_auth):
        ...
        result = await phase2_auth.authenticate_hkma_data(
            hkma_data, "test_hkma_001"
        )

        assert isinstance(result, AuthResult)
        assert result.data_id == "test_hkma_001"
        assert result.overall_verdict in [Verdict.AUTHENTIC, Verdict.SUSPICIOUS, Verdict.UNKNOWN, Verdict.ERROR]
    ...
    async def test_stock_data_authentication(self, phase2_auth):
        ...
        result = await phase2_auth.authenticate_stock_data(
...
class TestDigitalSignatureVerifier:
    ...
    async def test_invalid_signature_verification(self, ds_verifier):
        ...
        assert isinstance(result, AuthResult)
        assert result.overall_verdict in [Verdict.SUSPICIOUS, Verdict.FALSIFIED, Verdict.ERROR]
...
class TestRateLimitAnomalyDetector:
    ...
    async def test_rate_limit_exceeded(self, rl_detector):
        ...
        throttled_or_blocked = any(
            r.overall_verdict in [Verdict.SUSPICIOUS, Verdict.FALSIFIED] for r in results
        )
...
class TestIntegrationScenarios:
    ...
    async def test_full_authentication_flow(self):
        ...
        result = await auth.authenticate_api_request(request_info, "integration_test_002")
        assert isinstance(result, AuthResult)
