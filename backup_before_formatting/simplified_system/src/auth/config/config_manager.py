...
class ConfigChangeHandler(FileSystemEventHandler if FileSystemEventHandler is not None else object):
    """文件变更处理器"""
...
class ConfigManager:
    ...
    def _setup_file_watcher(self):
        ...
        if not WATCHDOG_AVAILABLE:
            logger.warning("File watching not available - install watchdog package for hot-reload functionality")
            return
    ...
    def update_verifier_config(self, verifier_type: str, config: Dict[str, Any], persist: bool = False) -> bool:
        """
        更新验证器配置
