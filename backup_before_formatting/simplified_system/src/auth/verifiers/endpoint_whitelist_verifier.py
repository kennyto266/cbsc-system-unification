...
class EndpointWhitelistVerifier(IVerifier):
    ...
    def __init__(self, name: str = "Endpoint Whitelist Verifier", config: Optional[Dict[str, Any]] = None):
        super().__init__(name, config)

        # Whitelist storage
        self.whitelist_path = self.config.get('whitelist_path', 'config/whitelist.json')
        self.approval_queue_path = self.config.get('approval_queue_path', 'config/approval_queue.json')
        ...
        self.suspicious_tlds = self.config.get('suspicious_tlds', [
        ...
        logger.info(f"Endpoint Whitelist Verifier initialized with {len(self.whitelist)} whitelisted endpoints")
        ...
        try:
            ...
            if whitelist_file.exists():
                with open(whitelist_file, 'r', encoding='utf-8') as f:
                    ...
                    # Merge with defaults
                    self.whitelist = {**self.default_trusted_endpoints, **loaded_whitelist}
            else:
    ...
    async def verify(self, data: Any, data_id: str, context: Optional[Dict[str, Any]] = None) -> AuthResult:
        ...
        try:
            ...
            # Check whitelist
            whitelist_result = await self._check_whitelist(hostname, port, endpoint_info)
            ...
            if (result.overall_verdict in [Verdict.AUTHENTIC, Verdict.SUSPICIOUS] and
                self.enable_dns_validation and DNS_AVAILABLE):

                dns_result = await self._validate_dns_records(hostname, whitelist_result['whitelist_entry'])
                ...
                if dns_result['valid']:
                    ...
                    result.overall_verdict = Verdict.SUSPICIOUS
                    result.overall_confidence = max(result.overall_confidence - 0.2, 0.3)

            # Perform health check if endpoint is approved
            if (result.overall_verdict == Verdict.AUTHENTIC and
                REQUESTS_AVAILABLE and
                endpoint_info.get('check_health', False)):

                health_result = await self._check_endpoint_health(hostname, port, endpoint_info)
                result.metadata['health_check'] = health_result

                if not health_result['healthy']:
                    result.overall_confidence = max(result.overall_confidence - 0.1, 0.6)

            logger.info(f"Endpoint whitelist verification completed for {hostname}: {result.overall_verdict.value}")
    ...
    def _extract_endpoint_info(self, data: Any, context: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if isinstance(data, str):
            if data.startswith('http://') or data.startswith('https://'):
                ...
                if ':' in host_part:
                    ...
                    endpoint_info['port'] = 443 if endpoint_info['protocol'] == 'https' else 80
            else:
                # Raw hostname
    ...
    async def _validate_endpoint_basic(self, hostname: str, port: Optional[int]) -> Dict[str, Any]:
        """Perform basic endpoint validation"""
        try:
            ...
            if self.block_private_ips:
                try:
                    ...
                    if ip.is_private or ip.is_loopback or ip.is_link_local:
                        if not any(localhost in hostname for localhost in ['localhost', '127.0.0.1']):
                            return {
                                'valid': False,
                                'error': f'Private IP address not allowed: {hostname}',
                                'metadata': {'ip_type': 'private', 'ip_address': str(ip)}
                            }
                except ValueError:
    ...
    async def _check_whitelist(self, hostname: str, port: Optional[int], endpoint_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
                        ...
                        if str(port) not in whitelist_entry['port_specific']:
                            return {
                                ...
                                'metadata': {
                                    ...
                                    'port_allowed': False,
                                    'allowed_ports': list(whitelist_entry['port_specific'].keys())
                                }
                            }
            ...
            for whitelist_hostname, whitelist_entry in self.whitelist.items():
                if whitelist_entry.get('approved', False):
                    ...
                    # Check subdomain
                    if whitelist_hostname.startswith('*.') and hostname.endswith(whitelist_hostname[1:]):
                        return {
                            'verdict': Verdict.AUTHENTIC,
                            'confidence': 0.8,
                            'whitelist_entry': whitelist_entry,
            ...
            return {
                ...
                'whitelist_entry': None,
                'metadata': {
                    'error': f'Whitelist check error: {str(e)}'
                }
            }

    async def _validate_dns_records(self, hostname: str, whitelist_entry: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    async def _check_endpoint_health(self, hostname: str, port: Optional[int], endpoint_info: Dict[str, Any]) -> Dict[str, Any]:
