...
from .content_validation_layer import ValidationSeverity, AnomalyType, AnomalyDetection
...
class StatisticalAnomalyDetector(IVerifier):
            ...
            result = AuthResult(
                ...
                metadata={
                    ...
                    'summary': {
                        ...
                        'total_anomalies': len(all_anomalies),
                        'overall_anomaly_rate': len(all_anomalies) / len(df) if len(df) > 0 else 0
                    }
                }
            )

            logger.info(f"Statistical anomaly detection completed for {data_id}: {verdict.value} "
                       f"(confidence: {confidence:.3f}, anomalies: {len(all_anomalies)})")
            return result

        except Exception as e:
            logger.error(f"Statistical anomaly detection failed for {data_id}: {str(e)}")
            execution_time = (time.time() - start_time) * 1000
            return AuthResult(
                data_id=data_id,
                data_type="unknown",
        ...
        try:
            ...
            for col in numeric_cols:
                ...
                for idx in outlier_indices:
                    timestamp = df.loc[idx, 'timestamp'] if 'timestamp' in df.columns else df.loc[idx, 'date'] if 'date' in df.columns else idx
                    ...
                    anomaly = AnomalyDetection(
                        ...
                        severity=ValidationSeverity.HIGH if outlier_mask.sum() > len(values) * 0.05 else ValidationSeverity.MEDIUM,
                        confidence=0.8,
                        context={
                            'method': 'IQR',
            ...
            anomaly_rate = len(anomalies) / (len(df) * len(numeric_cols)) if len(numeric_cols) > 0 else 0
            ...
            return AnomalyResult(
                ...
                details={
                    ...
                    'iqr_multiplier': self.iqr_multiplier,
                    'anomalies_per_column': {col: sum(1 for a in anomalies if a.field_name == col) for col in numeric_cols}
                }
            )
        ...
        try:
            ...
            for col in numeric_cols:
                ...
                for idx in outlier_indices:
                    ...
                    anomaly = AnomalyDetection(
                        ...
                        severity=ValidationSeverity.HIGH if z_scores[values.index.get_loc(idx)] > 4 else ValidationSeverity.MEDIUM,
                        confidence=0.85,
                        context={
                            'method': 'Z-Score',
            ...
            return AnomalyResult(
                ...
                details={
                    ...
                    'max_z_score': float(max([a.z_score or 0 for a in anomalies])) if anomalies else None
                }
            )
    ...
    async def _detect_isolation_forest_anomalies(self, df: pd.DataFrame) -> AnomalyResult:
        ...
        try:
            ...
            for idx in anomaly_indices:
                ...
                most_anomalous_feature = numeric_cols[np.argmax(np.abs(feature_values.values - feature_values.mean()))]
                ...
                anomaly = AnomalyDetection(
                    ...
                    isolation_score=float(anomaly_scores[feature_data.index.get_loc(idx)]),
                    severity=ValidationSeverity.HIGH if anomaly_scores[feature_data.index.get_loc(idx)] < -0.5 else ValidationSeverity.MEDIUM,
                    confidence=0.9,
                    context={
                        'method': 'Isolation Forest',
                        'contamination': self.isolation_forest_contamination,
                        'all_features': {col: df.loc[idx, col] for col in numeric_cols if not pd.isna(df.loc[idx, col])}
                    }
                )
                anomalies.append(anomaly)

            anomaly_rate = len(anomalies) / len(df)
            confidence = max(0.5, 1.0 - abs(anomaly_rate - self.isolation_forest_contamination))
        ...
        try:
            ...
            if not available_price_cols:
                return AnomalyResult(
                    ...
                    confidence=1.0,
                    execution_time_ms=(time.time() - start_time) * 1000,
                    details={'message': 'No price/volume columns for volatility analysis'}
                )
            ...
            for col in available_price_cols:
                ...
                for idx in extreme_indices:
                    ...
                    # 检查对应的绝对价格变动
                    if idx > 0:
                        price_change_pct = abs((df.loc[idx, col] - df.loc[idx-1, col]) / df.loc[idx-1, col]) if df.loc[idx-1, col] != 0 else 0
                    else:
                        price_change_pct = 0
                    ...
                    anomaly = AnomalyDetection(
                        ...
                        severity=ValidationSeverity.CRITICAL if price_change_pct > 0.1 else ValidationSeverity.HIGH,
                        confidence=0.8,
                        context={
            ...
            return AnomalyResult(
                ...
                details={
                    ...
                    'max_volatility': float(rolling_vol.max()) if 'rolling_vol' in locals() else None
                }
            )
        ...
        try:
            ...
            for col in numeric_cols:
                ...
                if abs(skewness) > 2:
                    ...
                    timestamp = df.loc[extreme_idx, 'timestamp'] if 'timestamp' in df.columns else df.loc[extreme_idx, 'date'] if 'date' in df.columns else extreme_idx
                ...
                if not (is_normal_jb or is_normal_da):
                    try:
                        ...
                        if n_clusters > 2:
                            ...
                            small_clusters = cluster_sizes[cluster_sizes < len(values) * 0.1].index
                        ...
                        logger.debug(f"Clustering analysis failed for {col}: {str(cluster_error)}")
            ...
            return AnomalyResult(
                ...
                details={
                    'columns_analyzed': list(numeric_cols),
                    'distribution_tests': ['jarque_bera', 'normaltest', 'skewness', 'kurtosis'],
                    'anomalies_by_type': {
                        'skewness': sum(1 for a in anomalies if a.context.get('distribution_test') == 'skewness_analysis'),
                        'kurtosis': sum(1 for a in anomalies if a.context.get('distribution_test') == 'kurtosis_analysis'),
                        'multimodal': sum(1 for a in anomalies if a.context.get('distribution_test') == 'multimodal_detection')
                    }
                }
            )
...
# Factory function
def create_statistical_anomaly_detector(config: Optional[Dict[str, Any]] = None) -> StatisticalAnomalyDetector:
    """创建统计异常检测器"""
