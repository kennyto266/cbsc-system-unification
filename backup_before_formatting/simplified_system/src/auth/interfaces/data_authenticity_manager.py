...
class DataAuthenticityManager:
    ...
    async def verify_data(
        ...
        try:
            ...
            logger.info(f"Starting verification for {data_id} using {len(verifiers_to_use)} verifiers")
            ...
            result.error_message = f"Verification timeout after {timeout or self.default_timeout}s"
    ...
    async def verify_batch(
        ...
        results = await asyncio.gather(
            *[verify_with_semaphore(item) for item in data_list],
    ...
    async def _execute_sequential(
        ...
        for verifier in verifiers:
            try:
                layer = await self._execute_single_verifier(verifier, data, data_id, context)
                layers.append(layer)
            except Exception as e:
    ...
    def _calculate_overall_result(self, layers: List[VerificationLayer]) -> tuple[Verdict, float]:
        """计算综合验证结果"""
        if not layers:
