...
def quick_signal_analysis(symbol: str, data: 'pd.DataFrame', indicators: dict) -> dict:
    ...
    # 生成綜合信號
    composite_signal = composite_signal_generator.generate_composite_signal(symbol, data, indicators)
...
def get_signal_recommendations(symbol: str, signal: CompositeSignal) -> list:
    ...
    if signal.signal_type == SignalType.BUY:
        recommendations.extend([
            f"立即買入 {symbol}",
            f"目標價格: {signal.target_price:.2f}" if signal.target_price else "未設置目標價格",
            f"止損價格: {signal.stop_loss:.2f}" if signal.stop_loss else "未設置止損價格",
...
def get_system_performance() -> dict:
    ...
    return {
        'signal_generator': {
            ...
            'average_confidence': np.mean([s.confidence for s in signal_generator.signal_history]) if signal_generator.signal_history else 0,
            'top_performing_indicators': signal_generator.get_signal_summary(signal_generator.signal_history)
        },
        'conflict_resolver': {
            ...
            'average_resolution_time': np.mean([
                perf.resolution_time for perf in conflict_resolver.strategy_performance.values()
                if perf.resolution_time > 0
            ]) if conflict_resolver.strategy_performance else 0
        },
        'composite_generator': {
            'total_composite_signals': len(composite_signal_generator.composite_signal_history),
            'average_quality_score': np.mean([
                s.quality_score for s in composite_signal_generator.composite_signal_history
            ]) if composite_signal_generator.composite_signal_history else 0,
            'success_rate': np.mean([
                s.success_probability for s in composite_signal_generator.composite_signal_history
            ]) if composite_signal_generator.composite_signal_history else 0
        }
    }
