...
class DynamicWeightManager:
    ...
    def create_portfolio(
        ...
        # 使用自定義權重或全局權重
        weight_source = custom_weights if custom_weights else self.config['default_weights']
    ...
    def update_weights(
        ...
        return {name: weight.current_weight for name, weight in portfolio.weights.items()}
    ...
    def _update_performance_metrics(
        ...
        for indicator, perf in performance_data.items():
            if indicator in portfolio.weights:
                ...
                # 記錄性能歷史
                performance_score = (weight.accuracy * 0.4 +
                ...
                if len(weight.performance_history) > max_history:
                    weight.performance_history = weight.performance_history[-max_history:]

    def _performance_based_rebalance(self, portfolio: WeightPortfolio) -> Dict[str, IndicatorWeight]:
        """基於性能的重平衡"""
        new_weights = {}
        ...
        for indicator, weight in weights.items():
            if weight.performance_history:
                ...
                # 應用性能衰減因子
                decay_factor = self.config['performance_decay'] ** len(weight.performance_history)
    ...
    def _risk_parity_rebalance(self, portfolio: WeightPortfolio) -> Dict[str, IndicatorWeight]:
        """風險平價重平衡"""
        weights = portfolio.weights.copy()
    ...
    def _momentum_based_rebalance(self, portfolio: WeightPortfolio) -> Dict[str, IndicatorWeight]:
        """基於動量的重平衡"""
        weights = portfolio.weights.copy()
        ...
        for indicator, weight in weights.items():
            if len(weight.performance_history) >= 3:
                ...
                older_performance = np.mean(weight.performance_history[-10:-3]) if len(weight.performance_history) >= 10 else np.mean(weight.performance_history[:-3])
    ...
    def _regime_based_rebalance(self, portfolio: WeightPortfolio) -> Dict[str, IndicatorWeight]:
        """基於市場狀態的重平衡"""
        weights = portfolio.weights.copy()
        ...
        if avg_performance > 0.6:  # 牛市
            ...
            for indicator, weight in weights.items():
                if indicator in ['SMA', 'EMA', 'MACD']:
                    weight.current_weight = weight.base_weight * (1 + regime_sensitivity)
                else:
                    weight.current_weight = weight.base_weight * (1 - regime_sensitivity * 0.5)
    ...
    def _volatility_adjusted_rebalance(self, portfolio: WeightPortfolio) -> Dict[str, IndicatorWeight]:
        """基於波動率調整的重平衡"""
        weights = portfolio.weights.copy()
        ...
        for indicator, weight in weights.items():
            # 基於穩定性調整權重
            stability_adjustment = weight.stability * risk_adjustment_factor + (1 - risk_adjustment_factor)
    ...
    def _apply_weight_constraints(self, weights: Dict[str, IndicatorWeight]) -> Dict[str, IndicatorWeight]:
        """應用權重約束條件"""
        min_weight = self.config['weight_constraints']['min_weight']
        max_weight = self.config['weight_constraints']['max_weight']
        ...
        # 應用最小最大權重約束
        for weight in weights.values():
            weight.current_weight = max(min_weight, min(max_weight, weight.current_weight))
    ...
    def _update_portfolio_weights(
        self,
        portfolio: WeightPortfolio,
        new_weights: Dict[str, IndicatorWeight]
    ):
        """更新投資組合權重"""
        ...
        # 重新計算總權重
        portfolio.total_weight = sum(w.current_weight for w in portfolio.weights.values())
    ...
    def get_global_weights(self) -> Dict[str, float]:
        ...
        return {
            name: weight.current_weight
            for name, weight in self.global_weights.items()
        }
    ...
    def analyze_weight_performance(self, symbol: str) -> Dict[str, Any]:
        ...
        for indicator, weight in portfolio.weights.items():
            indicator_analysis = {
                ...
                'performance_trend': self._calculate_trend(weight.performance_history) if weight.performance_history else 0,
                'volatility': np.std(weight.performance_history) if len(weight.performance_history) > 1 else 0,
    ...
    def rebalance_all_portfolios(self, performance_data: Dict[str, Dict[str, Dict[str, float]]]):
        """
        重平衡所有投資組合
    ...
    def export_weights(self, filepath: str):
        ...
        export_data = {
            ...
            'portfolio_weights': {
                symbol: self.get_weights(symbol)
