...
class DataValidator:
    ...
    def validate_stock_data(self, symbol: str = "0700.hk") -> Dict[str, Any]:
        ...
        try:
            ...
            if validation_result["is_valid"]:
                ...
                logger.info(f"Stock data validation PASSED for {symbol}, quality: {quality_score:.2f}")
            else:
    ...
    def _validate_government_endpoint(self, source_key: str, source_info: Dict) -> Dict[str, Any]:
        """
        验证政府数据端点
        ...
        try:
            ...
            if records:
                # 数据质量评分
                quality_score = self._calculate_government_data_quality(records, source_key)
                return {
                    "source": source_key,
                    "url": source_info["url"],
                    "is_valid": True,
    ...
    def _validate_stock_data_format(self, data: Dict, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(numeric_prices) < 10:
                validation_result["issues"].append("Insufficient price data (less than 10 records)")
            ...
            if min_price <= 0 or max_price > 100000:
                validation_result["issues"].append(f"Price range suspicious: {min_price:.2f} - {max_price:.2f}")
    ...
    def _calculate_government_data_quality(self, records: List[Dict], source_key: str) -> float:
        """
        计算政府数据质量评分
        ...
        try:
            ...
            if records and 'end_of_day' in records[0]:
                latest_date = datetime.fromisoformat(records[0]['end_of_day'].replace('Z', '+00:00'))
                days_old = (datetime.now(latest_date.tzinfo) - latest_date).days
    ...
    def generate_validation_report(self) -> Dict[str, Any]:
        ...
        for source_key, source_info in self.verified_sources.items():
            if source_key == "stock_api":
                ...
                if source_key in gov_results and gov_results[source_key].get("is_valid"):
                    valid_count += 1
        ...
        report["summary"]["average_quality_score"] = total_quality / valid_count if valid_count > 0 else 0.0
        ...
        if invalid_sources > 0:
            report["recommendations"].append(f"有{invalid_sources}个数据源无效，需要检查网络连接或API配置")
...
if __name__ == "__main__":
    ...
    print(f"Stock data validation: {stock_result['is_valid']}, quality: {stock_result.get('quality_score', 0):.2f}")

    # 验证政府数据
    print("\n2. Validating government data...")
    gov_results = validator.validate_government_data()
    for source, result in gov_results.items():
        print(f"{source}: {result['is_valid']}, quality: {result.get('quality_score', 0):.2f}")

    # 生成报告
    print("\n3. Generating validation report...")
    report = validator.generate_validation_report()
    print(f"Overall status: {report['overall_status']}")
    print(f"Valid sources: {report['summary']['valid_sources']}/{report['summary']['total_sources']}")
