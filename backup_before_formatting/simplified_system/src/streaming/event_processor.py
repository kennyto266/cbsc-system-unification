    ...
    async def handle_event(self, event: Event) -> bool:
        ...
        try:
            ...
            self.average_processing_time = (
                (self.average_processing_time * (self.processed_count - 1) + processing_time) /
                self.processed_count
            )

            logger.debug(f"Handler {self.handler_id} processed event {event.id} in {processing_time:.4f}s")
            return True

        except Exception as e:
            self.error_count += 1
            logger.error(f"Handler {self.handler_id} failed to process event {event.id}: {e}")
    ...
    async def put(self, event: Event) -> bool:
        ...
        async with self._lock:
            if self._total_size >= self.max_size:
                ...
                if not removed:
                    logger.warning("Event queue is full and no low priority events to remove")
                    return False
...
class EventProcessor:
        ...
        while self._running:
            try:
                if self.retry_queue:
                    ...
                    # 添加延迟后重新放入主队列
                    await asyncio.sleep(min(event.retry_count * 2, 30))  # 指数退避，最大30秒
    ...
    async def publish_price_update(self,
                                  symbol: str,
                                  price: float,
                                  volume: int = 0,
    ...
    async def publish_technical_signal(self,
                                      symbol: str,
                                      signal_type: str,
                                      signal_value: float,
                                      confidence: float,
    ...
    async def publish_risk_alert(self,
                                symbol: str,
                                alert_type: str,
                                alert_level: str,
                                message: str,
...
if __name__ == "__main__":
    async def test_event_processor():
        ...
        for i in range(10):
            ...
            await processor.publish_technical_signal("0700.HK", "BUY", 0.8, 0.9, source="test")
