...
class RealTimeDataStreamer:
    ...
    def __init__(self, max_concurrent_requests: int = 50):
        ...
        # 性能配置
        self.update_interval = perf_config.caching.get('api_cache_ttl', 5)  # 默认5秒更新
        ...
        logger.info(f"RealTime Data Streamer initialized with {max_concurrent_requests} concurrent requests")
    ...
    async def _initialize_historical_data(self, symbols: List[str]) -> None:
        ...
        logger.info(f"Historical data initialized for {len(self._historical_data)} symbols")

    async def _fetch_historical_data(self, symbol: str, duration_days: int = 30) -> None:
        """获取历史数据"""
        try:
    ...
    async def _update_single_stock_price(self, symbol: str) -> None:
        ...
        try:
            ...
            if symbol in self._price_cache:
                ...
                tick.change_percent = (tick.change / last_price) * 100 if last_price > 0 else 0
    ...
    async def _update_historical_data(self, symbol: str, price: float, timestamp: datetime) -> None:
        """更新历史数据"""
        ...
        self._historical_data[symbol] = pd.concat([self._historical_data[symbol], new_row])

        # 保持数据长度限制
        if len(self._historical_data[symbol]) > self.max_history_length:
            self._historical_data[symbol] = self._historical_data[symbol].tail(self.max_history_length)
    ...
    async def _indicator_calculation_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                tasks = []
                for symbol in self._watched_symbols:
                    if symbol in self._historical_data and len(self._historical_data[symbol]) >= 14:
    ...
    def _calculate_indicators_sync(self, df: pd.DataFrame, symbol: str, timestamp: datetime) -> TechnicalIndicators:
        """同步计算技术指标（在线程池中执行）"""
        prices = df['price']
        ...
        while self._running:
            try:
                ...
                if self._stats['last_update_time']:
                    time_diff = (datetime.now() - self._stats['last_update_time']).total_seconds()
                    if time_diff > 0:
                        self._stats['requests_per_second'] = len(self._watched_symbols) / time_diff

                # 记录性能指标
                if self._stats['total_updates'] % 100 == 0 and self._stats['total_updates'] > 0:
                    logger.info(f"Performance stats - Updates: {self._stats['total_updates']}, "
    ...
    def get_all_latest_data(self) -> Dict[str, Dict[str, Any]]:
        ...
        for symbol in self._watched_symbols:
            ...
            if symbol in self._indicator_cache:
                ...
                symbol_data['indicators']['timestamp'] = self._indicator_cache[symbol].timestamp.isoformat()
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self._stats,
            'watched_symbols_count': len(self._watched_symbols) if hasattr(self, '_watched_symbols') else 0,
