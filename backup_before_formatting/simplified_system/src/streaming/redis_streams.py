...
class RedisStreamManager:
    ...
    async def write_batch(self,
                         stream_name: str,
                         messages: List[Dict[str, Any]]) -> List[Optional[str]]:
        """
        批量写入消息
        ...
        try:
            ...
            logger.info(f"Batch written {len(messages)} messages to stream {stream_name}")
            return message_ids
    ...
    async def read_from_group(self,
                             stream_name: str,
                             group_name: str,
                             consumer_name: str,
                             count: int = 10,
    ...
    async def get_group_info(self, stream_name: str, group_name: str) -> Optional[Dict[str, Any]]:
        """
        获取消费者组信息
    ...
    async def start_stream_reader(self,
                                 stream_name: str,
                ...
                try:
                    if consumer_group and consumer_name:
                        ...
                        messages = await self.read_from_group(
                            ...
                            count=self.batch_size, block=True
                        )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self._stats,
            ...
            'active_consumer_groups': sum(len(groups) for groups in self.consumer_groups.values()),
...
if __name__ == "__main__":
    async def test_redis_streams():
        ...
        try:
            ...
            # 写入测试消息
            test_messages = [
                {'type': 'price_update', 'symbol': '0700.HK', 'price': 300.0, 'timestamp': datetime.now()},
                {'type': 'price_update', 'symbol': '0941.HK', 'price': 50.0, 'timestamp': datetime.now()},
                {'type': 'signal', 'symbol': '0700.HK', 'signal': 'BUY', 'confidence': 0.8}
            ]
            ...
            # 读取消费者组消息
            group_messages = await manager.read_from_group(stream_name, "test_group", "test_consumer")
            print(f"Read {len(group_messages)} messages from group:")
            for msg in group_messages:
                print(f"  {msg}")
                await manager.acknowledge_message(stream_name, "test_group", msg['message_id'])
