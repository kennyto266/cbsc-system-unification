...
class AdvancedRiskAnalyzer:
    ...
    def _default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'stress_scenarios': ['financial_crisis_2008', 'market_crash_2020', 'interest_rate_shock'],
    ...
    def analyze_comprehensive_risk(
        ...
        try:
            ...
            overall_risk_level = self.assess_overall_risk_level(
                basic_risk_metrics, risk_decomposition, concentration_risk, liquidity_risk
            )
            ...
            analysis_results = {
                ...
                'risk_decomposition': self._serialize_risk_decomposition(risk_decomposition),
        ...
        try:
            ...
            if portfolio_positions is not None and not portfolio_positions.empty:
                ...
                if 'asset' in portfolio_positions.columns:
                    asset_weights = portfolio_positions.set_index('asset')['weight'].to_dict()
                    asset_contributions = asset_weights

                # 貨幣暴露
                if 'currency' in portfolio_positions.columns:
                    currency_weights = portfolio_positions.groupby('currency')['weight'].sum()
    ...
    def analyze_concentration_risk(
        ...
        try:
            ...
            if 'sector' in portfolio_positions.columns:
                sector_weights = portfolio_positions.groupby('sector')['weight'].sum()
                single_sector_max = sector_weights.max() if len(sector_weights) > 0 else 0.0
            ...
            if 'currency' in portfolio_positions.columns:
                currency_weights = portfolio_positions.groupby('currency')['weight'].sum()
                single_currency_max = currency_weights.max() if len(currency_weights) > 0 else 0.0
                concentration_analysis['single_currency_max'] = single_currency_max
            else:
                concentration_analysis['single_currency_max'] = 0.0

            # 集中度風險評分
            concentration_score = self._calculate_concentration_score(concentration_analysis)
            concentration_level = self._assess_concentration_level(concentration_score)

            # 風險建議
            recommendations = self._generate_concentration_recommendations(concentration_analysis)
    ...
    def analyze_liquidity_risk(
        ...
        try:
            ...
            for _, position in portfolio_positions.iterrows():
                ...
                if 'asset_type' in position:
                    ...
                    if 'stock' in asset_type:
                        ...
                        volume_ratio = np.random.uniform(0.01, 0.1)  # 1-10% of daily volume
                        market_cap_score = np.random.uniform(0.5, 1.0)  # Market cap score
            ...
            if total_weight > 0:
                liquidity_analysis['bid_ask_spread_avg'] = weighted_spread / total_weight
                liquidity_analysis['daily_volume_ratio'] = weighted_volume_ratio / total_weight
                liquidity_analysis['market_cap_coverage'] = weighted_market_cap / total_weight
            ...
            liquidity_analysis['time_to_liquidate'] = {
                ...
                '30_days': self._estimate_liquidation_percentage(portfolio_positions, 30)
            }
            ...
            liquidity_analysis['recommendations'] = self._generate_liquidity_recommendations(liquidity_analysis)
    ...
    def check_risk_alerts(
        ...
        try:
            ...
            for category_dict in risk_dict.values():
                for metric, value in category_dict.items():
                    if isinstance(value, (int, float)):
                        alert = self._check_metric_threshold(metric, value, current_time)
                        if alert:
                            alerts.append(alert)
            ...
            if portfolio_positions is not None:
                concentration_risk = self.analyze_concentration_risk(portfolio_positions)
                if concentration_risk['concentration_score'] > 0.7:  # High concentration
                    alert = RiskAlert(
    ...
    def check_regulatory_compliance(
        ...
        try:
            ...
            if 'sector' in portfolio_positions.columns:
                sector_weights = portfolio_positions.groupby('sector')['weight'].sum()
                max_sector_weight = sector_weights.max() if len(sector_weights) > 0 else 0
            ...
            if 'currency' in portfolio_positions.columns:
                currency_weights = portfolio_positions.groupby('currency')['weight'].sum()
                max_currency_weight = currency_weights.max() if len(currency_weights) > 0 else 0
            ...
            if risk_metrics is not None:
                ...
                # 最大回撤限制檢查
                max_dd = risk_dict['drawdown_metrics']['max_drawdown'] / 100  # Convert to decimal
        ...
        try:
            dashboard_data = {
                'overview': {
                    ...
                    'compliance_status': analysis_results['regulatory_compliance']['compliant']
                },
    ...
    def _calculate_beta(self, returns: pd.Series, market_returns: pd.Series) -> float:
        """計算Beta係數"""
        aligned_data = pd.concat([returns, market_returns], axis=1, join='inner').dropna()
    ...
    def _calculate_concentration_score(self, concentration_analysis: Dict[str, Any]) -> float:
        """計算集中度風險評分"""
        score = 0.0
        ...
        # 單一部門暴露 (30%權重)
        single_sector_score = min(concentration_analysis['single_sector_max'] / 0.40, 1.0)
        score += single_sector_score * 0.3

        # 單一貨幣暴露 (20%權重)
        single_currency_score = min(concentration_analysis['single_currency_max'] / 0.30, 1.0)
    ...
    def _estimate_liquidation_percentage(
        ...
        max_single_position = portfolio_positions['weight'].max() if 'weight' in portfolio_positions.columns else 0.1

        liquidatable_percentage = min((daily_liquidation_capacity * total_days) / max_single_position, 1.0)
    ...
    def _serialize_risk_decomposition(self, decomposition: RiskDecomposition) -> Dict[str, Any]:
        """序列化風險分解結果"""
        return {
    ...
    def _calculate_overall_risk_score(self, analysis_results: Dict[str, Any]) -> float:
        ...
        # 集中度風險 (30%)
        concentration_score = analysis_results['concentration_risk']['concentration_score'] * 30
        score += concentration_score

        # 流動性風險 (20%)
        liquidity_score = (1 - analysis_results['liquidity_risk']['liquidity_score']) * 20
    ...
    def _prepare_factor_exposure_chart(self, risk_decomposition: Dict[str, Any]) -> Dict[str, Any]:
        """準備因子暴露圖表數據"""
    ...
    def _prepare_sector_allocation_chart(self, risk_decomposition: Dict[str, Any]) -> Dict[str, Any]:
        """準備部門配置圖表數據"""
    ...
    def _prepare_liquidity_profile_chart(self, liquidity_risk: Dict[str, Any]) -> Dict[str, Any]:
        """準備流動性狀況圖表數據"""
    ...
    def _generate_liquidity_recommendations(self, liquidity_analysis: Dict[str, Any]) -> List[str]:
        """生成流動性風險建議"""
        recommendations = []
