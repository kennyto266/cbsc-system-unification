...
class ProfessionalRiskMetrics:
    ...
    def calculate_sortino_ratio(self, returns: pd.Series,
                              downside_threshold: Optional[float] = None) -> float:
        """
        計算Sortino比率
        ...
        if downside_threshold is None:
            downside_threshold = self.config.benchmark_return / self.config.trading_days_per_year
    ...
    def calculate_alpha(self, returns: pd.Series, market_returns: pd.Series) -> float:
        ...
        # 年化回報
        asset_return = (np.prod(1 + returns) - 1) * (self.config.trading_days_per_year / min_length)
        market_return = (np.prod(1 + market_returns) - 1) * (self.config.trading_days_per_year / min_length)
    ...
    def calculate_omega_ratio(self, returns: pd.Series,
    ...
    def calculate_all_metrics(self, returns: pd.Series,
        ...
        for confidence_level in self.config.confidence_levels:
            ...
            metrics[f'cvar_{int(confidence_level*100)}'] = cvar_results['cvar_historical']
        ...
        metrics['average_win_loss_ratio'] = self.calculate_average_win_loss_ratio(returns)
