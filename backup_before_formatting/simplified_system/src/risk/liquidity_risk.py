...
class LiquidityRiskAnalyzer:
    ...
    def analyze_liquidity_risk(
        ...
        try:
            ...
            # 1. 市場流動性分析
            market_liquidity_metrics = self._analyze_market_liquidity(portfolio_positions, market_data)
            market_liquidity_risk = self._calculate_market_liquidity_risk(market_liquidity_metrics)

            # 2. 融資流動性分析
            funding_liquidity_metrics = self._analyze_funding_liquidity(funding_data or {})
            funding_liquidity_risk = self._calculate_funding_liquidity_risk(funding_liquidity_metrics)
            ...
            # 4. 監管流動性比率計算
            lcr = self._calculate_liquidity_coverage_ratio(portfolio_positions, market_data, funding_data)
            nsfr = self._calculate_net_stable_funding_ratio(portfolio_positions, funding_data)

            # 5. 現金流缺口分析
            cash_flow_gaps = self._analyze_cash_flow_gaps(portfolio_positions, funding_data)
            ...
            recommendations = self._generate_liquidity_recommendations(
                market_liquidity_risk, funding_liquidity_risk, lcr, nsfr, early_warning_indicators
            )
        ...
        try:
            for _, position in portfolio_positions.iterrows():
                ...
                metrics = self._calculate_liquidity_metrics(
                    asset_name, asset_type, prices, volumes, bid_prices, ask_prices, position_value
                )
    ...
    def _calculate_liquidity_metrics(
        ...
        try:
            ...
            if len(prices) > 1 and len(volumes) > 0:
                ...
                volumes_aligned = volumes.reindex(returns.index, method='ffill').fillna(1)
                price_impact = (abs(returns) / volumes_aligned).mean() * 1e6  # Scale factor
            else:
            ...
            if len(volumes) > 0 and len(prices) > 0:
                ...
                market_cap = avg_price * avg_volume if position_value == 0 else position_value
            ...
            # 10. Liquidity Class
            liquidity_class = self._classify_liquidity(liquidity_score, bid_ask_spread, turnover_ratio)
    ...
    def _calculate_liquidity_score(
        ...
        spread_score = max(0, 100 - bid_ask_spread * 5000)  # Lower spread = higher score
        impact_score = max(0, 100 - price_impact * 1000)    # Lower impact = higher score
        turnover_score = min(100, turnover_ratio * 10000)   # Higher turnover = higher score
        activity_score = max(0, 100 - zero_return_days * 2000)  # Fewer zero days = higher score
    ...
    def _analyze_funding_liquidity(self, funding_data: Dict[str, Any]) -> List[FundingLiquidityMetrics]:
        """分析融資流動性"""
        funding_metrics = []
        ...
        try:
            ...
            for source in funding_sources:
                funding_metrics.append(FundingLiquidityMetrics(
    ...
    def _calculate_market_liquidity_risk(self, liquidity_metrics: Dict[str, LiquidityMetrics]) -> float:
        """計算市場流動性風險評分 (0-100)"""
    ...
    def _calculate_funding_liquidity_risk(self, funding_metrics: List[FundingLiquidityMetrics]) -> float:
        """計算融資流動性風險評分 (0-100)"""
        ...
        for metrics in funding_metrics:
            ...
            short_term_weight = sum(
                amount for period, amount in metrics.maturity_profile.items()
            ...
            source_risk = np.mean([cost_risk, maturity_risk, concentration_risk, rollover_risk])
            ...
            if funding_data:
                ...
                for source in funding_sources:
                    ...
                    thirty_day_outflows = sum(
                        amount for period, amount in maturity_profile.items()
    ...
    def _analyze_cash_flow_gaps(
        ...
        try:
            ...
            # Return empty gaps
            return {bucket: 0.0 for bucket in ['1D', '1W', '2W', '1M', '3M', '6M', '1Y']}
    ...
    def _calculate_overall_liquidity_risk(
        ...
        weighted_liquidity_risk = (market_liquidity_risk * 0.6 + funding_liquidity_risk * 0.4)
    ...
    def _run_single_liquidity_stress_test(
        ...
        try:
            ...
            normal_liquidity_metrics = self._analyze_market_liquidity(portfolio_positions, market_data)
            ...
            funding_gap = 0
            if funding_data:
                total_funding = sum(source.get('amount', 0) for source in funding_data.get('funding_sources', []))
            ...
            contingency_actions = self._generate_contingency_actions(scenario, funding_gap, fire_sale_loss)
            ...
            logger.error(f"Single liquidity stress test failed for {scenario.get('name', 'unknown')}: {e}")
            raise
        ...
        try:
            ...
            for _, position in portfolio_positions.iterrows():
                ...
                if asset_name in liquidity_metrics:
                    ...
                    stressed_laf = base_laf * (1 + liquidity_deterioration) * price_impact_multiplier
...
def quick_liquidity_assessment(
    ...
    for asset in daily_volumes.keys():
        market_data[asset] = pd.DataFrame({
            'close': [100] * 10,
            'volume': [daily_volumes.get(asset, 1000000)] * 10
        })
