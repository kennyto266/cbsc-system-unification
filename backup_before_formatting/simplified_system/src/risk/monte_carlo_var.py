...
class MonteCarloVaRCalculator:
    ...
    def __init__(self, config: Optional[VaRConfig] = None):
        ...
        logger.info(f"Monte Carlo VaR Calculator initialized with {self.config.num_simulations} simulations")
    ...
    def calculate_var(
        ...
        try:
            ...
            logger.info(f"Starting Monte Carlo VaR calculation for {len(confidence_levels)} confidence levels and {len(time_horizons)} horizons")
            ...
            # 4. 計算不同時間範圍的累積回報
            horizon_returns = self._calculate_horizon_returns(simulated_returns, time_horizons)
            ...
            # 6. 收斂性統計
            convergence_stats = self._calculate_convergence_statistics(simulated_returns)
            ...
            logger.info(f"Monte Carlo VaR calculation completed in {computational_time:.2f} seconds")
            return result
        ...
        try:
            ...
            if self.config.distribution_type == DistributionType.NORMAL:
                ...
                def ged_loglikelihood(params, data):
                    ...
                    log_likelihood = (
                        np.log(v) - np.log(2) - np.log(gamma(1/v)) -
                        np.log(sigma) -
                ...
                result = minimize(
                    ...
                    args=(returns.values,),
                    method='L-BFGS-B',
                    bounds=[(0, 1), (None, None), (0.001, None), (None, None), (0.001, None)]
                )
    ...
    def _estimate_correlation_structure(self, returns: pd.Series) -> Optional[np.ndarray]:
        """估計相關性結構（為未來多資產擴展預留）"""
    ...
    def _generate_simulated_returns(
        ...
        try:
            ...
            if self.config.variance_reduction == VarianceReduction.ANTITHETIC:
                ...
                simulated_returns = np.concatenate([simulated_returns, antithetic_returns])
                ...
                simulated_returns = simulated_returns + (control_mean - np.mean(simulated_returns))

            elif self.config.variance_reduction == VarianceReduction.STRATIFIED_SAMPLING:
                ...
                for i in range(num_strata):
                    ...
                    if dist_type == 'normal':
                        strata_returns = norm.ppf(norm.cdf(strata_normal),
                                                distribution_params['mu'],
                                                distribution_params['sigma'])
                    else:
                        strata_returns = simulated_returns[i*strata_size:(i+1)*strata_size]
    ...
    def _calculate_horizon_returns(
        ...
        for horizon in time_horizons:
            ...
            horizon_return = np.random.choice(simulated_returns, size=(self.config.num_simulations, horizon))
    ...
    def _calculate_var_for_horizon(
        ...
        try:
            ...
            if len(tail_returns) > 0:
                ...
                expected_shortfall_value = portfolio_value * abs(expected_shortfall_percentage)
            else:
            ...
            for _ in range(n_bootstrap):
                bootstrap_sample = np.random.choice(horizon_returns, size=len(horizon_returns), replace=True)
                ...
                bootstrap_es = np.mean(bootstrap_tail) if len(bootstrap_tail) > 0 else bootstrap_var
    ...
    def _calculate_convergence_statistics(self, simulated_returns: np.ndarray) -> Dict[str, float]:
        """計算收斂性統計"""
        try:
            ...
            if var_first > 0 and var_last > 0:
                geweke_statistic = (mean_first - mean_last) / np.sqrt(var_first/len(first_10_percent) + var_last/len(last_50_percent))
            else:
            ...
            mc_standard_error = np.std(simulated_returns) / np.sqrt(len(simulated_returns))
    ...
    def _backtest_var_results(
        ...
        try:
            ...
            expected_violations = total_observations * (1 - 0.95)
            actual_violation_rate = violations / total_observations

            # Kupiec test (unconditional coverage)
            kupiec_lr = 2 * (
                violations * np.log(actual_violation_rate / 0.05) +
                (total_observations - violations) * np.log((1 - actual_violation_rate) / 0.95)
            )
    ...
    def calculate_incremental_var(
        ...
        try:
            ...
            portfolio_var = portfolio_var_result.var_results[confidence_level][time_horizon].var_value

            # Combined portfolio VaR (portfolio + position)
            if len(portfolio_returns) == len(position_returns):
                combined_returns = (portfolio_returns + position_returns) / 2  # Equal weights
                ...
                combined_var = combined_var_result.var_results[confidence_level][time_horizon].var_value
                ...
                return {
                    ...
                    'position_var': self.calculate_var(
                        position_returns, portfolio_value/2, [confidence_level], [time_horizon]
                    ).var_results[confidence_level][time_horizon].var_value,
                    'combined_var': combined_var,
                    'incremental_var': incremental_var,
                    'incremental_var_percentage': incremental_var_percentage,
                    'diversification_benefit': portfolio_var + self.calculate_var(
                        position_returns, portfolio_value/2, [confidence_level], [time_horizon]
                    ).var_results[confidence_level][time_horizon].var_value - combined_var
                }
            else:
                return {
                    'error': 'Return series lengths do not match'
                }
    ...
    def calculate_component_var(
        ...
        try:
            ...
            for i in range(num_assets):
                ...
                asset_var = asset_var_result.var_results[confidence_level][time_horizon].var_value
                ...
                portfolio_var_plus = self.calculate_var(
                ...
                component_var_results[f'asset_{i}'] = {
                    ...
                    'percentage_contribution': (component_var / portfolio_var) * 100 if portfolio_var != 0 else 0
                }

            # Verify sum of component VaRs equals portfolio VaR
            total_component_var = sum([result['component_var'] for result in component_var_results.values()])
            component_var_results['verification'] = {
                'portfolio_var': portfolio_var,
                'total_component_var': total_component_var,
                'difference': portfolio_var - total_component_var,
                'percentage_difference': ((portfolio_var - total_component_var) / portfolio_var) * 100 if portfolio_var != 0 else 0
            }
        ...
        try:
            ...
            for confidence, horizon_results in mc_result.var_results.items():
                ...
                for horizon, var_result in horizon_results.items():
                    report['var_summary'][f'{int(confidence*100)}%_confidence'][f'{horizon}_day'] = {
            ...
            # Risk metrics
            all_1day_var = [horizon_results[1].var_percentage for horizon_results in mc_result.var_results.values()]
            report['risk_metrics'] = {
                ...
                'convergence_achieved': mc_result.convergence_statistics['convergence_achieved']
            }
