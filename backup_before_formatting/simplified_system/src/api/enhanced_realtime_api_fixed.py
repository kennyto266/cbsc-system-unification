...
class EnhancedRealtimeAPI:
    ...
    def fetch_api_data_with_retry(self, url: str, source_name: str, limit: int = 1000) -> Optional[Dict[str, Any]]:
        """帶重試機制的API數據獲取"""

        for attempt in range(self.retry_attempts):
            try:
                logger.info(f"Fetching {source_name} (attempt {attempt + 1}/{self.retry_attempts})")
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(f"HTTP 400 for {source_name}, trying alternative URLs if available")
                    break
                    ...
                    logger.error(f"HTTP Error {response.status_code} for {source_name}: {response.text}")
...
# 便捷函數
def fetch_enhanced_data(source_name: str, limit: int = 1000) -> Optional[Dict[str, Any]]:
    """獲取增強版特定數據源"""
