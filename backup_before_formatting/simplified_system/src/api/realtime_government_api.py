...
class RealTimeGovernmentAPI:
    ...
    def fetch_hibor_rates(self, days_back: int = 365) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Successfully fetched {len(hibor_records)} HIBOR records from HKMA API")
            return result
    ...
    def fetch_exchange_rates(self, days_back: int = 365) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Successfully fetched {len(rate_records)} exchange rate records")
            return result
    ...
    def fetch_monetary_base(self, days_back: int = 365) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Successfully fetched {len(monetary_records)} monetary base records")
            return result
    ...
    def fetch_all_data(self, days_back: int = 365) -> Dict[str, Any]:
        ...
        for source_name, fetcher in data_fetchers:
            try:
                ...
                if data:
                    ...
                    logger.info(f"✅ {source_name}: {data.get('record_count', 0)} records")
                else:
                    results['data_sources'][source_name] = {
                        'success': False,
        ...
        results['summary'] = {
            ...
            'successful_sources': len([s for s in results['data_sources'].values() if s.get('success')]),
            'total_records': total_records,
            'max_records_per_source': self.max_records
        }

        logger.info(f"Fetch complete: {results['summary']['successful_sources']}/{results['summary']['total_sources']} sources, {total_records} total records")

        return results

    def save_real_time_data(self, data: Dict[str, Any], filename_prefix: str = "realtime_gov_data"):
        """保存實時數據到文件"""
        try:
...
if __name__ == "__main__":
    ...
    print(f"   Sources: {all_data['summary']['successful_sources']}/{all_data['summary']['total_sources']}")
    ...
    for source_name, source_data in all_data['data_sources'].items():
        if source_data.get('success'):
            print(f"   [OK] {source_name}: {source_data.get('record_count', 0)} records")
        else:
