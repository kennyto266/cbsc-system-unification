...
from multiprocessing import Manager, Queue, Lock, cpu_count
import threading
...
# 導入回測引擎
from .vectorbt_engine import VectorBTEngine, BacktestResult
...
class ParallelOptimizer:
    ...
    def __init__(self, config: Optional[OptimizationConfig] = None):
        ...
        # 結果緩存
        self.results_cache = {}
        self.cache_file = os.path.join(self.config.results_dir, "optimization_cache.pkl")
        ...
        logger.info(f"Parallel optimizer initialized with {self.config.max_workers} workers")
    ...
    def optimize_multiple_strategies(
        ...
        Returns:
            多策略優化結果
        """
        logger.info(f"Starting multi-strategy optimization for {len(strategy_names)} strategies")
        ...
        for strategy_name in strategy_names:
            param_space = self.parameter_space.get_strategy_space(strategy_name)
            if param_space is None:
                logger.warning(f"Parameter space not found for strategy: {strategy_name}")
                continue
        ...
        for strategy_name in strategy_names:
            strategy_results_list = [
                ...
                if result.strategy_name == strategy_name and result.backtest_result is not None
            ]

            if strategy_results_list:
                analysis = self._analyze_results(strategy_results_list, optimization_metric)
    ...
    def _execute_parallel_tasks(self, tasks: List[OptimizationTask]) -> List[OptimizationResult]:
        """執行並行任務"""
        results = []
    ...
    def _save_multi_strategy_results(self, symbol: str, strategy_results: Dict[str, Dict[str, Any]]):
        """保存多策略優化結果"""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        ...
        combined_results = {
            ...
            'summary': {
                ...
                'successful_strategies': len([
                    r for r in strategy_results.values()
    ...
    def _find_best_overall_strategy(self, strategy_results: Dict[str, Dict[str, Any]]) -> str:
        """找到整體最佳策略"""
        best_strategy = ""
        best_metric = float('-inf')
    ...
    def _save_progress(self):
        ...
        progress_data = {
            ...
            'progress_percent': (self.completed_count + self.failed_count) / self.total_tasks * 100,
    ...
    def _update_statistics(self):
        ...
        # 計算緩存命中率
        if self.config.cache_results:
            cache_hits = sum(1 for r in self.completed_tasks.values() if r.execution_time == 0)
            self.stats['cache_hit_rate'] = (cache_hits / self.completed_count) * 100 if self.completed_count > 0 else 0
