...
from .objective_functions import PortfolioObjective, ObjectiveConfig, ObjectiveFactory
...
class ParetoPoint:
    ...
    def to_dict(self) -> Dict[str, Any]:
        """转换为字典格式"""
        weights_dict = {asset: weight for asset, weight in zip(self.asset_names, self.weights)}
...
class ParetoFrontierCalculator:
    ...
    def calculate_pareto_frontier(
        ...
        logger.info(f"Starting Pareto frontier calculation with {len(objectives)} objectives")
        ...
        try:
            ...
            if algorithm == "nsga2" and PYMOO_AVAILABLE:
                ...
                points = self._weighted_sum_optimization(returns, objectives, objective_weights, n_solutions)
            ...
            logger.info(f"Pareto frontier calculated in {frontier.calculation_time:.3f}s with {len(points)} points")
            return frontier
    ...
    def _weighted_sum_optimization(
        ...
        # 生成权重组合
        weight_combinations = self._generate_weight_combinations(len(objectives), n_solutions)
    ...
    def _generate_weight_combinations(self, n_objectives: int, n_combinations: int) -> List[List[float]]:
        """生成目标函数权重组合"""
        combinations = []
    ...
    def _dominates(
        ...
        for obj1_val, obj2_val, objective in zip(point1.objectives, point2.objectives, objectives):
    ...
    def _calculate_crowding_distance(self, points: List[ParetoPoint]) -> List[ParetoPoint]:
        """计算拥挤距离"""
        if not points:
            return points
        ...
        for front_points in fronts.values():
            ...
            for obj_idx in range(n_objectives):
                # 按目标函数值排序
                sorted_points = sorted(front_points, key=lambda p: p.objectives[obj_idx])
                ...
                for i in range(1, len(sorted_points) - 1):
                    if sorted_points[i].crowding_distance != float('inf'):
                        distance = (sorted_points[i + 1].objectives[obj_idx] -
                                  sorted_points[i - 1].objectives[obj_idx]) / (obj_max - obj_min)
    ...
    def _identify_knee_points(self, points: List[ParetoPoint]) -> List[ParetoPoint]:
        ...
        if self.config.knee_detection_method == "curvature" and len(first_front[0].objectives) == 2:
            # 二目标曲率方法
    ...
    def _distance_knee_detection(self, points: List[ParetoPoint]) -> List[ParetoPoint]:
        ...
        for i, point in enumerate(points):
            ...
            dist_to_anti_ideal = np.linalg.norm(objectives_normalized[i] - anti_ideal_point)
    ...
    def _angle_knee_detection(self, points: List[ParetoPoint]) -> List[ParetoPoint]:
        ...
        for i in range(1, len(sorted_points) - 1):
            # 计算向量
            v1 = np.array(sorted_points[i - 1].objectives) - np.array(sorted_points[i].objectives)
            v2 = np.array(sorted_points[i + 1].objectives) - np.array(sorted_points[i].objectives)
    ...
    def visualize_pareto_frontier(
        ...
        objectives_data = np.array([p.objectives[objective_indices] for p in first_front])
        knee_points_data = np.array([p.objectives[objective_indices] for p in frontier.knee_points])

        if interactive:
            self._plot_interactive_pareto(
                objectives_data, knee_points_data, frontier, objective_indices, save_path
            )
        else:
    ...
    def _plot_interactive_pareto(
        ...
        if len(objective_indices) == 2:
            ...
            fig.add_trace(go.Scatter3d(
                ...
                mode='markers',
                name='Pareto Frontier',
                marker=dict(size=5, color=objectives_data[:, 2], colorscale='Viridis', opacity=0.6)
            ))
    ...
    def _plot_static_pareto(
        ...
        if len(objective_indices) == 2:
            ...
            ax.scatter(objectives_data[:, 0], objectives_data[:, 1],
                      c='blue', alpha=0.6, label='Pareto Frontier')

            if len(knee_points_data) > 0:
                ax.scatter(knee_points_data[:, 0], knee_points_data[:, 1],
            ...
            ax.set_title(f"Pareto Frontier: {frontier.objective_names[objective_indices[0]]} vs {frontier.objective_names[objective_indices[1]]}")
            ...
            for i in range(n_obj):
                for j in range(n_obj):
                    if i == j:
                        ...
                        axes[i, j].set_title(frontier.objective_names[objective_indices[i]])
                    else:
                        axes[i, j].scatter(objectives_data[:, j], objectives_data[:, i],
                                          c='blue', alpha=0.6, s=10)
                        axes[i, j].set_xlabel(frontier.objective_names[objective_indices[j]])
                        axes[i, j].set_ylabel(frontier.objective_names[objective_indices[i]])
...
# 便利函数
def create_pareto_calculator(config: Optional[ParetoConfig] = None) -> ParetoFrontierCalculator:
    """创建帕累托计算器"""
...
def calculate_pareto_frontier(
    ...
    return calculator.calculate_pareto_frontier(returns, objective_instances, objective_weights, n_solutions, algorithm)