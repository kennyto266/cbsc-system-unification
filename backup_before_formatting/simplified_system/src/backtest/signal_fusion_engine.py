...
from .enhanced_technical_indicators import (
    VectorizedTechnicalIndicators, CrossIndicatorAnalyzer,
    IndicatorSignal, IndicatorCategory
)
...
class RegimeDetector:
    ...
    def get_regime_transition_history(self, price_data: pd.DataFrame) -> List[Tuple[datetime, MarketRegime]]:
...
class WeightedSignalFusion:
    ...
    def _weighted_average_fusion(
        ...
        regime_weights = self.config.regime_weights.get(market_regime, self.indicator_weights)

        for indicator_name, signal in indicator_signals.items():
            weight = regime_weights.get(indicator_name, self.indicator_weights.get(indicator_name, 0.1))
    ...
    def _calculate_performance_weights(self, indicator_signals: Dict[str, IndicatorSignal]) -> Dict[str, float]:
...
class SignalConfidenceScoring:
    ...
    def calculate_signal_confidence(
        ...
        market_confidence = self._calculate_market_confidence(indicator_name, signal_value)
    ...
    def _calculate_performance_confidence(
        ...
        successful_signals = sum(1 for s in historical_signals if s.get('success', False))
    ...
    def _calculate_market_confidence(self, indicator_name: str, signal_value: float) -> float:
...
class SignalFusionEngine:
    ...
    def generate_fused_signals(
        ...
        for date in price_data.index:
            ...
            date_signals = {
                name: signal for name, signal in individual_signals.items()
            ...
            if date_signals:
                ...
                if self.config.strategy == FusionStrategy.MACHINE_LEARNING and self.ml_fusion.is_trained:
                    # ML-based fusion
                    indicator_features = self._extract_features(date_signals, indicators, date)
                    fused_signal = self.ml_fusion.predict_fused_signal(indicator_features)
                elif self.config.strategy == FusionStrategy.DYNAMIC_WEIGHTING:
                    # Regime-based fusion
                    fused_signal = self.regime_fusion.fuse_signals_by_regime(date_price_data, date_signals)
                else:
                    # Weighted fusion
                    fused_signal = self.weighted_fusion.fuse_signals(date_signals, current_regime)
    ...
    def _calculate_indicators(
        ...
        indicators_to_use = indicators_to_use or ['rsi', 'macd', 'bollinger_bands', 'moving_average']

        indicators = {}

        if 'rsi' in indicators_to_use:
            indicators['rsi'] = self.enhanced_indicators.calculate_rsi_vectorized(price_data)

        if 'macd' in indicators_to_use:
            indicators['macd'] = self.enhanced_indicators.calculate_macd_vectorized(price_data)

        if 'bollinger_bands' in indicators_to_use:
            indicators['bollinger_bands'] = self.enhanced_indicators.calculate_bollinger_bands_vectorized(price_data)

        if 'moving_average' in indicators_to_use:
            indicators['moving_average'] = self.enhanced_indicators.calculate_moving_averages_vectorized(price_data)

        if 'stochastic' in indicators_to_use:
            indicators['stochastic'] = self.enhanced_indicators.calculate_stochastic_oscillator_vectorized(price_data)

        if 'atr' in indicators_to_use:
            indicators['atr'] = self.enhanced_indicators.calculate_atr_vectorized(price_data)
    ...
    def _generate_rsi_signal(self, period: int, rsi_series: pd.Series) -> IndicatorSignal:
        """Generate RSI signal"""
        ...
        confidence = max(abs(latest_rsi - 50) / 50, 0.2)  # Higher confidence at extremes
    ...
    def _generate_macd_signal(self, params: str, macd_data: Dict[str, pd.Series]) -> IndicatorSignal:
        ...
        confidence = min(abs(latest_histogram) / 0.01, 0.9)  # Confidence based on histogram strength
    ...
    def _generate_bb_signal(self, params: str, bb_data: Dict[str, pd.Series]) -> IndicatorSignal:
        ...
        latest_percent_b = bb_data.get('percent_b', pd.Series([0.5], index=[bb_data['upper'].index[-1]])).iloc[-1]
    ...
    def get_fusion_performance_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'signal_distribution': {
                ...
                'neutral': len(signals_df[(signals_df['signal_value'] >= -0.2) & (signals_df['signal_value'] <= 0.2)])
            },
...
def test_signal_fusion_engine():
    ...
    fused_signals = fusion_engine.generate_fused_signals(
        price_data,
        indicators_to_use=['rsi', 'macd', 'bollinger_bands']
    )
