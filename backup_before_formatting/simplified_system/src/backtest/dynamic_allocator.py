...
from .market_regime import MarketRegimeDetector, RegimeState, RegimePrediction, RegimeConfig
...
class AllocationResult:
    ...
    # 執行計劃
    execution_schedule: Dict[str, List[Tuple[datetime, float]]] = field(default_factory=dict)
...
class DynamicAssetAllocator:
    ...
    def calculate_optimal_allocation(
        ...
        # 計算風險指標
        risk_metrics = self._calculate_portfolio_risk(cost_optimized_weights, market_data)
        ...
        result = AllocationResult(
            ...
            cost_impact=sum(transaction_costs.values()) / sum(trades.values()) if trades else 0,
        ...
        logger.info(f"Allocation calculated. Total cost: {result.total_transaction_cost:.4f}")
    ...
    def _calculate_base_weights(
        ...
        if current_regime and self.regime_strategies.get(current_regime.regime_name):
            vol_scaling = self.regime_strategies[current_regime.regime_name].get('volatility_scaling', 1.0)
    ...
    def _apply_risk_adjustments(
        ...
        if self.config.volatility_target > 0:
            portfolio_volatility = self._calculate_portfolio_volatility(weights, asset_risks, market_data)
        ...
        for symbol, asset in self.assets.items():
            if symbol in adjusted_weights:
                adjusted_weights[symbol] = max(
                    asset.min_weight,
                    min(asset.max_weight, adjusted_weights[symbol])
                )
    ...
    def _calculate_transaction_costs(
        ...
        for symbol, trade_size in trades.items():
            if symbol in self.assets:
                ...
                market_impact_cost = abs(trade_size) * asset.market_impact * abs(trade_size)
    ...
    def _create_execution_schedule(
        ...
        for symbol, trade_size in trades.items():
            if abs(trade_size) > 0:
                # 分批執行以減少市場衝擊
                n_batches = min(5, int(abs(trade_size) / self.config.min_trade_size) + 1)
    ...
    def run_backtest(
        ...
        if initial_weights is None:
            initial_weights = {symbol: 1.0/len(self.assets) for symbol in self.assets.keys()}
        ...
        return {
            ...
            'summary': {
                ...
                'average_transaction_cost': np.mean([r['allocation'].total_transaction_cost for r in results]) if results else 0,
                'final_weights': current_weights
            }
        }

    def _generate_rebalance_dates(self, start_date: datetime, end_date: datetime) -> List[datetime]:
        """生成再平衡日期"""
        dates = []
    ...
    def _get_historical_data(self, market_data: Dict[str, pd.DataFrame], date: datetime) -> Dict[str, pd.DataFrame]:
        """獲取歷史數據"""
        historical_data = {}
        ...
        for symbol, data in market_data.items():
            ...
            if len(filtered_data) >= self.config.lookback_period:
                # 使用回看期數據
                historical_data[symbol] = filtered_data.tail(self.config.lookback_period)

        return historical_data

    def _execute_allocation(self, allocation: AllocationResult, market_data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
        """模擬執行配置"""
        execution_result = {
            'executed_trades': {},
            'actual_costs': {},
        ...
        for symbol, trade_size in allocation.trades.items():
            if symbol in market_data and len(market_data[symbol]) > 0:
                ...
                # 實際成本
                actual_cost = allocation.expected_transaction_costs.get(symbol, 0) * (1 + abs(slippage_pct))
                execution_result['actual_costs'][symbol] = actual_cost

        return execution_result

    def _calculate_portfolio_value(self, weights: Dict[str, float], market_data: Dict[str, pd.DataFrame]) -> float:
        """計算投資組合價值（簡化）"""
        total_value = 0
    ...
    def _calculate_backtest_performance(self, portfolio_values: List[Dict]) -> Dict[str, float]:
        """計算回測性能指標"""
