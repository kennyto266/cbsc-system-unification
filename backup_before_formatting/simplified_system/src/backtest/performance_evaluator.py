...
class PerformanceEvaluator:
    ...
    def __init__(self, risk_free_rate: float = 0.03, benchmark_returns: Optional[pd.Series] = None):
        ...
        logger.info(f"Performance evaluator initialized with risk-free rate: {risk_free_rate}")
    ...
    def calculate_comprehensive_metrics(
        ...
        try:
            ...
            metrics.max_drawdown_duration = self._calculate_max_drawdown_duration(equity_curve)
            ...
            # 穩定性指標
            metrics.rolling_sharpe_std = self._calculate_rolling_sharpe_stability(returns)

            logger.info(f"Comprehensive metrics calculated for {backtest_result.symbol}")
            return metrics
    ...
    def detect_overfitting(
        ...
        try:
            ...
            # 計算樣本內外性能差異
            overfitting_metrics.in_sample_sharpe = max(r.sharpe_ratio for r in optimization_results)
            ...
            if validation_data is not None and len(optimization_results) > 0:
                ...
                out_sample_result = self._validate_out_of_sample(best_result, validation_data)
                overfitting_metrics.out_of_sample_sharpe = out_sample_result.sharpe_ratio

                # Sharpe衰減率
                if overfitting_metrics.in_sample_sharpe > 0:
                    overfitting_metrics.sharpe_decay = (
                        1 - overfitting_metrics.out_of_sample_sharpe / overfitting_metrics.in_sample_sharpe
                    )

            # 參數穩定性分析
            overfitting_metrics.parameter_stability = self._analyze_parameter_stability(optimization_results)
            overfitting_metrics.optimal_parameter_cluster = self._analyze_optimal_parameter_clustering(optimization_results)

            # 時間穩定性分析
            overfitting_metrics.walk_forward_performance = self._walk_forward_analysis(optimization_results)
            overfitting_metrics.rolling_performance_std = self._calculate_rolling_performance_stability(optimization_results)

            # 統計顯著性檢驗
            if len(optimization_results) > 1:
                sharpe_ratios = [r.sharpe_ratio for r in optimization_results]
                overfitting_metrics.p_value = self._calculate_statistical_significance(sharpe_ratios)
                overfitting_metrics.deflation_adjusted_sharpe = self._calculate_deflation_adjusted_sharpe(sharpe_ratios)
                overfitting_metrics.minimum_track_record_length = self._calculate_minimum_track_record_length(optimization_results[0])

            # 計算綜合過擬合風險評分
            overfitting_metrics.overfitting_risk_score = self._calculate_overfitting_risk_score(overfitting_metrics)
            overfitting_metrics.confidence_level = 100 - overfitting_metrics.overfitting_risk_score
    ...
    def generate_performance_report(
        ...
        try:
            ...
            # 計算綜合性能指標
            performance_metrics = self.calculate_comprehensive_metrics(backtest_result, benchmark_data)
            report['performance_metrics'] = performance_metrics.to_dict()

            # 過擬合分析
            if include_overfitting_analysis and optimization_results:
                overfitting_metrics = self.detect_overfitting(optimization_results, benchmark_data)
            ...
            total_score, objective_scores = self.calculate_multi_objective_score(performance_metrics, objectives)
            report['multi_objective_score'] = {
                'total_score': round(total_score * 100, 2),
                'objective_scores': {k: round(v * 100, 2) for k, v in objective_scores.items()},
                'objectives_used': objectives,
                'grade': self._get_performance_grade(total_score)
            }

            # 策略評級
            report['strategy_rating'] = self._generate_strategy_rating(performance_metrics)

            # 投資建議
            report['investment_recommendation'] = self._generate_investment_recommendation(performance_metrics)

            # 風險評估
            report['risk_assessment'] = self._generate_risk_assessment(performance_metrics)
    ...
    def _calculate_sortino_ratio(self, returns: pd.Series) -> float:
        ...
        excess_return = returns.mean() * self.trading_days_per_year - self.risk_free_rate
    ...
    def _calculate_information_ratio(self, returns: pd.Series, benchmark_returns: pd.Series) -> float:
        ...
        return active_returns.mean() * self.trading_days_per_year / tracking_error if tracking_error > 0 else 0.0
    ...
    def _calculate_capture_ratios(self, returns: pd.Series, benchmark_returns: pd.Series) -> Tuple[float, float]:
        ...
        downside_capture = portfolio_down / benchmark_down if benchmark_down < 0 else 0.0
    ...
    def _calculate_rolling_sharpe_stability(self, returns: pd.Series, window: int = 63) -> float:
        """計算滾動Sharpe比率穩定性"""
        if len(returns) < window * 2:
            return 0.0

        rolling_sharpe = returns.rolling(window=window).apply(
            lambda x: (x.mean() * np.sqrt(self.trading_days_per_year) - self.risk_free_rate) / (x.std() * np.sqrt(self.trading_days_per_year))
        ).dropna()

        return rolling_sharpe.std()

    # 私有方法：過擬合檢測
    def _validate_out_of_sample(self, in_sample_result: BacktestResult, validation_data: pd.DataFrame) -> BacktestResult:
        """樣本外驗證"""
    ...
    def _analyze_parameter_stability(self, optimization_results: List[BacktestResult]) -> float:
        """分析參數穩定性"""
        if len(optimization_results) < 10:
            return 0.0

        # 提取頂級結果的參數
        top_results = sorted(optimization_results, key=lambda r: r.sharpe_ratio, reverse=True)[:20]
        sharpe_values = [r.sharpe_ratio for r in top_results]

        # 計算Sharpe比率的穩定性
        return 1.0 - (np.std(sharpe_values) / np.mean(sharpe_values)) if np.mean(sharpe_values) > 0 else 0.0

    def _analyze_optimal_parameter_clustering(self, optimization_results: List[BacktestResult]) -> float:
        """分析最優參數聚集度"""
    ...
    def _walk_forward_analysis(self, optimization_results: List[BacktestResult]) -> float:
        """走步向前分析"""
        # 簡化版本的走步向前分析
        # 實際需要將數據分為多個時間窗口進行分析
    ...
    def _calculate_rolling_performance_stability(self, optimization_results: List[BacktestResult]) -> float:
        """計算滾動性能穩定性"""
        if len(optimization_results) < 10:
            return 0.0
    ...
    def _calculate_minimum_track_record_length(self, backtest_result: BacktestResult) -> int:
        """計算最小追蹤記錄長度"""
    ...
    def _calculate_overfitting_risk_score(self, metrics: OverfittingMetrics) -> float:
        ...
        if metrics.minimum_track_record_length > 0:
            ...
            trl_risk = max(0, 100 - (data_points / metrics.minimum_track_record_length) * 100)
    ...
    def _calculate_stability_score(self, metrics: PerformanceMetrics) -> float:
        ...
        if metrics.upside_capture > 0 and metrics.downside_capture > 0:
            capture_balance = 1 - abs(metrics.upside_capture - metrics.downside_capture) / 2
    ...
    def _generate_investment_recommendation(self, metrics: PerformanceMetrics) -> Dict[str, Any]:
        """生成投資建議"""
        recommendation = {
            'recommendation': "HOLD",
...
def evaluate_strategy_performance(
    ...
    return evaluator.generate_performance_report(backtest_result, benchmark_data=benchmark_data)
...
def detect_strategy_overfitting(
    ...
    """便利函數：檢測策略過擬合"""
    evaluator = PerformanceEvaluator()
    overfitting_metrics = evaluator.detect_overfitting(optimization_results, validation_data)
