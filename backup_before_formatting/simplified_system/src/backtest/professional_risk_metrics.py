...
class RiskCalculator:
    ...
    def calculate_basic_metrics(
        ...
        if benchmark_returns is not None:
            ...
            # Alpha (annualized)
            benchmark_ann_return = (1 + aligned_benchmark).prod() ** (trading_days_per_year / len(aligned_benchmark)) - 1
            ...
            information_ratio = (excess_returns.mean() * trading_days_per_year) / tracking_error if tracking_error != 0 else 0
    ...
    def calculate_risk_adjusted_performance_score(
        ...
        sharpe_score = max(0, risk_metrics.sharpe_ratio) / 3  # Normalize to 0-1 (assuming max Sharpe of 3)
        ...
        drawdown_score = max(0, 1 + risk_metrics.max_drawdown)  # Normalize (drawdown is negative)
        var_score = max(0, 1 + risk_metrics.var_95 * 10)  # Normalize VaR (VaR is negative)
...
def calculate_portfolio_risk(
    ...
    marginal_contributions = np.dot(returns_matrix.cov() * 252, weights) / portfolio_volatility
    ...
    return {
        ...
        'risk_contribution_percentages': (component_contributions / portfolio_volatility).tolist()
    }
...
def stress_test_returns(
    ...
    for scenario_name, params in stress_scenarios.items():
        ...
        results[scenario_name] = {
            ...
            'volatility_increase': stressed_volatility / (returns.std() * np.sqrt(252)) - 1,
