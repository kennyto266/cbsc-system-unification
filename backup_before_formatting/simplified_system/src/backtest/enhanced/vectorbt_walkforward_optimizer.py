...
class WalkForwardOptimizer:
    ...
    def _validate_data(self):
        ...
        if len(self.data) < self.config.window_len:
            raise ValueError(f"數據長度 {len(self.data)} 小於窗口長度 {self.config.window_len}")
    ...
    def optimize(self) -> WalkForwardResult:
        ...
        # 步驟5: 統計分析
        stats_result = self._perform_statistical_analysis(in_performance, out_performance)

        # 步驟6: 計算穩定性評分
        stability_score = self._calculate_stability_score(in_performance, out_performance)
    ...
    def _split_data(self) -> Tuple[pd.Series, pd.Series]:
        ...
        logger.info(f"創建了 {len(in_indexes)} 個In-Sample窗口，{len(out_indexes)} 個Out-of-Sample窗口")

        return (in_price, in_indexes), (out_price, out_indexes)

    def _optimize_in_sample(self, in_data: Tuple[pd.Series, pd.MultiIndex]) -> pd.Series:
        """
        In-Sample參數優化
    ...
    def _test_out_sample(self,
                        out_data: Tuple[pd.Series, pd.MultiIndex],
                        best_params: Dict[str, Any]) -> pd.Series:
        """
        Out-of-Sample策略測試
    ...
    def _perform_statistical_analysis(self,
                                    in_performance: pd.Series,
                                    out_performance: pd.Series) -> Dict[str, float]:
        """
        執行統計分析
        ...
        try:
            ...
            # 計算改善程度
            sharpe_improvement = out_mean - in_mean if not np.isnan(out_mean) and not np.isnan(in_mean) else 0.0
    ...
    def _calculate_stability_score(self,
                                 in_performance: pd.Series,
                                 out_performance: pd.Series) -> float:
        """
        計算穩定性評分
