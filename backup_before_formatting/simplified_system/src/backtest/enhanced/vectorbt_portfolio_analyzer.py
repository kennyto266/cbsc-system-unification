...
class AdvancedPortfolioAnalyzer:
    ...
    def analyze_portfolio(self, portfolio: 'vbt.Portfolio',
    ...
    def _analyze_performance(self, portfolio: 'vbt.Portfolio') -> PerformanceMetrics:
        ...
        annualized_return = portfolio.total_return() * (self.config.trading_days_per_year / len(returns))
        sharpe_ratio = portfolio.sharpe_ratio()
        sortino_ratio = portfolio.sortino_ratio()
        calmar_ratio = portfolio.total_return() / abs(portfolio.max_drawdown()) if portfolio.max_drawdown() != 0 else 0

        # 交易統計
        win_rate = portfolio.win_rate()
        profit_factor = portfolio.profit_factor()
        avg_trade_return = portfolio.trades.returns().mean() if hasattr(portfolio.trades, 'returns') else 0
    ...
    def _calculate_risk_metrics(self, portfolio: 'vbt.Portfolio') -> RiskMetrics:
        ...
        cvar_95 = returns[returns <= var_95_threshold].mean() if len(returns[returns <= var_95_threshold]) > 0 else 0
        cvar_99 = returns[returns <= var_99_threshold].mean() if len(returns[returns <= var_99_threshold]) > 0 else 0
    ...
    def _compare_with_benchmark(self,
                               ...
                               benchmark_prices: Optional[pd.Series]) -> Dict[str, Any]:
        """與基準比較"""
        ...
        try:
            ...
            # 計算跟蹤誤差
            tracking_error = (portfolio_aligned - benchmark_aligned).std() * np.sqrt(self.config.trading_days_per_year)
            ...
            up_capture = portfolio_aligned[up_market].mean() / benchmark_aligned[up_market].mean() if up_market.any() else 0
            down_capture = portfolio_aligned[down_market].mean() / benchmark_aligned[down_market].mean() if down_market.any() else 0
    ...
    def _analyze_trades(self, portfolio: 'vbt.Portfolio') -> Dict[str, Any]:
        ...
        winning_trades = len(trades[trades.return_series > 0]) if hasattr(trades, 'return_series') else 0
    ...
    def _analyze_monthly_performance(self, portfolio: 'vbt.Portfolio') -> pd.DataFrame:
        ...
        yearly_stats = monthly_stats.groupby('year')['return'].agg(['mean', 'std', 'sum'])
...
"""
    ...
    def create_performance_visualization(self, analysis_result: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # 4. 收益分布直方圖
            portfolio.returns().hist(bins=50, ax=axes[1, 1], title='日收益分布', alpha=0.7)
