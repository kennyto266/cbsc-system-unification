...
try:
    from .vectorbt_walkforward_optimizer import WalkForwardOptimizer, WalkForwardConfig, WalkForwardResult
    from .vectorbt_strategy_builder import CookbookStrategyBuilder, StrategyConfig
    from .vectorbt_portfolio_analyzer import AdvancedPortfolioAnalyzer, PortfolioAnalysisConfig, RiskMetrics, PerformanceMetrics
    from .gpu_vectorbt_accelerator import GPUVectorBTAccelerator, GPUConfig, GPUBenchmarkResult

    # Strategy convenient imports
    from .cookbook_strategies.ma_crossover_strategy import ma_crossover_strategy, optimize_ma_crossover
    from .cookbook_strategies.rsi_mean_reversion_strategy import rsi_mean_reversion_strategy, rsi_with_stop_loss_strategy, optimize_rsi_strategy
...
def get_enhanced_backtest_engine(price_data=None, gpu_enabled=True):
    ...
    if price_data is not None:
        components['walkforward_optimizer'] = WalkForwardOptimizer(
            price_data,
            rsi_mean_reversion_strategy  # 使用RSI策略作為默認示例
        )
