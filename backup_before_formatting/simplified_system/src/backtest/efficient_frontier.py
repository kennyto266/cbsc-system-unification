...
from .mpt_optimizer import MPTOptimizer, MPTConfig, OptimizationResult
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            'efficient_frontier': {
                ...
                'min_volatility': {
                    'return': round(self.min_volatility_portfolio.expected_return * 100, 2),
                    'volatility': round(self.min_volatility_portfolio.volatility * 100, 2),
            ...
            'capital_market_line': {
                ...
                'intercept': round(self.cml_intercept, 4) if self.cml_intercept else None,
            ...
            'asset_characteristics': {
                ...
                for asset, ret, vol in zip(self.asset_names, self.asset_returns, self.asset_volatilities)
            },
            'metadata': {
...
class EfficientFrontierCalculator:
    ...
    def calculate_efficient_frontier(
        ...
        try:
            logger.info(f"Starting efficient frontier calculation for {returns.shape[1]} assets")
            ...
            logger.info(f"Efficient frontier calculation completed in {result.calculation_time:.3f}s")
            return result
    ...
    def calculate_target_risk_portfolios(
        ...
        try:
            logger.info(f"Calculating {len(target_volatilities)} target volatility portfolios")
            ...
            for target_vol in target_volatilities:
                ...
                portfolio_result = self._optimize_for_target_volatility(
                    mean_returns, cov_matrix, target_vol, num_assets, list(returns.columns)
                )
                target_portfolios.append(portfolio_result)

            logger.info(f"Target volatility portfolios calculated in {time.time() - start_time:.3f}s")
    ...
    def plot_efficient_frontier(
        ...
        try:
            ...
            # 設置圖表標題和標籤
            plt.title('Efficient Frontier and Optimal Portfolios', fontsize=16, fontweight='bold')
    ...
    def generate_efficient_frontier_report(
        ...
        try:
            ...
            report_lines.append(f"Calculation Time: {ef_result.calculation_time:.3f} seconds")
            report_lines.append(f"Risk-free Rate: {ef_result.risk_free_rate * 100:.2f}%")
            report_lines.append("")

            # 投資組合統計
            report_lines.append("PORTFOLIO STATISTICS")
            report_lines.append("-" * 30)
            report_lines.append(f"Total Portfolios Generated: {ef_result.total_portfolios:,}")
            report_lines.append(f"Efficient Portfolios: {ef_result.efficient_portfolios_count:,}")
            report_lines.append(f"Efficiency Ratio: {ef_result.efficient_portfolios_count/ef_result.total_portfolios:.2%}")
            report_lines.append("")
            ...
            report_lines.append(f"  Expected Return: {max_sharpe.expected_return * 100:.2f}%")
            ...
            # Top 5 holdings
            weights_dict = {asset: weight for asset, weight in zip(max_sharpe.assets, max_sharpe.weights)}
            top_assets = sorted(weights_dict.items(), key=lambda x: x[1], reverse=True)[:5]
            ...
            report_lines.append(f"  Expected Return: {min_vol.expected_return * 100:.2f}%")
            report_lines.append(f"  Volatility: {min_vol.volatility * 100:.2f}%")
            report_lines.append(f"  Sharpe Ratio: {min_vol.sharpe_ratio:.3f}")

            # Top 5 holdings
            weights_dict = {asset: weight for asset, weight in zip(min_vol.assets, min_vol.weights)}
            top_assets = sorted(weights_dict.items(), key=lambda x: x[1], reverse=True)[:5]
            report_lines.append("  Top 5 Holdings:")
            for asset, weight in top_assets:
                report_lines.append(f"    {asset}: {weight * 100:.2f}%")
            ...
            if ef_result.cml_slope is not None:
                ...
                report_lines.append(f"Market Return Premium: {ef_result.cml_slope * 100:.2f}% per unit of risk")
                report_lines.append("")
            ...
            if max_sharpe.sharpe_ratio > 1.0:
                report_lines.append("✓ Maximum Sharpe portfolio shows excellent risk-adjusted returns")
            elif max_sharpe.sharpe_ratio > 0.5:
                report_lines.append("✓ Maximum Sharpe portfolio shows good risk-adjusted returns")
            else:
                report_lines.append("⚠ Consider additional assets or different time periods")

            if max_sharpe.volatility < 0.20:
                report_lines.append("✓ Optimal portfolio has acceptable volatility level")
    ...
    def _generate_random_portfolios(
        ...
        for _ in range(self.ef_config.num_portfolios):
            ...
            if portfolio_volatility > 0:
                sharpe_ratio = (portfolio_return - self.mpt_config.risk_free_rate) / portfolio_volatility
            else:
                sharpe_ratio = 0
    ...
    def _calculate_capital_market_line(
        ...
        try:
            ...
            if max_sharpe_portfolio.volatility > 0:
                cml_slope = (max_sharpe_portfolio.expected_return - self.mpt_config.risk_free_rate) / max_sharpe_portfolio.volatility
