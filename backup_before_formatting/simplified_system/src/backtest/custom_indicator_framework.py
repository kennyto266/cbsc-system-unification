...
class BaseCustomIndicator(ABC):
    ...
    @abstractmethod
    def generate_signals(self, indicator_values: Union[pd.Series, Dict[str, pd.Series]]) -> pd.Series:
        """
    ...
    def set_parameter(self, name: str, value: Any) -> bool:
        ...
        if not param_def:
            logger.error(f"Parameter '{name}' not found in indicator '{self.spec.name}'")
            return False
...
class VectorizedIndicatorMixin:
    ...
    def vectorized_calculate(self, data: pd.DataFrame) -> Union[pd.Series, Dict[str, pd.Series]]:
        """
    ...
    def batch_calculate(self, data_list: List[pd.DataFrame]) -> List[Union[pd.Series, Dict[str, pd.Series]]]:
        """
...
class CustomIndicatorRegistry:
    ...
    def _validate_indicator(self, indicator: BaseCustomIndicator) -> bool:
        ...
        if not indicator.spec.name or not indicator.spec.description:
            logger.error(f"Indicator '{indicator.spec.name}' missing name or description")
            return False
        ...
        for method_name in required_methods:
            if not hasattr(indicator, method_name):
                logger.error(f"Indicator '{indicator.spec.name}' missing required method: {method_name}")
                return False
...
class IndicatorTester:
    ...
    def test_indicator(
        ...
        test_results['validation_results'] = self._test_data_validation(indicator, test_data)
        ...
        if test_results['validation_results']['data_valid']:
            ...
            if calculation_results['calculation_success']:
                signal_results = self._test_signal_generation(indicator, calculation_results['values'])
    ...
    def _test_data_validation(self, indicator: BaseCustomIndicator, test_data: pd.DataFrame) -> Dict[str, Any]:
        """Test data validation"""
    ...
    def _test_calculation(self, indicator: BaseCustomIndicator, test_data: pd.DataFrame) -> Dict[str, Any]:
    ...
    def _test_signal_generation(self, indicator: BaseCustomIndicator, indicator_values: Union[pd.Series, Dict[str, pd.Series]]) -> Dict[str, Any]:
        """Test signal generation"""
        try:
            # Generate signals
    ...
    def _test_error_handling(self, indicator: BaseCustomIndicator) -> Dict[str, Any]:
        ...
        error_tests['invalid_parameter'] = not indicator.set_parameter('invalid_param', 'invalid_value')
...
class PerformanceBenchmark:
    ...
    def benchmark_indicator(
        ...
        all_records_per_second = [
            results['records_per_second'] for results in benchmark_results['data_sizes'].values()
        ]
    ...
    def compare_indicators(
        ...
        for indicator_name in indicator_names:
            if indicator_name in self.registry.list_all_indicators():
                results = self.benchmark_indicator(indicator_name, [data_size], iterations)
        ...
        sorted_indicators = sorted(records_per_second.items(), key=lambda x: x[1], reverse=True)
        ...
        comparison_results['best_performer'] = sorted_indicators[0][0] if sorted_indicators else None
        comparison_results['worst_performer'] = sorted_indicators[-1][0] if sorted_indicators else None
...
class CustomMeanReversionIndicator(BaseCustomIndicator, VectorizedIndicatorMixin):
    ...
    def validate_data(self, data: pd.DataFrame) -> bool:
        ...
        return all(col in data.columns for col in required_columns) and len(data) >= self.parameters['lookback_period']
...
class CustomVolatilityRegimeIndicator(BaseCustomIndicator, VectorizedIndicatorMixin):
    ...
    def validate_data(self, data: pd.DataFrame) -> bool:
        """Validate input data"""
        required_columns = ['close']
        return all(col in data.columns for col in required_columns) and len(data) >= self.parameters['long_window']
...
def test_custom_indicator_framework():
    ...
    logger.info(f"  Records/sec: {comparison.get('performance_ranking', [{}])[0].get('records_per_second', 0):.0f}")
