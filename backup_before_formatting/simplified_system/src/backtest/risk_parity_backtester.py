...
try:
    ...
    from .risk_budgeting import RiskBudgetingFramework, RiskBudgetConfig
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'benchmark_comparison': {
                'tracking_error': round(self.tracking_error, 4) if self.tracking_error else None,
                'excess_return': round(self.excess_returns.mean() * 252 * 100, 2) if self.excess_returns is not None else None
            },
            'metadata': {
...
class RiskParityBacktester:
    ...
    def backtest_equal_risk_parity(
        ...
        try:
            ...
            result = self._execute_backtest(
                filtered_returns, "equal_budget", name
            )

            result.calculation_time = time.time() - start_time

            logger.info(f"Equal risk parity backtest completed: {result.calculation_time:.2f}s")
            return result
    ...
    def backtest_risk_budgeting(
        ...
        try:
            ...
            self.risk_budgeting.create_custom_risk_budget(assets, risk_allocations, budget_name)

            # 执行回测
            result = self._execute_backtest(
                filtered_returns, budget_name, name
            )

            result.calculation_time = time.time() - start_time

            logger.info(f"Risk budgeting backtest completed: {result.calculation_time:.2f}s")
            return result
    ...
    def backtest_hierarchical_risk_parity(
        ...
        try:
            ...
            logger.info(f"Hierarchical risk parity backtest completed: {result.calculation_time:.2f}s")
            return result
        ...
        for strategy_type, params in strategies:
            try:
                if strategy_type == "equal_risk_parity":
                    result = self.backtest_equal_risk_parity(returns, params.get("name", "Equal Risk Parity"))
    ...
    def _prepare_data(self, returns: pd.DataFrame) -> pd.DataFrame:
        """准备回测数据"""
        # 过滤日期范围
        mask = (returns.index >= self.config.start_date) & (returns.index <= self.config.end_date)
        ...
        filtered_returns = filtered_returns[valid_assets].fillna(method='ffill').fillna(0)

        logger.info(f"Prepared data: {len(filtered_returns)} periods, {len(filtered_returns.columns)} assets")
    ...
    def _execute_backtest(
        ...
        for i in range(1, len(dates)):
            ...
            portfolio_values.iloc[i] = portfolio_values.iloc[i-1] * (1 + daily_return - trading_cost)

        # 计算性能指标
        performance_metrics = self._calculate_performance_metrics(portfolio_returns, portfolio_values)
        ...
        # 计算基准比较
        benchmark_results = self._calculate_benchmark_comparison(returns, portfolio_returns)

        # 计算风险贡献历史
        contribution_history = None
        if self.config.calculate_contributions:
            try:
                contribution_history = self._calculate_contribution_history(returns, weights_history)
            except Exception as e:
            ...
            if current_date in rebalance_dates or i == 1:
                ...
                if len(window_returns) >= self.config.min_observations:
                    try:
                        ...
                        logger.warning(f"HRP optimization failed at {current_date}: {e}")
                        trading_cost = 0.0
                else:
                    trading_cost = 0.0
            else:
    ...
    def _calculate_trading_metrics(self, weights_history: pd.DataFrame) -> Dict[str, Any]:
        """计算交易指标"""
        # 计算权重变化
    ...
    def _calculate_contribution_history(
        ...
        # 每月计算一次风险贡献
        monthly_dates = weights_history.index[weights_history.index.to_period('M').drop_duplicates()]
        ...
        for date in monthly_dates:
            if date in weights_history.index:
                ...
                if len(historical_returns) >= self.config.min_observations:
                    try:
                        # 计算风险贡献
                        analysis = self.risk_contribution.calculate_marginal_contributions(
                        ...
                        contribution_data.append({
                            ...
                            **{f"{asset}_contribution": mc.component_contribution
                               for asset, mc in zip(returns.columns, analysis.marginal_contributions)},
                            'portfolio_volatility': analysis.portfolio_volatility,
                            'diversification_ratio': analysis.diversification_ratio
                        })

                    except Exception as e:
                        logger.warning(f"Contribution calculation failed at {date}: {e}")
                        continue

        return pd.DataFrame(contribution_data).set_index('date') if contribution_data else pd.DataFrame()

    def plot_results(self, result: BacktestResult, save_path: Optional[str] = None) -> None:
        """绘制回测结果"""
...
def run_risk_parity_backtest(
    returns: pd.DataFrame,
    strategy_type: str = "equal_risk_parity",
    **kwargs
) -> BacktestResult:
    """便利函数：运行风险平价回测"""
