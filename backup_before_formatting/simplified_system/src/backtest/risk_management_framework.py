...
from .professional_risk_metrics import (
    ...
    stress_test_returns, create_default_stress_scenarios
)
...
class RiskBudgeting:
    ...
    def _risk_parity_weights(self, cov_matrix: pd.DataFrame, assets: List[str]) -> Dict[str, float]:
    ...
    def _apply_risk_constraints(self, weights: Dict[str, float], assets: List[str]) -> Dict[str, float]:
        """Apply risk budget constraints"""
        # Apply position size limits
        weights = {k: min(v, self.risk_budget.max_position_size) for k, v in weights.items()}
...
class StressTesting:
    ...
    def _test_scenario(
        ...
        return {
            ...
            'impact_analysis': {
                'volatility_increase': (stressed_volatility / base_metrics['volatility']) - 1,
                'var_deterioration': stressed_var_95 / base_metrics['var_95'] - 1,
                'max_portfolio_decline': max_decline
            },
            'risk_assessment': self._assess_scenario_risk(max_decline, stressed_volatility)
        }

    def _assess_scenario_risk(self, max_decline: float, volatility: float) -> Dict[str, Any]:
...
class RiskMonitoring:
    ...
    def monitor_portfolio_risk(
        ...
        if abs(risk_metrics.max_drawdown) > self.risk_budget.max_drawdown_limit:
            alerts.append(RiskAlert(
                ...
                recommendations=["Consider reducing position sizes", "Review risk tolerance"]
            ))
        ...
        if len(recent_returns) > 30:
            ...
            if current_var_95 < -self.risk_budget.var_limit_95:
                alerts.append(RiskAlert(
                    ...
                    timestamp=current_time,
                    recommendations=["Review portfolio volatility", "Consider hedging strategies"]
                ))

        # Check concentration risk
        for symbol, position in positions.items():
            position_weight = (position * current_prices.get(symbol, 0)) / portfolio_value
            if position_weight > self.risk_budget.concentration_limit:
                alerts.append(RiskAlert(
                    ...
                    timestamp=current_time,
                    symbol=symbol,
                    recommendations=["Consider reducing position size", "Diversify exposure"]
                ))
    ...
    def _calculate_overall_risk_level(self) -> RiskLevel:
        ...
        critical_alerts = [a for a in self.active_alerts if a.severity == RiskLevel.CRITICAL]
        if critical_alerts:
        ...
        medium_alerts = [a for a in self.active_alerts if a.severity == RiskLevel.MEDIUM]
        if medium_alerts:
...
class IntegratedRiskManager:
    ...
    def generate_risk_report(
        ...
        report = {
            ...
            'recommendations': self._generate_recommendations(risk_metrics, current_alerts, stress_results)
        }
    ...
    def _check_budget_compliance(self, risk_metrics: RiskMetrics) -> Dict[str, bool]:
        ...
        return {
            'volatility_limit': risk_metrics.volatility <= self.risk_budget.max_portfolio_volatility,
            'max_drawdown_limit': abs(risk_metrics.max_drawdown) <= self.risk_budget.max_drawdown_limit,
    ...
    def _summarize_stress_tests(self, stress_results: Dict) -> Dict[str, Any]:
        ...
        return {
            ...
            'high_risk_scenarios': [
                ...
                if results['risk_assessment']['risk_level'] in [RiskLevel.HIGH, RiskLevel.CRITICAL]
            ]
        }
    ...
    def _generate_recommendations(
        ...
        if abs(risk_metrics.max_drawdown) > self.risk_budget.max_drawdown_limit * 0.7:
            recommendations.append("Consider reducing overall portfolio risk due to high drawdown")

        # Check Sharpe ratio
        if risk_metrics.sharpe_ratio < 0.5:
            recommendations.append("Portfolio Sharpe ratio is low - consider strategy review")

        # Check concentration
        concentration_alerts = [a for a in alerts if a.alert_type == AlertType.CONCENTRATION_RISK]
        ...
        if high_risk_scenarios:
            recommendations.append(f"High risk under stress scenarios: {', '.join(high_risk_scenarios)}")
...
def test_integrated_risk_management():
    ...
    logger.info(f"Risk report generated with {len(risk_report['current_alerts'])} alerts")
    logger.info(f"Overall risk level: {risk_report['risk_monitoring_summary']['risk_level']}")
