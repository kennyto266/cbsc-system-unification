...
from .objective_functions import (
    PortfolioObjective, ObjectiveConfig, ObjectiveFactory,
    SharpeRatioObjective, VarianceObjective, TradingCostObjective
)
from .pareto_frontier import (
    ParetoFrontier, ParetoFrontierCalculator, ParetoPoint, ParetoConfig
)
...
class MultiObjectiveOptimizer:
    ...
    def __init__(self, config: Optional[MultiObjectiveConfig] = None):
        ...
        logger.info(f"Multi-Objective Optimizer initialized with {self.config.algorithm} algorithm")
    ...
    def optimize_portfolio(
        ...
        try:
            ...
            if custom_objectives:
                ...
                objective_instances = self._prepare_objectives(objectives, objective_configs)
            ...
            if self.config.algorithm == "nsga2" and PYMOO_AVAILABLE:
                pareto_frontier = self._nsga2_optimization(returns, objective_instances, constraints)
            elif self.config.algorithm == "spea2" and PYMOO_AVAILABLE:
                pareto_frontier = self._spea2_optimization(returns, objective_instances, constraints)
            elif self.config.algorithm == "moead" and PYMOO_AVAILABLE:
                pareto_frontier = self._moead_optimization(returns, objective_instances, constraints)
            else:
                pareto_frontier = self._weighted_sum_optimization(returns, objective_instances, constraints)

            # 后处理分析
            self._post_process_analysis(pareto_frontier, objective_instances)

            logger.info(f"Multi-objective optimization completed in {time.time() - start_time:.3f}s")
    ...
    def preference_optimization(
        ...
        try:
            ...
            # 计算帕累托边界
            pareto_frontier = self.optimize_portfolio(returns, objectives, constraints=constraints)

            # 应用偏好
            if preference_method == "weighted_sum":
                preferred_solution = self._weighted_sum_preference(
                    pareto_frontier, preference_weights or self.config.preference_weights
                )
            ...
            satisfaction_level = self._calculate_satisfaction(
                preferred_solution, objective_instances, preference_weights, aspiration_levels
            )
            ...
            logger.info(f"Preference optimization completed in {time.time() - start_time:.3f}s")
            return result
    ...
    def robust_optimization(
        ...
        try:
            ...
            for i, scenario_returns in enumerate(uncertainty_scenarios):
                ...
                frontier = self.optimize_portfolio(scenario_returns, objectives, constraints=constraints)
            ...
            logger.info(f"Robust optimization completed in {time.time() - start_time:.3f}s")
    ...
    def dynamic_rebalancing(
        ...
        """
        start_time = time.time()
        logger.info(f"Starting dynamic rebalancing with {rebalance_frequency} frequency")

        try:
            # 确定再平衡日期
            rebalance_dates = self._get_rebalance_dates(returns_history, rebalance_frequency)
            ...
            for rebalance_date in rebalance_dates:
                # 获取优化窗口数据
                window_data = self._get_rebalance_window(returns_history, rebalance_date, window_size)
                ...
                # 优化
                pareto_frontier = self.optimize_portfolio(window_data, objectives, constraints=constraints)
            ...
            logger.info(f"Dynamic rebalancing completed in {time.time() - start_time:.3f}s")
    ...
    def _prepare_objectives(
        ...
        for i, obj_name in enumerate(objectives):
            config = objective_configs[i] if objective_configs and i < len(objective_configs) else None
    ...
    def _nsga2_optimization(
        ...
        algorithm = NSGA2(
            ...
            crossover=get_crossover("real_sbx", prob=self.config.crossover_prob, eta=self.config.eta_crossover),
    ...
    def _weighted_sum_optimization(
        ...
        weight_combinations = self._generate_objective_weights(n_objectives, self.config.population_size)
    ...
    def _generate_objective_weights(self, n_objectives: int, n_combinations: int) -> List[List[float]]:
        """生成目标函数权重组合"""
        combinations = []
        ...
        if n_objectives == 2:
            ...
            for i in range(n1):
                for j in range(n2):
                    for k in range(n3):
                        w1, w2, w3 = i/(n1-1) if n1 > 1 else 0, j/(n2-1) if n2 > 1 else 0, k/(n3-1) if n3 > 1 else 0
                        total = w1 + w2 + w3
                        if total > 0:
    ...
    def _post_process_analysis(
        ...
        # 计算收敛指标
        convergence_metrics = self._calculate_convergence_metrics(pareto_frontier, objectives)
        ...
        if frontier.n_objectives == 2:
            ...
            for i in range(len(sorted_points)):
                if i == 0:
                    ...
                    width = sorted_points[i-1].objectives[0] - sorted_points[i].objectives[0]
    ...
    def _calculate_diversity_metrics(self, frontier: ParetoFrontier) -> Dict[str, float]:
        """计算多样性指标"""
        first_front = [p for p in frontier.points if p.rank == 0]
        if not first_front:
            return {}
        ...
        metrics['min_crowding_distance'] = np.min([cd for cd in crowding_distances if cd != float('inf')])
    ...
    def _generate_uncertainty_scenarios(self, returns: pd.DataFrame) -> List[pd.DataFrame]:
        """生成不确定性情景"""
        scenarios = []
    ...
    def _compute_robust_frontier(
        ...
        # 重新排序
        robust_solutions = self.pareto_calculator._non_dominated_sorting(robust_solutions, [])
    ...
    def _get_rebalance_dates(self, returns_history: pd.DataFrame, frequency: str) -> List[str]:
        """获取再平衡日期"""
    ...
    def _calculate_satisfaction(
        ...
        if preference_weights and aspiration_levels:
            # 基于期望水平的满意度
            for i, (obj_val, asp_level) in enumerate(zip(solution.objectives, aspiration_levels)):
    ...
    def _sensitivity_analysis(
        ...
        for i in range(n_assets):
            ...
            # 计算敏感性
            for j, (original, perturbed) in enumerate(zip(original_values, perturbed_values)):
                if abs(epsilon) > 1e-8:
    ...
    def plot_optimization_progress(self, save_path: Optional[str] = None) -> None:
        ...
        fig = make_subplots(
            rows=2, cols=2,
            subplot_titles=("Hypervolume", "Average Distance to Ideal",
                          "Average Crowding Distance", "Solution Diversity"),
        ...
        # 超体积
        fig.add_trace(
            go.Scatter(x=generations, y=hypervolumes, mode='lines+markers', name='Hypervolume'),
            row=1, col=1
        )

        # 收敛指标
        avg_distances = [h.convergence_metrics.get('avg_distance_to_ideal', 0)
                        for h in self.optimization_history]
        fig.add_trace(
            go.Scatter(x=generations, y=avg_distances, mode='lines+markers', name='Avg Distance'),
            row=1, col=2
        )

        # 拥挤距离
        avg_crowding = [h.diversity_metrics.get('avg_crowding_distance', 0)
                       for h in self.optimization_history]
        fig.add_trace(
            go.Scatter(x=generations, y=avg_crowding, mode='lines+markers', name='Avg Crowding'),
            row=2, col=1
        )

        # 多样性
        diversity_spread = [h.diversity_metrics.get('diversity_spread', 0)
                           for h in self.optimization_history]
        fig.add_trace(
            go.Scatter(x=generations, y=diversity_spread, mode='lines+markers', name='Diversity'),
            row=2, col=2
        )
...
# 便利函数
def create_multi_objective_optimizer(config: Optional[MultiObjectiveConfig] = None) -> MultiObjectiveOptimizer:
    """创建多目标优化引擎"""
