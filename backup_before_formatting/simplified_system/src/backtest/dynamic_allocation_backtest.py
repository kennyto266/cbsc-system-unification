...
from .dynamic_allocator import DynamicAssetAllocator, AssetConfig, AllocationConfig, AllocationResult
from .tactical_overlay import TacticalOverlaySystem, OverlayConfig, OverlayResult
...
class DynamicAllocationBacktester:
    ...
    def _validate_scenario_data(
        ...
        for asset in scenario.assets:
            ...
            if len(data) < 252:  # 至少1年數據
                raise ValueError(f"Insufficient data for {asset.symbol}: {len(data)} records")
            ...
            if data_start > scenario.start_date:
                raise ValueError(f"Data for {asset.symbol} starts after scenario start date")

            if data_end < scenario.end_date:
                raise ValueError(f"Data for {asset.symbol} ends before scenario end date")
    ...
    def _run_dynamic_backtest(
        ...
        allocator = DynamicAssetAllocator(
            scenario.assets,
            scenario.allocation_config,
            regime_detector
        )
        overlay_system = TacticalOverlaySystem(
            allocator,
            scenario.overlay_config,
            regime_detector
        )
        ...
        current_weights = {asset.symbol: 1.0/len(scenario.assets) for asset in scenario.assets}
        ...
        for i, rebalance_date in enumerate(rebalance_dates):
            try:
                ...
                historical_data = self._get_historical_data(
                    market_data, rebalance_date, scenario.allocation_config.lookback_period
                )
                ...
                overlay_result = overlay_system.apply_tactical_overlay(
                    strategic_allocation.target_weights, historical_data, regime_prediction
                )
                ...
                allocation_record = {
                    ...
                    'transaction_costs': strategic_allocation.total_transaction_cost + overlay_result.total_overlay_cost,
        ...
        # 計算最終性能指標
        final_metrics = self._calculate_final_metrics(portfolio_values, scenario.initial_capital)
        ...
        return {
            ...
            'final_portfolio_value': portfolio_values[-1]['value'] if portfolio_values else scenario.initial_capital
        }
    ...
    def _run_benchmark_backtest(
        ...
        if scenario.benchmark_weights:
            ...
            benchmark_weights = {asset.symbol: 1.0/len(scenario.assets) for asset in scenario.assets}
    ...
    def _compare_performance(
        ...
        comparison['returns'] = {
            ...
            'excess': dynamic_metrics.get('total_return', 0) - benchmark_metrics.get('total_return', 0)
        }

        comparison['sharpe'] = {
            'dynamic': dynamic_metrics.get('sharpe_ratio', 0),
            'benchmark': benchmark_metrics.get('sharpe_ratio', 0),
            'excess': dynamic_metrics.get('sharpe_ratio', 0) - benchmark_metrics.get('sharpe_ratio', 0)
        }

        comparison['volatility'] = {
            'dynamic': dynamic_metrics.get('volatility', 0),
            'benchmark': benchmark_metrics.get('volatility', 0),
            'difference': dynamic_metrics.get('volatility', 0) - benchmark_metrics.get('volatility', 0)
        }

        comparison['max_drawdown'] = {
            'dynamic': dynamic_metrics.get('max_drawdown', 0),
            'benchmark': benchmark_metrics.get('max_drawdown', 0),
            'improvement': abs(benchmark_metrics.get('max_drawdown', 0)) - abs(dynamic_metrics.get('max_drawdown', 0))
        }
        ...
        comparison['calmar'] = {
            ...
            'excess': dynamic_metrics.get('calmar_ratio', 0) - benchmark_metrics.get('calmar_ratio', 0)
        }

        comparison['sortino'] = {
            'dynamic': dynamic_metrics.get('sortino_ratio', 0),
            'benchmark': benchmark_metrics.get('sortino_ratio', 0),
            'excess': dynamic_metrics.get('sortino_ratio', 0) - benchmark_metrics.get('sortino_ratio', 0)
        }
    ...
    def _analyze_trades(self, dynamic_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for record in allocation_history:
            ...
            if weight_changes:
                ...
                total_change = sum(abs(curr_weights.get(k, 0) - prev_weights.get(k, 0)) for k in set(prev_weights) | set(curr_weights))
        ...
        return {
            ...
            'cost_as_percentage_of_final_value': total_cost / dynamic_results.get('final_portfolio_value', 1) * 100,
    ...
    def _analyze_risk(
        ...
        return {
            ...
            'conditional_var_95': float(returns[returns <= var_95].mean() * np.sqrt(252)) if len(returns[returns <= var_95]) > 0 else 0
        }
    ...
    def _analyze_attribution(self, dynamic_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for alloc_record, regime_record in zip(allocation_history, regime_history):
            ...
            performance_change = alloc_record['portfolio_value'] / 100000 - 1  # 假設基準100k
        ...
        return {
            ...
            'average_overlay_per_period': overlay_contribution / len(overlay_history) if overlay_history else 0,
            'regime_switching_frequency': len(set(r['current_regime'] for r in regime_history)) / len(regime_history),
    ...
    def _execute_allocation(
        ...
        return {
            ...
            'overlay_cost': overlay_result.total_overlay_cost,
            'total_cost': strategic_allocation.total_transaction_cost + overlay_result.total_overlay_cost,
    ...
    def save_results(self, results: BacktestResults, filepath: str) -> None:
        ...
        serializable_results = {
            ...
            'dynamic_summary': {
                'final_portfolio_value': results.dynamic_results.get('final_portfolio_value', 0),
                'total_transactions': results.dynamic_results.get('total_transactions', 0),
                'total_costs': results.dynamic_results.get('total_costs', 0),
                'final_metrics': results.dynamic_results.get('final_metrics', {})
            },
            'benchmark_summary': {
                'final_portfolio_value': results.benchmark_results.get('final_portfolio_value', 0),
                'final_metrics': results.benchmark_results.get('final_metrics', {}),
                'benchmark_weights': results.benchmark_results.get('benchmark_weights', {})
            },
