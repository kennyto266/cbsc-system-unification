...
class VectorizedTechnicalIndicators:
        ...
        if VECTORTBT_AVAILABLE and 'close' in price_data.columns:
            ...
            for period in periods:
                try:
                    ...
                    logger.warning(f"VectorBT RSI({period}) failed, using fallback: {e}")
                    # Fallback to core indicators
                    if self.core_indicators:
                        rsi_series = self.core_indicators.calculate_rsi(price_data['close'], period)
                        results[f'rsi_{period}'] = rsi_series
                    else:
                        # Manual RSI calculation
                        results[f'rsi_{period}'] = self._calculate_rsi_manual(price_data['close'], period)
        else:
            # Fallback to manual calculations
            results = {}
            for period in periods:
                if self.core_indicators:
                    results[f'rsi_{period}'] = self.core_indicators.calculate_rsi(price_data['close'], period)
                else:
                    results[f'rsi_{period}'] = self._calculate_rsi_manual(price_data['close'], period)
                ...
                for slow in slow_periods:
                    for signal in signal_periods:
                        ...
                        try:
                            ...
                            logger.debug(f"VectorBT MACD({fast},{slow},{signal}) calculated successfully")
                        except Exception as e:
                            logger.warning(f"VectorBT MACD({fast},{slow},{signal}) failed, using fallback: {e}")
            ...
            for period in periods:
                for std_dev in std_devs:
                    ...
                    try:
                        ...
                        logger.debug(f"VectorBT BB({period},{std_dev}) calculated successfully")
                    except Exception as e:
                        logger.warning(f"VectorBT BB({period},{std_dev}) failed, using fallback: {e}")
                        # Fallback calculation
                ...
                for ma_type in ma_types:
                    ...
                    try:
                        if ma_type == 'sma':
                            ...
                            ma = vbt.MA.run(price_data['close'], window=period, ewm=True)
                        else:
                            continue

                        results[param_key] = ma.ma
                        logger.debug(f"VectorBT {ma_type.upper()}({period}) calculated successfully")
                    except Exception as e:
                        logger.warning(f"VectorBT {ma_type.upper()}({period}) failed, using fallback: {e}")
                        # Fallback calculation
    ...
    def calculate_stochastic_oscillator_vectorized(
        ...
        if VECTORTBT_AVAILABLE and all(col in price_data.columns for col in ['high', 'low', 'close']):
            results = {}
            for k_period in k_periods:
                for d_period in d_periods:
                    ...
                    try:
                        ...
                        logger.debug(f"VectorBT Stoch({k_period},{d_period}) calculated successfully")
                    except Exception as e:
                        logger.warning(f"VectorBT Stoch({k_period},{d_period}) failed, using fallback: {e}")
                        # Fallback calculation
                        results[param_key] = self._calculate_stochastic_manual(
                            price_data['high'], price_data['low'], price_data['close'], k_period, d_period
                        )
        else:
            # Fallback to manual calculations
            results = {}
        ...
        if VECTORTBT_AVAILABLE and all(col in price_data.columns for col in ['high', 'low', 'close']):
            ...
            for period in periods:
                ...
                try:
                    ...
                    logger.warning(f"VectorBT ATR({period}) failed, using fallback: {e}")
                    # Fallback calculation
    ...
    def calculate_adaptive_parameters(
        ...
        vol_norm = (volatility - volatility.min()) / (volatility.max() - volatility.min() + 1e-10)
        trend_norm = (trend_strength - trend_strength.min()) / (trend_strength.max() - trend_strength.min() + 1e-10)
    ...
    def _calculate_ma_manual(self, prices: pd.Series, period: int, ma_type: str = 'sma') -> pd.Series:
...
class CrossIndicatorAnalyzer:
    ...
    def generate_weighted_signals(
        ...
        for indicator_name, indicator_data in indicators.items():
            indicator_signals = self._indicator_to_signals(indicator_name, indicator_data)
    ...
    def detect_regime_based_signals(
        ...
        if 'ma' in str(indicators.keys()).lower():
            regime_signals['trending'] = self._generate_trending_signals(price_data, indicators, trending_regime)

        # Volatile regime signals
        if 'bb' in str(indicators.keys()).lower():
            regime_signals['volatile'] = self._generate_volatility_signals(price_data, indicators, volatile_regime)

        # Calm regime signals
        regime_signals['calm'] = self._generate_mean_reversion_signals(price_data, indicators, calm_regime)

        return regime_signals

    def _indicator_to_signals(self, indicator_name: str, indicator_data: Any) -> pd.Series:
        ...
        if isinstance(indicator_data, pd.Series):
            if 'rsi' in indicator_name.lower():
                ...
                signals = np.where(indicator_data > 70, -1, np.where(indicator_data < 30, 1, 0))
                ...
                if isinstance(indicator_data, dict) and 'stoch_k' in indicator_data:
                    signals = np.where(indicator_data['stoch_k'] > 80, -1,
    ...
    def _generate_trending_signals(self, price_data, indicators, regime_mask):
        ...
        if regime_mask.any():
            ...
            for indicator_name, indicator_data in indicators.items():
                if 'ma' in indicator_name.lower() and isinstance(indicator_data, pd.Series):
            ...
            for indicator_name, indicator_data in indicators.items():
                if 'bb' in indicator_name.lower() and isinstance(indicator_data, dict):
                    if 'percent_b' in indicator_data:
                        ...
                        if len(bb_percent) > 1:
                            band_signals = np.where(bb_percent > 0.8, -1,
                                                   np.where(bb_percent < 0.2, 1, 0))
                            band_series = pd.Series(band_signals, index=bb_percent.index)
                            band_series = band_series.reindex(signals.index, fill_value=0)
    ...
    def _generate_mean_reversion_signals(self, price_data, indicators, regime_mask):
        ...
        if regime_mask.any():
            ...
            for indicator_name, indicator_data in indicators.items():
                if 'rsi' in indicator_name.lower() and isinstance(indicator_data, pd.Series):
                    rsi_values = indicator_data[regime_mask]
                    if len(rsi_values) > 1:
                        rsi_signals = np.where(rsi_values > 70, -1,
                                            np.where(rsi_values < 30, 1, 0))
...
class AdaptiveIndicatorParameters:
    ...
    def _optimize_single_window(self, price_data: pd.DataFrame, indicator_type: str) -> Dict[str, float]:
    ...
    def _optimize_rsi_parameters(self, price_data: pd.DataFrame) -> Dict[str, float]:
        ...
        for period in range(5, 31, 2):  # Test odd periods from 5 to 29
            rsi = VectorizedTechnicalIndicators()._calculate_rsi_manual(price_data['close'], period)
        ...
        for fast in [8, 10, 12, 15]:
            for slow in [21, 24, 26, 30]:
                for signal in [6, 8, 9, 12]:
                    if fast < slow:
                        macd_data = VectorizedTechnicalIndicators()._calculate_macd_manual(
                            price_data['close'], fast, slow, signal
                        )

                        # Generate signals based on MACD crossover
                        signals = np.where(macd_data['macd'] > macd_data['signal'], 1, -1)
    ...
    def _optimize_bb_parameters(self, price_data: pd.DataFrame) -> Dict[str, float]:
        ...
        for period in [10, 15, 20, 25]:
            for std_dev in [1.5, 2.0, 2.5]:
                bb_data = VectorizedTechnicalIndicators()._calculate_bollinger_bands_manual(
                    price_data['close'], period, std_dev
                )

                # Generate signals based on Bollinger Bands
                signals = np.where(
                    price_data['close'] > bb_data['upper'], -1,
...
def benchmark_indicator_performance(
    ...
    for indicator_name, indicator_data in indicators.items():
        ...
        if len(strategy_returns) > 30:
            ...
            avg_signal_return = strategy_returns[strategy_returns != 0].mean() if len(strategy_returns[strategy_returns != 0]) > 0 else 0
...
def test_enhanced_technical_indicators():
    ...
    stoch_results = enhanced_indicators.calculate_stochastic_oscillator_vectorized(price_data)
