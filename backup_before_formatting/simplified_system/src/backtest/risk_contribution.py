...
class RiskContributionConfig:
    ...
    # 风险度量
    risk_measures: List[str] = field(default_factory=lambda: ["volatility", "var", "cvar"])
        ...
        return {
            'marginal_contributions': {
                mc.asset_name: {
                    ...
                    'percentage_contribution': round(mc.percentage_contribution * 100, 2),
            ...
            'factor_contributions': {
                fc.factor_name: {
                    ...
                    'contribution_to_portfolio_risk': round(fc.contribution_to_portfolio_risk, 6),
                    'eigenvalue': round(fc.eigenvalue, 6) if fc.eigenvalue is not None else None
                }
...
class RiskContributionCalculator:
    ...
    def calculate_marginal_contributions(
        ...
        try:
            ...
            # 计算因子贡献
            factor_results = self._calculate_factor_contributions(returns_clean, weights)
    ...
    def calculate_rolling_contributions(
        ...
        for i in range(window, len(returns)):
            ...
            try:
                ...
                logger.warning(f"Rolling contribution calculation failed at index {i}: {e}")
                continue
    ...
    def attribution_analysis(
        ...
        try:
            ...
            # 计算两个时期的风险贡献
            current_analysis = self.calculate_marginal_contributions(returns, current_weights)
            previous_analysis = self.calculate_marginal_contributions(returns, previous_weights)
    ...
    def _calculate_marginal_contributions_by_measure(
        ...
        if risk_measure == "volatility":
            ...
            return self._var_contributions(weights, cov_matrix, returns, confidence_level)
        elif risk_measure == "cvar":
            return self._cvar_contributions(weights, cov_matrix, returns, confidence_level)
        else:
    ...
    def _var_contributions(
        ...
        # 调整系数 (简化版本)
        adjustment_factor = 1 + (skewness / 6) * (norm.ppf(confidence_level) ** 2 - 1) + \
    ...
    def _cvar_contributions(
        ...
        return {
            'marginal': conditional_contributions / np.sum(np.abs(conditional_contributions)),
            'component': conditional_contributions,
            'percentage': np.abs(conditional_contributions) / np.sum(np.abs(conditional_contributions)),
            'volatility': np.zeros_like(weights),  # 占位符
            'var': conditional_contributions,
            'cvar': conditional_contributions / np.mean(portfolio_sim_returns[tail_scenarios])
        }
    ...
    def _calculate_factor_contributions(
        ...
        try:
            ...
            # 标准化数据
            standardized_returns = (returns_array - np.mean(returns_array, axis=0)) / np.std(returns_array, axis=0)
            ...
            for i in range(min(self.config.num_factors, len(weights))):
                factor_results.append({
                    ...
                    'exposure': weights[i],
                    'risk': np.sqrt(np.diag(returns.cov().values)[i]) if i < len(weights) else 0.0,
                    'contribution': weights[i] * np.sqrt(np.diag(returns.cov().values)[i]) if i < len(weights) else 0.0,
                    'eigenvalue': None
                })
    ...
    def _calculate_weight_attribution(
        ...
        """计算权重变化归因"""
        # 权重变化对风险贡献的影响
        current_contributions = np.array([mc.component_contribution for mc in current_analysis.marginal_contributions])
        previous_contributions = np.array([mc.component_contribution for mc in previous_analysis.marginal_contributions])
        ...
        return {
            ...
            'attribution_by_asset': {
                asset: {
                    'weight_change': float(wc),
                    'contribution_change': float(cc)
                }
    ...
    def _calculate_volatility_attribution(
        ...
        """计算波动率归因"""
        vol_change = current_analysis.portfolio_volatility - previous_analysis.portfolio_volatility
        div_ratio_change = current_analysis.diversification_ratio - previous_analysis.diversification_ratio
        ...
        return {
            ...
            'volatility_attribution_sources': {
                'component_contributions_change': (
                    sum([mc.component_contribution for mc in current_analysis.marginal_contributions]) -
                    sum([mc.component_contribution for mc in previous_analysis.marginal_contributions])
                ),
                'diversification_effect': div_ratio_change * current_analysis.portfolio_volatility
            }
        }
...
def calculate_risk_contributions(
    ...
    return calculator.calculate_marginal_contributions(returns, weights, risk_measure, confidence_level)