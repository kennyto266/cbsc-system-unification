...
from .professional_risk_metrics import RiskCalculator, RiskMetrics, calculate_portfolio_risk
...
class DynamicPositionSizer:
    ...
    def calculate_kelly_criterion_weights(
        self,
        returns: pd.DataFrame,
        lookahead_returns: Optional[pd.DataFrame] = None
    ) -> Dict[str, float]:
        """
        ...
        for i in range(lookback_window, len(dates)):
            ...
            if i % rebalance_window == 0:
                ...
                if strategy == 'max_sharpe':
                    allocation = self.portfolio_optimizer.maximize_sharpe_ratio(hist_returns)
                elif strategy == 'min_volatility':
                    allocation = self.portfolio_optimizer.minimize_volatility(hist_returns)
                elif strategy == 'risk_parity':
                    risk_parity_weights = self.position_sizer.calculate_risk_parity_weights(hist_returns)
...
def test_portfolio_optimization():
    ...
    ef_returns, ef_volatilities, ef_weights = optimizer.calculate_efficient_frontier(returns, 50)
    ...
    logger.info(f"Max Sharpe portfolio: Return={max_sharpe_portfolio.expected_return:.3f}, "
               f"Vol={max_sharpe_portfolio.expected_volatility:.3f}, "
               f"Sharpe={max_sharpe_portfolio.sharpe_ratio:.3f}")
    logger.info(f"Min Vol portfolio: Return={min_vol_portfolio.expected_return:.3f}, "
