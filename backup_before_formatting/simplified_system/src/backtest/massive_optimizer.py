...
from .parallel_optimizer import ParallelOptimizer, OptimizationConfig, parallel_optimizer
...
class MassiveOptimizer:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # 優化配置
        self.results_dir = self.config.get('results_dir', 'massive_optimization_results')
        ...
        try:
            ...
            final_results = {
                'optimization_summary': {
                    ...
                    'optimization_metric': optimization_metric,
                    'max_combinations': max_combinations,
                    'actual_combinations': optimization_results.get('successful_combinations', 0),
                ...
                'best_strategy': {
                    ...
                    'performance_metrics': optimization_results.get('best_performance', {}),
                    'backtest_result': best_result.to_dict() if best_result else None
                },

                'optimization_statistics': optimization_results.get('performance_statistics', {}),
                'top_strategies': optimization_results.get('top_results', [])[:10],

                'performance_report': performance_report,
                'overfitting_analysis': overfitting_analysis.to_dict() if overfitting_analysis else None,

                'parameter_space_analysis': {
                    'total_possible_combinations': param_space.get_total_combinations(),
                    'efficiency_ratio': len(parameter_combinations) / param_space.get_total_combinations(),
                    'parameter_importance': self._analyze_parameter_importance(optimization_results)
                },

                'system_performance': {
                    'parallel_efficiency': self.parallel_optimizer.stats.get('parallel_efficiency', 0),
                    'tasks_per_second': self.parallel_optimizer.stats.get('tasks_per_second', 0),
                    'cache_hit_rate': self.parallel_optimizer.stats.get('cache_hit_rate', 0)
                },
            ...
            logger.info(f"Massive optimization completed for {strategy_name} on {symbol}")
    ...
    def optimize_multiple_strategies(
        ...
        Returns:
            多策略優化結果
        """
        logger.info(f"Starting multi-strategy massive optimization for {len(strategy_names)} strategies")
        ...
        try:
            ...
            # 執行多策略優化
            multi_strategy_results = self.parallel_optimizer.optimize_multiple_strategies(
            ...
            final_results = {
                'multi_strategy_summary': {
                    ...
                    'total_combinations': sum(
                        r.get('total_combinations', 0) for r in multi_strategy_results.values()
                    ),
                    'total_successful': sum(
                        r.get('successful_combinations', 0) for r in multi_strategy_results.values()
                    ),
                    'execution_time': time.time() - start_time,
    ...
    def run_million_parameter_challenge(
        ...
        Returns:
            百萬參數挑戰結果
        """
        logger.info(f"Starting MILLION PARAMETER CHALLENGE: {target_combinations:,} combinations")
        ...
        try:
            ...
            if total_possible < target_combinations:
                logger.warning(f"Strategy only supports {total_possible:,} combinations, "
            ...
            combinations_per_second = optimization_results.get('successful_combinations', 0) / challenge_time
            parallel_efficiency = high_performance_optimizer.stats.get('parallel_efficiency', 0)
            ...
            challenge_results = {
                'challenge_summary': {
                    ...
                    'best_metric_found': optimization_results.get('best_performance', {}).get(optimization_metric, 0)
                },

                'performance_breakdown': {
                    'total_execution_time': challenge_time,
                    'average_task_time': high_performance_optimizer.stats.get('average_task_time', 0),
                    'cache_hit_rate': high_performance_optimizer.stats.get('cache_hit_rate', 0),
                    'system_resources': {
            ...
            with open(challenge_path, 'w', encoding='utf-8') as f:
                json.dump(challenge_results, f, indent=2, ensure_ascii=False, default=str)

            logger.info(f"MILLION PARAMETER CHALLENGE COMPLETED!")
            logger.info(f"Performance: {combinations_per_second:,.1f} combinations/second")
    ...
    def _split_data(self, data: pd.DataFrame, validation_split: float) -> Tuple[pd.DataFrame, Optional[pd.DataFrame]]:
        """分割訓練和驗證數據"""
    ...
    def _get_all_backtest_results(
        ...
        for result_info in top_results:
            try:
                ...
                logger.warning(f"Failed to get backtest result for params {params}: {e}")
                continue

        return all_results

    def _analyze_parameter_importance(self, optimization_results: Dict[str, Any]) -> Dict[str, float]:
        """分析參數重要性"""
        # 簡化實現 - 實際需要更複雜的統計分析
        ...
        for param_name, value_counts in param_frequency.items():
            ...
            entropy = -sum((count / total_count) * np.log2(count / total_count)
    ...
    def _calculate_challenge_grade(self, combinations_per_second: float,
                                 parallel_efficiency: float,
                                 target_combinations: int) -> Dict[str, Any]:
        """計算百萬參數挑戰等級"""
        performance_score = 0
    ...
    def _save_optimization_results(self, strategy_name: str, symbol: str, results: Dict[str, Any]):
        """保存優化結果"""
    ...
    def _generate_html_report(self, strategy_name: str, symbol: str, results: Dict[str, Any]):
        """生成HTML報告"""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    def _create_html_report_content(self, results: Dict[str, Any]) -> str:
        ...
        for param, value in best_params.items():
            params_html += f'<div class="metric"><strong>{param}:</strong> {value}</div>'

        # 過擬合分析部分
        overfitting_section = ""
        if results.get('overfitting_analysis'):
            overfitting = results['overfitting_analysis']
            risk_level = overfitting.get('overfitting_assessment', {}).get('risk_level', 'UNKNOWN')
            risk_score = overfitting.get('overfitting_assessment', {}).get('overfitting_risk_score', 0)
        ...
        return html_template.format(
            ...
            best_sharpe=best_strategy.get('performance_metrics', {}).get('sharpe_ratio', 0),
            total_return=best_strategy.get('performance_metrics', {}).get('total_return', 0),
            max_drawdown=best_strategy.get('performance_metrics', {}).get('max_drawdown', 0),
            win_rate=best_strategy.get('performance_metrics', {}).get('win_rate', 0),
            best_parameters_html=params_html,
            total_score=performance_report.get('multi_objective_score', {}).get('total_score', 0),
            grade=performance_report.get('multi_objective_score', {}).get('grade', 'N/A'),
            recommendation=performance_report.get('investment_recommendation', {}).get('recommendation', 'N/A'),
...
if __name__ == "__main__":
    ...
    parser.add_argument('--million-challenge', action='store_true', help='運行百萬參數挑戰')
    parser.add_argument('--target-combinations', type=int, default=1000000, help='百萬挑戰目標組合數')
    ...
    if 'error' in results:
        ...
        if args.million_challenge:
            ...
            print(f"   處理速度: {challenge_summary.get('combinations_per_second', 0):,.1f} 組合/秒")
            print(f"   並行效率: {challenge_summary.get('parallel_efficiency_percent', 0):.1f}%")
            print(f"   挑戰等級: {results.get('challenge_grade', {}).get('grade', 'N/A')}")
        else:
            optimization_summary = results.get('optimization_summary', {})
            best_strategy = results.get('best_strategy', {})
