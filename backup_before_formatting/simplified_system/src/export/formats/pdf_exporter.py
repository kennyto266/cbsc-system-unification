...
class PDFExporter:
    ...
    def _export_with_weasyprint(self, data: Any, output_path: Path, options: Dict = None) -> bool:
        """使用WeasyPrint導出PDF"""
        try:
    ...
    def _export_with_reportlab(self, data: Any, output_path: Path, options: Dict = None) -> bool:
        """使用ReportLab導出PDF"""
        try:
    ...
    def _export_with_pdfkit(self, data: Any, output_path: Path, options: Dict = None) -> bool:
        """使用PDFKit導出PDF"""
        try:
            import pdfkit
...
<body>
    ...
    def _generate_content_sections(self, data: Any) -> str:
        ...
        if isinstance(data, dict):
            ...
            if 'performance_metrics' in data:
                content += self._generate_performance_section(data['performance_metrics'])
            ...
            content += f"<div class='data-section'><h2>數據</h2><p>{str(data)}</p></div>"
    ...
    def _generate_reportlab_content(self, data: Any, styles) -> List:
        ...
        if isinstance(data, dict):
            # 摘要表格
            if 'summary' in data:
                content.extend(self._create_summary_table_reportlab(data['summary'], styles))

            # 性能指標表格
            if 'performance_metrics' in data:
                content.extend(self._create_performance_table_reportlab(data['performance_metrics'], styles))

            # 交易記錄表格
            if 'trades' in data and not data['trades'].empty:
                content.extend(self._create_trades_table_reportlab(data['trades'], styles))
