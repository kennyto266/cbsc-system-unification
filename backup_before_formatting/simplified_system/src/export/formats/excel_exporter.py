...
class ExcelExporter:
    ...
    def _export_dict_data(self, writer: pd.ExcelWriter, data: Dict, options: Dict = None):
        """導出字典數據到多個工作表"""
        sheet_index = 0
        ...
        for key, value in data.items():
            ...
            try:
                if isinstance(value, pd.DataFrame):
                    # 導出DataFrame
                    self._export_dataframe(writer, value, options, sheet_name=str(key)[:31])
                elif isinstance(value, (dict, list)):
                    # 轉換為DataFrame
                    df = pd.DataFrame(value) if isinstance(value, list) else pd.DataFrame([value])
                    self._export_dataframe(writer, df, options, sheet_name=str(key)[:31])
                else:
                    # 單個值
    ...
    def _export_dataframe(self, writer: pd.ExcelWriter, df: pd.DataFrame, options: Dict = None, sheet_name: str = None):
    ...
    def _export_large_dataframe(self, writer: pd.ExcelWriter, df: pd.DataFrame, options: Dict = None, sheet_name: str = None):
        """導出大DataFrame到多個工作表"""
        if sheet_name is None:
            sheet_name = 'Data'
    ...
    def _add_conditional_formatting(self, worksheet, df: pd.DataFrame):
        ...
        try:
            ...
            for idx, col_name in enumerate(numeric_columns):
                ...
                worksheet.conditional_formatting.add(f'{col_letter}2:{col_letter}{len(df)+1}', positive_rule)
                worksheet.conditional_formatting.add(f'{col_letter}2:{col_letter}{len(df)+1}', negative_rule)
    ...
    def _export_list_data(self, writer: pd.ExcelWriter, data: List, options: Dict = None):
        """導出列表數據"""
        try:
            if len(data) == 0:
                return
        ...
        try:
            with pd.ExcelWriter(output_path, engine=self.engine) as writer:
                ...
                summary_df = pd.DataFrame([
                    {'指標': '總回報率', '數值': f"{summary_data.get('total_return', 0):.2%}"},
                    {'指標': '年化回報率', '數值': f"{summary_data.get('annual_return', 0):.2%}"},
                    {'指標': 'Sharpe比率', '數值': f"{summary_data.get('sharpe_ratio', 0):.3f}"},
                    {'指標': '最大回撤', '數值': f"{summary_data.get('max_drawdown', 0):.2%}"},
                    {'指標': '波動率', '數值': f"{summary_data.get('volatility', 0):.2%}"},
                    {'指標': '交易次數', '數值': summary_data.get('trade_count', 0)},
                    {'指標': '勝率', '數值': f"{summary_data.get('win_rate', 0):.2%}"}
                ])
