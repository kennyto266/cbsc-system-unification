...
class ExportManager:
    ...
    def _load_config(self, config_path: str = None) -> Dict:
        """加載導出配置"""
        default_config_path = Path(__file__).parent.parent.parent.parent / "config" / "export_config.json"
    ...
    def batch_export(self, requests: List[ExportRequest]) -> List[ExportResult]:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            # 提交所有任務
            future_to_request = {
                executor.submit(self.export, request): request
            ...
            for future in as_completed(future_to_request):
                ...
                try:
                    ...
                    if result.success:
                        ...
                        logger.error(f"❌ 導出失敗: {result.filename} - {result.error_message}")
    ...
    def _generate_output_path(self, request: ExportRequest) -> Path:
        """生成輸出文件路徑"""
        output_dir = Path(self.config.get('export_settings', {}).get('output_directory', 'exports'))
