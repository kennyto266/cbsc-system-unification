...
class ExportMenu:
    ...
    def display_menu(self):
        ...
        print("\n" + "-"*60)
        print(f"📁 默認輸出目錄: {self.export_manager.config.get('export_settings', {}).get('output_directory', 'exports')}")
        print(f"📄 默認格式: {self.export_manager.config.get('export_settings', {}).get('default_format', 'xlsx')}")
        print(f"⏰ 自動時間戳: {'啟用' if self.export_manager.config.get('export_settings', {}).get('auto_timestamp', True) else '禁用'}")
        print("-"*60)
    ...
    def _export_technical_indicators(self):
        ...
        try:
            ...
            filename = input("請輸入文件名 (留空使用默認): ").strip()
            if not filename:
                filename = f"technical_indicators_{symbol}_{time.strftime('%Y%m%d_%H%M%S')}"
    ...
    def _batch_export_stocks(self):
        ...
        try:
            ...
            # 輸入股票列表
            stocks_input = input("請輸入股票代碼列表 (用逗號分隔，如 0700.HK,0941.HK): ").strip()
            if not stocks_input:
    ...
    def _export_optimization_results(self):
        ...
        try:
            ...
            choice = input(f"\n請選擇文件 (1-{min(10, len(optimization_files))}): ").strip()
            try:
    ...
    def _manage_export_settings(self):
        ...
        try:
            ...
            print(f"3. 自動時間戳: {'啟用' if settings.get('auto_timestamp', True) else '禁用'}")
            print(f"4. 包含圖表: {'啟用' if settings.get('include_charts', True) else '禁用'}")
            print(f"5. 包含原始數據: {'啟用' if settings.get('include_raw_data', False) else '禁用'}")
    ...
    def _view_export_history(self):
        ...
        try:
            ...
            output_dir = Path(self.export_manager.config.get('export_settings', {}).get('output_directory', 'exports'))
            ...
            for file_path in export_files[:20]:
                ...
                mod_time = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(file_path.stat().st_mtime))
                file_ext = file_path.suffix[1:].upper()

                print(f"{file_path.name:<30} {file_ext:<8} {file_size_str:<10} {mod_time:<20}")

            print(f"\n📁 總共 {len(export_files)} 個導出文件")
            print(f"💾 總大小: {self._format_file_size(sum(f.stat().st_size for f in export_files))}")
    ...
    def _update_export_setting(self, setting_choice: str):
        ...
        current_value = self.export_manager.config.get('export_settings', {}).get(setting_key)
        ...
        # 更新配置
        self.export_manager.config.setdefault('export_settings', {})[setting_key] = new_value
