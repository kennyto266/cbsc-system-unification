...
class BaseStrategy(ABC):
    ...
    @abstractmethod
    def generate_signals(self, data: pd.DataFrame, parameters: Dict[str, Any]) -> pd.Series:
        """
        生成交易信号
    ...
    def execute(self, data: pd.DataFrame, parameters: Optional[Dict[str, Any]] = None) -> pd.Series:
        """
        执行策略
        ...
        for param, value in parameters.items():
            if param in self.default_parameters:
                ...
                if not isinstance(value, type(default_value)):
                    raise TypeError(f"Parameter {param} should be {type(default_value)}, got {type(value)}")

    def calculate_performance(self, data: pd.DataFrame, signals: pd.Series) -> Dict[str, float]:
        """
        计算策略性能指标
