...
def test_rsi_combinations(data):
    ...
    if results:
        best = results[0]
        print(f"  RSI({best['period']}, {best['oversold']}, {best['overbought']}): "
...
def test_macd_combinations(data):
    ...
    if results:
        best = results[0]
        print(f"  MACD({best['fast']}, {best['slow']}, {best['signal']}): "
              f"SR={best['sharpe_ratio']:.3f}, MDD={best['max_drawdown']:.1%}, "
              f"Return={best['total_return']:.1%}")
...
def test_ma_combinations(data):
    ...
    if results:
        best = results[0]
        print(f"  MA({best['short_period']}, {best['long_period']}): "
              f"SR={best['sharpe_ratio']:.3f}, MDD={best['max_drawdown']:.1%}, "
              f"Return={best['total_return']:.1%}")
...
def analyze_results(rsi_results, macd_results, ma_results):
    ...
    print(f"Data period: {len(all_results[0].get('returns', []))} days" if all_results else "")

    # Show top 10 strategies
    print(f"\nTOP 10 STRATEGIES BY SHARPE RATIO:")
    print("-" * 80)
    print(f"{'Rank':<4} {'Strategy':<10} {'Parameters':<20} {'Sharpe':<8} {'Max DD':<10} {'Return':<8}")
    print("-" * 80)
    ...
    for i, result in enumerate(all_results[:10]):
        ...
        print(f"{i+1:<4} {result['strategy']:<10} {params:<20} "
...
def main():
    ...
    print(f"📈 Best Sharpe Ratio: {top_results[0]['sharpe_ratio']:.3f}" if top_results else "N/A")
    print(f"📉 Best Max Drawdown: {top_results[0]['max_drawdown']:.1%}" if top_results else "N/A")
