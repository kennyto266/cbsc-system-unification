...
from data_quality_validator import (
    ...
    validate_stock_data, quick_data_quality_check
)
...
class EnhancedStockAPI(StockDataAPI):
    ...
    def get_stock_data_with_validation(self, symbol: str, duration_days: int = 1095,
                                     require_validation: bool = True) -> Optional[Dict[str, Any]]:
        """
        獲取股票數據並進行質量驗證
        """
        ...
        if require_validation:
            ...
            if quality_report.quality_score < self.quality_threshold:
                logger.warning(f"⚠️ {symbol} 數據質量評分 {quality_report.quality_score:.1f} 低於要求 {self.quality_threshold}")
            ...
            if self.auto_fix_enabled and quality_report.quality_score < self.quality_threshold:
                ...
                if fixed_data:
                    # 重新驗證修復後的數據
                    fixed_report = self.quality_validator.validate_stock_data(fixed_data)

                    if fixed_report.quality_score > quality_report.quality_score:
                        logger.info(f"✅ {symbol} 自動修復成功，質量評分從 {quality_report.quality_score:.1f} 提升到 {fixed_report.quality_score:.1f}")
                        raw_data = fixed_data
            ...
            raw_data['quality_report'] = {
                ...
                'issues': [issue.message for issue in quality_report.critical_issues + quality_report.high_issues]
            }

        return raw_data

    def get_stock_prices_dataframe_with_validation(self, symbol: str,
                                                 duration_days: int = 1095) -> Optional[Tuple[pd.DataFrame, DataQualityReport]]:
        """
    ...
    def get_multiple_stocks_with_validation(self, symbols: List[str],
                                         duration_days: int = 1095) -> Dict[str, Tuple[Optional[pd.DataFrame], Optional[DataQualityReport]]]:
        """
        批量獲取多只股票數據並進行質量驗證
        """
        results = {}
        ...
        for symbol in symbols:
            ...
            try:
                result = self.get_stock_prices_dataframe_with_validation(symbol, duration_days)
                results[symbol] = result

                if result:
                    df, quality_report = result
                    if quality_report:
                        logger.info(f"✅ {symbol}: 質量評分 {quality_report.quality_score:.1f}")
                    else:
    ...
    def _convert_stock_dict_to_dataframe(self, data: Dict[str, Any]) -> pd.DataFrame:
        ...
        if 'data' in data and 'close' in data['data']:
            ...
            if 'close' in df.columns:
                df['open'] = df['close'].shift(1).fillna(df['close'])  # 前一日收盤價作為開盤價
    ...
    def _auto_fix_stock_data_issues(self, data: Dict[str, Any], quality_report: DataQualityReport) -> Optional[Dict[str, Any]]:
        """自動修復股票數據問題"""
        try:
            fixed_data = data.copy()

            # 修復1：OHLC邏輯問題
            if any("OHLC邏輯檢查" in issue.message for issue in quality_report.critical_issues):
                fixed_data = self._fix_ohlc_logic_issues(fixed_data)

            # 修復2：價格異常跳變
            if any("價格跳變檢測" in issue.message for issue in quality_report.high_issues):
                fixed_data = self._fix_price_spike_issues(fixed_data)

            # 修復3：缺失數據
            if any("完整性" in issue.message for issue in quality_report.critical_issues):
    ...
    def detect_market_anomalies(self, symbol: str, duration_days: int = 30) -> Dict[str, Any]:
        """
        檢測市場異常情況
        """
        logger.info(f"🔍 檢測 {symbol} 市場異常情況")

        df_result = self.get_stock_prices_dataframe_with_validation(symbol, duration_days)
        ...
        if 'close' in df.columns:
            ...
            consecutive_flat = self._find_consecutive_periods(flat_periods, min_length=3)

            if consecutive_flat:
                anomalies["anomalies_detected"].append({
                    "type": "potential_trading_halt",
                    "flat_periods": consecutive_flat
                })

        # 異常檢測4：數據缺失
        expected_trading_days = len([d for d in pd.date_range(df.index.min(), df.index.max(), freq='B')
                                   if d.weekday() < 5])  # 只計算工作日
        ...
        logger.info(f"✅ {symbol} 異常檢測完成，發現 {len(anomalies['anomalies_detected'])} 種異常情況")
        return anomalies
    ...
    def generate_data_quality_summary(self, symbols: List[str],
                                    duration_days: int = 1095) -> Dict[str, Any]:
        """
        生成多只股票的數據質量摘要
        """
            ...
            try:
                ...
                if df_result:
                    ...
                    symbol_analysis = {
                        ...
                        "validation_passed": quality_report.quality_score >= self.quality_threshold
                    }
        ...
        if all_quality_scores:
            summary["overall_statistics"] = {
                ...
                "validation_pass_rate": validation_passed / successful_collections if successful_collections > 0 else 0
            }

        logger.info(f"✅ 數據質量摘要生成完成")
        logger.info(f"   收集成功率: {summary['overall_statistics'].get('collection_success_rate', 0):.1%}")
        logger.info(f"   驗證通過率: {summary['overall_statistics'].get('validation_pass_rate', 0):.1%}")
        logger.info(f"   平均質量評分: {summary['overall_statistics'].get('average_quality_score', 0):.1f}")
...
# 便捷函數
def get_stock_data_with_validation(symbol: str, duration_days: int = 1095) -> Optional[Dict[str, Any]]:
    """獲取並驗證股票數據"""
    return enhanced_stock_api.get_stock_data_with_validation(symbol, duration_days)

def get_stock_prices_dataframe_with_validation(symbol: str, duration_days: int = 1095) -> Optional[Tuple[pd.DataFrame, DataQualityReport]]:
    """獲取股票DataFrame並返回質量報告"""
    return enhanced_stock_api.get_stock_prices_dataframe_with_validation(symbol, duration_days)

def get_multiple_stocks_with_validation(symbols: List[str], duration_days: int = 1095) -> Dict[str, Tuple[Optional[pd.DataFrame], Optional[DataQualityReport]]]:
    """批量獲取並驗證多只股票數據"""
    return enhanced_stock_api.get_multiple_stocks_with_validation(symbols, duration_days)
...
def generate_data_quality_summary(symbols: List[str], duration_days: int = 1095) -> Dict[str, Any]:
    """生成數據質量摘要"""
