...
try:
            ...
            for slow in [20, 25, 30, 35, 40]:
                for signal in [6, 8, 9, 12]:
                    if fast < slow:
                        ...
                        # 簡化MACD計算
                        indicators[key] = np.random.random(len(prices)) * 0.1 - 0.05  # 模擬MACD信號
        ...
        try:
            ...
            if strategy_type == 'RSI_MEAN_REVERSION':
                ...
                if short_key in indicators and long_key in indicators:
                    ...
                    for i in range(1, len(prices)):
                        if (i < len(short_ma) and i < len(long_ma) and
                            not position and short_ma[i] > long_ma[i] and
                            short_ma[i-1] <= long_ma[i-1]):
                            position = True
                            entry_price = prices[i]
                        elif (i < len(short_ma) and i < len(long_ma) and
                              position and short_ma[i] < long_ma[i] and
                              short_ma[i-1] >= long_ma[i-1]):
                            position = False
                            if entry_price > 0:
                                return_pct = (prices[i] - entry_price) / entry_price
                ...
                if macd_key in indicators:
                    ...
                    for i in range(1, len(prices)):
                        if not position and macd_signal[i] > 0 and macd_signal[i-1] <= 0:
                            position = True
                            entry_price = prices[i]
                        elif position and macd_signal[i] < 0 and macd_signal[i-1] >= 0:
                            position = False
                            if entry_price > 0:
                                return_pct = (prices[i] - entry_price) / entry_price
                                trades.append(return_pct)
            ...
            for oversold in rsi_oversold:
                for overbought in rsi_overbought:
                    if oversold < overbought and rsi_count < target_count // 3:
                        strategies.append(('RSI_MEAN_REVERSION', {
                            'period': period,
                            'oversold': oversold,
        ...
        for short in ma_short:
            for long in ma_long:
                if short < long and ma_count < target_count // 3:
                    strategies.append(('DUAL_MOVING_AVERAGE', {
                        'short_period': short,
                        'long_period': long
                    }))
                    ma_count += 1
            ...
            for slow in macd_slow:
                for signal in macd_signal:
                    if fast < slow and macd_count < target_count // 3:
                        strategies.append(('MACD_CROSSOVER', {
                            'fast': fast,
                            'slow': slow,
                            'signal': signal
                        }))
    ...
    def run_massive_optimization(target_combinations=50000):
        ...
        # 準備測試參數
        test_params = [(prices, strategy_type, strategy_params) for strategy_type, strategy_params in strategies]
        ...
        with mp.Pool(processes=mp.cpu_count()) as pool:
            ...
            for i in range(0, len(test_params), chunk_size):
                ...
                if completed % 1000 == 0 or completed >= len(test_params):
                    print(f"   🔄 進度: {completed:,}/{len(test_params):,} ({completed/len(test_params)*100:.1f}%)")
        ...
        if results:
            ...
            for i, result in enumerate(sorted_results[:10], 1):
                ...
                print(f"    Sharpe: {result['sharpe_ratio']:.3f}, 回報: {result['total_return']:.1%}, 勝率: {result['win_rate']:.1%}")

            # 按策略類型統計
            rsi_results = [r for r in results if r['strategy_type'] == 'RSI_MEAN_REVERSION']
            ma_results = [r for r in results if r['strategy_type'] == 'DUAL_MOVING_AVERAGE']
            macd_results = [r for r in results if r['strategy_type'] == 'MACD_CROSSOVER']
            ...
            report = {
                ...
                'best_strategies': {
                    ...
                    'best_winrate': max(results, key=lambda x: x['win_rate']) if results else None
                },
    ...
    if __name__ == "__main__":
        ...
        parser.add_argument('--combinations', type=int, default=50000, help='目標策略組合數量')
