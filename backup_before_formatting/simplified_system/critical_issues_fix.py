...
class CriticalIssuesFix:
                    ...
                    for missing_field in missing_fields:
                        for alt_field in field_mappings.get(missing_field, []):
                            if alt_field in sample_record:
                                print(f"[INFO] 找到替代字段: {alt_field} -> {missing_field}")
                                break
                else:
    ...
    def generate_fix_report(self):
        ...
        for test_name, result in self.test_results.items():
            ...
            status_icon = "[OK]" if status == 'PASS' else "[PARTIAL]" if status == 'PARTIAL' else "[FAIL]"
        ...
        try:
            full_report = {
                ...
                'system_status': 'GOOD' if success_rate >= 80 else 'ACCEPTABLE' if success_rate >= 60 else 'NEEDS_IMPROVEMENT'
            }
