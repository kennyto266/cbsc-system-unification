...
# 導入核心模塊
from src.backtest.vectorbt_engine import VectorBTEngine, VectorBTConfig
from src.backtest.vectorbt_optimizer import VectorBTOptimizer, OptimizationConfig
...
from src.backtest.signal_fusion_engine import SignalFusionEngine, SignalFusionConfig
...
def example_1_basic_vectorbt_engine():
    ...
    rsi_result = engine.backtest_strategy(data, 'RSI_MEAN_REVERSION', {
        'period': 14,
        'oversold': 30,
        'overbought': 70
    })
...
def example_2_batch_backtesting(data, engine):
    ...
    strategies = [
        ...
        ('VOLATILITY_BREAKOUT', {'volatility_period': 20, 'volatility_multiplier': 2.0})
    ]
    ...
    for i, result in enumerate(sorted_results, 1):
        ...
        print(f"{i:<4} {strategy_name:<20} {result.total_return:<10.2%} "
...
def example_4_bayesian_optimization(data):
    ...
    bayesian_results = bayesian_optimizer.optimize(
        evaluation_func=evaluate_rsi_strategy,
        maximize=True
    )
...
def example_5_professional_risk_management(data, engine):
    ...
    information_ratio = risk_metrics.calculate_information_ratio(returns, returns.mean())
...
def example_6_signal_fusion_engine(data):
    ...
    macd_signals = ((macd_line > macd_signal) &
                   (macd_histogram > 0)).astype(int) - ((macd_line < macd_signal)).astype(int)

    # 布林帶信號
    bb_upper, bb_middle, bb_lower = indicators.calculate_bollinger_bands(
        data['close'], 20, 2.0
    )
    bb_signals = ((data['close'] < bb_lower)).astype(int) - ((data['close'] > bb_upper)).astype(int)
...
def example_7_performance_benchmark():
    ...
    for size in sizes:
        ...
        start_time = time.time()
        result = engine.backtest_strategy(test_data, 'RSI_MEAN_REVERSION', {
            'period': 14, 'oversold': 30, 'overbought': 70
        })
        ...
        start_time = time.time()
        batch_results = engine.batch_backtest(test_data, strategies)
    ...
    print(f"\n📊 性能基準總結:")
    print("-" * 50)
    print(f"{'數據大小':<10} {'單策略(s)':<12} {'批量(s)':<10} {'策略/秒':<12} {'數據點/秒':<12}")
    print("-" * 50)

    for result in results:
        print(f"{result['data_size']:<10} {result['single_strategy_time']:<12.3f} "
...
def main():
    ...
    try:
        ...
        # 示例2: 批量回測
        batch_results = example_2_batch_backtesting(data, engine)

        # 示例3: 參數優化
        basic_opt, multi_opt = example_3_parameter_optimization(data, engine)

        # 示例4: 貝葉斯優化
        bayesian_results = example_4_bayesian_optimization(data)

        # 示例5: 專業風險管理
        risk_results = example_5_professional_risk_management(data, engine)

        # 示例6: 信號融合
        fusion_results = example_6_signal_fusion_engine(data)

        # 示例7: 性能基準
        benchmark_results = example_7_performance_benchmark()
