...
def analyze_factor_performance(data, factors, returns):
    ...
    try:
        ...
        if not report.empty:
            ...
            top_factors = report.sort_values('composite_score', ascending=False).head(10)
    ...
    try:
        ...
        for symbol in symbols:
            for factor_name, score in factor_scores.items():
                ...
                expanded_factor_scores[symbol][factor_name] = score * (1 + np.random.randn() * 0.1)

        # 選擇股票（這裡簡化處理）
        selected_stocks = portfolio_manager.select_stocks(
            pd.DataFrame(expanded_factor_scores),
            symbols
        )
        print(f"✓ 選中 {len(selected_stocks)} 個股票進行投資組合")

        # 分配權重
        weights = portfolio_manager.allocate_weights(selected_stocks, pd.DataFrame(expanded_factor_scores))

        # 計算投資組合績效
        portfolio_metrics = portfolio_manager.calculate_portfolio_metrics(
            weights,
            pd.DataFrame(expanded_factor_scores).mean(axis=1)  # 簡化處理
        )
...
def compare_strategies(original_results, alpha_optimized):
    ...
    try:
        ...
        improvement = {
            ...
            'Improvement_%': [
                ((comparison_df.loc[1, 'Sharpe_Ratio'] / comparison_df.loc[0, 'Sharpe_Ratio']) - 1) * 100,
                ((comparison_df.loc[1, 'Annualized_Return'] / comparison_df.loc[0, 'Annualized_Return']) - 1) * 100,
                ((comparison_df.loc[1, 'Max_Drawdown'] / comparison_df.loc[0, 'Max_Drawdown']) - 1) * 100,  # Drawdown改進是正的
                ((comparison_df.loc[1, 'Win_Rate'] / comparison_df.loc[0, 'Win_Rate']) - 1) * 100
            ]
        }
        ...
        # 生成優化建議
        generate_optimization_recommendations(original_results, alpha_optimized, comparison_df)
...
def generate_optimization_recommendations(original_results, alpha_optimized, comparison):
    """生成優化建議"""
    print("\n💡 Alpha因子優化建議:")
    print("=" * 60)

    try:
        sharpe_improvement = comparison.loc[1, 'Sharpe_Ratio'] - comparison.loc[0, 'Sharpe_Ratio']
        return_improvement = comparison.loc[1, 'Annualized_Return'] - comparison.loc[0, 'Annualized_Return']

        print(f"🎯 核心成果:")
        print(f"   Sharpe比率提升: {sharpe_improvement:+.3f} ({'顯著改善' if sharpe_improvement > 0.1 else '輕微改善'})")
        print(f"   年化回報提升: {return_improvement:+.2%} ({'顯著改善' if return_improvement > 0.05 else '輕微改善'})")
...
def save_optimization_results(original_results, comparison, alpha_optimized):
    ...
    try:
        ...
        results = {
            'timestamp': timestamp,
            'original_best_strategy': original_results.loc[original_results['sharpe_ratio'].idxmax()].to_dict(),
            'comparison_table': comparison.to_dict(),
            'alpha_optimized_portfolio': {
                'portfolio_metrics': alpha_optimized['portfolio_metrics'].__dict__ if hasattr(alpha_optimized['portfolio_metrics'], '__dict__') else str(alpha_optimized['portfolio_metrics']),
                'factor_importance': alpha_optimized['factor_importance'].to_dict() if hasattr(alpha_optimized.get('factor_importance'), 'to_dict') else {},
