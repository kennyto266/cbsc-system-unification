...
def demo_strategy_comparison(price_data):
    ...
    try:
        ...
        display_cols = ['description'] + [col for col in comparison.columns if col in key_metrics]
...
def demo_walkforward_optimization(price_data):
    ...
    try:
        # 創建簡化的Walk-Forward配置
        from simplified_system.src.backtest.enhanced.vectorbt_walkforward_optimizer import WalkForwardConfig
...
def main():
    ...
    # 2. 策略比較演示
    comparison = demo_strategy_comparison(price_data)

    # 3. 單策略深度分析演示
    analysis = demo_single_strategy_analysis(price_data)

    # 4. Walk-Forward優化演示
    wf_result = demo_walkforward_optimization(price_data)

    # 5. 高級投資組合分析演示
    portfolio_analysis = demo_advanced_portfolio_analysis(price_data)
