...
class SimpleBehavioralAnalysisTest:
    ...
    def generate_test_data(self):
        ...
        test_data = {
            ...
            'high_vol_data': pd.Series(prices + np.random.normal(0, 10, n_points), index=dates)
        }
        ...
        # 盘中数据
        intraday_dates = pd.date_range(start='2024-01-15 09:00:00', end='2024-01-15 16:30:00', freq='15min')
        intraday_prices = []
        base_price = 100
    ...
    def test_basic_statistics(self, data):
        ...
        try:
            ...
            basic_stats = {
                ...
                'skewness': float(((data['basic_data'] - data['basic_data'].mean()) / data['basic_data'].std()).skew()),
            ...
            logger.info(f"   Mean: {basic_stats['mean']:.2f}, Std: {basic_stats['std']:.2f}")
            logger.info(f"   Volatility ratio: {result['details']['volatility_analysis']['volatility_ratio']:.2f}")
        ...
        try:
            ...
            if len(ma_short.dropna()) > 0 and len(ma_long.dropna()) > 0:
                ...
                result['details']['moving_average_trend'] = {
                    ...
                    'trend_signal': 'bullish' if ma_relationship > 0.6 else 'bearish' if ma_relationship < 0.4 else 'neutral'
                }
    ...
    def test_seasonality_detection(self, data):
        ...
        try:
            ...
            # 判断是否存在季节性
            has_seasonality = any(
                scores['max_correlation'] > 0.3 for scores in seasonality_scores.values()
            )
            result['details']['has_seasonality'] = has_seasonality

            if has_seasonality:
                best_period = max(seasonality_scores.items(),
                                key=lambda x: x[1]['max_correlation'])[0]
        ...
        try:
            ...
            if len(lunch_data) > 0:
                result['details']['lunch_analysis'] = {
                    ...
                    'lunch_volatility': float(lunch_data.pct_change().std()) if len(lunch_data) > 1 else 0
                }
        ...
        try:
            ...
            for window in windows:
                if len(basic_returns) >= window:
                    ...
                    volatility_analysis[f'window_{window}'] = {
                        ...
                        'volatility_ratio': float(high_vol.mean() / basic_vol.mean()) if basic_vol.mean() > 0 else 0
                    }
            ...
            if len(basic_returns) >= 20:
                ...
                result['details']['volatility_clustering'] = {
                    'autocorr_lag1': float(autocorr_1) if not np.isnan(autocorr_1) else 0,
                    'autocorr_lag5': float(autocorr_5) if not np.isnan(autocorr_5) else 0,
                    'has_clustering': abs(autocorr_1) > 0.2 if not np.isnan(autocorr_1) else False
                }
            ...
            logger.info(f"   Volatility clustering detected: {clustering['has_clustering']}")
    ...
    def test_realtime_simulation(self, data):
        ...
        try:
            ...
            for i, (timestamp, value) in enumerate(stream_data.items()):
                ...
                if len(window_data) == window_size:
                    ...
                    if z_score > 2.5:  # 简单的异常阈值
                        alert_count += 1
                        logger.info(f"Simulated alert at index {i}: z-score = {z_score:.2f}")
            ...
            result['details']['realtime_simulation'] = {
                ...
                'samples_per_second': processed_count / processing_time if processing_time > 0 else 0,
                'window_size': window_size,
                'alert_rate': alert_count / processed_count if processed_count > 0 else 0
            }
            ...
            logger.info(f"   Processing rate: {result['details']['realtime_simulation']['samples_per_second']:.1f} samples/sec")
        ...
        for test_func, task_name in test_tasks:
            ...
            try:
                ...
                if task_result['status'] == 'passed':
                    ...
                    failed_tasks += 1
                    logger.error(f"[FAIL] {task_name} FAILED: {task_result.get('error', 'Unknown error')}")
    ...
    def print_final_report(self, report):
        ...
        for task_name, result in report['task_results'].items():
            ...
            if result['status'] == 'passed' and 'details' in result:
                ...
                if 'basic_statistics' in details:
                    ...
                    print(f"      - Data points: {stats['data_points']}, Mean: {stats['mean']:.2f}")
                elif 'linear_trend' in details:
                    trend = details['linear_trend']
                    print(f"      - Trend: {trend['trend_direction']}, R-squared: {trend['r_squared']:.3f}")
                    ...
                    print(f"      - Processing rate: {sim['samples_per_second']:.1f} samples/sec")
        ...
        if report['task_results'].get('Basic Statistical Analysis', {}).get('status') == 'passed':
        ...
        if report['task_results'].get('Seasonality Detection', {}).get('status') == 'passed':
            achievements.append("[PASS] Seasonal pattern identification")

        if report['task_results'].get('Simple Anomaly Detection', {}).get('status') == 'passed':
            achievements.append("[PASS] Anomaly detection with multiple methods")

        if report['task_results'].get('Intraday Pattern Recognition', {}).get('status') == 'passed':
            achievements.append("[PASS] Hong Kong trading session pattern analysis")

        if report['task_results'].get('Volatility Analysis', {}).get('status') == 'passed':
            achievements.append("[PASS] Multi-timeframe volatility analysis")

        if report['task_results'].get('Real-time Monitoring Simulation', {}).get('status') == 'passed':
        ...
        if summary['success_rate'] == '100.0%':
            ...
            print(f"   The simplified Phase 4 Behavioral Analysis Layer is fully functional.")
        else:
