...
def test_basic_system_components():
    ...
    try:
        ...
        if stock_data is not None and len(stock_data) > 0:
            ...
            test_results['stock_api'] = {'status': 'success', 'records': record_count, 'latest_price': latest_price}
        else:
    ...
    try:
        ...
        test_results['government_api'] = {
            ...
            'enabled_sources': sum(1 for config in government_api.data_sources.values() if config.get('enabled'))
        }
    except Exception as e:
    ...
    try:
        ...
        try:
            ...
            # 如果SMA计算失败，使用默认值
            sma_value = test_prices.rolling(5).mean().iloc[-1] if len(test_prices) >= 5 else test_prices.mean()
    ...
    try:
        ...
        if data_dir.exists():
            ...
            if complete_files:
                ...
                successful_sources = data.get('collection_info', {}).get('successful_sources', 0)
                total_sources = data.get('collection_info', {}).get('total_sources', 0)
                total_records = data.get('collection_info', {}).get('total_records', 0)

                print(f"   [+] Latest collection: {successful_sources}/{total_sources} sources")
                ...
                test_results['data_integrity'] = {'status': 'no_data', 'error': 'No collection files'}
        else:
            print("   [!] Government data directory not found")
            test_results['data_integrity'] = {'status': 'no_directory', 'error': 'Data directory missing'}
...
def generate_system_report(test_results, execution_time):
    ...
    if overall_status == 'PASS':
        report['recommendations'].append("System is fully operational and ready for production trading")
        report['recommendations'].append("All 8 government data sources integrated successfully")
        report['recommendations'].append("Technical indicators and APIs working correctly")
    else:
        report['recommendations'].append("Some components require attention before production use")
        report['recommendations'].append("Review failed components and fix integration issues")
...
def main():
    ...
    print(f"Tests Passed: {report['test_summary']['successful_tests']}/{report['test_summary']['total_tests']}")
