...
def demo_trend_indicators(indicators, stock_data):
    ...
    print(f"   趨勢強度: {'強' if abs(close_data.iloc[-1] - tema.iloc[-1]) > 5 else '弱'}")
...
def demo_momentum_indicators(indicators, stock_data):
    ...
    print(f"   計算時間: {stoch_result['adaptation_info']['calculation_time_ms']:.3f}ms")
...
def demo_volatility_indicators(indicators, stock_data):
    ...
    # ATR
    print("\n3. 平均真實範圍 (ATR)")
    atr_percent, atr_info = indicators.calculate_atr(
...
def demo_data_source_indicators(indicators, hibor_data):
    ...
    if 'term_spread' in hibor_result:
        ...
        print(f"   計算時間: {hibor_result['adaptation_info']['calculation_time_ms']:.3f}ms")
        print(f"   最新利差: {spread.iloc[-1]:.3f}%")
        print(f"   Z-Score: {zscore.iloc[-1]:.2f}")
        print(f"   信號: {signal.iloc[-1]:.0f} ({'看漲' if signal.iloc[-1] > 0 else '看跌' if signal.iloc[-1] < 0 else '中性'})")
    ...
    print(f"   計算時間: {spread_result['adaptation_info']['calculation_time_ms']:.3f}ms")
    print(f"   最新利差: {spread_result['spread'].iloc[-1]:.3f}%")
    print(f"   信號強度: {spread_result['signal_strength'].iloc[-1]:.2f}")
    print(f"   信號方向: {spread_result['signal_direction'].iloc[-1]:.0f} ({'正值' if spread_result['signal_direction'].iloc[-1] > 0 else '負值'})")
...
def generate_comprehensive_signals(trend_results, momentum_results, volatility_results, data_source_results):
    """生成綜合交易信號"""
    print("\n" + "="*60)
    print("🎯 綜合交易信號分析")
    print("="*60)
    ...
    if 'structure_signal' in data_source_results.get('HIBOR_TERM_STRUCTURE', {}):
        latest_hibor_signal = data_source_results['HIBOR_TERM_STRUCTURE']['structure_signal'].iloc[-1]
    ...
    return {
        ...
        'signal_counts': {'buy': buy_signals, 'sell': sell_signals, 'neutral': neutral_signals},
