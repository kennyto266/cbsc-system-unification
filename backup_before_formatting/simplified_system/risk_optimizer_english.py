...
class RiskAdjustedOptimizer:
    ...
    def validate_walk_forward_requirements(self, row: pd.Series) -> Tuple[bool, Dict]:
        ...
        validation_results['min_trades_passed'] = estimated_trades_per_window >= self.risk_constraints.min_trades_per_window

        validation_results['drawdown_passed'] = row['max_drawdown'] >= self.risk_constraints.max_drawdown_limit

        validation_results['sharpe_passed'] = row['sharpe_ratio'] >= self.risk_constraints.min_sharpe_threshold

        estimated_volatility = abs(row['total_return']) * 2
        validation_results['volatility_passed'] = estimated_volatility <= self.risk_constraints.max_volatility_limit

        validation_results['win_rate_passed'] = row['win_rate'] >= self.risk_constraints.min_win_rate
    ...
    def optimize_parameters(self, strategy_data: pd.DataFrame) -> List[OptimizationResult]:
        ...
        for idx, row in strategy_data.iterrows():
            try:
                ...
                validation_passed, risk_metrics = self.validate_walk_forward_requirements(row)
    ...
    def generate_optimization_report(self, results: List[OptimizationResult]) -> str:
        ...
        if valid_strategies:
            ...
            report += "{:<3} {:<25} {:<8} {:<10} {:<10} {:<10} {:<8}\n".format(
                "Rank", "Strategy", "Risk Score", "WF Sharpe", "WF Return", "WF Drawdown", "Trade Freq"
            )
            report += "-"*80 + "\n"
        ...
        if valid_strategies:
            ...
            report += f"Expected WF Drawdown: {best_strategy.walk_forward_drawdown:.2%}\n"
