...
def test_sharpe_calculator_directly():
    ...
    try:
        ...
        for method in methods:
            ...
            print(f"  Annual return: {result['annual_return']:.4f} ({result['annual_return']*100:.2f}%)")
        ...
        np.random.seed(123)
        realistic_returns = np.random.normal(0.0008, 0.025, 252)  # ~20% annual return, 40% volatility
        realistic_result = calculator.calculate_sharpe_ratio(realistic_returns)

        print(f"  Annual return: {realistic_result['annual_return']:.4f} ({realistic_result['annual_return']*100:.2f}%)")
        print(f"  Annual volatility: {realistic_result['annual_volatility']:.4f} ({realistic_result['annual_volatility']*100:.2f}%)")
...
def test_formula_comparison():
    ...
    print(f"  Max difference: {max(abs(sharpe_incorrect - sharpe_correct), abs(sharpe_compounded - sharpe_correct)):.4f}")
