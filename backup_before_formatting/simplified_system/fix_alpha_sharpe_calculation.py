    ...
    try:
        ...
        for i, factor in enumerate(alpha_results['top_factors']):
            if 'sharpe_ratio' in factor:
                ...
                print(f"  {i+1:2d}. {factor['name']:15s}: {original_sharpe:7.3f} -> {corrected_sharpe:7.3f}")

        # 修復其他相關字段
        alpha_results['sharpe_correction_applied'] = True
        alpha_results['sharpe_correction_timestamp'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                ...
                if 'Sharpe:' in rec:
                    ...
                    if len(parts) > 1:
                        ...
                        for factor in alpha_results['top_factors']:
                            if factor['name'] in prefix or str(factor['name']) in old_sharpe_part:
                        ...
                        if factor_name:
                            # 找到對應的修復後Sharpe比率
                            for factor in alpha_results['top_factors']:
                                if factor['name'] == factor_name and 'corrected_sharpe' in factor.get('sharpe_correction_details', {}):
                                    new_sharpe = factor['sharpe_ratio']
                                    corrected_rec = f"{prefix}Sharpe: {new_sharpe:.3f} (corrected)"
        ...
        sorted_factors = sorted(alpha_results['top_factors'],
        ...
        for factor in sorted_factors:
            ...
            print(f"  {name:15s}: IC={ic_mean:6.3f}, Sharpe: {original_sharpe:6.2f} -> {corrected_sharpe:6.2f}")
...
def main():
    ...
    if success:
        ...
        print("2. The corrected Sharpe ratios should now be realistic (typically -3 to 3)")
