...
class QuantitativeChartTemplates:
    ...
    def candlestick_chart(self,
                         ...
                         show_volume_bars: bool = True) -> go.Figure:
        """創建專業K線圖
        ...
        if volume and show_volume_bars and 'Volume' in data.columns:
            # 根據漲跌設置顏色
            colors = [
                self.colors['bull_market'] if close >= open else self.colors['bear_market']
        ...
        # 更新坐標軸
        fig.update_yaxes(title_text="價格 (HKD)", row=1, col=1, gridcolor=self.colors['grid'])
        if rows == 2:
            fig.update_yaxes(title_text="成交量", row=2, col=1, gridcolor=self.colors['grid'])

        fig.update_xaxes(showgrid=True, gridcolor=self.colors['grid'])

        return fig

    def technical_indicators_chart(self,
                                 data: pd.DataFrame,
                                 indicators: Dict[str, pd.Series],
                                 title: str = "技術指標分析") -> go.Figure:
        """創建技術指標組合圖表
    ...
    def portfolio_performance_chart(self,
                                  portfolio: pd.DataFrame,
                                  benchmark: Optional[pd.Series] = None,
                                  title: str = "投資組合表現") -> go.Figure:
        """創建投資組合表現圖表
    ...
    def risk_return_scatter(self,
        ...
        # 根據最大回撤設置大小
        sizes = strategies.get('max_drawdown', pd.Series([0.1] * len(strategies))).abs() * 1000 + 10
    ...
    def performance_attribution_chart(self,
                                     attribution_data: Dict[str, float],
                                     title: str = "績效歸因分析") -> go.Figure:
        """創建績效歸因瀑布圖
    ...
    def rolling_metrics_chart(self,
                            returns: pd.Series,
                            window: int = 252,
                            title: str = "滾動風險指標") -> go.Figure:
        """創建滾動風險指標圖表
        ...
        rolling_sharpe = (returns.rolling(window=window).mean() * 252 - 0.03) / rolling_vol
        ...
        fig = make_subplots(
            ...
            shared_xaxes=True,
            vertical_spacing=0.05,
            subplot_titles=(f"滾動波動率 ({window}天)",
        ...
        # 添加Sharpe = 1的參考線
        fig.add_hline(y=1, line_dash="dash", line_color=self.colors['neutral'],
    ...
    def exposure_analysis_chart(self,
                               exposure_data: pd.DataFrame,
                               title: str = "風險暴露分析") -> go.Figure:
        """創建風險暴露分析圖表
...
if __name__ == "__main__":
    ...
    print(f"📊 Available templates: {len([method for method in dir(chart_templates) if not method.startswith('_')])}")
