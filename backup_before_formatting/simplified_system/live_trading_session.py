...
def get_government_data():
    ...
    try:
        from api.government_data import get_latest_hibor, get_hibor_data
...
def generate_trading_signals(indicators, government_data):
    ...
    if current_price > sma_20 and sma_20 > sma_50:
        ...
        print(f"🟢 MA买入信号: 价格 {current_price:.2f} > SMA20 {sma_20:.2f} > SMA50 {sma_50:.2f}")
        ...
        print(f"🔴 MA卖出信号: 价格 {current_price:.2f} < SMA20 {sma_20:.2f} < SMA50 {sma_50:.2f}")
    else:
...
def run_vectorbt_backtest(stock_data):
    ...
    try:
        ...
        print(f"🔹 策略超额收益: {(rsi_results['annual_return'] - buy_hold_annual):.2%}")
...
def generate_trading_report(stock_data, indicators, signals, backtest_results):
    ...
    report = {
        ...
        'current_market': {
            'current_price': indicators['current_price'],
            'daily_change': (stock_data['close'].iloc[-1] / stock_data['close'].iloc[-2] - 1) * 100 if len(stock_data) > 1 else 0,
...
""")
        ...
        for signal in signals:
            ...
            f.write(f"- {icon} **{signal['indicator']}**: {signal['signal']} - {signal['reason']}\n")
...
def main():
    ...
    # 步骤6: 生成报告
    report_files = generate_trading_report(stock_data, indicators, signals, backtest_results)
