...
def test_cache_system():
    ...
    try:
        ...
        from src.performance.high_performance_cache import LRUCache, HighPerformanceCache
        ...
        assert cached_result == {"result": "success"}, "High performance cache test failed"
...
def test_parallel_optimizer():
    ...
    try:
        from src.performance.parallel_optimizer import ParallelOptimizer, ParallelConfig
        ...
        # 测试并行执行
        config = ParallelConfig(max_workers=2, use_processes=False)  # 使用线程避免进程问题
        ...
        assert results == expected, f"Parallel execution failed: {results} != {expected}"
        print("   ✓ Parallel Execution: PASSED")

        stats = optimizer.get_performance_stats()
        print(f"   ✓ Worker Configuration: {stats['worker_configuration']['optimal_workers']} workers")
...
def test_gpu_manager():
    ...
    try:
        ...
        print(f"   ✓ GPU Detection: Available={gpu_env.is_available}, Count={gpu_env.gpu_count}")
        ...
        gpu_array = gpu_manager.asarray(test_array)
        ...
        print(f"   ✓ GPU Manager: Backend={gpu_manager.get_backend_info()['backend_type']}")
...
def test_core_indicators():
    ...
    try:
        ...
        # 测试批量计算
        start_time = time.time()
        all_indicators = indicators.calculate_all_indicators(test_data, ['RSI', 'MACD', 'SMA'])
        batch_time = time.time() - start_time
        print(f"   ✓ Batch Calculation: {batch_time:.4f}s, {len(all_indicators)} indicators")
...
def test_vectorbt_engine():
    ...
    try:
        ...
        print(f"   ✓ Performance Metrics: Return={result.total_return:.3f}, Sharpe={result.sharpe_ratio:.3f}")
