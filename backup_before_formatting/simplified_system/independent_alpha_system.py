    ...
    def _generate_reversion_signals(self, data: pd.DataFrame) -> List[AlphaSignal]:
        ...
        for i in range(len(data)):
            ...
            if (current_rsi < self.parameters['rsi_oversold'] and
                current_price <= current_lower * 1.02):

                signal_strength = min(1.0, (self.parameters['rsi_oversold'] - current_rsi) / 20)
                confidence = 0.7
            ...
            # RSI超买 + 价格接近布林带上轨 = 卖出信号
                  ...
                  current_price >= current_upper * 0.98):

                signal_strength = min(1.0, (current_rsi - self.parameters['rsi_overbought']) / 20)
                confidence = 0.7
    ...
    def _generate_momentum_signals(self, data: pd.DataFrame) -> List[AlphaSignal]:
        ...
        for i in range(len(data)):
            ...
            if (current_ma_short > current_ma_long and
                ...
                current_price > current_ma_short):
...
class MacroEconomicAlpha(AlphaSource):
    ...
    def _analyze_interest_rate_cycle(self, econ_data: pd.DataFrame,
        ...
        # 市场估值水平 (简化PE)
        market_pe = market_data['close'] / market_data['close'].rolling(window=252).mean()
    ...
    def _analyze_growth_cycle(self, econ_data: pd.DataFrame,
                             market_data: pd.DataFrame) -> List[AlphaSignal]:
        """分析增长周期"""
        signals = []
...
class CrossAssetAlpha(AlphaSource):
    ...
    def generate_signals(self, market_data: pd.DataFrame,
        ...
        # 商品股票相对强度
        commodity_equity_signals = self._analyze_commodity_equity_ratio(market_data, cross_data)
    ...
    def _analyze_equity_bond_ratio(self, equity_data: pd.DataFrame,
    ...
    def _analyze_commodity_equity_ratio(self, equity_data: pd.DataFrame,
                                       cross_data: pd.DataFrame) -> List[AlphaSignal]:
        """分析商品股票比率"""
        signals = []
...
class IndependentAlphaSystem:
    ...
    def generate_all_signals(self, market_data: pd.DataFrame,
                            additional_data: Dict = None) -> Dict[str, List[AlphaSignal]]:
        """生成所有Alpha信号"""
        all_signals = {}
    ...
    def analyze_independence(self) -> Dict[str, float]:
        ...
        source_names = [s.name for s in self.alpha_sources]
    ...
    def calculate_portfolio_weights(self, signals: Dict[str, List[AlphaSignal]],
        ...
        for source_name in signals.keys():
            ...
            for other_name in signals.keys():
                if source_name != other_name:
                    correlations.append(abs(correlation_matrix[source_name][other_name]))

            avg_correlations[source_name] = np.mean(correlations) if correlations else 0

        # 相关性越低，权重越高
        inv_correlations = {name: 1.0 / (1.0 + corr)
                           for name, corr in avg_correlations.items()}
        total_inv_corr = sum(inv_correlations.values())

        weights = {name: inv_corr / total_inv_corr for name, inv_corr in inv_correlations.items()}

        return weights

    def generate_portfolio_signals(self, market_data: pd.DataFrame,
        ...
        # 聚合信号
        portfolio_signals = self.signal_aggregator.aggregate_signals(all_signals, weights)
        ...
        return {
            ...
            'independence_score': self._calculate_independence_score(correlation_matrix)
        }
...
class SignalAggregator:
    ...
    def aggregate_signals(self, all_signals: Dict[str, List[AlphaSignal]],
                         weights: Dict[str, float]) -> List[AlphaSignal]:
        """聚合信号"""
        # 按时间对齐信号
        ...
        for timestamp in time_aligned_signals:
            ...
            for source_name, signal in time_aligned_signals[timestamp].items():
                if signal and source_name in weights:
                    ...
                    weighted_value += signal.signal_value * signal.signal_strength * weight * signal.confidence
                    total_weight += weight
            ...
            if total_weight > 0:
                ...
                aggregated_signals.append(AlphaSignal(
...
class PerformanceAnalyzer:
    ...
    def analyze_portfolio_signals(self, portfolio_signals: List[AlphaSignal],
    ...
    def _align_with_market_data(self, signal_series: pd.Series,
                               market_data: pd.DataFrame) -> Dict:
        """与市场数据对齐"""
        # 计算市场收益率
    ...
    def _calculate_portfolio_returns(self, signals: pd.Series,
    ...
    def _calculate_performance_metrics(self, returns: pd.Series) -> Dict:
        ...
        # 与市场相关性
        market_returns_sample = np.random.normal(0, 0.01, len(returns))  # 模拟市场收益率
        correlation_to_market = np.corrcoef(returns, market_returns_sample)[0, 1]
        correlation_to_market = correlation_to_market if not np.isnan(correlation_to_market) else 0
...
def main():
    ...
    alpha_system.add_alpha_source(
        MacroEconomicAlpha("Interest_Rate_Cycle")
    )
    alpha_system.add_alpha_source(
        CrossAssetAlpha("Equity_Bond_Rotation")
    )
    ...
    economic_data = pd.DataFrame({
        'interest_rate': 3.0 + np.sin(np.linspace(0, 4*np.pi, len(econ_dates))) * 1.5,
    ...
    print(f"Market Correlation: {result['performance_metrics']['correlation_to_market']:.3f}")
