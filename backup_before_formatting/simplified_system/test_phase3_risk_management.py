...
from src.backtest.advanced_portfolio_manager import (
    PortfolioOptimizer, DynamicPositionSizer,
    MultiAssetPortfolioBacktester, create_multi_asset_test_data
)
from src.backtest.risk_management_framework import (
    IntegratedRiskManager, create_default_risk_budget,
    RiskBudget, RiskLevel, AlertType
)
...
def test_portfolio_optimization():
    ...
    try:
        ...
        assert abs(sum(max_sharpe_portfolio.weights.values()) - 1.0) < 1e-6, "Weights should sum to 1"

        # Test minimum volatility optimization
        min_vol_portfolio = optimizer.minimize_volatility(returns)

        assert min_vol_portfolio.expected_volatility >= 0, "Volatility should be non-negative"
        assert abs(sum(min_vol_portfolio.weights.values()) - 1.0) < 1e-6, "Weights should sum to 1"
        ...
        assert len(vol_adj_weights) == len(returns.columns), "Should return weight for each asset"
        assert abs(sum(vol_adj_weights.values()) - 1.0) < 1e-6, "Weights should sum to 1"

        # Test risk parity weights
        risk_parity_weights = position_sizer.calculate_risk_parity_weights(returns)

        assert len(risk_parity_weights) == len(returns.columns), "Should return weight for each asset"
        assert abs(sum(risk_parity_weights.values()) - 1.0) < 1e-6, "Weights should sum to 1"

        logger.info(f"✓ Portfolio optimization successful:")
        logger.info(f"  Efficient Frontier: {len(ef_returns)} portfolios")
        logger.info(f"  Max Sharpe Portfolio: {max_sharpe_portfolio.sharpe_ratio:.3f} Sharpe")
        logger.info(f"  Min Vol Portfolio: {min_vol_portfolio.expected_volatility:.2%} volatility")
        logger.info(f"  Risk Parity Weights: {list(risk_parity_weights.values())[:3]}...")
...
def test_risk_management_framework():
    ...
    try:
        ...
        portfolio_value = sum(pos * price for pos, price in zip(positions.values(), current_prices.values()))
        ...
        assert 'stress_test_summary' in risk_report, "Report should contain stress test summary"
        ...
        # Test risk budgeting
        returns_matrix = pd.DataFrame(np.random.normal(0.001, 0.02, (252, 3)), columns=['A', 'B', 'C'])
        risk_budget_weights = risk_manager.risk_budgeting.calculate_risk_budget_weights(returns_matrix)

        assert abs(sum(risk_budget_weights.values()) - 1.0) < 1e-6, "Risk budget weights should sum to 1"
...
def test_multi_asset_backtesting():
    ...
    try:
        ...
        for strategy in strategies:
            ...
            assert strategy_results['num_rebalances'] >= 0, "Rebalance count should be non-negative"
        ...
        for strategy, result in results.items():
            logger.info(f"  {strategy}: {result['total_return']:.2%} return, "
