...
def test_phase4_1_signal_generation():
    ...
    if success:
        ...
        if signals:
            ...
            logger.info(f"示例信號: {example_signal.indicator_name} - {example_signal.signal_type.value} - 強度:{example_signal.strength:.1f} - 置信度:{example_signal.confidence:.2f}")
    else:
...
def test_phase4_2_weight_management():
    ...
    try:
        ...
        updated_weights = weight_manager.update_weights("0700.HK", performance_data, "performance_based")
...
def test_phase4_3_conflict_resolution():
    ...
    try:
        from src.signal_fusion.signal_generator import TradingSignal, SignalType
        ...
        # 解決衝突
        start_time = time.time()
        resolutions = conflict_resolver.resolve_conflicts(conflicts, strategy="weighted_vote")
...
def test_phase4_4_composite_signal_generation():
    ...
    try:
        ...
        # 檢查信號屬性
        required_attributes = [
            'signal_type', 'strength', 'confidence', 'quality_score',
            'risk_level', 'decision_reason', 'component_signals'
        ]
...
def test_integration():
    ...
    try:
        ...
        # 獲取建議
        if 'composite_signal' in analysis_result and analysis_result['composite_signal']:
            recommendations = get_signal_recommendations(
                "0700.HK",
                analysis_result['composite_signal']
            )
