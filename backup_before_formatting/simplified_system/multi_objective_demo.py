...
def demo_objective_functions():
    ...
    for i, asset in enumerate(asset_names):
        ...
        print(f"  {asset}: 年化收益{annual_return:.2%}, 年化波动{annual_vol:.2%}, 夏普{sharpe:.2f}")
...
def demo_pareto_frontier(returns_df):
    ...
    if pareto_portfolios:
        ...
        plt.scatter(all_vols, all_returns, c='lightblue', alpha=0.6, s=10, label='所有投资组合')
        ...
        plt.scatter(min_vol_portfolio['volatility'], min_vol_portfolio['return'],
                   c='green', s=100, marker='*', label='最小波动率')
        plt.scatter(max_sharpe_portfolio['volatility'], max_sharpe_portfolio['return'],
                   c='blue', s=100, marker='*', label='最高夏普比率')
        plt.scatter(max_return_portfolio['volatility'], max_return_portfolio['return'],
                   c='purple', s=100, marker='*', label='最大收益')
...
def demo_multi_objective_optimization(returns_df):
    ...
    def multi_objective_function(weights, lambda_return, lambda_vol, lambda_dd):
        ...
        # 组合目标函数 (负值用于最小化)
        objective = -lambda_return * portfolio_return + \
    ...
    optimization_cases = [
        ...
        {"name": "均衡偏好", "lambda_return": 0.33, "lambda_vol": 0.33, "lambda_dd": 0.34},
    ]
    ...
    for case in optimization_cases:
        print(f"\n优化案例: {case['name']}")
        print(f"权重 - 收益:{case['lambda_return']:.2f}, 波动:{case['lambda_vol']:.2f}, 回撤:{case['lambda_dd']:.2f}")
        ...
        # 优化
        result = minimize(
            lambda w: multi_objective_function(w,
                                            case['lambda_return'],
                                            case['lambda_vol'],
                                            case['lambda_dd']),
            initial_weights,
    ...
    if results:
        print(f"\n多目标优化结果比较:")
        print("-" * 80)
        print(f"{'案例':<10} {'收益':<8} {'波动':<8} {'夏普':<8} {'最大回撤':<10} {'VaR(95%)':<10}")
        print("-" * 80)

        for r in results:
            print(f"{r['case']:<10} {r['return']:<8.2%} {r['volatility']:<8.2%} {r['sharpe']:<8.3f} {r['max_drawdown']:<10.2%} {r['var_95']:<10.2%}")
        ...
        for i, case in enumerate(case_names):
            ax2.annotate(case, (vols[i], returns_vals[i]), xytext=(5, 5),
...
def demo_trading_costs(returns_df):
    ...
    def objective_with_costs(new_weights):
        ...
        portfolio_volatility = np.sqrt(np.dot(new_weights.T, np.dot(cov_matrix, new_weights)))
        sharpe_ratio = (portfolio_return - 0.03) / portfolio_volatility
    ...
    if result_with_cost.success:
        ...
        vol_no_cost = np.sqrt(np.dot(weights_no_cost.T, np.dot(cov_matrix, weights_no_cost)))
        adjusted_return_no_cost = return_no_cost - trading_cost_no_cost

        return_with_cost = np.sum(mean_returns * weights_with_cost)
        vol_with_cost = np.sqrt(np.dot(weights_with_cost.T, np.dot(cov_matrix, weights_with_cost)))
        adjusted_return_with_cost = return_with_cost - trading_cost_with_cost

        print(f"\n绩效比较:")
        print(f"{'策略':<15} {'期望收益':<10} {'交易成本':<10} {'净收益':<10} {'波动率':<10}")
        print("-" * 60)
        print(f"{'不考虑成本':<15} {return_no_cost:<10.2%} {trading_cost_no_cost:<10.4f} {adjusted_return_no_cost:<10.2%} {vol_no_cost:<10.2%}")
        print(f"{'考虑成本':<15} {return_with_cost:<10.2%} {trading_cost_with_cost:<10.4f} {adjusted_return_with_cost:<10.2%} {vol_with_cost:<10.2%}")
...
def demo_sensitivity_analysis(returns_df):
    ...
    result = minimize(sharpe_objective, initial_weights, method='SLSQP',
    ...
    for asset_idx, asset_name in enumerate(returns_df.columns):
        ...
        print(f"{asset_name}: 基准夏普{base_sharpe:.3f}, " +
    ...
    for i, asset_name in enumerate(returns_df.columns):
        plt.plot(sensitivity_ranges, sensitivity_results[i],
                marker='o', label=asset_name, linewidth=2)
...
def main():
    ...
    try:
        ...
        # 2. 帕累托边界演示
        pareto_portfolios = demo_pareto_frontier(returns_df)

        # 3. 多目标优化演示
        optimization_results = demo_multi_objective_optimization(returns_df)

        # 4. 交易成本演示
        cost_analysis = demo_trading_costs(returns_df)
        ...
        print("✓ Multi-objective optimization framework - supports various objective combinations")
        print("✓ Pareto frontier calculation - efficient frontier identification and visualization")
        print("✓ Custom objective functions - return, risk, drawdown, and other metrics")
        ...
        print("✓ Preference elicitation tools - strategies for different risk preferences")
