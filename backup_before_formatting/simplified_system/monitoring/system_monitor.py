...
class PerformanceMonitor:
        ...
        if metric.name in self.thresholds:
            ...
            for level, threshold_value in thresholds.items():
                if metric.value >= threshold_value:
                    ...
                    logger.warning(f"Alert triggered: {alert.message} (Value: {metric.value}, Threshold: {threshold_value})")
    ...
    def _check_trend_alerts(self, metric_name: str, values: List[float], trend_slope: float):
        """检查趋势警报"""
        # 如果指标持续增长或下降
        if len(values) >= 5:
        ...
        if metric.name in self.thresholds:
            ...
            for level, threshold_value in thresholds.items():
                if latest_value >= threshold_value:
                    ...
                    # 检查是否已经存在相同级别的警报
                    existing_alert = next(
                        (alert for alert in reversed(self.alerts)
    ...
    def _create_trend_alert(self, metric_name: str, trend_type: str,
                            values: List[float], slope: float):
        """创建趋势警报"""
    ...
    def create_performance_dashboard_data(self) -> Dict[str, Any]:
        ...
        dashboard_data = {
            ...
            'system_health': {
                'overall_status': 'healthy' if alerts_summary['total_alerts'] < 5 else 'degraded',
        ...
        # 关键性能指标
        key_metric_names = ['cpu_usage_percent', 'memory_usage_percent', 'disk_usage_percent']
        for metric_name in key_metric_names:
            if metric_name in metrics_summary:
                dashboard_data['key_metrics'][metric_name] = metrics_summary[metric_name]
...
def example_usage():
    ...
    try:
        ...
        logger.info(f"  Critical alerts: {alerts_summary['alert_counts'].get('critical', 0)}")
