                ...
                try:
                    ...
                    if result:
                        ...
                        if sharpe > 1.0 or max_dd > -0.15:
                            print(f"  [優異] RSI({period},{oversold},{overbought}): "
                ...
                try:
                    ...
                    if result:
                        ...
                        if sharpe > 1.0 or max_dd > -0.20:
                            print(f"  [優異] MACD({fast},{slow},{signal}): "
                                  f"SR={sharpe:.3f}, MDD={max_dd:.1%}, Return={result.total_return:.1%}")
...
def analyze_best_strategies(all_results):
    ...
    print("\n綜合前10名策略:")
    print("-" * 80)
    print(f"{'排名':<4} {'策略類型':<20} {'參數':<25} {'Sharpe':<8} {'最大回撤':<10} {'總回報':<8}")
    print("-" * 80)
...
def main():
    ...
    print(f"最佳Sharpe: {top_strategies[0]['sharpe_ratio']:.3f}" if top_strategies else "N/A")
    print(f"最大回撤控制: {top_strategies[0]['max_drawdown']:.1%}" if top_strategies else "N/A")
