...
def demo_basic_multi_objective():
    ...
    for asset in asset_names:
        ...
        print(f"  {asset:<10}: Return={ann_return:.1%}, Vol={ann_vol:.1%}, Sharpe={sharpe:.2f}")
...
def demo_pareto_frontier(mean_returns, cov_matrix, asset_names):
    ...
    for i, p1 in enumerate(portfolios):
        ...
        for j, p2 in enumerate(portfolios):
            if i != j:
                if (p2['return'] >= p1['return'] and p2['volatility'] <= p1['volatility'] and
    ...
    if pareto_portfolios:
        ...
        plt.scatter(all_vols, all_returns, c='lightblue', alpha=0.6, s=10, label='All Portfolios')
        ...
        plt.scatter(min_vol_portfolio['volatility'], min_vol_portfolio['return'],
                   c='green', s=100, marker='*', label='Min Volatility')
        plt.scatter(max_sharpe_portfolio['volatility'], max_sharpe_portfolio['return'],
                   c='blue', s=100, marker='*', label='Max Sharpe')
...
def demo_multi_objective_optimization(mean_returns, cov_matrix, asset_names):
    ...
    def multi_objective(weights, lambda_ret, lambda_vol, lambda_dd):
        ...
        objective = -lambda_ret * portfolio_return + lambda_vol * portfolio_volatility + lambda_dd * abs(max_drawdown_est)
        return objective

    # Different optimization scenarios
    scenarios = [
        {"name": "Return Focus", "lambda_ret": 0.6, "lambda_vol": 0.3, "lambda_dd": 0.1},
        {"name": "Risk Averse", "lambda_ret": 0.2, "lambda_vol": 0.6, "lambda_dd": 0.2},
        {"name": "Drawdown Control", "lambda_ret": 0.3, "lambda_vol": 0.2, "lambda_dd": 0.5},
    ...
    print(f"{'Scenario':<15} {'Return':<8} {'Volatility':<10} {'Sharpe':<8} {'MaxDD':<8}")
    print("-" * 65)

    for scenario in scenarios:
        result = minimize(
            lambda w: multi_objective(w, scenario['lambda_ret'], scenario['lambda_vol'], scenario['lambda_dd']),
            initial_weights,
            method='SLSQP',
            bounds=bounds,
        ...
        if result.success:
            ...
            print(f"{scenario['name']:<15} {portfolio_return:<8.2%} {portfolio_volatility:<10.2%} {sharpe_ratio:<8.3f} {max_dd_est:<8.2%}")
    ...
    if results:
        ...
        for i, result in enumerate(results[:4]):
            ax = axes[i]
            ax.pie(result['weights'], labels=asset_names, autopct='%1.1f%%', startangle=90)
...
def demo_trading_costs(returns_df, mean_returns, cov_matrix):
    ...
    if result_no_cost.success and result_with_cost.success:
        ...
        cost_data = [0, turnover_no_cost * trading_cost_rate, turnover_with_cost * trading_cost_rate]
...
def main():
    ...
    try:
        ...
        # 2. Pareto frontier
        pareto_portfolios = demo_pareto_frontier(mean_returns, cov_matrix, asset_names)

        # 3. Multi-objective optimization
        optimization_results = demo_multi_objective_optimization(mean_returns, cov_matrix, asset_names)
