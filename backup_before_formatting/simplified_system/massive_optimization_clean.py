...
try:
    ...
    def simple_ma_backtest(prices, short=10, long=30):
        ...
        for i in range(1, len(prices)):
            if not position and short_ma.iloc[i] > long_ma.iloc[i] and short_ma.iloc[i-1] <= long_ma.iloc[i-1]:
                position = True
                entry_price = prices[i]
            elif position and short_ma.iloc[i] < long_ma.iloc[i] and short_ma.iloc[i-1] >= long_ma.iloc[i-1]:
                position = False
                if entry_price > 0:
                    return_pct = (prices[i] - entry_price) / entry_price
                    trades.append(return_pct)
    ...
    def run_massive_optimization():
        ...
        prices_for_test = (prices, ) * len(test_params)
            ...
            for i in range(0, len(test_params), chunk_size):
                ...
                for j, (strategy_type, params) in enumerate(chunk):
                    if strategy_type == 'RSI':
                        args_list.append((chunk_prices[j][0], params[0], params[1], params[2]))
                ...
                for j, (strategy_type, params) in enumerate(chunk):
                    if strategy_type == 'RSI':
                        result = simple_rsi_backtest(chunk_prices[j][0], params[0], params[1], params[2])
                    elif strategy_type == 'MA':
                        result = simple_ma_backtest(chunk_prices[j][0], params[0], params[1])
                ...
                if completed % 200 == 0 or completed >= len(test_params):
                    ...
                    print(f"   [CPU] {mp.cpu_count()} cores should be running at full capacity!")
        ...
        if results:
            ...
            for i, result in enumerate(sorted_results[:10], 1):
                ...
                print(f"    Sharpe: {result['sharpe']:.3f}, Return: {result['total_return']:.1%}, Trades: {result['trades']}")
