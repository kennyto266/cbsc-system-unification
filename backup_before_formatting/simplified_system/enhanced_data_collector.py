...
from data_quality_validator import (
    DataQualityValidator, DataQualityReport, ValidationResult,
    validate_government_data, quick_data_quality_check
)
from src.data.government_data import GovernmentDataCollector, DataCollectionResult
...
class EnhancedGovernmentDataCollector(GovernmentDataCollector):
    ...
    async def collect_with_validation(self, source_config) -> EnhancedDataCollectionResult:
        """
        收集政府數據並進行質量驗證
        """
        ...
        # 第二步：數據質量驗證
        records = self._load_records_from_file(original_result.file_path) if original_result.file_path else []
        ...
        # 第三步：執行質量驗證
        quality_report = self.quality_validator.validate_government_data(records, source_config.data_type)

        logger.info(f"📊 {source_config.name} 質量評分: {quality_report.quality_score:.1f}")
        ...
        if self.auto_fix_enabled and quality_report.quality_score < self.quality_threshold:
            ...
            if fixed_records:
                # 重新驗證修復後的數據
                fixed_report = self.quality_validator.validate_government_data(fixed_records, source_config.data_type)

                if fixed_report.quality_score > quality_report.quality_score:
                    logger.info(f"✅ {source_config.name}: 自動修復成功，質量評分從 {quality_report.quality_score:.1f} 提升到 {fixed_report.quality_score:.1f}")

                    # 保存修復後的數據
                    fixed_file_path = await self._save_fixed_data(fixed_records, source_config.name)
        ...
        # 第五步：重試機制（如果仍有問題）
        if self.retry_on_failure and len(quality_report.critical_issues) > 0 and fix_attempts < self.max_retries:
            logger.info(f"🔄 {source_config.name}: 檢測到嚴重問題，嘗試重新收集數據")

            retry_result = await self._retry_collection_with_fixes(source_config, quality_report)
            if retry_result and retry_result.quality_report.quality_score > quality_report.quality_score:
        ...
        logger.info(f"📋 {source_name} 最終質量評分: {quality_report.quality_score:.1f}")
    ...
    def _create_enhanced_result(self, original_result: DataCollectionResult,
                              quality_report: Optional[DataQualityReport]) -> EnhancedDataCollectionResult:
        """創建增強收集結果"""
    ...
    async def _auto_fix_data_issues(self, records: List[Dict[str, Any]],
                                  quality_report: DataQualityReport,
                                  source_config) -> Tuple[List[Dict[str, Any]], int]:
        """自動修復數據問題"""
        fix_attempts = 0
        fixed_records = records.copy()

        # 修復1：數據量不足問題
        if any("數據量不足" in issue.message for issue in quality_report.critical_issues):
            logger.info(f"🔧 嘗試修復數據量不足問題: {source_config.name}")
            fixed_records = await self._fix_data_volume_issue(source_config, fixed_records)
            fix_attempts += 1

        # 修復2：範圍錯誤問題
        range_issues = [issue for issue in quality_report.critical_issues + quality_report.high_issues
        ...
        # 修復3：完整性問題
        completeness_issues = [issue for issue in quality_report.critical_issues + quality_report.high_issues
                             if "缺少" in issue.message or "字段" in issue.message]
        if completeness_issues:
            logger.info(f"🔧 嘗試修復完整性問題: {source_config.name}")
            fixed_records = self._fix_completeness_issues(fixed_records, completeness_issues)
            fix_attempts += 1

        return fixed_records, fix_attempts

    async def _fix_data_volume_issue(self, source_config, existing_records: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """修復數據量不足問題"""
        try:
    ...
    def _fix_range_issues(self, records: List[Dict[str, Any]], issues: List[ValidationResult]) -> List[Dict[str, Any]]:
        """修復範圍錯誤問題"""
        fixed_records = []
    ...
    def _fix_completeness_issues(self, records: List[Dict[str, Any]],
                               issues: List[ValidationResult]) -> List[Dict[str, Any]]:
        """修復完整性問題"""
        fixed_records = []
        ...
        for record in records:
            ...
            for field, default_value in numeric_defaults.items():
                if field in fixed_record and (fixed_record[field] is None or fixed_record[field] == ''):
    ...
    async def _save_fixed_data(self, records: List[Dict[str, Any]], source_name: str) -> str:
        """保存修復後的數據"""
    ...
    async def _retry_collection_with_fixes(self, source_config,
                                         original_quality_report: DataQualityReport) -> Optional[EnhancedDataCollectionResult]:
        """基於質量問題重試數據收集"""
        try:
            # 根據質量問題調整收集參數
            adjusted_config = self._adjust_config_based_on_issues(source_config, original_quality_report)
    ...
    def _adjust_config_based_on_issues(self, source_config, quality_report: DataQualityReport):
        """根據質量問題調整收集配置"""
        # 這裡可以根據具體問題調整API參數
        # 暫時返回原始配置
    ...
    async def collect_all_with_validation(self) -> List[EnhancedDataCollectionResult]:
        ...
        for source_config in sorted_sources:
            logger.info(f"收集並驗證: {source_config.name} (優先級: {source_config.priority})")
        ...
        # 生成增強收集報告
        self._generate_enhanced_collection_report(results, successful_collections, total_records, average_quality)

        return results

    def _generate_enhanced_collection_report(self, results: List[EnhancedDataCollectionResult],
                                           successful_collections: int, total_records: int, average_quality: float):
        """生成增強的收集報告"""
        report = {
            "collection_time": datetime.now().isoformat(),
            ...
            "validation_passed_collections": len([r for r in results if r.validation_passed]),
        ...
        logger.info(f"   自動修復: {len([r for r in results if r.auto_fixed])} 個數據源")
...
# 便捷函數
async def collect_government_data_with_validation(source_name: str) -> Optional[EnhancedDataCollectionResult]:
    """收集特定政府數據源並進行質量驗證"""
...
async def collect_all_government_data_with_validation() -> List[EnhancedDataCollectionResult]:
    """收集所有政府數據並進行質量驗證"""
    return await enhanced_collector.collect_all_with_validation()

async def get_validated_government_data(source_name: str,
                                       min_quality_score: float = 70.0,
                                       require_validation_passed: bool = True) -> Optional[Dict[str, Any]]:
    """獲取通過質量驗證的政府數據"""
    ...
    if result.quality_report and result.quality_report.quality_score < min_quality_score:
        logger.warning(f"{source_name} 質量評分 {result.quality_report.quality_score:.1f} 低於要求 {min_quality_score}")
        return None
