...
def calculate_sharpe_from_prices(
    ...
    return calculate_correct_sharpe_ratio(portfolio_returns, risk_free_rate, trading_days)
...
def validate_existing_sharpe_calculations(csv_file_path: str):
    ...
    try:
        ...
        if abnormal_count > 0:
            ...
            abnormal_results = df[abnormal_mask][['strategy', 'params', 'sharpe', 'total_return']]
        ...
        if normal_mask.sum() > 0:
            ...
            print(f"\n✅ 正常範圍內的前5個策略:")
            print(normal_results[['strategy', 'params', 'sharpe', 'total_return']].to_string(index=False))
