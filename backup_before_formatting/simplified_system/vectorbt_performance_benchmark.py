...
class VectorBTPerformanceBenchmark:
    ...
    def benchmark_strategy(self, strategy_name: str, parameters: Dict, data: pd.DataFrame, iterations: int = 10) -> Dict:
        """基準測試單個策略"""
    ...
    def run_comprehensive_benchmark(self) -> Dict:
        ...
        for strategy_name, parameters in test_strategies:
            result = self.benchmark_strategy(strategy_name, parameters, data, iterations=20)
            self.results['strategies'][strategy_name] = result

        # 計算整體性能統計
        successful_strategies = [r for r in self.results['strategies'].values() if 'performance' in r]

        if successful_strategies:
            avg_strategies_per_sec = np.mean([r['performance']['strategies_per_second'] for r in successful_strategies])
            total_time = np.sum([r['execution_times']['average'] * r['iterations'] for r in successful_strategies])
            ...
            self.results['summary'] = {
                ...
                'performance_improvement_needed': max(0, 600.0 - avg_strategies_per_sec)
            }
    ...
    def generate_report(self) -> str:
        ...
        if 'summary' in self.results:
            ...
            report.append(f"  Average Performance: {summary['average_strategies_per_second']:.2f} strategies/sec")
            report.append(f"  Target Performance: {summary['target_performance']:.2f} strategies/sec")
            report.append(f"  Target Met: {'YES' if summary['performance_target_met'] else 'NO'}")

            if not summary['performance_target_met']:
                report.append(f"  Improvement Needed: {summary['performance_improvement_needed']:.2f} strategies/sec")
            report.append("")
        ...
        for strategy_name, result in self.results['strategies'].items():
            ...
            if 'performance' in result:
                ...
                report.append(f"  Performance: {perf['strategies_per_second']:.2f} strategies/sec")
                report.append(f"  Avg Time: {exec_time['average']:.4f}s ({perf['execution_time_ms']:.2f}ms)")
                report.append(f"  Success Rate: {result['success_rate']*100:.1f}%")
                report.append(f"  Time Range: {exec_time['minimum']:.4f}s - {exec_time['maximum']:.4f}s")
            else:
