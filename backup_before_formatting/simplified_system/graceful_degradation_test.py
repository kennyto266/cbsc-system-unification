...
class GracefulDegradationTest:
    ...
    def test_scenario_1_api_failure_with_local_backup(self):
        ...
        try:
            ...
            test_result = {
                ...
                'has_data': result is not None and 'data' in result and len(result['data']) > 0,
                'source': result.get('source', 'Unknown') if result else None,
                'verification_passed': result.get('real_data_verification', {}).get('verified', False) if result else False
            }

            logger.info(f"Scenario 1 Result: {test_result['success']}, Source: {test_result['source']}")
    ...
    def test_scenario_2_simulate_api_unavailable(self):
        ...
        try:
            ...
            logger.info(f"Scenario 2 Result: {test_result['success']}, Records: {test_result['record_count']}")
            self.test_results.append(test_result)
    ...
    def test_scenario_3_no_fallback_to_simulated_data(self):
        ...
        try:
            ...
            if result and 'data' in result:
                ...
                if isinstance(result['data'], list) and result['data']:
                    ...
                    if isinstance(sample_record, dict):
                        # 檢查真實數據特徵
                        has_real_fields = any(field in sample_record for field in
                                              ['source', 'date', 'hibor_overnight', 'ir_1m', 'ir_3m'])
            ...
            test_result = {
                ...
                'rejects_simulated': not (result and result.get('fallback_used') == 'simulated'),
                'record_count': len(result.get('data', [])) if result else 0
            }

            logger.info(f"Scenario 3 Result: Real Data={test_result['is_real_data']}, Rejects Simulated={test_result['rejects_simulated']}")
            self.test_results.append(test_result)
        ...
        try:
            ...
            for data_type in data_types:
                ...
                prioritization_results[data_type] = {
                    ...
                    'has_records': result is not None and len(result.get('data', [])) > 0,
                    'verified': result.get('real_data_verification', {}).get('verified', False) if result else False
                }

            # 檢查優先級是否正確
            priority_test_passed = all(
                result['verified'] for result in prioritization_results.values() if result['available']
            )
            ...
            test_result = {
                ...
                'successful_sources': sum(1 for r in prioritization_results.values() if r['available']),
                'detailed_results': prioritization_results
            }

            logger.info(f"Scenario 4 Result: {test_result['successful_sources']}/{len(data_types)} sources available, all verified: {test_result['all_sources_verified']}")
            self.test_results.append(test_result)
    ...
    def test_scenario_5_error_handling_and_recovery(self):
        ...
        try:
            ...
            # 測試多種錯誤情況
            error_scenarios = [
                ('Invalid data type', lambda: gov_api._fetch_from_api('invalid_type', 7)),
            ...
            # 計算錯誤處理成功率
            handled_successfully = sum(1 for r in error_handling_results if r['handled_gracefully'])
            ...
            logger.info(f"Scenario 5 Result: {test_result['handled_successfully']}/{len(error_scenarios)} scenarios handled gracefully")
            self.test_results.append(test_result)
    ...
    def calculate_overall_results(self):
        ...
        overall_status = 'EXCELLENT' if success_rate >= 0.9 else \
                        ...
                        'ACCEPTABLE' if success_rate >= 0.7 else \
                        'POOR'
        ...
        self.overall_results = {
            ...
            'recommendations': self._generate_degradation_recommendations(scenario_analysis)
        }

    def _generate_degradation_recommendations(self, scenario_analysis: Dict) -> List[str]:
        """生成優雅降級改進建議"""
        recommendations = []

        # 檢查各場景的表現
        for scenario, analysis in scenario_analysis.items():
            rate = analysis['passed'] / analysis['total'] if analysis['total'] > 0 else 0

            if rate < 0.8:
                recommendations.append(f"改進{scenario}的實現，成功率僅為{rate:.1%}")

        # 通用建議
        if scenario_analysis.get('API Failure with Local Backup', {}).get('passed', 0) == 0:
            recommendations.append("增強API失敗時的本地備份機制")

        if scenario_analysis.get('No Fallback to Simulated Data', {}).get('passed', 0) == 0:
    ...
    def generate_test_report(self):
        ...
        for scenario, analysis in self.overall_results['scenario_analysis'].items():
            rate = analysis['passed'] / analysis['total'] if analysis['total'] > 0 else 0
            status_icon = "[OK]" if rate >= 0.8 else "[WARN]" if rate >= 0.6 else "[FAIL]"
            print(f"  {scenario}: {status_icon} {rate:.1%} ({analysis['passed']}/{analysis['total']})")
