...
class VerificationSystemRunner:
    ...
    async def _display_system_info(self):
        ...
        print("\n📋 Configuration:")
        print(f"  🎯 Verification enabled: {self.config.get('verification_config', {}).get('verification_pipeline', {}).get('layers', {}).get('source_auth', {}).get('enabled', False)}")
        print(f"  💾 Cache enabled: {self.config.get('cache', {}).get('enable_compression', False)}")
        print(f"  🚀 Async processing: {self.config.get('async_processor', {}).get('enable_monitoring', False)}")
        print(f"  📱 Telegram alerts: {self.config.get('telegram', {}).get('enabled', False)}")
        print("="*60 + "\n")
        ...
        try:
            ...
            try:
                ...
                if result.composite_score > 0:
                    ...
                    logger.info(f"    ✅ Verification score: {result.composite_score:.3f}")
                    logger.info(f"    ⏱️  Verification time: {result.verification_time_ms:.2f}ms")
                else:
    ...
    async def _interactive_verify(self, args):
        ...
        try:
            result = await self.system.verify_data(test_data, data_type, "http://interactive.test")
        ...
        try:
            ...
            if data and data.get('data'):
                ...
                for record in data['data'][:3]:  # 顯示前3條
                    print(f"  {record.get('date', 'N/A')}: {record.get('overnight', 'N/A')}%")
            else:
...
async def main():
    ...
    parser.add_argument("--config", default="verification_config.yaml", help="Configuration file path")
