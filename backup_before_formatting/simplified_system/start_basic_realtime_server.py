...
class BasicRealtimeServer:
    ...
    async def start(self, symbols: List[str]):
        ...
        try:
            ...
            print(f"WebSocket server: ws://{self.websocket_server.host}:{self.websocket_server.port}")
    ...
    def _register_handlers(self):
        ...
        async def signal_handler(event):
            ...
            print(f"Signal: {symbol} - {signal_data.get('signal_type', 'N/A')} (confidence: {signal_data.get('confidence', 'N/A')})")

        from src.streaming.event_processor import EventType, EventFilter
        ...
        self.event_processor.register_handler("price_updates", price_handler, price_filter, async_handler=True)
        self.event_processor.register_handler("signal_updates", signal_handler, signal_filter, async_handler=True)
        ...
        try:
            ...
            if active_signals:
                for symbol, signal in list(active_signals.items())[:3]:  # 显示前3个信号
                    print(f"     {symbol}: {signal.signal_type.value} ({signal.confidence:.2f})")
