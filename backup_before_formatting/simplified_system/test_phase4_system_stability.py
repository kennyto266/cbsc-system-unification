...
class StabilityTest:
    ...
    def monitor_system_resources(self, duration: int, interval: int = 5) -> Dict[str, List]:
        ...
        while time.time() < end_time:
            ...
            resource_data['memory_mb'].append(self.process.memory_info().rss / 1024 / 1024)
    ...
    def test_long_running_operations(self) -> Dict[str, Any]:
        ...
        monitor_thread = threading.Thread(
            target=lambda: results.update({'resource_monitoring': self.monitor_system_resources(duration, 5)})
        )
        ...
        if results['resource_monitoring']:
            ...
            results['resource_analysis'] = {
                ...
                'memory_growth_mb': resource_data['memory_mb'][-1] - resource_data['memory_mb'][0],
        ...
        logger.info(f"Long-running test completed: {results['operations_completed']} operations, {results['errors_encountered']} errors")
    ...
    def test_memory_leak_detection(self) -> Dict[str, Any]:
        ...
        for i in range(iterations):
            try:
                ...
                if memory_growth > 500:  # 500MB threshold
                    logger.warning(f"Excessive memory growth detected: {memory_growth:.1f}MB")
        ...
        memory_analysis = {
            ...
            'memory_leak_detected': (final_memory - initial_memory) > 100,  # 100MB threshold
            'memory_volatility_mb': np.std(memory_snapshots),
            'iterations_completed': iterations,
            'memory_snapshots': memory_snapshots[::10]  # Every 10th snapshot
        }
        
        logger.info(f"Memory leak test completed: Growth={memory_analysis['memory_growth_mb']:.1f}MB, Leak detected={memory_analysis['memory_leak_detected']}")
    ...
    def test_concurrent_access_stability(self) -> Dict[str, Any]:
            ...
            try:
                for i in range(operations_per_thread):
                    try:
                        ...
                        strategies = ['RSI', 'MACD', 'Moving Average', 'Bollinger Bands']
                        strategy = random.choice(strategies)
                        
                        result = self._execute_strategy_operation(strategy, test_data)
        ...
        concurrent_analysis = {
            ...
            'operations_per_second': results['total_operations'] / total_time if total_time > 0 else 0,
            'avg_operations_per_thread': results['total_operations'] / results['threads_completed'] if results['threads_completed'] > 0 else 0,
            'error_rate': results['errors_encountered'] / results['total_operations'] if results['total_operations'] > 0 else 0,
            'thread_success_rate': results['threads_completed'] / num_threads,
            'max_thread_duration': max([r.get('duration', 0) for r in results['thread_results']]) if results['thread_results'] else 0,
            'min_thread_duration': min([r.get('duration', 0) for r in results['thread_results']]) if results['thread_results'] else 0
        }
        
        results.update(concurrent_analysis)
        
        logger.info(f"Concurrent access test completed: {results['total_operations']} operations, {results['errors_encountered']} errors")
    ...
    def test_data_integrity_under_stress(self) -> Dict[str, Any]:
        ...
        for i in range(50):
            try:
                ...
                with ThreadPoolExecutor(max_workers=2) as executor:
                    futures = [
                        executor.submit(self._perform_intensive_calculations, test_data.copy()),
                        executor.submit(self._perform_intensive_calculations, test_data.copy())
                    ]
        ...
        logger.info(f"Data integrity test completed: {results['integrity_tests_passed']} passed, {results['integrity_tests_failed']} failed")
        
        return results
    
    def _execute_strategy_operation(self, strategy: str, data: pd.DataFrame) -> Optional[Dict[str, Any]]:
    ...
    def run_all_stability_tests(self) -> Dict[str, Any]:
        ...
        self.results['concurrent_access_stability'] = self.test_concurrent_access_stability()
        self.results['resource_exhaustion_handling'] = self.test_resource_exhaustion_handling()
        self.results['data_integrity_under_stress'] = self.test_data_integrity_under_stress()
        ...
        report = {
            ...
            'summary': {
                ...
                'memory_leaks_detected': self.results['memory_leak_detection']['memory_leak_detected'],
                'data_integrity_maintained': not self.results['data_integrity_under_stress']['data_corruption_detected'],
                'recovery_successful': self.results['resource_exhaustion_handling']['recovery_success']
            }
        }
        
        # Save report
        report_path = os.path.join(os.path.dirname(__file__), 'phase4_stability_test_report.json')
        ...
        logger.info("=" * 80)
        logger.info(f"Overall Stability Score: {stability_score['overall_score']:.1f}/10")
        logger.info(f"System Stable: {report['summary']['system_stable']}")
        logger.info(f"Memory Leaks Detected: {report['summary']['memory_leaks_detected']}")
        logger.info(f"Data Integrity Maintained: {report['summary']['data_integrity_maintained']}")
    ...
    def _calculate_stability_score(self) -> Dict[str, float]:
        ...
        if long_running['errors_encountered'] == 0:
            ...
            error_rate = long_running['errors_encountered'] / max(long_running['operations_completed'], 1)
        ...
        if not memory_leak['memory_leak_detected']:
            ...
            scores['memory_leak'] = max(10.0 - memory_growth / 20, 0.0)  # Deduct points for memory growth
...
if __name__ == "__main__":
    ...
    print(f"Overall stability score: {report['stability_score']['overall_score']:.1f}/10")
