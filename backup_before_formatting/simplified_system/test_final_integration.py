...
try:
    ...
    # 檢查組件配置
    print(f"  - Signal weights: {len(advanced_signals.signal_weights)} indicators configured")
    ...
    if extension_result.get('success'):
        ...
        print(f"  Original: {extension_result['original_count']} -> Extended: {extension_result['final_count']}")
        ...
        print(f"  Quality metrics: {len(extension_result.get('metadata', {}).get('quality_metrics', {}))} columns")
    else:
    ...
    if stock_data and 'data' in stock_data and 'close' in stock_data['data']:
        ...
        if analysis_result.get('success'):
            ...
            # 檢查信號組成
            components = ['indicators', 'cross_indicator_signals', 'multi_timeframe_signals', 'trading_patterns', 'composite_signals', 'risk_assessment']
            ...
            print(f"[FAIL] Advanced analysis failed: {analysis_result.get('error', 'Unknown error')}")
    else:
            ...
            if symbol_data and 'data' in symbol_data and 'close' in symbol_data['data']:
                ...
                for date in recent_dates:
                    ...
                    symbol_records.append({
                        'date': date,
                        'price': price,
                        'volume': symbol_data['data'].get('volume', {}).get(date, 0),
                        'high': symbol_data['data'].get('high', {}).get(date, price),
                        'low': symbol_data['data'].get('low', {}).get(date, price),
                        'open': symbol_data['data'].get('open', {}).get(date, price)
                    })
                ...
                if symbol_result.get('success'):
                    batch_results[symbol] = {
                        ...
                        'recommendations': len(symbol_result.get('recommendations', []))
                    }
    ...
    successful_symbols = sum(1 for r in batch_results.values() if r.get('success', False))
    print(f"  Success rate: {successful_symbols}/{len(test_symbols)} ({successful_symbols/len(test_symbols)*100:.0f}%)")

    # 顯示質量分數
    qualities = [r.get('signal_quality', 0) for r in batch_results.values() if r.get('success', False)]
    if qualities:
    ...
    for test_name, size in benchmark_tests.items():
        ...
        if result.get('success'):
            ...
            print(f"  [OK] {test_name}: {processing_time:.3f}s, {len(test_records) / processing_time:.1f} records/s")
        else:
            performance_results[test_name] = {
                'success': False,
    ...
    # 計算平均吞吐量
    successful_benchmarks = [r for r in performance_results.values() if r.get('success', False)]
    if successful_benchmarks:
        avg_throughput = sum(r['throughput'] for r in successful_benchmarks) / len(successful_benchmarks)
    ...
    integration_report = {
        ...
        'data_quality': {
            'stock_data_available': stock_data is not None and 'data' in stock_data and 'close' in stock_data['data'],
            'extension_successful': extension_result.get('success', False),
            'extension_ratio': extension_result.get('extension_ratio', 0),
            'signal_generation': analysis_result.get('success', False) if 'analysis_result' in locals() else False
        },
        'recommendations': []
    }
    ...
    if len(successful_benchmarks) == len(benchmark_tests):
        recommendations.append("Performance benchmarks show excellent system throughput")

    if analysis_result.get('success', False):
        recommendations.append("Advanced technical analysis is generating high-quality signals")
    ...
    status_checks = [
        ...
        ('Data Extension', 'operational' if extension_result.get('success') else 'failed'),
