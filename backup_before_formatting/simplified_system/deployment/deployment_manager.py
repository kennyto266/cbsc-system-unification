...
class DeploymentManager:
    ...
    def check_deployment_readiness(self, environment: Environment) -> Tuple[bool, List[str]]:
        """
        检查部署准备状态
        ...
        if is_ready:
            ...
            logger.warning(f"Deployment readiness check: FAILED - {len(issues)} issues found")
            for issue in issues:
            ...
            if flag.enabled and flag.dependencies:
                for dependency in flag.dependencies:
                    try:
                        ...
                        issues.append(f"Feature '{flag_name}' requires '{dependency}' but it's not installed")
                ...
                if 'memory_gb' in flag.conditions:
                    try:
                        ...
                        if memory_gb < flag.conditions['memory_gb']:
                            issues.append(f"Feature '{flag_name}' requires {flag.conditions['memory_gb']}GB memory, available: {memory_gb:.1f}GB")
                    except ImportError:
                        issues.append(f"Cannot check memory requirements for feature '{flag_name}'")

        return issues

    def execute_deployment(self, environment: Environment, version: str) -> Dict[str, Any]:
        """
        执行部署
        ...
        for check_name, check_func in health_checks:
            try:
                ...
                if not check_result.get('healthy', False):
                    ...
                    health_results['issues'].append(f"{check_name}: {check_result.get('message', 'Unknown issue')}")

            except Exception as e:
                health_results['checks'][check_name] = {'healthy': False, 'error': str(e)}
                health_results['healthy'] = False
    ...
    def _check_memory_limits(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'healthy': True, 'message': 'psutil not available - cannot check memory'}
    ...
    def _get_config(self, environment: Environment) -> DeploymentConfig:
        ...
        return DeploymentConfig(
            environment=environment,
            version="current",
            features_enabled=[name for name, flag in self.feature_flags.items() if flag.enabled],
            monitoring_enabled=True,
            auto_rollback=True
        )

    def execute_rollback(self, environment: Environment, version: str) -> Dict[str, Any]:
        """执行回滚"""
        ...
        try:
            ...
            if os.path.exists(backup_dir):
                backup_subdirs = [d for d in os.listdir(backup_dir) if d.startswith('backup_')]
                if backup_subdirs:
        ...
        logger.info(f"Rollback {'completed' if rollback_result['status'] == 'completed' else 'failed'}")
    ...
    def get_deployment_status(self, environment: Environment) -> Dict[str, Any]:
        ...
        recent_deployments = [
            ...
            if d['environment'] == environment.value
        ]
        ...
        return {
            ...
            ).total_seconds() if latest_deployment.get('end_time') else None,
    ...
    def enable_feature_flag(self, flag_name: str, rollout_percentage: float = 100.0):
        ...
        if flag_name in self.feature_flags:
            ...
            logger.info(f"Feature flag '{flag_name}' enabled with {rollout_percentage}% rollout")
        else:
...
# 便利函数
def create_deployment_manager(config_file: str = "deployment_config.yaml") -> DeploymentManager:
    """创建部署管理器"""
