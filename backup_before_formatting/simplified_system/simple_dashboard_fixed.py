...
def generate_mock_data():
    ...
    df['High'] = np.maximum(df['Open'], df['Close']) * (1 + np.random.rand(len(df)) * 0.01)
    df['Low'] = np.minimum(df['Open'], df['Close']) * (1 - np.random.rand(len(df)) * 0.01)
...
def standardize_dataframe(df):
    ...
    for col in df.columns:
        ...
        elif 'price' in col_lower and 'close' not in column_mapping:  # Handle 'price' column
            column_mapping[col] = 'Close'
...
def create_layout():
    ...
    return html.Div([
        html.H1("0700.HK Trading Dashboard", style={'textAlign': 'center', 'color': '#2c3e50'}),
        ...
        html.Div([
            ...
            html.Div([
                ...
                html.P(id='last-update', children=datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
            ], style={'width': '30%', 'display': 'inline-block', 'vertical-align': 'top'}),
