...
from data_quality_validator import (
    DataQualityValidator, validate_government_data, validate_stock_data,
    quick_data_quality_check
)
from enhanced_data_collector import (
    EnhancedGovernmentDataCollector, collect_government_data_with_validation,
    collect_all_government_data_with_validation
)
from enhanced_stock_api import (
    EnhancedStockAPI, get_stock_data_with_validation,
    generate_data_quality_summary, detect_market_anomalies
)
...
class DataQualitySystemDemo:
    ...
    def demo_government_data_validation(self):
        ...
        # 匯率數據驗證
        print("\n💱 匯率數據驗證:")
        exchange_report = validate_government_data(test_data['exchange'], "exchange_rate")
    ...
    def demo_stock_data_validation(self):
        ...
        print(f"   驗證狀態: {'通過' if len(good_data_report.critical_issues) == 0 else '失敗'}")
    ...
    def demo_auto_fix_functionality(self):
        ...
        # 模擬修復過程
        fixed_gov_data = self._simulate_auto_fix_government_data(broken_data['government'])
        fixed_gov_report = validate_government_data(fixed_gov_data, "hibor")
        print(f"   修復後質量評分: {fixed_gov_report.quality_score:.1f}/100")
        print(f"   質量提升: {fixed_gov_report.quality_score - broken_gov_report.quality_score:.1f} 分")
        ...
        print(f"   質量提升: {fixed_stock_report.quality_score - broken_stock_report.quality_score:.1f} 分")
            ...
            try:
                ...
                for result in successful_gov:
                    if result.quality_report:
                        print(f"   {result.source_name}: 質量評分 {result.quality_report.quality_score:.1f}")
    ...
    def demo_anomaly_detection(self):
        ...
        price_issues = [issue for issue in price_anomaly_report.high_issues if "跳變" in issue.message]
        ...
        volume_issues = [issue for issue in volume_anomaly_report.medium_issues if "成交量" in issue.message]
        ...
        halt_issues = [issue for issue in halt_report.medium_issues if "交易日" in issue.message]
        ...
        try:
            ...
            if 'error' not in anomalies:
                print(f"   0700.hk 異常檢測完成，發現 {len(anomalies['anomalies_detected'])} 種異常情況")
    ...
    def demo_quality_reporting(self):
        ...
        passing_datasets = len([r for r in reports.values() if len(r.critical_issues) == 0])
    ...
    def _create_test_stock_data(self) -> dict:
        ...
        # 確保OHLC邏輯正確
        good_quality_data['high'] = good_quality_data[['open', 'close']].max(axis=1) * 1.02
        good_quality_data['low'] = good_quality_data[['open', 'close']].min(axis=1) * 0.98

        # 低質量數據（包含問題）
        poor_quality_data = good_quality_data.copy()
        poor_quality_data.iloc[10:15, 1] = poor_quality_data.iloc[10:15, 3] * 0.8  # high < close
        poor_quality_data.iloc[20:25, 2] = poor_quality_data.iloc[20:25, 0] * 1.2  # low > open
        poor_quality_data.iloc[30, 0] = -50  # 負價格
        poor_quality_data.iloc[35:40, 3] = 0  # 零價格

        return {
            'good_quality': good_quality_data,
    ...
    def _simulate_auto_fix_stock_data(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'close' in fixed_data.columns:
            ...
            if 'open' not in fixed_data.columns:
                fixed_data['open'] = fixed_data['close'].shift(1).fillna(fixed_data['close'])
            ...
            fixed_data['high'] = np.maximum(fixed_data['high'], fixed_data[['open', 'close']].max(axis=1))
            fixed_data['low'] = np.minimum(fixed_data['low'], fixed_data[['open', 'close']].min(axis=1))
