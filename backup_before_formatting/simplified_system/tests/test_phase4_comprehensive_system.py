...
class TestConfigurationManagement(unittest.TestCase):
    ...
    def test_config_file_existence(self):
        ...
        for config_file in required_configs:
            ...
            self.assertTrue(os.path.exists(config_path), f"Config file {config_file} should exist")
...
class TestStrategyManagement(unittest.TestCase):
    ...
    def test_strategy_availability(self):
        ...
        try:
            ...
            core_strategies = ['RSI_MEAN_REVERSION', 'MACD_CROSSOVER', 'DUAL_MOVING_AVERAGE']
            for strategy in core_strategies:
                self.assertIn(strategy, available_strategies, f"Core strategy {strategy} should be available")
            
            logger.info(f"Strategy availability test passed: {len(available_strategies)} strategies")
    ...
    def test_strategy_execution(self):
        ...
        try:
            ...
            logger.warning("Strategy manager not available - testing basic strategy logic")
            # Basic strategy test
        ...
        try:
            ...
            try:
                ...
                if result is not None:
                    logger.warning("Invalid parameters were accepted without validation")
            except (ValueError, TypeError):
                # Expected behavior for invalid parameters
                pass
                
        except ImportError:
            logger.warning("Strategy manager not available for parameter validation test")
...
class TestPerformanceOptimization(unittest.TestCase):
    ...
    def test_parallel_processing(self):
        ...
        np.testing.assert_array_almost_equal(sequential_results, parallel_results, decimal=5)
        ...
        self.assertGreater(speedup, 1.5, "Parallel processing should provide at least 1.5x speedup")
    ...
    def test_caching_performance(self):
        ...
        try:
            ...
            # Test cache miss
            start_time = time.time()
            result1 = cache.get_or_compute('test_key', lambda: np.sum(self.large_dataset['close']))
            cache_miss_time = time.time() - start_time
            
            # Test cache hit
            start_time = time.time()
            result2 = cache.get_or_compute('test_key', lambda: np.sum(self.large_dataset['close']))
            ...
            logger.warning("High performance cache not available - skipping caching test")
    ...
    def test_memory_efficiency(self):
        ...
        self.assertGreater(memory_recovered, memory_increase * 0.7, "Should recover at least 70% of memory")
        
        logger.info(f"Memory efficiency test passed: Increase={memory_increase:.1f}MB, Recovered={memory_recovered:.1f}MB")
...
class TestVectorBTEngine(unittest.TestCase):
    ...
    def test_vectorbt_availability(self):
        ...
        try:
            ...
            logger.info(f"VectorBT availability test passed: Total Return={portfolio.total_return():.3f}")
    ...
    def test_vectorbt_performance(self):
        ...
        try:
            ...
            for fast in fast_periods:
                for slow in slow_periods:
                    if fast < slow:
                        ...
                        portfolio = vbt.Portfolio.from_signals(large_data['close'], entries, exits)
            ...
            logger.info(f"VectorBT performance test passed: Best Return={best_return:.3f}, Time={execution_time:.2f}s, Best Params={best_params}")
    ...
    def test_vectorbt_integration_with_strategies(self):
        ...
        try:
            ...
            logger.info(f"VectorBT RSI strategy test passed: Return={portfolio.total_return():.3f}")
...
class TestGovernmentDataValidation(unittest.TestCase):
    ...
    def setUp(self):
        """Set up test fixtures"""
        self.data_dir = os.path.join(os.path.dirname(__file__), '..', 'data', 'government')
    ...
    def test_government_data_availability(self):
        ...
        self.assertGreater(len(data_files), 0, "Should have at least one government data file")
    ...
    def test_hibor_data_structure(self):
        ...
        hibor_files = [f for f in os.listdir(self.data_dir) if 'hibor' in f.lower() and f.endswith('.json')]
        ...
        try:
            ...
            if hibor_data:
                ...
                for field in required_fields:
                    self.assertIn(field, sample_record, f"HIBOR record should have {field} field")
    ...
    def test_data_freshness(self):
        ...
        self.assertLess(days_old, 30, f"Government data should be fresher than 30 days (current: {days_old} days)")
        
        logger.info(f"Data freshness test passed: Most recent data is {days_old} days old")
...
class TestMockDataRemoval(unittest.TestCase):
    ...
    def test_api_endpoints_are_real(self):
        ...
        # Validate URL format
        self.assertTrue(stock_api_url.startswith('http://'), "Stock API should use HTTP")
        self.assertIn('18.180.162.113', stock_api_url, "Should use the correct IP address")
    ...
    def test_real_hkma_apis(self):
        ...
        for endpoint in hkma_endpoints:
            # Validate URL format
            self.assertTrue(endpoint.startswith('https://api.hkma.gov.hk/'), f"Should use HKMA domain: {endpoint}")
            self.assertIn('public', endpoint, f"Should use public API: {endpoint}")
            self.assertIn('market-data-and-statistics', endpoint, f"Should use market data endpoint: {endpoint}")
...
class TestEndToEndWorkflow(unittest.TestCase):
                ...
                try:
                    ...
                    try:
                        ...
                        if hasattr(result, 'total_return'):
                            ...
                            logger.info(f"Complete workflow test passed: Return={result.total_return():.3f}")
                        ...
                        logger.info(f"Simple workflow test passed: Return={total_return:.3f}")
                        
                except ImportError:
                    logger.warning("Core indicators not available - using basic workflow")
                    
            else:
                logger.warning("Stock API returned mock data - testing with synthetic data")
    ...
    def _create_synthetic_data(self) -> pd.DataFrame:
        ...
        close_prices = 100 * np.exp(np.cumsum(np.random.normal(0.001, 0.02, self.test_period)))
        
        return pd.DataFrame({
            'open': np.roll(close_prices, 1),
            'high': close_prices * (1 + np.random.uniform(0, 0.02, self.test_period)),
            'low': close_prices * (1 - np.random.uniform(0, 0.02, self.test_period)),
            'close': close_prices,
    ...
    def test_performance_of_complete_workflow(self):
        ...
        try:
            ...
            rsi = pd.Series(np.random.uniform(20, 80, len(market_data)), index=market_data.index)
        ...
        logger.info(f"Workflow performance test passed: Time={execution_time:.3f}s, Return={total_return:.3f}, Sharpe={sharpe_ratio:.3f}")
...
def run_phase4_comprehensive_tests():
    ...
    # Save test report
    report_path = os.path.join(os.path.dirname(__file__), '..', 'phase4_comprehensive_test_report.json')
