...
class SharpeRatioAnalyzer:
    ...
    def calculate_correct_sharpe(self, returns: pd.Series, trading_days: int = None) -> dict:
        """
        計算正確的Sharpe Ratio
    ...
    def validate_calculation_method(self, result: dict) -> dict:
        ...
        # 檢查Sharpe Ratio的合理性
        if abs(sharpe) > 10:
            validation['warnings'].append(f'Unrealistic Sharpe ratio: {sharpe:.3f} (typical range: -3 to 3)')
        elif abs(sharpe) > 5:
            validation['warnings'].append(f'High Sharpe ratio: {sharpe:.3f} (consider validation)')

        # 檢查年化因子
        if 'annual_factor' in result:
            annual_factor = result['annual_factor']
            if annual_factor < 10 or annual_factor > 20:
                validation['warnings'].append(f'Unusual annualization factor: {annual_factor:.2f}')
                    ...
                    if 'returns' in strategy:
                        try:
                            ...
                            if 'error' not in corrected_sharpe:
                                ...
                                strategy['sharpe_ratio'] = corrected_sharpe['sharpe_ratio']
                                strategy['sharpe_calculation_details'] = {
                                    'original_sharpe': old_sharpe,
                                    'corrected_sharpe': corrected_sharpe['sharpe_ratio'],
                                    'method': 'Fixed annualized Sharpe with 3% risk-free rate'
                                }
                                fixed_count += 1

                                # 添加驗證
                                validation = self.validate_calculation_method(corrected_sharpe)
                                if not validation['is_valid']:
                                    error_count += 1
                                    print(f"  WARNING: Sharpe calculation issue for strategy: {validation['issues'] + validation['warnings']}")

                        except Exception as e:
                            error_count += 1
                            print(f"  ERROR fixing strategy {strategy.get('name', 'unknown')}: {e}")
...
def analyze_sharpe_issues():
    ...
    # 檢查現有結果文件
    import glob
    result_files = glob.glob("*sharpe*.json") + glob.glob("*optimization*.json") + glob.glob("*backtest*.json")
    ...
    for file_path in result_files:
        try:
            ...
            if high_sharpe_count > 0:
                ...
                print(f"   WARNING: Found {high_sharpe_count} unrealistic Sharpe ratios")
    ...
    if problematic_files:
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            ...
            for file_info in problematic_files:
                ...
                f.write(f"- Unrealistic Sharpe ratios: {file_info['high_sharpe_count']}\n")
            ...
            f.write("   sharpe = excess_returns.mean() / excess_returns.std() * sqrt(252)\n")
            f.write("   ```\n\n")
