...
def backtest_rsi_strategy(params):
    ...
    try:
        ...
        if trades:
            ...
            sharpe_ratio = excess_returns.mean() / excess_returns.std() * np.sqrt(252) if std_return > 0 else 0
...
def backtest_macd_strategy(params):
    ...
    try:
        ...
        result = engine.backtest_strategy(data, 'MACD_CROSSOVER', {
            'fast': fast,
            'slow': slow,
            'signal': signal
        })
...
def backtest_ma_strategy(params):
    ...
    try:
        ...
        # 使用VectorBT引擎回測
        engine = VectorBTEngine()
        result = engine.backtest_strategy(data, 'DUAL_MOVING_AVERAGE', {
...
def main():
    ...
    total_tests = (len(param_combinations['RSI']) +
    ...
    with ProcessPoolExecutor(max_workers=num_processes) as executor:
        future_to_task = {executor.submit(func, params): (strategy_type, params)
