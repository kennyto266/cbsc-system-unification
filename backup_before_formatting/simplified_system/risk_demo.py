...
def main():
    ...
    portfolio_positions = pd.DataFrame([
        {'asset': 'AAPL', 'asset_type': 'equity', 'market_value': 800000, 'weight': 0.267, 'sector': 'Technology'},
        {'asset': 'MSFT', 'asset_type': 'equity', 'market_value': 600000, 'weight': 0.200, 'sector': 'Technology'},
        {'asset': 'NVDA', 'asset_type': 'equity', 'market_value': 400000, 'weight': 0.133, 'sector': 'Technology'},
        {'asset': 'JPM', 'asset_type': 'equity', 'market_value': 300000, 'weight': 0.100, 'sector': 'Financial'},
        {'asset': 'BRK.B', 'asset_type': 'equity', 'market_value': 250000, 'weight': 0.083, 'sector': 'Financial'},
        {'asset': 'US_Treasury_10Y', 'asset_type': 'government_bond', 'market_value': 400000, 'weight': 0.133, 'sector': 'Fixed Income'},
        {'asset': 'Gold', 'asset_type': 'commodity', 'market_value': 150000, 'weight': 0.050, 'sector': 'Commodities'},
        {'asset': 'REIT_Index', 'asset_type': 'real_estate', 'market_value': 100000, 'weight': 0.033, 'sector': 'Real Estate'}
    ])
    ...
    print(f"[SUCCESS] Portfolio Composition: {len(portfolio_positions)} assets across {len(portfolio_positions['sector'].unique())} sectors")
    ...
    try:
        from risk.monte_carlo_var import MonteCarloVaRCalculator, VaRConfig, DistributionType
        ...
        for confidence, horizon_results in results.var_results.items():
            for horizon, var_result in horizon_results.items():
                ...
                print(f"   {int(confidence*100)}% VaR ({horizon_str}): ${var_result.var_value:,.2f} ({var_result.var_percentage:.2%})")
                print(f"   {int(confidence*100)}% Expected Shortfall ({horizon_str}): ${var_result.expected_shortfall:,.2f} ({var_result.expected_shortfall_percentage:.2%})")

        # Show distribution fitting results
        print(f"\n📊 Distribution Fitting Results:")
        if results.distribution_params['type'] == 'student_t':
            print(f"   Student-t Degrees of Freedom: {results.distribution_params['df']:.2f}")
        ...
        print(f"❌ Monte Carlo VaR demo failed: {e}")
    ...
    try:
        ...
        stress_report = engine.run_stress_tests(
            portfolio_data,
            scenario_ids=['2008_financial_crisis', '2020_covid_crash', 'interest_rate_shock']
        )
        ...
        print(f"   Worst Case Loss Percentage: {stress_report.worst_case_loss_percentage:.2%}")
        ...
        for result in stress_report.detailed_results:
            ...
            print(f"     Portfolio Loss: ${result.portfolio_loss:,.2f} ({result.portfolio_loss_percentage:.2%})")
        ...
        print(f"❌ Stress testing demo failed: {e}")
    ...
    try:
        ...
        for _, position in portfolio_positions.iterrows():
            ...
            prices = pd.Series(
                base_price * np.cumprod(1 + np.random.normal(0, 0.02, 100)),
                index=dates
            )
            volumes = pd.Series(
        ...
        funding_data = {
            'funding_sources': [
                {
                    'name': 'Revolver_Credit_Line',
                    'amount': 400000,
                    'cost': 0.03,
                    'maturity_profile': {'1M': 100000, '3M': 100000, '6M': 100000, '12M': 100000},
                ...
                },
                {
                    ...
                    'amount': 600000,
                    'cost': 0.04,
                    'maturity_profile': {'2Y': 300000, '5Y': 300000},
                    'collateral_ratio': 1.0,
        ...
        print(f"   Overall Liquidity Risk Score: {liquidity_results.overall_liquidity_risk:.1f}/100")
        ...
        print(f"\n📊 Regulatory Ratios:")
        print(f"   Liquidity Coverage Ratio (LCR): {liquidity_results.liquidity_coverage_ratio:.2f}")
        print(f"   Net Stable Funding Ratio (NSFR): {liquidity_results.net_stable_funding_ratio:.2f}")
        ...
        for period, gap in liquidity_results.cash_flow_gap_analysis.items():
            gap_str = f"${abs(gap):,.2f} surplus" if gap > 0 else f"${abs(gap):,.2f} shortfall"
            print(f"   {period}: {gap_str}")

    except Exception as e:
        print(f"❌ Liquidity risk demo failed: {e}")
    ...
    try:
        ...
        print(f"   Sector Diversification: {len(portfolio_positions['sector'].unique())} sectors")
        print(f"   Largest Position: {portfolio_positions['market_value'].max():,.2f} ({portfolio_positions.loc[portfolio_positions['market_value'].idxmax(), 'asset']})")
        print(f"   Portfolio Concentration: {(portfolio_positions['market_value'] / portfolio_value).max():.2%}")
        ...
        print(f"   1. VaR analysis provides quantitative risk estimates at multiple confidence levels")
        print(f"   2. Stress testing evaluates portfolio resilience under extreme market conditions")
        print(f"   3. Liquidity analysis ensures ability to meet obligations and market trading needs")
        ...
        print(f"❌ Risk summary demo failed: {e}")
