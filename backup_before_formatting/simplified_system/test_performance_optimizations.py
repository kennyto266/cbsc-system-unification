...
def test_high_performance_cache():
    ...
    try:
        ...
        # 测试缓存键生成
        cache_key = global_cache.generate_cache_key(test_data, "test_operation", {'param': 14})
...
def test_parallel_optimizer():
    ...
    try:
        ...
        # 生成参数组合
        param_combinations = [
            {'period': p} for p in range(10, 31, 5)
        ]

        # 测试并行执行
        start_time = time.time()
        results = global_parallel_optimizer.parallel_execute(compute_rsi_data, param_combinations)
        ...
        logger.info(f"✅ 性能统计: 工作线程数 {stats['worker_configuration']['optimal_workers']}, 任务成功率 {stats['task_statistics']['success_rate']:.1%}")
...
def test_gpu_manager():
    ...
    try:
        from src.performance.gpu_manager import get_gpu_manager, get_gpu_environment

        # 测试GPU环境检测
        gpu_env = get_gpu_environment()
        logger.info(f"✅ GPU环境检测: 可用={gpu_env.is_available}, 数量={gpu_env.gpu_count}")

        # 测试GPU管理器初始化
        gpu_manager = get_gpu_manager(auto_fallback=True)
        backend_info = gpu_manager.get_backend_info()
        logger.info(f"✅ GPU管理器: 后端类型={backend_info.get('backend_type', 'Unknown')}")
...
def test_performance_monitor():
    ...
    try:
        ...
        logger.info(f"✅ 基准比较: 性能提升 {comparison.get('improvement_percent', 0):.1f}%")
...
def test_core_indicators_optimization():
    ...
    try:
        ...
        logger.info(f"✅ 批量指标计算完成: {batch_time:.4f}s, 指标数量: {len(all_indicators)}")

        # 测试缓存优化
        start_time = time.time()
        for _ in range(10):
            cache_key = global_cache.generate_cache_key(test_data, 'rsi_batch', {'period': 14})
...
"""
    ...
    for test_name, result in test_results['tests'].items():
        ...
        text_report += f"{status_icon} {test_name}: {result['status']} ({execution_time:.3f}s)\n"
