...
class ImprovedRealDataIntegration:
    ...
    def create_alpha_signals_with_real_data(self, symbol: str = "0700.HK",
        ...
        try:
            ...
            alpha_result['data_sources_used'].append(f"stock_data: {len(stock_data)} records")
            ...
            for data_type, data in available_data.items():
                alpha_result['data_sources_used'].append(f"{data_type}: {len(data['data'])} records")
    ...
    def _generate_technical_signals(self, stock_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        # 價格動量信號
        momentum_5d = (close_prices.iloc[-1] / close_prices.iloc[-5] - 1) * 100 if len(close_prices) >= 5 else 0
        momentum_20d = (close_prices.iloc[-1] / close_prices.iloc[-20] - 1) * 100 if len(close_prices) >= 20 else 0

        signals['momentum'] = {
            'momentum_5d': momentum_5d,
            'momentum_20d': momentum_20d,
            'signal': 'STRONG_MOMENTUM' if abs(momentum_5d) > 3 else 'WEAK_MOMENTUM',
            'trend': 'BULLISH' if momentum_20d > 5 else 'BEARISH' if momentum_20d < -5 else 'SIDEWAYS',
            'weight': 0.15
        }

        # 波動率信號
        returns = close_prices.pct_change().dropna()
        volatility = returns.rolling(window=20).std().iloc[-1] * np.sqrt(252) * 100 if len(returns) >= 20 else 0

        signals['volatility'] = {
            'annualized_volatility': volatility,
            'signal': 'HIGH_VOLATILITY' if volatility > 30 else 'LOW_VOLATILITY' if volatility < 15 else 'NORMAL_VOLATILITY',
            'recent_volatility': returns.tail(5).std() * np.sqrt(252) * 100 if len(returns.tail(5)) >= 5 else 0,
            'weight': 0.10
        }

        return signals

    def _generate_macro_signals(self, government_data: Dict[str, Any],
        ...
        if 'hibor_rates' in government_data and government_data['hibor_rates']:
            ...
            if hibor_data:
                ...
                # HIBOR水平信號
                rate_level_signal = 'ACCOMMODATIVE' if hibor_overnight < 3.5 else 'RESTRICTIVE' if hibor_overnight > 5.0 else 'NEUTRAL'
        ...
        # 銀行流動性信號
        if 'interbank_liquidity' in government_data and government_data['interbank_liquidity']:
            ...
            if liquidity_data:
                ...
                signals['liquidity'] = {
                    ...
                    'signal': 'AMPLE' if liquidity_amount > 1000 else 'TIGHT' if liquidity_amount < 500 else 'NORMAL',
                    'weight': 0.20
                }

        return signals

    def _generate_composite_signals(self, tech_signals: Dict, macro_signals: Dict,
                                  stock_data: pd.DataFrame) -> Dict[str, Any]:
        """生成綜合Alpha信號"""
        composite = {
        ...
        try:
            ...
            for signal_name, signal_data in tech_signals.items():
                if 'weight' in signal_data:
                    signal_score = self._signal_to_numeric(signal_data.get('signal', 'NEUTRAL'))
            ...
            for signal_name, signal_data in macro_signals.items():
                if 'weight' in signal_data:
                    signal_score = self._macro_signal_to_numeric(signal_data, signal_name)
    ...
    def _signal_to_numeric(self, signal: str) -> float:
        """將信號轉換為數值"""
        signal_map = {
            'STRONG_BUY': 1.0, 'BUY': 0.5, 'BULLISH': 0.4,
            'OVERBOUGHT': -0.2, 'OVERSOLD': 0.3, 'NEUTRAL': 0,
            'SELL': -0.5, 'STRONG_SELL': -1.0, 'BEARISH': -0.4
        }
    ...
    def _calculate_signal_performance(self, signals: Dict, stock_data: pd.DataFrame) -> Dict:
        """計算信號性能指標"""
        performance = {
            'signal_count': 0,
        ...
        try:
            ...
            # 數據源多樣性
            performance['data_source_diversity'] = min(1.0, performance['signal_count'] / 5.0)

            # 計算預期Alpha（基於信號強度和信心度）
            composite = signals.get('composite', {})
            if composite:
                performance['signal_consistency'] = composite.get('confidence', 0)
                performance['expected_alpha'] = composite.get('alpha_score', 0) * 0.15  # 年化預期回報
    ...
    def _generate_portfolio_signals(self, individual_results: Dict) -> Dict:
        ...
        try:
            ...
            if quality_scores:
                ...
                top_picks = [
                    symbol for symbol, score in quality_scores.items()
