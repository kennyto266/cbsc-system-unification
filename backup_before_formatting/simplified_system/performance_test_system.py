...
from src.performance.performance_monitor import get_performance_monitor, start_global_monitoring, stop_global_monitoring
...
class PerformanceTestSuite:
    ...
    def test_cache_performance(self):
        ...
        start_time = time.time()
        for _ in range(100):
            cache_key = global_cache.generate_cache_key(test_data, 'rsi_test', {'period': 14})
            cached_rsi = global_cache.get(cache_key)
            if cached_rsi is None:
                rsi = indicators.calculate_rsi(test_data['close'], 14)
                global_cache.put(cache_key, rsi)

            cache_key = global_cache.generate_cache_key(test_data, 'sma_test', {'period': 20})
            cached_sma = global_cache.get(cache_key)
            if cached_sma is None:
                sma = indicators.calculate_sma(test_data['close'], 20)
                global_cache.put(cache_key, sma)

            cache_key = global_cache.generate_cache_key(test_data, 'macd_test', {'fast': 12, 'slow': 26})
        ...
        self.performance_monitor.record_operation_time('indicators_with_cache', with_cache_time)

        logger.info(f"缓存性能测试完成: 加速 {speedup:.2f}x, 命中率 {cache_stats['overall_stats']['hit_rate']:.1%}")
    ...
    def test_parallel_computation(self):
        ...
        def evaluate_rsi_params(params):
            """评估RSI参数"""
            try:
                result = engine.backtest_strategy(test_data, "RSI_MEAN_REVERSION", params, "TEST")
                return {
                    'parameters': params,
        ...
        self.performance_monitor.record_operation_time('rsi_optimization_parallel', parallel_time)

        logger.info(f"并行计算测试完成: 加速 {speedup:.2f}x, 使用 {parallel_stats['worker_configuration']['optimal_workers']} 个工作线程")
    ...
    def test_gpu_acceleration(self):
        ...
        for _ in range(50):
            ...
            cpu_results = cpu_backend.compute_indicators(prices, indicators_config)
        ...
        if gpu_manager.is_gpu_available():
            start_time = time.time()
            for _ in range(50):
                gpu_results = gpu_manager.compute_indicators(prices, indicators_config)
    ...
    def test_vectorbt_optimization(self):
        ...
        try:
            ...
            self.test_results['vectorbt_optimization'] = {
                ...
                'combinations_per_second': optimization_result.get('total_combinations', 0) / optimization_time if optimization_time > 0 else 0
            }

            self.performance_monitor.record_operation_time('vectorbt_optimization', optimization_time)

            logger.info(f"VectorBT优化测试完成: {optimization_result.get('total_combinations', 0)} 个组合, {optimization_time:.2f}秒")
    ...
    def test_indicator_optimization(self):
        ...
        start_time = time.time()
        for _ in range(100):
            all_indicators = indicators.calculate_all_indicators(test_data, ['RSI', 'MACD', 'SMA', 'BOLLINGER'])
        ...
        logger.info(f"技术指标优化测试完成: RSI {100/rsi_time:.1f} 次/秒, MACD {100/macd_time:.1f} 次/秒")
    ...
    def test_comprehensive_performance(self):
        ...
        try:
            ...
            logger.info(f"综合性能测试完成: {len(strategy_results)}/{len(strategies)} 策略成功, {total_time:.2f}秒")
    ...
    def generate_test_report(self) -> Dict[str, Any]:
        ...
        # 性能改进总结
        cache_speedup = self.test_results.get('cache_performance', {}).get('speedup', 1.0)
        parallel_speedup = self.test_results.get('parallel_computation', {}).get('speedup', 1.0)
        gpu_speedup = self.test_results.get('gpu_acceleration', {}).get('speedup', 1.0)

        # 目标达成情况
        cache_target_met = cache_speedup >= 2.0  # 目标: 2倍性能提升
        parallel_target_met = self.test_results.get('parallel_computation', {}).get('optimal_workers', 1) >= 4  # 目标: 4+工作线程
        gpu_available = self.test_results.get('gpu_acceleration', {}).get('gpu_available', False)
        ...
        performance_grade = "A" if successful_optimizations >= 3 else "B" if successful_optimizations >= 2 else "C"
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        cache_hit_rate = self.test_results.get('cache_performance', {}).get('cache_hit_rate', 0)
