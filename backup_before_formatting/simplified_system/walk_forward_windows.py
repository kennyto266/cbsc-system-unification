...
# -*- coding: utf-8 -*-
"""
Walk-Forward Analysis for 0700.HK Trading Strategies (Windows Compatible)
专业级Walk-Forward分析系统 - Windows兼容版本
...
class WalkForwardAnalyzer:
    ...
    def run_walk_forward_analysis(self, data: pd.DataFrame, strategy_configs: List[Dict]) -> Dict:
        ...
        for i, config in enumerate(strategy_configs):
            ...
            while current_train_start + self.config.train_period + self.config.test_period <= total_periods:
                ...
                print(f"  窗口 {window_count + 1}: 训练期{train_data.index[0].date()}至{train_data.index[-1].date()}, "
                ...
                if result.trades_count >= self.config.min_trades:
                    ...
                    print(f"    OK 回报: {result.total_return:.2%}, Sharpe: {result.sharpe_ratio:.2f}, 交易: {result.trades_count}")
                else:
                    print(f"    FAIL 交易次数不足({result.trades_count} < {self.config.min_trades})")
            ...
            if strategy_results:
                ...
                print(f"{strategy_name} 总体表现: 平均Sharpe {all_results[strategy_name]['summary']['mean_sharpe']:.2f}, "
                      f"平均回報 {all_results[strategy_name]['summary']['mean_return']:.2%}")
            else:
                print(f"FAIL {strategy_name} 无有效窗口结果")
    ...
    def generate_report(self, results: Dict) -> str:
        ...
        report += f"配置: 训练期{self.config.train_period}天, 测试期{self.config.test_period}天, 步长{self.config.step_size}天\n"
        report += "="*80 + "\n\n"
        ...
        report += "{:<15} {:<25} {:<8} {:<12} {:<12} {:<15} {:<10} {:<12}\n".format(
            "策略", "参数", "窗口数", "平均回报", "平均Sharpe", "平均回撤", "胜率", "成功率"
        )
        report += "-"*100 + "\n"
        ...
        if results:
            ...
            report += f"预期年化回报: {best_strategy[1]['summary']['mean_return']:.2%}\n"
