...
app.layout = html.Div([
    html.H1("🎯 Jupyter Analytics Dashboard - 0700.HK",
             style={'textAlign': 'center', 'color': '#2c3e50', 'marginBottom': 30}),

    html.Div([
        html.Div([
            html.H3("Data Source Status", style={'color': '#34495e'}),
            html.P(f"Simplified System: {'✅ Available' if SIMPLIFIED_SYSTEM_AVAILABLE else '⚠️ Mock Data'}",
                  style={'color': 'green' if SIMPLIFIED_SYSTEM_AVAILABLE else 'orange'}),
            html.P(f"Data Records: {len(processed_data)}"),
            html.P(f"Date Range: {processed_data.index[0].date()} to {processed_data.index[-1].date()}"),
        ], className='four columns', style={'padding': 20, 'backgroundColor': '#ecf0f1', 'borderRadius': 10}),

        html.Div([
            html.H3("Latest Indicators", style={'color': '#34495e'}),
            html.P(f"Close Price: {processed_data['Close'].iloc[-1]:.2f} HKD"),
            html.P(f"RSI(14): {processed_data['RSI14'].iloc[-1]:.2f}"),
            html.P(f"MA20: {processed_data['MA20'].iloc[-1]:.2f} HKD"),
            html.P(f"Daily Return: {processed_data['Returns'].iloc[-1]:.2%}"),
        ], className='four columns', style={'padding': 20, 'backgroundColor': '#e8f5e8', 'borderRadius': 10}),
        ...
        ], className='four columns', style={'padding': 20, 'backgroundColor': '#fdeaea', 'borderRadius': 10}),
    ...
    html.Div([
        html.Hr(),
        html.P("🎉 Jupyter Notebook Analytics Dashboard",
               style={'textAlign': 'center', 'color': '#7f8c8d', 'margin': 20}),
        html.P("Built with Simplified System Integration | Real-time 0700.HK Analysis",
