...
class BacktestEngineConfig(BaseModel):
    ...
    commission_rate: Decimal = Field(
        Decimal("0.001"), ge=0, le=0.01, description="手续费率"
    )
    slippage_rate: Decimal = Field(
        Decimal("0.0005"), ge=0, le=0.01, description="滑点率"
    )
    benchmark_symbol: Optional[str] = Field(None, description="基准指数")
    risk_free_rate: Decimal = Field(
        Decimal("0.03"), ge=0, le=0.1, description="无风险利率"
    )
    max_position_size: Decimal = Field(
        Decimal("0.1"), gt=0, le=1, description="最大仓位比例"
    )
...
class BacktestMetrics(BaseModel):
    ...
    benchmark_performance: Optional[StrategyPerformance] = Field(
        None, description="基准绩效"
    )
    ...
    portfolio_analysis: Dict[str, Any] = Field(
        default_factory=dict, description="投资组合分析"
    )
...
class BaseBacktestEngine(ABC):
    ...
    def __init__(self, config: BacktestEngineConfig):
        ...
        self.logger = logging.getLogger(
            f"hk_quant_system.backtest.{config.engine_type}"
        )
    ...
    async def run_backtest(
        self, strategy: Dict[str, Any], market_data: List[RealMarketData]
    ) -> BacktestMetrics:
        """
        运行回测
