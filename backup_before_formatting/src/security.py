...
class SecurityManager:
    ...
    def validate_input(self, data: Any, data_type: str = "general") -> bool:
        ...
        try:
            if data_type == "email":
                ...
                # 通用验证：检查长度和特殊字符
                data_str = str(data)
                return 0 < len(data_str) < 1000 and not any(
                    ord(c) < 32 for c in data_str
                )
    ...
    def check_rate_limit(
        self, identifier: str, max_attempts: int = None, window_seconds: int = 3600
    ) -> bool:
        """检查速率限制"""
        ...
        self.login_attempts[identifier] = [
            attempt
            for attempt in self.login_attempts[identifier]
    ...
    def sanitize_data(
        self, data: Dict[str, Any], sensitive_fields: List[str] = None
    ) -> Dict[str, Any]:
        """数据清理（移除敏感信息）"""
