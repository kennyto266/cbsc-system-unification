...
class AsyncTaskManager:
    ...
    async def run_in_process(self, func: Callable, *args, **kwargs) -> Any:
        ...
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            self.process_executor, partial(func, *args, **kwargs)
        )

    async def gather_with_limit(
        self, tasks: List[asyncio.Task], concurrency_limit: int = None
    ) -> List[Any]:
        """限制并发数的任务收集"""
    ...
    async def async_file_operation(
        self, operation: str, file_path: str, mode: str = "r", content: str = None
    ) -> Any:
        """异步文件操作"""
        try:
            if operation == "read":
    ...
    def record_metrics(self, operation: str, start_time: float, success: bool = True):
        ...
        logger.info(
            f"Performance: {operation} took {duration:.3f}s, CPU: {cpu_usage}%, Memory: {memory_usage}%"
        )
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        for op, durations in operations.items():
            stats[op] = {
                ...
                "success_rate": sum(
                    1 for m in self.metrics if m.operation == op and m.success
                )
                / len(durations),
            }
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                ...
                if hasattr(args[0], "record_metrics"):
                    args[0].record_metrics(
                        operation_name or func.__name__, start_time, True
                    )
                return result
            except Exception as e:
                # 记录失败指标
                if hasattr(args[0], "record_metrics"):
                    args[0].record_metrics(
                        operation_name or func.__name__, start_time, False
                    )
                raise
...
class ResourcePool:
    ...
    async def acquire(self, timeout: float = 30.0) -> Optional[str]:
        ...
        try:
            resource = await asyncio.wait_for(
                self.available_resources.get(), timeout=timeout
            )
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "utilization": (self.max_resources - self.available_resources.qsize())
            / self.max_resources,
        }
...
async def run_concurrent_tasks(
    tasks: List[Callable], concurrency_limit: int = None
) -> List[Any]:
    """并发运行多个任务"""
