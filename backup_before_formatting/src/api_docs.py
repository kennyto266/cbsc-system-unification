        ...
        openapi_schema.update(
            {
                ...
                "servers": [
                    ...
                    {"url": "https://api.codex - quant.com", "description": "生产服务器"},
                ],
                "security": [{"BearerAuth": []}],
                "components": {
                    "securitySchemes": {
                ...
                "tags": [
                    {"name": "股票分析", "description": "股票技术分析和基本面分析"},
                    {"name": "策略回测", "description": "量化策略开发和回测"},
                    {"name": "机器学习", "description": "AI模型训练和预测"},
                    {"name": "实时数据", "description": "WebSocket实时数据流"},
                    {"name": "系统监控", "description": "系统状态和性能监控"},
        ...
        for path_data in openapi_schema.get("paths", {}).values():
            for operation_data in path_data.values():
                if isinstance(operation_data, dict):
                    operation_data.setdefault("responses", {}).setdefault(
                        "429",
                        {
                            ...
                            "content": {
                                "application / json": {
                                    "schema": {
                                        "$re": "#/components / schemas / ErrorResponse"
                                    }
                                }
                            },
                        },
