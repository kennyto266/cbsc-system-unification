...
class TelegramUser(BaseModel):
    ...
    alert_levels: List[str] = Field(
        default_factory=lambda: ["critical"], description="Alert levels to receive"
    )

    # User status
    is_active: bool = Field(True, description="User active status")
    last_seen: datetime = Field(
        default_factory=datetime.now, description="Last seen timestamp"
    )
...
class TelegramBotInterface:
    ...
    async def _register_default_handlers(self) -> None:
        ...
        try:
            ...
            self.command_handlers[CommandType.POSITIONS] = (
                self._handle_positions_command
            )
            ...
            self.command_handlers[CommandType.PERFORMANCE] = (
            ...
            self.logger.info(
                f"Registered {len(self.command_handlers)} command handlers"
            )
    ...
    async def _handle_command(self, command: TelegramCommand) -> None:
        ...
        try:
            ...
            if not await self._check_user_permission(
                command.user_id, command.command_type
            ):
                await self.send_message(
            ...
            if not handler:
                await self.send_message(
                    chat_id=command.chat_id, text="❌ Command handler not found."
                )
                return
            ...
            # Send response
            if response:
                await self.send_message(chat_id=command.chat_id, text=response)
            ...
            await self.send_message(
                chat_id=command.chat_id, text=f"❌ Error executing command: {str(e)}"
            )

    async def _check_user_permission(
        self, user_id: int, command_type: CommandType
    ) -> bool:
    ...
    async def send_notification(
        self, user_id: int, message: str, alert_level: str = "INFO"
    ) -> bool:
    ...
    async def broadcast_message(
        self, message: str, role_filter: Optional[UserRole] = None
    ) -> int:
    ...
    async def shutdown(self) -> None:
        ...
        try:
            ...
            await self.broadcast_message(
                "🛑 Bot is shutting down. Goodbye!", role_filter=UserRole.ADMIN
            )
