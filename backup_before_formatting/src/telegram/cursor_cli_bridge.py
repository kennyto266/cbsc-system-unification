...
from .bot_interface import MessageType, TelegramBotInterface, TelegramUser, UserRole
...
class CursorCLIConfig(BaseModel):
    ...
    enable_trading_signals: bool = Field(
        True, description="Enable trading signal integration"
    )
...
class CursorCLIBridge:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.bot_interface = TelegramBotInterface(
                bot_token=self.config.bot_token, webhook_url=None
            )
    ...
    async def _register_custom_handlers(self) -> None:
        ...
        try:
            ...
            self.bot_interface.add_command_handler(
                "trading_signals", self._handle_trading_signals_command
            )

            self.bot_interface.add_command_handler(
                "portfolio_status", self._handle_portfolio_status_command
            )
            ...
            self.bot_interface.add_command_handler(
                "system_metrics", self._handle_system_metrics_command
            )

            self.bot_interface.add_command_handler(
                "risk_alerts", self._handle_risk_alerts_command
            )

            # Register Cursor CLI specific commands
            if self.config.enable_cursor_commands:
                self.bot_interface.add_command_handler(
                    "cursor_help", self._handle_cursor_help_command
                )

                self.bot_interface.add_command_handler(
                    "cursor_query", self._handle_cursor_query_command
                )
    ...
    async def _start_cursor_cli(self) -> bool:
        ...
        try:
            ...
            if self.config.allowed_user_ids:
                env["TG_ALLOWED_USER_IDS"] = ",".join(
                    map(str, self.config.allowed_user_ids)
                )

            if self.config.allowed_chat_ids:
                env["TG_ALLOWED_CHAT_IDS"] = ",".join(
                    map(str, self.config.allowed_chat_ids)
                )
    ...
    async def _send_trading_signal(self, signal: TradingSignalNotification) -> None:
        ...
        try:
            ...
            await self.bot_interface.broadcast_message(
                message=message, role_filter=UserRole.TRADER
            )

            await self.bot_interface.broadcast_message(
                message=message, role_filter=UserRole.ANALYST
            )
        ...
        try:
            ...
            if status.severity in ["critical", "error"]:
                ...
                for role in [UserRole.TRADER, UserRole.ANALYST, UserRole.ADMIN]:
                    await self.bot_interface.broadcast_message(
                        message, role_filter=role
                    )
            else:
                await self.bot_interface.broadcast_message(
                    message, role_filter=UserRole.ADMIN
                )

        except Exception as e:
            self.logger.error(f"Error sending system status: {e}")

    async def _format_trading_signal_message(
        self, signal: TradingSignalNotification
    ) -> str:
        ...
        try:
            side_emoji = (
                "🟢"
                if signal.side == "BUY"
                else "🔴" if signal.side == "SELL" else "🟡"
            )
            strength_bar = "█" * int(signal.strength * 10) + "░" * (
                10 - int(signal.strength * 10)
            )
            confidence_bar = "█" * int(signal.confidence * 10) + "░" * (
                10 - int(signal.confidence * 10)
            )
    ...
    async def _format_system_status_message(
        self, status: SystemStatusNotification
    ) -> str:
        ...
        try:
            ...
            if status.agents_status:
                active_agents = len(
                    [s for s in status.agents_status.values() if s == "active"]
                )
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    "http://127.0.0.1:8000 / api / dashboard / signals / latest"
                ) as resp:
                    if resp.status != 200:
                        return "❌ 无法获取最新信号。"
            ...
            side_emoji = (
                ...
                else "🔴" if str(side).upper() == "SELL" else "🟡"
            )
            sharpe_text = (
                f"{sharpe:.3f}"
                if isinstance(sharpe, (int, float)) and sharpe is not None
                else "-"
            )
            maxdd_text = (
                f"{max_dd:.2%}"
                if isinstance(max_dd, (int, float)) and max_dd is not None
                else "-"
            )
    ...
    async def _handle_agent_status_command(self, command) -> str:
        ...
        try:
            ...
            for agent_id, info in agents.items():
                ...
                performance_bar = "█" * int(info["performance"] * 10) + "░" * (
                    10 - int(info["performance"] * 10)
                )
    ...
    async def _handle_risk_alerts_command(self, command) -> str:
        ...
        try:
            ...
            alerts = [
                {
                    "level": "WARNING",
                    "message": "High volatility detected in 0700.HK",
            ...
            for alert in alerts:
                level_emoji = {"INFO": "ℹ️", "WARNING": "⚠️", "CRITICAL": "🚨"}.get(
                    alert["level"], "ℹ️"
                )
    ...
    async def _execute_cursor_command(self, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            cmd = [
                "bash",
                "-lc",
                'cursor -agent --print --output --format text --model gpt-5 --agent "$CURSOR_API_KEY"',
            ]
