...
class IntegrationManager:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            cursor_config = CursorCLIConfig(
                ...
                enable_system_monitoring=self.config.get(
    ...
    async def _handle_trading_signal(self, signal: TradingSignalNotification) -> None:
        ...
        try:
            # Log signal
            self.logger.info(
                f"Processing trading signal: {signal.symbol} {signal.side}"
            )
        ...
        try:
            ...
            if self.bot_interface:
                ...
                health["components"]["bot_interface"] = {
                    "status": (
                        "healthy" if bot_stats.get("is_running", False) else "unhealthy"
                    ),
                    "stats": bot_stats,
                }
            else:
            ...
            if self.cursor_bridge:
                ...
                health["components"]["cursor_bridge"] = {
                    "status": (
                        "healthy"
                        if bridge_stats.get("is_running", False)
                        else "unhealthy"
                    ),
                    "stats": bridge_stats,
                }
            ...
            if (
                "unhealthy" in component_statuses
                or "not_initialized" in component_statuses
            ):
