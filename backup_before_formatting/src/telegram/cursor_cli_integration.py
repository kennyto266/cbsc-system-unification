...
class SystemStatusNotification(BaseModel):
    ...
    agents_status: Dict[str, str] = Field(
        default_factory=dict, description="Agent statuses"
    )
...
class CursorCLIIntegration:
    ...
    async def _handle_command_response(self, response: UserCommandResponse) -> None:
        ...
        try:
            ...
            await self.bot_interface.send_message(
                chat_id=response.user_id, text=message
            )
    ...
    async def _format_command_response_message(
        self, response: UserCommandResponse
    ) -> str:
    ...
    async def _send_signal_notification(
        self, signal: TradingSignalNotification, message: str
    ) -> None:
    ...
    async def _send_status_notification(
        self, status: SystemStatusNotification, message: str
    ) -> None:
