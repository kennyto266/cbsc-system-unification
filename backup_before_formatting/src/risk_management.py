...
class RiskMetrics:
    ...
    def calculate_portfolio_risk(
        strategy_returns: Dict[str, pd.Series], weights: Dict[str, float]
    ) -> Dict:
        """
        計算組合風險指標
    ...
    def apply_stop_loss_take_profit(
        ...
        try:
            ...
            for i in range(len(signals)):
                ...
                current_signal = (
                    signals.iloc[i]["signal"] if "signal" in signals.columns else 0
                )
                ...
                if position != 0:
                    ...
                    if position > 0:  # 多頭
                        ...
                        if current_price <= entry_price * (1 - stop_loss_pct):
                            adjusted_signals.iloc[
                                i, adjusted_signals.columns.get_loc("signal")
                            ] = -position
                            position = 0
                            continue
