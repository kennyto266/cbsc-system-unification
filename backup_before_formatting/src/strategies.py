...
def run_brute_force(stock_list=None):
    ...
    for symbol in stock_list:
        csv_path = os.path.join(
            csv_dir,
            f"{symbol.replace('.', '_')}_2020 - 01 - 01_{datetime.now().strftime('%Y-%m-%d')}.csv",
        )
        ...
        if valid_results:
            ...
            out_path = os.path.join(
                csv_dir, f"{symbol.replace('.', '_')}_rsi_vs_stock.csv"
            )
            out.to_csv(out_path)
...
def run_macd_strategy(stock_list=None):
    ...
    for symbol in stock_list:
        ...
        out_path = os.path.join(
            csv_dir, f"{symbol.replace('.', '_')}_macd_vs_stock.csv"
        )
        if not os.path.exists(csv_path):
            print(f"找不到 {csv_path}，請先下載數據")
            continue
...
def run_sma_ema_strategy(stock_list=None):
    ...
    for symbol in stock_list:
        ...
        out_path = os.path.join(
            csv_dir, f"{symbol.replace('.', '_')}_smaema_vs_stock.csv"
        )
        if not os.path.exists(csv_path):
            print(f"找不到 {csv_path}，請先下載數據")
            continue
...
def run_bollinger_strategy(stock_list=None):
    ...
    for symbol in stock_list:
        ...
        out_path = os.path.join(
            csv_dir, f"{symbol.replace('.', '_')}_boll_vs_stock.csv"
        )
        if not os.path.exists(csv_path):
            print(f"找不到 {csv_path}，請先下載數據")
            continue
...
def run_momentum_strategy(stock_list=None):
    ...
    for symbol in stock_list:
        ...
        out_path = os.path.join(
            csv_dir, f"{symbol.replace('.', '_')}_momentum_vs_stock.csv"
        )
        if not os.path.exists(csv_path):
            print(f"找不到 {csv_path}，請先下載數據")
            continue
