...
class StrategyDataService:
    ...
    async def _subscribe_to_strategy_updates(self):
        ...
        try:
            ...
            await self.message_queue.subscribe(
                "strategy_updates", self._handle_strategy_update
            )

            # 订阅策略性能更新消息
            await self.message_queue.subscribe(
                "strategy_performance", self._handle_strategy_performance_update
            )

            # 订阅回测结果消息
            await self.message_queue.subscribe(
                "backtest_results", self._handle_backtest_result
            )
    ...
    async def _get_agent_strategy_info(
    ...
    async def _get_default_strategy_info(
        self, agent_id: str, agent_type: str
    ) -> StrategyInfo:
        """获取默认策略信息"""
        return StrategyInfo(
        ...
        try:
            return StrategyInfo(
                ...
                strategy_type=StrategyType(
                    strategy_data.get("strategy_type", "momentum")
                ),
                ...
                created_at=datetime.fromisoformat(
                    strategy_data.get("created_at", datetime.utcnow().isoformat())
                ),
                last_updated=datetime.fromisoformat(
                    strategy_data.get("last_updated", datetime.utcnow().isoformat())
                ),
    ...
    async def _parse_live_metrics(
        ...
        try:
            return LiveMetrics(
                ...
                last_trade_time=(
                    datetime.fromisoformat(performance_data["last_trade_time"])
                    if performance_data.get("last_trade_time")
                    else None
                ),
                live_period_start=datetime.fromisoformat(
                    performance_data.get(
                        "live_period_start", datetime.utcnow().isoformat()
                    )
                ),
            )
        except Exception as e:
    ...
    async def _parse_backtest_metrics(
        ...
        try:
            return BacktestMetrics(
                ...
                backtest_period_start=datetime.fromisoformat(
                    backtest_data.get("backtest_period_start", "2023 - 01 - 01")
                ).date(),
                backtest_period_end=datetime.fromisoformat(
                    backtest_data.get("backtest_period_end", "2023 - 12 - 31")
                ).date(),
    ...
    async def _add_strategy_to_history(
        self, agent_id: str, strategy_info: StrategyInfo
    ):
        """添加策略到历史记录"""
        try:
            ...
            if len(self._strategy_history[agent_id]) > self.config.max_strategy_history:
                self._strategy_history[agent_id] = self._strategy_history[agent_id][
                    -self.config.max_strategy_history :
                ]
