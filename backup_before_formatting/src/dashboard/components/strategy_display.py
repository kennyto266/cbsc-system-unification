...
class StrategyDisplayComponent:
    ...
    def render_html(self, strategy_info: StrategyInfo) -> str:
        ...
        try:
            ...
            if self.config.show_parameters and strategy_info.parameters:
                strategy_parameters_html = self._generate_strategy_parameters_html(
            ...
            if self.config.show_backtest_results and strategy_info.backtest_metrics:
                backtest_results_html = self._generate_backtest_results_html(
                    strategy_info.backtest_metrics
                )

            # 生成实时表现
            live_performance_html = ""
            if self.config.show_live_performance and strategy_info.live_metrics:
                live_performance_html = self._generate_live_performance_html(
                    strategy_info.live_metrics
                )

            # 生成性能图表
            performance_charts_html = ""
            if self.config.show_performance_charts:
                performance_charts_html = self._generate_performance_charts_html(
                    strategy_info
                )
    ...
    def _generate_strategy_parameters_html(
        ...
        for param in display_parameters:
            ...
            <div class="parameter - item">
                ...
                {"<div class="parameter - description">{param.description}</div>' if param.description else ''}
            </div>
            """
