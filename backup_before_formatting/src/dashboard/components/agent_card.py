...
class AgentCardComponent:
    ...
    def render_html(self, agent_data: AgentDashboardData) -> str:
        ...
        try:
            ...
            if self.config.show_performance_metrics and agent_data.performance_metrics:
                performance_metrics_html = self._generate_performance_metrics_html(
            ...
            if self.config.show_resource_usage and agent_data.resource_usage:
                resource_usage_html = self._generate_resource_usage_html(
                    agent_data.resource_usage
                )

            # 生成策略信息
            strategy_info_html = ""
            if self.config.show_strategy_info and agent_data.current_strategy:
                strategy_info_html = self._generate_strategy_info_html(
                    agent_data.current_strategy
                )
            ...
            if self.config.enable_controls and self.agent_control_service:
                control_buttons_html = self._generate_control_buttons_html(
                    agent_data.agent_id, agent_data.status
                )
    ...
    def _generate_performance_metrics_html(
    ...
    def _generate_strategy_info_html(self, strategy: StrategyInfo) -> str:
        ...
        <div class="strategy - info">
            ...
            <div class="strategy - details">
                ...
                {"<div class="strategy - description">{strategy.description}</div>' if strategy.description else ''}
            </div>
        </div>
        """
    ...
    def _generate_detailed_info_html(self, agent_data: AgentDashboardData) -> str:
        ...
        <div class="detailed - info">
            ...
            <div class="detail - grid">
                ...
                {"<div class="detail - item"><span class="detail - label">最后错误:</span><span class="detail - value error - message">{agent_data.last_error}</span></div>' if agent_data.last_error else ''}
                <div class="detail - item">
    ...
    def _get_strategy_status_text(self, status) -> str:
        ...
        return status_texts.get(
            status.value if hasattr(status, "value") else status, "未知"
        )
    ...
    async def control_agent(self, agent_id: str, action: str) -> bool:
        ...
        try:
            ...
            if action == "start":
                ...
                await self.agent_control_service.restart_agent(
                    agent_id, "dashboard_user"
                )
            elif action == "pause":
                await self.agent_control_service.pause_agent(agent_id, "dashboard_user")
            elif action == "resume":
                await self.agent_control_service.resume_agent(
                    agent_id, "dashboard_user"
                )
            else:
