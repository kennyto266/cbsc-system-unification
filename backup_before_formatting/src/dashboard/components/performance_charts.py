...
class PerformanceChartsComponent:
    ...
    def __init__(self, config: PerformanceChartsConfig = None):
        ...
        self._update_callbacks: List[
            Callable[[str, List[PerformanceMetrics]], None]
        ] = []
    ...
    async def _check_alerts(self):
        ...
        try:
            ...
            for agent_id, history in self._performance_history.items():
                ...
                # 检查夏普比率告警
                if (
                    latest_performance.sharpe_ratio
                    < self.config.alert_thresholds["sharpe_ratio_min"]
                ):
                    new_alerts.append(
                        {
                ...
                # 检查最大回撤告警
                if (
                    latest_performance.max_drawdown
                    > self.config.alert_thresholds["max_drawdown_max"]
                ):
                    new_alerts.append(
                        {
                            "type": "drawdown_high",
                ...
                # 检查波动率告警
                if (
                    latest_performance.volatility
                    > self.config.alert_thresholds["volatility_max"]
                ):
                    new_alerts.append(
                        {
                ...
                if (
                    ...
                    < self.config.alert_thresholds["win_rate_min"]
                ):
                    new_alerts.append(
                        {
                            "type": "win_rate_low",
    ...
    def _render_single_agent_charts(self, agent_id: str) -> str:
        ...
        if self.config.show_sharpe_ratio_chart:
            charts_html.append(
                self._generate_sharpe_ratio_chart(agent_id, performance_history)
            )

        if self.config.show_return_chart:
            charts_html.append(
                self._generate_return_chart(agent_id, performance_history)
            )

        if self.config.show_drawdown_chart:
            charts_html.append(
                self._generate_drawdown_chart(agent_id, performance_history)
            )

        if self.config.show_risk_metrics_chart:
            charts_html.append(
                self._generate_risk_metrics_chart(agent_id, performance_history)
            )
    ...
    def _generate_sharpe_ratio_chart(
        self, agent_id: str, performance_history: List[PerformanceMetrics]
    ) -> str:
        """生成夏普比率图表"""
    ...
    def _generate_risk_metrics_chart(
        self, agent_id: str, performance_history: List[PerformanceMetrics]
    ) -> str:
        """生成风险指标图表"""
        chart_data = {
    ...
    def _generate_alerts_html(self, agent_id: str) -> str:
        """生成告警HTML"""
        agent_alerts = [
            alert for alert in self._alerts if alert.get("agent_id") == agent_id
        ]
    ...
    def _prepare_chart_data(
        self, performance_history: List[PerformanceMetrics], metric: str
    ) -> Dict[str, Any]:
        """准备图表数据"""
        labels = []
        values = []
    ...
    def add_update_callback(
        self, callback: Callable[[str, List[PerformanceMetrics]], None]
    ):
        """添加更新回调函数"""
