...
class AdapterConfigEntry(BaseModel):
    ...
    priority: int = Field(
        1, ge=1, le=10, description="优先级（1 - 10，数字越小优先级越高）"
    )
...
class DataAdapterConfigManager:
    ...
    def _load_default_configs(self) -> None:
        ...
        self.adapters["raw_data"] = AdapterConfigEntry(
            name="raw_data", enabled=True, priority=1, config=raw_data_config.dict()
        )
    ...
    async def load_config_from_file(self) -> bool:
        ...
        try:
            ...
            if not config_path.exists():
                self.logger.warning(
                    f"Config file not found: {config_path}, using default configs"
                )
                return True
            ...
            self.logger.info(
                f"Loaded {len(self.adapters)} adapter configurations from {config_path}"
            )
            return True
    ...
    async def save_config_to_file(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Saved {len(self.adapters)} adapter configurations to {config_path}"
            )
            return True
    ...
    def add_adapter_config(
        ...
        try:
            if name in self.adapters:
                self.logger.warning(
                    f"Adapter {name} already exists, updating configuration"
                )

            self.adapters[name] = AdapterConfigEntry(
                name=name, enabled=enabled, priority=priority, config=config
            )
            ...
            try:
                ...
                if entry.config.source_type == DataSourceType.RAW_DATA:
                    if not hasattr(entry.config, "data_directory"):
                        ...
                        errors.append(
                            f"Data directory does not exist: {entry.config.data_directory}"
                        )
                ...
                if (
                    entry.config.quality_threshold < 0.0
                    or entry.config.quality_threshold > 1.0
                ):
