...
class RiskMetrics(BaseModel):
    ...
    calculation_date: datetime = Field(
        default_factory=datetime.now, description="計算日期"
    )
...
class RiskCalculator:
    ...
    async def calculate_portfolio_risk(
        ...
        try:
            ...
            if len(returns) < 30:
                raise ValueError(
                    "Insufficient data for risk calculation (minimum 30 observations)"
                )
            ...
            sharpe_ratio = (
                excess_return / returns.std() * np.sqrt(252) if returns.std() > 0 else 0
            )
            ...
            if benchmark_returns is not None and len(benchmark_returns) == len(returns):
                ...
                information_ratio = (
                    active_return / tracking_error if tracking_error > 0 else 0
                )
    ...
    async def calculate_portfolio_var(
        ...
        try:
            ...
            portfolio_variance = np.dot(
                weights_aligned, np.dot(cov_aligned, weights_aligned)
            )
    ...
    async def calculate_marginal_var(
        ...
        try:
            ...
            for symbol in common_symbols:
                marginal_var[symbol] = (
                    z_score
                    * np.dot(cov_aligned[symbol], weights_aligned)
    ...
    async def calculate_component_var(
        ...
        """計算成分VaR"""
        try:
            marginal_var = await self.calculate_marginal_var(
                weights, covariance_matrix, confidence_level
            )
    ...
    async def calculate_stress_test(
        self, returns: pd.Series, stress_scenarios: Dict[str, float]
    ) -> Dict[str, Any]:
        """計算壓力測試"""
        try:
            stress_results = {}
    ...
    async def calculate_monte_carlo_var(
        ...
        try:
            ...
            simulated_returns = np.random.normal(
                mean_return, std_return, (num_simulations, time_horizon)
            )
    ...
    async def calculate_historical_var(
        self, returns: pd.Series, confidence_level: float = 0.95, time_horizon: int = 1
    ) -> Dict[str, Any]:
        """歷史模擬VaR計算"""
        try:
            # 滾動窗口計算
    ...
    async def calculate_risk_budget(
        self, weights: pd.Series, risk_limits: RiskLimits
    ) -> Dict[str, Any]:
        """計算風險預算"""
        try:
            risk_budget = {}
