...
class ConfigManager:
    ...
    def _merge_configs(self):
        ...
        for key, value in self.config.items():
            if key in self.merged_config:
                if isinstance(self.merged_config[key], dict) and isinstance(
                    value, dict
                ):
    ...
    def export_config(
        self, export_path: Union[str, Path], include_system: bool = False
    ):
        """導出配置到文件"""
        try:
    ...
    def get_config_summary(self) -> Dict[str, Any]:
        """獲取配置摘要信息"""
        return {
            "config_version": self.system_config.get("system", {}).get(
                "version", "1.0.0"
            ),
