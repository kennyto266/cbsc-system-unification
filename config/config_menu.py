...
class ConfigMenu:
    ...
    def _show_current_config(self, trader_instance):
        ...
        print(
            f"  用戶配置文件: {'✅ 存在' if summary['user_config_exists'] else '❌ 不存在'}"
        )
        print(
            f"  系統配置文件: {'✅ 存在' if summary['system_config_exists'] else '❌ 不存在'}"
        )
        ...
        print(
            f"  自動刷新: {'開啟' if self.config_manager.get('trading.auto_refresh') else '關閉'}"
        )
        ...
        print("-" * 30)
        telegram_enabled = self.config_manager.get(
            "notifications.enable_telegram", False
        )
        print(f"  Telegram通知: {'開啟' if telegram_enabled else '關閉'}")
        if telegram_enabled:
            print(
                f"  警報閾值: {self.config_manager.get('notifications.alert_threshold', 0.02):.1%}"
            )
    ...
    def _edit_default_symbol(self, trader_instance):
        ...
        new_symbol = trader_instance._get_user_input(
            f"默認股票代碼 (當前: {current})", None
        )
        ...
        if new_symbol and new_symbol.lower() not in ["q", "quit", "exit"]:
            if self.config_manager.set("trading.default_symbol", new_symbol.upper()):
                ...
                print(
                    f"{trader_instance._get_colored_text('✅ 默認股票已更新', 'green')}"
                )
            else:
    ...
    def _edit_default_duration(self, trader_instance):
        ...
        new_duration = trader_instance._get_user_input(
            f"默認時間範圍天數 (當前: {current})", None
        )
            ...
            try:
                ...
                if 1 <= duration <= 3650:
                    if self.config_manager.set("trading.default_duration", duration):
                        self.config_manager.save_config()
                        print(
                            f"{trader_instance._get_colored_text('✅ 默認時長已更新', 'green')}"
                        )
                    else:
                        print(
                            f"{trader_instance._get_colored_text('❌ 更新失敗', 'red')}"
                        )
                else:
                    print(
                        f"{trader_instance._get_colored_text('⚠️  時長必須在1 - 3650天之間', 'yellow')}"
                    )
            except ValueError:
                print(
                    f"{trader_instance._get_colored_text('⚠️  請輸入有效的數字', 'yellow')}"
                )
    ...
    def _edit_favorite_symbols(self, trader_instance):
        ...
        new_symbols = trader_instance._get_user_input(
            "輸入股票代碼列表 (用逗號分隔)", None
        )
        ...
        if new_symbols and new_symbols.lower() not in ["q", "quit", "exit"]:
            ...
            if symbols:
                if self.config_manager.set("trading.favorite_symbols", symbols):
                    self.config_manager.save_config()
                    print(
                        f"{trader_instance._get_colored_text('✅ 收藏股票已更新', 'green')}"
                    )
                else:
                    print(f"{trader_instance._get_colored_text('❌ 更新失敗', 'red')}")
            else:
                print(
                    f"{trader_instance._get_colored_text('⚠️  未輸入有效股票代碼', 'yellow')}"
                )
    ...
    def _edit_auto_refresh(self, trader_instance):
        ...
        enable = trader_instance._get_user_input(
            "是否啟用自動刷新? (y / n)", ["y", "n", "Y", "N"]
        )

        if enable.lower() == "y":
            if self.config_manager.set("trading.auto_refresh", True):
                # 詢問刷新間隔
                interval = trader_instance._get_user_input(
                    f"刷新間隔秒數 (當前: {current_interval})", None
                )
                try:
                    if interval:
                        ...
                        if 10 <= interval_int <= 3600:
                            self.config_manager.set(
                                "trading.refresh_interval", interval_int
                            )
                        else:
                            print(
                                f"{trader_instance._get_colored_text('⚠️  間隔範圍: 10 - 3600秒', 'yellow')}"
                            )
                except ValueError:
                    print(
                        f"{trader_instance._get_colored_text('⚠️  保持原間隔設置', 'yellow')}"
                    )

                self.config_manager.save_config()
                print(
                    f"{trader_instance._get_colored_text('✅ 自動刷新已啟用', 'green')}"
                )
            else:
                print(f"{trader_instance._get_colored_text('❌ 更新失敗', 'red')}")
        else:
            if self.config_manager.set("trading.auto_refresh", False):
                self.config_manager.save_config()
                print(
                    f"{trader_instance._get_colored_text('✅ 自動刷新已關閉', 'green')}"
                )
            else:
                print(f"{trader_instance._get_colored_text('❌ 更新失敗', 'red')}")
    ...
    def _edit_rsi_config(self, trader_instance):
        ...
        try:
            ...
            if 2 <= period <= 100 and 1 <= oversold < overbought <= 99:
                ...
                self.config_manager.save_config()
                print(
                    f"{trader_instance._get_colored_text('✅ RSI配置已更新', 'green')}"
                )
            else:
                print(
                    f"{trader_instance._get_colored_text('⚠️  參數範圍無效', 'yellow')}"
                )
        except ValueError:
    ...
    def _edit_macd_config(self, trader_instance):
        ...
        try:
            ...
            if (
                ...
                self.config_manager.save_config()
                print(
                    f"{trader_instance._get_colored_text('✅ MACD配置已更新', 'green')}"
                )
            else:
                print(
                    f"{trader_instance._get_colored_text('⚠️  參數範圍無效，快線必須小於慢線', 'yellow')}"
                )
        except ValueError:
            print(f"{trader_instance._get_colored_text('⚠️  請輸入有效數字', 'yellow')}")
    ...
    def _edit_ma_config(self, trader_instance):
        ...
        try:
            ...
            if 5 <= short <= 50 and 51 <= long <= 300 and short < long:
                ...
                self.config_manager.save_config()
                print(
                    f"{trader_instance._get_colored_text('✅ 移動平均線配置已更新', 'green')}"
                )
            else:
                print(
                    f"{trader_instance._get_colored_text('⚠️  參數範圍無效，短期必須小於長期', 'yellow')}"
                )
        except ValueError:
            print(f"{trader_instance._get_colored_text('⚠️  請輸入有效數字', 'yellow')}")
    ...
    def _edit_bollinger_config(self, trader_instance):
        ...
        try:
            ...
            if 5 <= period <= 50 and 1.0 <= std_dev <= 3.0:
                ...
                self.config_manager.save_config()
                print(
                    f"{trader_instance._get_colored_text('✅ 布林帶配置已更新', 'green')}"
                )
            else:
                print(
                    f"{trader_instance._get_colored_text('⚠️  參數範圍無效', 'yellow')}"
    ...
    def _edit_kdj_config(self, trader_instance):
        ...
        try:
            ...
            if 5 <= k_period <= 50 and 1 <= d_period <= 10 and 1 <= j_period <= 10:
                ...
                self.config_manager.save_config()
                print(
                    f"{trader_instance._get_colored_text('✅ KDJ配置已更新', 'green')}"
                )
            else:
                print(
                    f"{trader_instance._get_colored_text('⚠️  參數範圍無效', 'yellow')}"
    ...
    def _reset_config(self, trader_instance):
        ...
        if choice == "1":
            confirm = trader_instance._get_user_input(
                "確認重置交易配置? (y / n)", ["y", "n", "Y", "N"]
            )
            if confirm.lower() == "y":
                if self.config_manager.reset_to_default("trading"):
                    self.config_manager.save_config()
                    print(
                        f"{trader_instance._get_colored_text('✅ 交易配置已重置', 'green')}"
                    )
                else:
                    print(f"{trader_instance._get_colored_text('❌ 重置失敗', 'red')}")
        elif choice == "2":
            confirm = trader_instance._get_user_input(
                "確認重置指標配置? (y / n)", ["y", "n", "Y", "N"]
            )
            if confirm.lower() == "y":
                if self.config_manager.reset_to_default("indicators"):
                    self.config_manager.save_config()
                    print(
                        f"{trader_instance._get_colored_text('✅ 指標配置已重置', 'green')}"
                    )
                else:
                    print(f"{trader_instance._get_colored_text('❌ 重置失敗', 'red')}")
        elif choice == "3":
            confirm = trader_instance._get_user_input(
                "確認重置所有配置? (y / n)", ["y", "n", "Y", "N"]
            )
            if confirm.lower() == "y":
                if self.config_manager.reset_to_default():
                    self.config_manager.save_config()
                    print(
                        f"{trader_instance._get_colored_text('✅ 所有配置已重置', 'green')}"
                    )
                else:
                    print(f"{trader_instance._get_colored_text('❌ 重置失敗', 'red')}")
