...
class TestPhase2ExtendedIndicators(unittest.TestCase):
    ...
    def generate_test_data(cls):
        ...
        cls.hibor_data = {
            "ON": pd.Series(
                np.clip(np.random.normal(3.5, 0.3, n_points), 2.0, 5.0),
                index = pd.date_range("2020 - 01 - 01", periods = n_points, freq="D"),
            ),
            "1W": pd.Series(
                np.clip(np.random.normal(4.0, 0.4, n_points), 2.5, 6.0),
                index = pd.date_range("2020 - 01 - 01", periods = n_points, freq="D"),
            ),
            "1M": pd.Series(
                np.clip(np.random.normal(4.5, 0.5, n_points), 3.0, 7.0),
                index = pd.date_range("2020 - 01 - 01", periods = n_points, freq="D"),
            ),
            "3M": pd.Series(
                np.clip(np.random.normal(5.0, 0.6, n_points), 3.5, 8.0),
                index = pd.date_range("2020 - 01 - 01", periods = n_points, freq="D"),
            ),
        }

        # High / Low數據用於需要OHLC的指標
        cls.high_data = cls.price_data * (
            1 + np.abs(np.random.normal(0, 0.01, n_points))
        )
        cls.low_data = cls.price_data * (
            1 - np.abs(np.random.normal(0, 0.01, n_points))
        )
        ...
        logger.info(
            f"  Price data: {len(cls.price_data)} points, range: {cls.price_data.min():.2f}-{cls.price_data.max():.2f}"
        )
        logger.info(
            f"  Rate data: {len(cls.rate_data)} points, range: {cls.rate_data.min():.2f}-{cls.rate_data.max():.2f}"
        )
        logger.info(
            f"  Flow data: {len(cls.flow_data)} points, range: {cls.flow_data.min():.2f}-{cls.flow_data.max():.2f}"
        )
        logger.info(
            f"  Ratio data: {len(cls.ratio_data)} points, range: {cls.ratio_data.min():.2f}-{cls.ratio_data.max():.2f}"
        )
    ...
    def test_dema_calculation(self):
        ...
        for data_name, data in [("price", self.price_data), ("rate", self.rate_data)]:
            with self.subTest(data_type = data_name):
                result, adaptation_info = self.indicators.calculate_dema(
                    data, period = 21
                )
                ...
                self.assertLess(
                    adaptation_info["calculation_time_ms"], 1.0
                )  # < 1ms target

                # 性能記錄
                self.performance_results[f"DEMA_{data_name}"] = adaptation_info[
                    "calculation_time_ms"
                ]

                logger.info(
                    f"  {data_name} DEMA: {adaptation_info['calculation_time_ms']:.3f}ms"
                )
    ...
    def test_tema_calculation(self):
        ...
        for data_name, data in [("price", self.price_data), ("rate", self.rate_data)]:
            with self.subTest(data_type = data_name):
                result, adaptation_info = self.indicators.calculate_tema(
                    data, period = 15
                )
                ...
                self.performance_results[f"TEMA_{data_name}"] = adaptation_info[
                    "calculation_time_ms"
                ]
                logger.info(
                    f"  {data_name} TEMA: {adaptation_info['calculation_time_ms']:.3f}ms"
                )
    ...
    def test_trima_calculation(self):
        ...
        for data_name, data in [("price", self.price_data), ("rate", self.rate_data)]:
            with self.subTest(data_type = data_name):
                result, adaptation_info = self.indicators.calculate_trima(
                    data, period = 18
                )
                ...
                self.performance_results[f"TRIMA_{data_name}"] = adaptation_info[
                    "calculation_time_ms"
                ]
                logger.info(
                    f"  {data_name} TRIMA: {adaptation_info['calculation_time_ms']:.3f}ms"
                )
    ...
    def test_macd_extended_calculation(self):
        ...
        for method in methods:
            with self.subTest(method = method):
                ...
                self.performance_results[f"MACD_EXTENDED_{method}"] = result[
                    "adaptation_info"
                ]["calculation_time_ms"]
                logger.info(
                    f"  MACD {method}: {result['adaptation_info']['calculation_time_ms']:.3f}ms"
                )
    ...
    def test_stochastic_f_calculation(self):
        ...
        for data_name, data in [("price", self.price_data), ("rate", self.rate_data)]:
            with self.subTest(data_type = data_name):
                ...
                self.performance_results[f"STOCHASTIC_F_{data_name}"] = result[
                    "adaptation_info"
                ]["calculation_time_ms"]
                logger.info(
                    f"  {data_name} Stochastic F: {result['adaptation_info']['calculation_time_ms']:.3f}ms"
                )
    ...
    def test_williams_r_calculation(self):
        ...
        for data_name, data in [("price", self.price_data), ("rate", self.rate_data)]:
            with self.subTest(data_type = data_name):
                ...
                self.performance_results[f"WILLIAMS_R_{data_name}"] = adaptation_info[
                    "calculation_time_ms"
                ]
                logger.info(
                    f"  {data_name} Williams %R: {adaptation_info['calculation_time_ms']:.3f}ms"
                )
    ...
    def test_cci_calculation(self):
        ...
        for data_name, data in [("price", self.price_data), ("rate", self.rate_data)]:
            with self.subTest(data_type = data_name):
                ...
                self.performance_results[f"CCI_{data_name}"] = adaptation_info[
                    "calculation_time_ms"
                ]
                logger.info(
                    f"  {data_name} CCI: {adaptation_info['calculation_time_ms']:.3f}ms"
                )
    ...
    def test_rsi_extended_calculation(self):
        ...
        for data_name, data in [("price", self.price_data), ("rate", self.rate_data)]:
            with self.subTest(data_type = data_name):
                ...
                self.performance_results[f"RSI_EXTENDED_{data_name}"] = adaptation_info[
                    "calculation_time_ms"
                ]
                logger.info(
                    f"  {data_name} RSI Extended: {adaptation_info['calculation_time_ms']:.3f}ms"
                )
    ...
    def test_bollinger_bands_calculation(self):
        ...
        for data_name, data in [("price", self.price_data), ("rate", self.rate_data)]:
            with self.subTest(data_type = data_name):
                ...
                self.performance_results[f"BOLLINGER_BANDS_{data_name}"] = result[
                    "adaptation_info"
                ]["calculation_time_ms"]
                logger.info(
                    f"  {data_name} Bollinger Bands: {result['adaptation_info']['calculation_time_ms']:.3f}ms"
                )
    ...
    def test_keltner_channels_calculation(self):
        ...
        for data_name, data in [("price", self.price_data), ("rate", self.rate_data)]:
            with self.subTest(data_type = data_name):
                ...
                self.performance_results[f"KELTNER_CHANNELS_{data_name}"] = result[
                    "adaptation_info"
                ]["calculation_time_ms"]
                logger.info(
                    f"  {data_name} Keltner Channels: {result['adaptation_info']['calculation_time_ms']:.3f}ms"
                )
    ...
    def test_hibor_term_structure(self):
        ...
        self.performance_results["HIBOR_TERM_STRUCTURE"] = result["adaptation_info"][
            "calculation_time_ms"
        ]
        logger.info(
            f"  HIBOR Term Structure: {result['adaptation_info']['calculation_time_ms']:.3f}ms"
        )
    ...
    def test_rate_spread_analysis(self):
        ...
        self.performance_results["RATE_SPREAD_ANALYSIS"] = result["adaptation_info"][
            "calculation_time_ms"
        ]
        logger.info(
            f"  Rate Spread Analysis: {result['adaptation_info']['calculation_time_ms']:.3f}ms"
        )
    ...
    def test_parameter_adaptation(self):
        ...
        for data_type in data_types:
            with self.subTest(data_type = data_type):
                ...
                logger.info(
                    f"  {data_type}: {adapted_params.get('adaptation_reason', 'No reason')}"
                )
    ...
    def test_performance_benchmarks(self):
        ...
        performance_summary = {
            ...
            "tests_passing": sum(
                1 for r in all_performance_results if r["meets_target"]
            ),
            "average_calc_time": np.mean(
                [r["calculation_time_ms"] for r in all_performance_results]
            ),
            "max_calc_time": np.max(
                [r["calculation_time_ms"] for r in all_performance_results]
            ),
            "min_calc_time": np.min(
                [r["calculation_time_ms"] for r in all_performance_results]
            ),
            "pass_rate": (
                sum(1 for r in all_performance_results if r["meets_target"])
                / len(all_performance_results)
            )
            * 100,
        }
    ...
    def test_comprehensive_indicator_validation(self):
        ...
        # 驗證性能目標
        performance_targets_met = comprehensive_results["test_summary"][
        ...
        logger.info(
            f"  System Rating: {comprehensive_results['performance_analysis'].get('system_performance_rating', 'Unknown')}"
        )
    ...
    def save_test_results(cls):
        ...
        if cls.performance_results:
            performance_file = (
                output_dir / f"phase2_performance_benchmark_{timestamp}.json"
            )
        ...
        if cls.test_results:
            comprehensive_file = (
                output_dir / f"phase2_comprehensive_test_{timestamp}.json"
            )
