...
def test_standardized_sharpe_calculator():
    ...
    try:
        from backtest.standardized_sharpe_calculator import (
        ...
        for method in methods:
            ...
            print(
                f"  年化回報: {result['annual_return']:.4f} ({result['annual_return']*100:.2f}%)"
            )
            print(
                f"  年化波動: {result['annual_volatility']:.4f} ({result['annual_volatility']*100:.2f}%)"
            )
            print(
        ...
        consistency = (
            "EXCELLENT" if max_diff < 0.1 else "GOOD" if max_diff < 0.3 else "POOR"
        )
        ...
        print(
            f"與標準方法一致性: {abs(sharpe_convenient - results['standard']['sharpe_ratio']) < 0.0001}"
        )
        ...
        realistic_result = calculator.calculate_sharpe_ratio(
            realistic_returns, "standard"
        )

        print(f"現實市場數據結果:")
        print(
            f"  年化回報: {realistic_result['annual_return']:.4f} ({realistic_result['annual_return']*100:.2f}%)"
        )
        print(
            f"  年化波動: {realistic_result['annual_volatility']:.4f} ({realistic_result['annual_volatility']*100:.2f}%)"
        )
        ...
        data = pd.DataFrame(
            {
                ...
                "close": prices,
                "volume": [
                    np.random.randint(1000000, 5000000) for _ in range(len(prices))
                ],
            },
            index = dates,
        )
...
def generate_test_report():
    ...
    report = {
        ...
        "summary": {"total_tests": 2, "passed_tests": 0, "failed_tests": 0},
    }

    report["summary"]["passed_tests"] = sum(
        [report["sharpe_calculator_test"], report["vectorbt_integration_test"]]
    )
    ...
    print(
        f"通過測試: {report['summary']['passed_tests']}/{report['summary']['total_tests']}"
    )
