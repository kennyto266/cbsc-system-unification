...
from src.backtest.constraint_system import (
    ...
    SectorConstraint,
    WeightBoundConstraint,
)
from src.backtest.efficient_frontier import (
    EfficientFrontierCalculator,
    EfficientFrontierConfig,
)
from src.backtest.mpt_optimizer import MPTConfig, MPTOptimizer, create_mpt_optimizer
...
def generate_test_returns_data(
    num_assets: int = 5, num_days: int = 252
) -> pd.DataFrame:
    """生成測試回報率數據"""
...
def test_mpt_optimizer():
    ...
    logger.info(
        f"   Expected return: {max_sharpe_result.expected_return:.4f} ({max_sharpe_result.expected_return * 100:.2f}%)"
    )
    logger.info(
        f"   Volatility: {max_sharpe_result.volatility:.4f} ({max_sharpe_result.volatility * 100:.2f}%)"
    )
    ...
    logger.info(
        f"   Expected return: {min_vol_result.expected_return:.4f} ({min_vol_result.expected_return * 100:.2f}%)"
    )
    logger.info(
        f"   Volatility: {min_vol_result.volatility:.4f} ({min_vol_result.volatility * 100:.2f}%)"
    )
    ...
    logger.info(
        f"   Expected return: {risk_parity_result.expected_return:.4f} ({risk_parity_result.expected_return * 100:.2f}%)"
    )
    logger.info(
        f"   Volatility: {risk_parity_result.volatility:.4f} ({risk_parity_result.volatility * 100:.2f}%)"
    )
    ...
    logger.info(
        f"   Actual return: {target_result.expected_return:.4f} ({target_result.expected_return * 100:.2f}%)"
    )
    logger.info(f"   Optimization successful: {target_result.success}")
    logger.info(
        f"   Volatility: {target_result.volatility:.4f} ({target_result.volatility * 100:.2f}%)"
    )
...
def test_efficient_frontier():
    ...
    logger.info(
        f"   Max Sharpe - Return: {ef_result.max_sharpe_portfolio.expected_return:.4f}, "
        f"Vol: {ef_result.max_sharpe_portfolio.volatility:.4f}, "
        f"Sharpe: {ef_result.max_sharpe_portfolio.sharpe_ratio:.4f}"
    )

    logger.info(
        f"   Min Vol   - Return: {ef_result.min_volatility_portfolio.expected_return:.4f}, "
    ...
    target_vol_portfolios = calculator.calculate_target_risk_portfolios(
        returns, target_vols
    )

    for i, (target_vol, portfolio) in enumerate(
        zip(target_vols, target_vol_portfolios)
    ):
        logger.info(
            f"   Target {target_vol:.2f} - Actual: {portfolio.volatility:.4f}, "
            f"Return: {portfolio.expected_return:.4f}, Sharpe: {portfolio.sharpe_ratio:.4f}"
        )

    # Generate report
    logger.info(f"\n4. Generating Efficient Frontier Report...")
    calculator.generate_efficient_frontier_report(ef_result)
...
def test_constraint_system():
    ...
    config = ConstraintConfig(min_weight = 0.05, max_weight = 0.40, max_positions = 4)
    ...
    constraint_system.add_constraint(
        WeightBoundConstraint(min_weight = 0.05, max_weight = 0.40)
    )
    logger.info("   Added weight bound constraint")

    # Position limit
    constraint_system.add_constraint(
        PositionLimitConstraint(max_positions = 4, min_weight_threshold = 0.01)
    )
    ...
    constraint_system.add_constraint(
        SectorConstraint(sector_mapping = sector_mapping, sector_limits = sector_limits)
    )
    ...
    for result in results:
        if not result.is_satisfied:
            logger.info(
                f"   ✗ {result.constraint_name}: {result.current_value:.4f} > {result.limit_value:.4f}"
            )
        else:
    ...
    scipy_constraints, bounds = constraint_system.generate_scipy_constraints(
        asset_names
    )
    ...
    constraint_system.generate_constraint_report(invalid_weights, data)
    logger.info("   Report generated successfully")
...
def test_integration():
    ...
    constraint_config = ConstraintConfig(
        min_weight = 0.02, max_weight = 0.35, max_positions = 6, max_turnover = 0.8
    )
    ...
    # Add constraints
    constraint_system.add_constraint(
        WeightBoundConstraint(min_weight = 0.02, max_weight = 0.35)
    )
    constraint_system.add_constraint(
        PositionLimitConstraint(max_positions = 6, min_weight_threshold = 0.02)
    )

    # Mock sector mapping for 8 assets
    sector_mapping = {
        f"Asset_{i:02d}": ["Tech", "Finance", "Healthcare", "Energy"][i % 4]
        for i in range(8)
    }
    sector_limits = {
        "Tech": (0.0, 0.4),
    ...
    mpt_config = MPTConfig(risk_free_rate = 0.025, min_weight = 0.02, max_weight = 0.35)
        ...
        "sector_constraints": [
            ...
            },
            {
                "assets": [
                    i
                    for i, asset in enumerate(asset_names)
                    if sector_mapping[asset] == "Finance"
                ],
                "min_weight": 0.0,
                "max_weight": 0.3,
            },
    ...
    max_sharpe_constrained = optimizer.maximize_sharpe_ratio(
        returns, custom_constraints
    )
    logger.info(
        f"   Max Sharpe (constrained): Return={max_sharpe_constrained.expected_return:.4f}, "
    ...
    ef_config = EfficientFrontierConfig(
        num_portfolios = 150, show_optimal_portfolios = True, show_individual_assets = True
    )
    ...
    logger.info(
        f"   Efficient frontier: {ef_result.total_portfolios} portfolios, "
        f"{ef_result.efficient_portfolios_count} efficient"
    )
    logger.info(
        f"   Max Sharpe portfolio: Return={ef_result.max_sharpe_portfolio.expected_return:.4f}, "
        f"Vol={ef_result.max_sharpe_portfolio.volatility:.4f}"
    )
    ...
    is_valid, validation_results = constraint_system.validate_portfolio(
    ...
    if not is_valid:
        ...
        for result in validation_results:
            if not result.is_satisfied:
                logger.info(
                    f"     ✗ {result.constraint_name}: {result.current_value:.4f} vs {result.limit_value:.4f}"
                )
...
def main():
    ...
    try:
        ...
        logger.info(
            f"   - Applied {integration_results['constraint_count']} constraints"
        )
        logger.info(
            f"   - Optimal Sharpe ratio: {integration_results['max_sharpe_portfolio']['sharpe_ratio']:.4f}"
        )
        logger.info(
            f"   - Constraints satisfied: {integration_results['max_sharpe_portfolio']['constraints_satisfied']}"
        )
