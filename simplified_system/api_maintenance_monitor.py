...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("api_maintenance.log"), logging.StreamHandler()],
)
...
class APIMaintenanceMonitor:
        ...
        for endpoint_id, health_check in self.health_checks.items():
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        f"{health_check.endpoint_name}: 健康 ({response_time:.0f}ms)"
                    )

                else:
                    # HTTP錯誤
                    self._handle_api_failure(
                        health_check, f"HTTP {response.status_code}"
                    )
            ...
            if "records" in data or "data" in data:
                ...
                if len(records) > 0:
                    ...
                    if isinstance(sample_record, dict) and len(sample_record) > 0:
                        ...
                        # 檢查關鍵字段
                        if any(
                            key in sample_record
                            for key in ["date", "timestamp", "value", "rate"]
                        ):
                            score += 0.2
    ...
    def _handle_api_failure(self, health_check: APIHealthCheck, error_message: str):
        ...
        logger.warning(
            f"{health_check.endpoint_name}: 失敗 ({health_check.consecutive_failures}連續失敗) - {error_message}"
        )
    ...
    def _analyze_health_status(self):
        ...
        healthy_endpoints = sum(
            ...
            if check.status == APIStatus.HEALTHY
        )
        critical_endpoints = sum(
            1
            for check in self.health_checks.values()
            if check.status == APIStatus.CRITICAL
        )
        ...
        logger.info(
            f"健康狀態: {healthy_endpoints}/{total_endpoints} 端點健康, {critical_endpoints} 個嚴重"
        )
    ...
    def _check_for_alerts(self):
        ...
        for endpoint_id, health_check in self.health_checks.items():
            ...
            if health_check.status == APIStatus.CRITICAL:
                self._send_alert(
                    f"CRITICAL: {health_check.endpoint_name} API處於嚴重狀態",
                    health_check,
                )
            ...
            if (
                ...
                self._send_alert(
                    f"WARNING: {health_check.endpoint_name} API響應時間過長",
                    health_check,
                )

            # 檢查數據質量
            if (
                health_check.data_quality_score
                < self.monitoring_config["data_quality_threshold"]
            ):
                self._send_alert(
                    f"WARNING: {health_check.endpoint_name} API數據質量下降",
                    health_check,
                )
    ...
    def get_health_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "overall_status": {
                ...
                "warning_count": sum(
                    1
                    for check in self.health_checks.values()
                    if check.status == APIStatus.WARNING
                ),
                "critical_count": sum(
                    1
                    for check in self.health_checks.values()
        ...
        for endpoint_id, health_check in self.health_checks.items():
            report["endpoint_details"][endpoint_id] = {
                ...
                "data_quality_score": health_check.data_quality_score,
                "last_success": (
                    health_check.last_success.isoformat()
                    if health_check.last_success
                    else None
                ),
                "last_failure": (
                    health_check.last_failure.isoformat()
                    if health_check.last_failure
                    else None
                ),
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if avg_response_time > 3000:  # 3秒
            recommendations.append(
                f"平均響應時間過長({avg_response_time:.0f}ms)，建議優化網絡或增加超時時間"
            )

        # 分析失敗端點
        failing_endpoints = [
            endpoint_id
            for endpoint_id, check in self.health_checks.items()
            if check.consecutive_failures > 0
        ]

        if failing_endpoints:
            recommendations.append(
                f"以下端點存在連續失敗: {', '.join(failing_endpoints)}"
            )
        ...
        if low_quality_endpoints:
            recommendations.append(
                f"以下端點數據質量較低: {', '.join(low_quality_endpoints)}"
            )
    ...
    def _check_single_endpoint(self, endpoint_id: str):
        ...
        try:
            start_time = time.time()
            response = requests.get(
                health_check.url, params={"pagesize": 1, "page": 1}, timeout = 10
            )
            response_time = (time.time() - start_time) * 1000
    ...
    try:
        ...
        for endpoint_id, details in health_report["endpoint_details"].items():
            ...
            status_icon = (
                "[OK]"
                if status == "HEALTHY"
                else "[WARN]" if status == "WARNING" else "[CRITICAL]"
            )
        ...
        try:
            while True:
                ...
                print(
                    f"\r[{current_time}] 健康率: {overall['health_rate']:.1%} ({overall['healthy_count']}/{overall['total_endpoints']})",
                    end="",
                    flush = True,
                )
