...
def analyze_best_strategies(all_results):
    ...
    print("-" * 80)
    print(
        f"{'Rank':<4} {'Strategy Type':<20} {'Parameters':<25} {'Sharpe':<8} {'Max DD':<10} {'Return':<8}"
    )
    print("-" * 80)
...
def main():
    ...
    print(
        f"Best Sharpe: {top_strategies[0]['sharpe_ratio']:.3f}"
        if top_strategies
        else "N / A"
    )
    print(
        f"Max drawdown control: {top_strategies[0]['max_drawdown']:.1%}"
        if top_strategies
        else "N / A"
    )
