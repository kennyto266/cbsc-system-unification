...
class AntiOverfittingFramework:
    ...
    def _calculate_strategy_metrics(self, data: pd.DataFrame) -> StrategyMetrics:
        ...
        profit_factor = (
            winning_returns / losing_returns if losing_returns > 0 else float("inf")
        )
    ...
    def _walk_forward_analysis(self, data: pd.DataFrame) -> Dict:
        ...
        for i in range(n_windows):
            ...
            # 检查窗口是否成功
            if (
                test_metrics.trades_count >= self.config.min_trades_per_window
                and test_metrics.sharpe_ratio > 0
            ):
        ...
        if len(results["window_results"]) > 0:
            first_performance = results["window_results"][0]["test_metrics"][
                "sharpe_ratio"
            ]
            last_performance = results["window_results"][-1]["test_metrics"][
                "sharpe_ratio"
            ]
        ...
        print(
            f"SUCCESS Walk - Forward Analysis: {successful_windows}/{n_windows} windows successful"
        )
        return results
        ...
        if walk_forward_results["window_results"]:
            ...
            if len(sharpe_scores) > 1:
                tests["walk_forward_stability"] = {
                    ...
                    "coefficient_of_variation": np.std(sharpe_scores)
                    / np.mean(sharpe_scores),
                    "stability_score": 1
                    - (np.std(sharpe_scores) / np.mean(sharpe_scores)),
                }

        # 5. 过拟合检测指标
        tests["overfitting_indicators"] = {
            "trade_frequency": walk_forward_results["trade_frequency"],
            "successful_window_ratio": walk_forward_results["successful_windows"]
            / len(walk_forward_results["window_results"]),
    ...
    def _assess_overfitting_risk(
        ...
        if in_sample.sharpe_ratio > 0:
            sharpe_degradation = (
                in_sample.sharpe_ratio - out_sample.sharpe_ratio
        ...
        # 2. Walk - Forward成功率
        successful_ratio = walk_forward["successful_windows"] / len(
            walk_forward["window_results"]
        )
    ...
    def _calculate_validation_score(
        ...
        # Walk - Forward成功率加分
        success_ratio = walk_forward["successful_windows"] / len(
            walk_forward["window_results"]
        )
        ...
        if "walk_forward_stability" in statistical_tests:
            stat_stability = statistical_tests["walk_forward_stability"][
                "stability_score"
            ]
    ...
    def generate_validation_report(self, results: ValidationResults) -> str:
        ...
        if "overfitting_indicators" in results.statistical_tests:
            ...
            report += (
                f"  Performance Degradation: {oi['performance_degradation']:.2%}\n"
            )
...
def main():
    ...
    accepted_count = sum(
        1 for r in results.values() if r["validation_result"].is_accepted
    )
    print(
        f"Strategies accepted: {accepted_count}/{len(results)} ({accepted_count / len(results):.1%})"
    )
...
def generate_strategy_data(dates, quality="good"):
    ...
    df = pd.DataFrame(
        {"close": prices, "volume": np.random.randint(1000000, 5000000, n_days)},
        index = dates,
    )
