...
class ExportSystemTester:
    ...
    def run_all_tests(self):
        ...
        # 顯示失敗的測試
        failed_tests = [
            (name, error) for name, success, error in self.test_results if not success
        ]
        if failed_tests:
            print(f"\n❌ 失敗的測試:")
    ...
    def test_json_export(self):
        ...
        request = ExportRequest(
            data = test_data, format="json", filename="test_json_export"
        )
    ...
    def test_excel_export(self):
        ...
        request = ExportRequest(
            data = test_data, format="xlsx", filename="test_excel_export"
        )
    ...
    def test_csv_export(self):
        ...
        request = ExportRequest(
            data = test_data, format="csv", filename="test_csv_export"
        )
    ...
    def test_html_export(self):
        ...
        request = ExportRequest(
            data = test_data, format="html", filename="test_html_export"
        )
    ...
    def test_pdf_export(self):
        ...
        request = ExportRequest(
            data = test_data, format="pdf", filename="test_pdf_export"
        )
    ...
    def test_backtest_results_export(self):
        ...
        result = self.export_manager.export_backtest_results(
            backtest_data, format="xlsx", filename="test_backtest_results"
        )
    ...
    def test_batch_export(self):
        ...
        requests = []
        for i in range(3):
            test_data = {"batch_id": i, "data": list(range(10 + i))}

            request = ExportRequest(
                data = test_data, format="json", filename = f"test_batch_{i}"
            )
    ...
    def test_error_handling(self):
        ...
        # 測試無效格式
        request = ExportRequest(
            data={"test": "data"}, format="invalid_format", filename="test_error"
        )
    ...
    def save_test_report(self):
        ...
        report = {
            ...
            "failed_tests": sum(
                1 for _, success, _ in self.test_results if not success
            ),
            "success_rate": sum(1 for _, success, _ in self.test_results if success)
            / len(self.test_results)
            * 100,
            "results": [
                {"test_name": name, "success": success, "error": error}
