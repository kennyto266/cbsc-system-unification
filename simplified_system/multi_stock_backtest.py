    ...
    def run_multi_stock_analysis():
        ...
        for symbol in symbols:
            ...
            try:
                ...
                if data is not None and not data.empty:
                    ...
                    print(
                        f"價格範圍: ${data['low'].min():.2f} - ${data['high'].max():.2f}"
                    )
                    ...
                    strategies = [
                        (
                            "RSI_MEAN_REVERSION",
                            {"period": 18, "oversold": 25, "overbought": 70},
                        ),
                        (
                            "DUAL_MOVING_AVERAGE",
                            {"short_period": 10, "long_period": 70},
                        ),
                    ...
                    for strategy_name, params in strategies:
                        try:
                            ...
                            if result:
                                ...
                                print(
                                    f"  {strategy_name}: 回報 {result.get('total_return', 0):.1%}, Sharpe {result.get('sharpe_ratio', 0):.3f}"
                                )
                            else:
                    ...
                    if symbol_results:
                        best_strategy = max(
                            symbol_results, key = lambda x: x["sharpe_ratio"]
                        )
                        results[symbol] = {
                        ...
                        print(
                            f"  [OK] 最佳策略: {best_strategy['strategy']} (Sharpe: {best_strategy['sharpe_ratio']:.3f})"
                        )
                    else:
        ...
        if valid_results:
            ...
            sorted_stocks = sorted(
                valid_results.items(),
                key = lambda x: x[1]["best_strategy"]["sharpe_ratio"],
                reverse = True,
            )
            ...
            for i, (symbol, data) in enumerate(sorted_stocks, 1):
                ...
                print(
                    f"{i}. {symbol}: Sharpe {best['sharpe_ratio']:.3f}, 回報 {best['total_return']:.1%}, {best['strategy']}"
                )
            ...
            for symbol, data in valid_results.items():
                ...
                strategy_counts[strategy_name] = (
                    strategy_counts.get(strategy_name, 0) + 1
                )
