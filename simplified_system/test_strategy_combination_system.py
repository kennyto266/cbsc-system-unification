...
def test_correlation_analyzer():
    ...
    try:
        ...
        for method, result in correlation_results.items():
            ...
            print(
                f"平均相关性: {result.correlation_matrix.values[np.triu_indices_from(result.correlation_matrix.values, k = 1)].mean():.4f}"
            )
            print(
                f"显著相关性数量: {result.significant_correlations.notna().sum().sum()}"
            )
        ...
        print(
            f"平均相关性波动率: {rolling_result.correlation_volatility.values[np.triu_indices_from(rolling_result.correlation_volatility.values, k = 1)].mean():.4f}"
        )
        ...
        print(
            f"前两个主成分方差解释: {pca_result.explained_variance_ratio[:2].sum():.4f}"
        )
...
def test_integration_with_existing_components():
    ...
    try:
        ...
        signals = expanded_strategies.generate_signals(
            price_data, test_strategy, test_params
        )
        ...
        try:
            ...
            mo_config = MultiObjectiveConfig(
                algorithm="weighted_sum", population_size = 50, n_generations = 20
            )
            ...
            if len(asset_returns.columns) <= 8:  # 限制资产数量
                ...
                pareto_result = mo_optimizer.optimize_portfolio(
                    asset_returns, objectives
                )
...
def test_risk_management_features():
    ...
    try:
        ...
        print(f"\n风险管理约束验证:")
        print(
            f"最大回撤限制 (0.15): {best_combination.max_drawdown:.4f} {'✅' if best_combination.max_drawdown <= 0.15 else '❌'}"
        )
        print(
            f"最大波动率限制 (0.20): {best_combination.volatility:.4f} {'✅' if best_combination.volatility <= 0.20 else '❌'}"
        )
        ...
        print(f"\n权重约束验证:")
        print(
            f"最大权重 (0.50): {max_weight:.4f} {'✅' if max_weight <= 0.50 else '❌'}"
        )
        print(
            f"最小权重 (0.05): {min_weight:.4f} {'✅' if min_weight >= 0.05 or min_weight == 0 else '❌'}"
        )
        print(
            f"权重和: {np.sum(best_combination.weights):.6f} {'✅' if abs(np.sum(best_combination.weights) - 1.0) < 1e - 6 else '❌'}"
        )
...
def test_cost_analysis_features():
    ...
    try:
        ...
        print(
            f"成本占回报比: {best_combination.total_costs / abs(best_combination.expected_return) * 100:.2f}%"
        )

        # 成本效率分析
        cost_adjusted_sharpe = (
            best_combination.sharpe_ratio - best_combination.total_costs * 100
        )
        print(f"\n成本效率分析:")
