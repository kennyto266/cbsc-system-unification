...
# -*- coding: utf - 8 -*-
"""
0700.HK Professional Backtest System - English Version
                ...
                try:
                    ...
                    if result:
                        ...
                        if sharpe > 1.0:
                            ...
                            print(
                                f"  [GOOD] RSI({period},{oversold},{overbought}): "
                                f"SR={sharpe:.3f}, MDD={max_dd:.1%}, Return={result.total_return:.1%}"
                            )
...
def analyze_comprehensive_results(all_results):
    ...
    print("-" * 100)
    print(
        f"{'Rank':<4} {'Strategy Type':<25} {'Parameters':<20} {'Sharpe':<8} {'Max DD':<10} {'Return':<8}"
    )
    print("-" * 100)
    ...
    for i, res in enumerate(combined_results[:10]):
        ...
        print(
            f"{i + 1:<4} {strategy_name:<25} {params:<20} "
...
def save_comprehensive_results(data, all_results, top_strategies):
    ...
    # Save JSON report
    json_filename = f"professional_0700_backtest_results_{timestamp}.json"
    ...
    if top_strategies:
        ...
        csv_filename = f"professional_0700_top_strategies_{timestamp}.csv"
...
def main():
    ...
    print(
        f"Best Sharpe Ratio: {top_strategies[0]['sharpe_ratio']:.3f}"
        if top_strategies
        else "N / A"
    )
    print(
        f"Best Max Drawdown: {top_strategies[0]['max_drawdown']:.1%}"
        if top_strategies
        else "N / A"
    )
