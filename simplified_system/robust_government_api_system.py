...
class RobustGovernmentAPISystem:
    ...
    def _try_alternative_endpoints(
        ...
        for i, alt_url in enumerate(alternatives):
            ...
            try:
                ...
                if parsed_data and len(parsed_data.get("data", [])) > 0:
                    logger.info(
                        f"備選端點 {i + 1} 成功: {len(parsed_data['data'])} 條記錄"
                    )
    ...
    def _parse_response(
        self, response_data: Dict, data_type: str
    ) -> Optional[Dict[str, Any]]:
        """解析API響應數據"""
        try:
        ...
        for record in records:
            try:
                ...
                hibor_record = {
                    ...
                    "hibor_2_months": self._safe_float(
                        record.get("hibor_2_months") or record.get("ir_2m")
                    ),
    ...
    def _parse_exchange_data(self, data: Dict) -> Optional[Dict[str, Any]]:
        ...
        for record in records:
            try:
                ...
                exchange_record = {
                    "date": date_field,
                    "usd_hkd": self._safe_float(
                        record.get("usd_hkd") or record.get("usd_hkd_mid")
                    ),
            ...
            if data_type == "hibor_rates":
                ...
                for i in range(min(days_back, 30)):
                    ...
                    record = {
                        ...
                        "hibor_3_months": round(
                            base_rate + 0.6 + (i % 4 - 2) * 0.01, 4
                        ),
    ...
    def _record_api_success(self, data_type: str, endpoint_type: str, url: str):
        ...
        if len(self.api_status_history[data_type]) > 100:
            self.api_status_history[data_type] = self.api_status_history[data_type][
                -50:
            ]
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        for data_type, history in self.api_status_history.items():
            ...
            recent_failures = sum(
                1 for record in history[-10:] if record["status"] == "FAILURE"
            )
            ...
            if all(
                "fallback" in source.lower() or "emergency" in source.lower()
    ...
    def start_scheduled_maintenance(self):
        ...
        def maintenance_job():
            ...
            success_rate = (
                ...
                / test_results["summary"]["total_endpoints"]
            )
