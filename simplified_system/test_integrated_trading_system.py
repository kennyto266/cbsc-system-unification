...
try:
    ...
    if signal:
        ...
        if trade_result:
            ...
            print(
                "[INFO] No trade executed (signal confidence too low or other constraints)"
            )
    ...
    if trader.positions:
        ...
        for symbol, position in trader.positions.items():
            ...
            print(
                f"    PnL %: {((position.current_price - position.entry_price) / position.entry_price) * 100:.1f}%"
            )
            print(
                f"    Holding Period: {(datetime.now() - position.entry_date).days} days"
            )
    else:
    ...
    if extension_result.get("success"):
        ...
        print(
            f"[OK] Data extension successful: {len(test_data)} -> {len(extended_data)} records"
        )
    ...
    integrity_checks = {
        ...
        "position_quantity_valid": all(
            p.quantity > 0 for p in trader.positions.values()
        ),
    ...
    assessment = {
        "trading_system_status": (
            "OPERATIONAL" if all(integrity_checks.values()) else "WARNING"
        ),
        "performance_grade": self._calculate_performance_grade(metrics),
        "signal_quality": (
            "GOOD"
            if len(test_signals) > 0 and all(s.confidence > 0.5 for s in test_signals)
            else "NEEDS_IMPROVEMENT"
        ),
        "risk_management": (
            "ADEQUATE" if trader.config["risk_limit"] <= 0.05 else "HIGH_RISK"
        ),
    ...
    def _generate_recommendations(
        self, metrics: Dict, cycle_results: Dict
    ) -> List[str]:
        """生成建議"""
        recommendations = []
