    ...
    if result_file:
        ...
        try:
            ...
            if "successful_combinations" in results:
                ...
                print(
                    f"Successful: {results['successful_combinations']} ({success_rate:.1f}%)"
                )

            if "combinations_per_second" in results:
                print(
                    f"Performance: {results['combinations_per_second']:.1f} combos / sec"
                )
            ...
            if "analysis" in results:
                ...
                if "top_10_strategies" in analysis:
                    ...
                    print("-" * 80)
                    print(
                        f"{'Rank':<5} {'Strategy ID':<20} {'Sharpe':<8} {'Return':<8} {'MDD':<8} {'Win%':<6}"
                    )
                    print("-" * 80)

                    for i, strategy in enumerate(top_strategies, 1):
                        strategy_id = strategy.get("combination_id", f"Strategy_{i}")[
                            :18
                        ]
                        ...
                        print(
                            f"{i:<5} {strategy_id:<20} {sharpe:<8.3f} {return_pct:<8.1%} {mdd:<8.1%} {win_rate:<6.1%}"
                        )
                ...
                if "strategy_distribution" in analysis:
                    ...
                    if "sharpe_distribution" in dist:
                        ...
                        print(
                            f"Strategies with Sharpe > 2.0: {sharpe_dist.get('sharpe_gt_2', 0)}"
                        )
                        print(
                            f"Strategies with Sharpe > 1.5: {sharpe_dist.get('sharpe_gt_1_5', 0)}"
                        )
                        print(
                            f"Strategies with Sharpe > 1.0: {sharpe_dist.get('sharpe_gt_1', 0)}"
                        )
