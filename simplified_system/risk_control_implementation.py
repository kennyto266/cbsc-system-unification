...
class RiskControlParameters:
    ...
    position_sizing_method: str = (
        "volatility_based"  # volatility_based, kelly_criterion, fixed_fractional
    )
...
class RiskControlImplementation:
    ...
    def load_optimization_results(self) -> Dict:
        ...
        try:
            ...
            result_files = glob.glob(
                "simplified_system / risk_adjusted_optimization_*.json"
            )
            if not result_files:
    ...
    def _load_mock_results(self) -> Dict:
        """載入模擬優化結果"""
        return {
            "optimization_metadata": {"total_strategies": 144, "valid_strategies": 12},
            "results": [
                {
    ...
    def calculate_portfolio_risk_metrics(self, returns: pd.Series) -> RiskMetrics:
        ...
        try:
            ...
            current_drawdown = (
                cumulative.iloc[-1] - running_max.iloc[-1]
            ) / running_max.iloc[-1]
            ...
            consecutive_losses = loss_series.groupby(
                (loss_series != loss_series.shift()).cumsum()
            ).cumsum()
            max_consecutive_losses = consecutive_losses.max()

            # 組合熱度 (基於波動率調整的頭寸)
            portfolio_heat = (
                abs(returns).mean() / current_volatility
    ...
    def calculate_optimal_position_size(
        ...
        try:
            if self.risk_params.position_sizing_method == "volatility_based":
                # 波動率基礎頭寸
                abs(strategy_result["walk_forward_return"])
                ...
                position_size = min(
                    kelly_fraction * 0.5, self.risk_params.max_position_size
                )
        ...
        try:
            ...
            if strategy_result["validation_passed"]:
                ...
                rsi_signal = self._calculate_rsi_signal(
                    market_data, strategy_result["params"]
                )
    ...
    def implement_risk_controls(self) -> Dict:
        ...
        # 選擇最佳策略
        valid_strategies = [
            r for r in optimization_data["results"] if r.get("validation_passed", False)
        ]
        ...
        best_strategy = max(
            valid_strategies, key = lambda x: x.get("risk_adjusted_score", 0)
        )
        ...
        trading_signals = self.generate_trading_signals(
            best_strategy, market_data, current_risk_metrics
        )
    ...
    def _generate_sample_market_data(self) -> pd.DataFrame:
        ...
        market_data = pd.DataFrame(
            {"date": dates[: len(prices)], "close": prices}
        ).set_index("date")
    ...
    def generate_implementation_report(self, results: Dict) -> str:
        ...
        if results["implementation_status"] == "success":
            ...
            report += (
                f"Max Consecutive Losses: {risk_metrics['max_consecutive_losses']}\n\n"
            )
            ...
            if signals.get("risk_adjustments"):
                report += (
                    f"Risk Adjustments: {', '.join(signals['risk_adjustments'])}\n"
                )
            ...
            report += (
                f"Walk - Forward Drawdown: {expected['walk_forward_drawdown']:.2%}\n"
            )
