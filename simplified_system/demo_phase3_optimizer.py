...
def demo_phase3_parameter_space():
    ...
    try:
        ...
        for strategy_name in ["RSI_MEAN_REVERSION", "MACD_CROSSOVER"]:
            ...
            generated_combinations = space.generate_parameter_combinations(
            ...
            print(
                f"     篩選效率: {len(generated_combinations)/total_combinations:.1%}"
            )
...
def demo_phase3_parallel_optimization():
    ...
    try:
        ...
        print(
            f"   數據期間: {test_data.index[0].strftime('%Y-%m-%d')} 到 {test_data.index[-1].strftime('%Y-%m-%d')}"
        )
...
def demo_phase3_performance_evaluation():
    ...
    try:
        ...
        if "performance_report" in report:
            ...
            print(
                f"   綜合評分: {perf_report.get('multi_objective_score', {}).get('total_score', 0):.1f}"
            )
            print(
                f"   策略評級: {perf_report.get('multi_objective_score', {}).get('grade', 'N / A')}"
            )
...
def demo_integrated_system():
    ...
    try:
        ...
        print(
            f"   成功組合數: {optimization_summary.get('successful_combinations', 0):,}"
        )
        ...
        if system_performance:
            print(f"\n💻 系統性能:")
            print(
                f"   並行效率: {system_performance.get('parallel_efficiency', 0):.1f}%"
            )
            print(
                f"   處理速度: {system_performance.get('tasks_per_second', 0):.1f} 策略 / 秒"
            )
...
def main():
    ...
    for demo_name, demo_func in demos:
        ...
        try:
            ...
            demo_results[demo_name] = {"success": success, "execution_time": demo_time}
