...
class MassiveOptimizerTest:
    ...
    def test_parameter_space(self):
        ...
        all_combinations = space_manager.get_all_combinations(
            test_strategies, max_combinations_per_strategy = 100
        )
        ...
        filtered_combinations = rsi_space.generate_parameter_combinations(
            smart_filter = True
        )
    ...
    def test_parallel_optimization(self):
        ...
        logger.info(
            f"✅ 最佳Sharpe: {result['best_performance'].get('sharpe_ratio', 0):.3f}"
        )
    ...
    def test_performance_evaluation(self):
        ...
        assert (
            0 <= overfitting_metrics.overfitting_risk_score <= 100
        ), "過擬合風險評分應該在0 - 100之間"

        logger.info(
            f"✅ 過擬合風險評分: {overfitting_metrics.overfitting_risk_score:.1f}"
        )
    ...
    def test_single_strategy_optimization(self):
        ...
        assert (
            result["optimization_summary"]["successful_combinations"] > 0
        ), "應該有成功組合"
        assert (
            result["best_strategy"]["performance_metrics"] is not None
        ), "應該有性能指標"
        ...
        logger.info(
            f"✅ 最佳Sharpe: {best_strategy['performance_metrics']['sharpe_ratio']:.3f}"
        )
    ...
    def test_multi_strategy_optimization(self):
        ...
        assert len(result["individual_results"]) == len(
            strategies
        ), "應該返回所有策略結果"
    ...
    def test_million_parameter_challenge(self):
        ...
        logger.info(
            f"✅ 處理速度: {challenge_summary['combinations_per_second']:.1f} 組合 / 秒"
        )
    ...
    def test_performance_benchmarks(self):
        ...
        logger.info(
            f"✅ 回測性能: {result['successful_combinations']} 策略耗時 {backtest_time:.3f}秒"
        )
    ...
    def generate_test_report(self):
        ...
        if performance_metrics:
            print(f"\n⚡ 性能指標:")
            print(
                f"   策略處理速度: {performance_metrics.get('strategies_per_second', 0):.1f} 策略 / 秒"
            )
            print(
                f"   平均每策略時間: {performance_metrics.get('avg_time_per_strategy', 0):.3f}秒"
            )
        ...
        for test_name, result in self.test_results.items():
            ...
            print(
                f"   {status_emoji} {test_name}: {result['status']} ({result['execution_time']:.2f}秒)"
            )
