...
def calculate_alpha_factors(data):
    ...
    try:
        ...
        print(
            f"SUCCESS: Converted {len(technical_factors.columns)} technical indicators"
        )
...
def generate_optimization_recommendations(data, factor_analysis, multifactor_model):
    ...
    try:
        ...
        if factor_analysis is not None and not factor_analysis.empty:
            top_factors = factor_analysis.sort_values(
        ...
        recommendations["strategic_recommendations"] = generate_strategic_advice(
            recommendations
        )
...
def generate_strategic_advice(recommendations):
    ...
    if "top_factors" in recommendations and recommendations["top_factors"]:
        ...
        if best_factor["sharpe_ratio"] > 1.0:
            advice.append(
                f"STRONG: {best_factor['name']} shows excellent predictive power (Sharpe: {best_factor['sharpe_ratio']:.3f})"
            )
        elif best_factor["sharpe_ratio"] > 0.5:
            advice.append(
                f"MODERATE: {best_factor['name']} shows good predictive power (Sharpe: {best_factor['sharpe_ratio']:.3f})"
            )
    ...
    if "multifactor_model" in recommendations:
        ...
        if r_squared > 0.05:
            advice.append(
                f"Model explains {r_squared:.1%} of return variance - good predictive power"
            )
        elif r_squared > 0.02:
            advice.append(
                f"Model explains {r_squared:.1%} of return variance - moderate predictive power"
            )
...
def display_recommendations(recommendations):
    ...
    print(
        f"  Price Range: ${recommendations['price_range']['min']:.1f} - ${recommendations['price_range']['max']:.1f} HKD"
    )
    ...
    if "top_factors" in recommendations:
        ...
        for i, factor in enumerate(recommendations["top_factors"], 1):
            ...
            print(
                f"     IC: {factor['ic_mean']:.4f}, Sharpe: {factor['sharpe_ratio']:.3f}, Hit Rate: {factor['hit_rate']:.1%}"
            )
    ...
    if "multifactor_model" in recommendations:
        ...
        print(
            f"  Selected Factors: {len(recommendations['multifactor_model']['selected_factors'])}"
        )
...
def main():
    ...
    recommendations = generate_optimization_recommendations(
        data, validation_report, multifactor_model
    )
