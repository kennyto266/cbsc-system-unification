    ...
    if data_dir.exists():
        ...
        if complete_files:
            ...
            try:
                ...
                print(
                    f"  Success Rate: {collection_info.get('successful_sources', 0)}/{collection_info.get('total_sources', 0)}"
                )
                print(
                    f"  Success Rate %: {collection_info.get('success_rate', 0):.1f}%"
                )
...
def generate_trading_report(technical_analysis, government_data):
    ...
    if technical_analysis:
        ...
        print(
            f"  Price vs SMA20: {'Above' if technical_analysis['current_price'] > technical_analysis['sma_20'] else 'Below'}"
        )
        ...
        buy_signals = len(
            [
                s
                for s in technical_analysis["signals"]
                if "Buy" in s or "Potential Buy" in s
            ]
        )
        sell_signals = len(
            [
                s
                for s in technical_analysis["signals"]
                if "Sell" in s or "Potential Sell" in s
            ]
        )
    ...
    if government_data:
        ...
        if "success_rate" in government_data:
            print(f"\nSystem Integration:")
            print(
                f"  Data Sources: {government_data.get('successful_sources', 'N / A')}/{government_data.get('total_sources', 'N / A')}"
            )
            print(f"  Success Rate: {government_data.get('success_rate', 'N / A'):.1f}%")
            print(
                f"  Status: {'EXCEPTIONAL' if government_data.get('success_rate', 0) == 100 else 'GOOD'}"
            )
    ...
    return {
        ...
        "data_sources": (
            government_data.get("success_rate", 0) if government_data else 0
        ),
    }
