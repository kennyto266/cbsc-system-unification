...
class SystemIntegrationTester:
    ...
    def test_api_data_collection(self):
        ...
        try:
            ...
            avg_collection_time = np.mean(
                [r.get("collection_time_ms", 0) for r in results.values()]
            )
            ...
            if total_records > 30000:  # 超過30k記錄
                self.test_results["key_achievements"][
    ...
    def test_data_alignment(self):
        ...
        try:
            ...
            # 檢查對齊報告
            alignment_report_files = list(
                aligned_data_dir.glob("alignment_report_*.json")
            )
            ...
            alignment_test_result = {
                ...
                "missing_data_filled": summary.get("total_records_after", 0)
                - summary.get("total_records_before", 0),
            }
    ...
    def test_intelligent_indicator_selection(self):
        ...
        try:
            ...
            recommendation_files = list(
                recommendations_dir.glob("*_indicator_recommendations_*.json")
            )
            ...
            for rec_file in recommendation_files:
                ...
                source_analyses[source_name] = {
                    "total_indicators_evaluated": summary.get(
                        "total_indicators_evaluated", 0
                    ),
            ...
            # 計算整體指標
            high_suitability_rate = (
                total_highly_suitable / total_indicators_evaluated
                if total_indicators_evaluated > 0
                else 0
            )
            ...
            indicator_test_result = {
                ...
                "best_performers": [
                    {
                        "source": source,
                        "indicator": analysis["best_indicator"],
            ...
            self.test_results["system_status"][
                "intelligent_indicator_selection"
            ] = indicator_test_result
    ...
    def test_enhanced_core_indicators(self):
        ...
        try:
            ...
            indicator_files = list(
                enhanced_indicators_dir.glob("enhanced_indicators_*.json")
            )
            ...
            self.test_results["system_status"][
                "enhanced_core_indicators"
            ] = indicators_test_result

            # 添加關鍵成就
            if total_indicators_calculated > 200:
                self.test_results["key_achievements"][
                    "comprehensive_indicator_support"
    ...
    def generate_integration_summary(self):
        ...
        passed_tests = sum(
            1 for status in system_statuses.values() if status.get("status") == "PASS"
        )
        ...
        if "api_data_collection" in system_statuses:
            ...
            performance_metrics["data_collection"] = {
                "records_per_second": api_result.get("total_records_collected", 0)
                / (api_result.get("avg_collection_time_ms", 1) / 1000),
        ...
        # 核心問題解决驗證
        core_issues_solved = {
            "data_time_mismatch": self.test_results["system_status"]
            .get("data_alignment", {})
            .get("status")
            == "PASS",
            "indicator_applicability": self.test_results["system_status"]
            .get("intelligent_indicator_selection", {})
            .get("status")
            == "PASS",
            "api_optimization": self.test_results["system_status"]
            .get("api_data_collection", {})
            .get("status")
            == "PASS",
            "non_price_data_support": self.test_results["system_status"]
            .get("enhanced_core_indicators", {})
            .get("status")
            == "PASS",
        }
        ...
        if not all(core_issues_solved.values()):
            recommendations.append(
                "Focus on the unsolved core issues for complete functionality"
            )
        if self.test_results["key_achievements"]:
            recommendations.append(
                "System shows excellent performance - consider production deployment"
            )
    ...
    def _generate_readable_report(self, report_path):
        ...
        # 核心問題解决
        report_lines.extend(["## Core Issues Resolution", ""])
        ...
        for achievement, description in self.test_results["key_achievements"].items():
            report_lines.append(
                f"- **{achievement.replace('_', ' ').title()}:** {description}"
            )
        ...
        for test_name, result in self.test_results["system_status"].items():
            ...
            report_lines.append(
                f"**Status:** {status_emoji} {result.get('status', 'UNKNOWN')}"
            )
    ...
    def run_full_test_suite(self):
        ...
        tests = [
            ...
            (
                "Intelligent Indicator Selection",
                self.test_intelligent_indicator_selection,
            ),
        ...
        logger.info(
            f"Success Rate: {summary['success_rate']:.1%} ({summary['tests_passed']}/{summary['total_tests']})"
        )
        logger.info(f"Core Issues Solved: {summary['all_issues_resolved']}")

        if self.test_results["key_achievements"]:
            logger.info("\nKey Achievements:")
            for achievement, description in self.test_results[
                "key_achievements"
            ].items():
...
def main():
    ...
    if success:
        ...
        print(
            "All core issues have been resolved and the system is ready for production use."
        )
    else:
