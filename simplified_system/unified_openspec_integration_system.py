...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("openspec_integration.log"), logging.StreamHandler()],
)
...
class UnifiedOpenSpecIntegrationSystem:
    ...
    def __init__(self):
        ...
        expected_gov_indicators = (
            len([ds for ds in self.data_sources if ds != "stock_price"]) * 6
        )  # 8 * 6 = 48
        ...
        logger.info(
            f"[性能模式] GPU: {'啟用' if self.gpu_mode else '禁用'}, VectorBT: {'啟用' if self.vectorbt_mode else '禁用'}"
        )
    ...
    def calculate_all_477_indicators(
        ...
        try:
            ...
            # 5. 價格形態指標 (24 種參數組合)
            price_pattern_indicators = self._calculate_price_pattern_indicators(
                price_data
            )
            ...
            logger.info(
                f"[指標計算] 完成 {actual_count} 種指標，耗時 {execution_time:.3f}秒"
            )
        ...
        for period in periods:
            if self.gpu_mode:
                ...
                indicators[f"SMA_{period}"] = pd.Series(
                    sma.get(), index = close_prices.index
                )
            else:
                # CPU 計算
            ...
            for slow in slow_periods:
                if fast < slow:
                    for signal in signal_periods:
                        ...
                        indicators[f"MACD_Hist_{fast}_{slow}_{signal}"] = (
        ...
        for k_period in k_periods:
            for d_period in d_periods:
                for slowing in slowing_periods:
                    ...
                    k_percent = 100 * (
                        (close_prices - lowest_low) / (highest_high - lowest_low)
                    )
    ...
    def _calculate_volume_indicators(
        ...
        obv = (price_change > 0).astype(int) * volume - (price_change < 0).astype(
            int
        ) * volume
        ...
        for period in vwap_periods:
            ...
            vwap = (typical_price * volume).rolling(
                window = period
            ).sum() / volume.rolling(window = period).sum()
            indicators[f"VWAP_{period}"] = vwap

        return indicators

    def _calculate_price_pattern_indicators(
        self, price_data: pd.DataFrame
    ) -> Dict[str, pd.Series]:
        """計算價格形態指標 (24 種組合)"""
        indicators = {}
    ...
    def _calculate_government_indicators(
    ...
    def _calculate_custom_indicators(
        ...
        macd_indicators = [
            name for name in indicator_names if "MACD_" in name and "_Hist" in name
        ]
        ...
        for rsi_name in rsi_indicators[:5]:  # 取前5個RSI
            for macd_name in macd_indicators[:3]:  # 取前3個MACD直方圖
                ...
                macd_norm = (
                    existing_indicators[macd_name]
                    - existing_indicators[macd_name].min()
                ) / (
                    existing_indicators[macd_name].max()
                    - existing_indicators[macd_name].min()
                )
        ...
        for sma_name in sma_indicators[:4]:  # 取前4個SMA
            for ema_name in ema_indicators[:5]:  # 取前5個EMA
                crossover = (
                    existing_indicators[sma_name] > existing_indicators[ema_name]
                ).astype(int)
        ...
        for roc_name in roc_indicators[:6]:  # 取前6個ROC
            for pivot_name in pivot_indicators[:9]:  # 取前9個Pivot
                ...
                indicators[f"MOMENTUM_PATTERN_{roc_name}_{pivot_name}"] = (
                    momentum_pattern_combo
                )
    ...
    def backtest_all_combinations(
        ...
        try:
            ...
            if self.gpu_mode and len(combinations) > 50:
                ...
                results = self._gpu_parallel_backtest(
                    combinations, stock_data, gov_data
                )
            else:
                # CPU 並行處理
            ...
            logger.info(
                f"[成功率] {len(valid_results)}/{len(combinations)} = {len(valid_results)/len(combinations)*100:.1f}%"
            )
        ...
        try:
            ...
            for data_source in combination:
                if data_source == "stock_price":
                    ...
                    # 政府數據信號
                    gov_score = self._calculate_government_signals(
    ...
    def _calculate_government_signals(
        ...
        try:
            ...
            if data_source == "hibor_rates":
                ...
                # 貨幣基礎相關指標
                monetary_indicators = [
                    k for k in indicators.keys() if "monetary" in k.lower()
                ]
                ...
                # 匯率相關指標
                exchange_indicators = [
                    k for k in indicators.keys() if "exchange" in k.lower()
                ]
    ...
    def _gpu_parallel_backtest(
        ...
        try:
            # 將數據移到 GPU
            if self.gpu_mode:
                cudf.DataFrame.from_pandas(stock_data)
                gov_gpu = {
                    k: cudf.DataFrame.from_pandas(v) for k, v in gov_data.items()
                }
            ...
            for i in range(0, len(combinations), batch_size):
                ...
                logger.info(
                    f"[GPU] 處理批次 {i / /batch_size + 1}/{(len(combinations)-1)//batch_size + 1}"
                )
...
def main():
    ...
    try:
        ...
        print(
            f"✅ 價格範圍: {stock_df['close'].min():.2f} - {stock_df['close'].max():.2f}"
        )
    ...
    if user_input == "y":
        ...
        if "error" not in full_test:
            ...
            print(
                f"✅ 性能目標: {'達成' if full_test['performance_target_met'] else '未達成'} (目標: 600+)"
            )
