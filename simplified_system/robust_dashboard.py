...
def calculate_comprehensive_indicators(df):
    ...
    # Volatility measures
    data["Volatility_20"] = data["Price_Change"].rolling(
...
print(
    f"📊 Price range: {processed_data['Close'].min():.2f} - {processed_data['Close'].max():.2f} HKD"
)
...
app.layout = html.Div(
    [
        html.H1(
            "🎯 Jupyter Analytics Dashboard - 0700.HK (Tencent)",
            style={
                "textAlign": "center",
                "color": "#2c3e50",
                "marginBottom": 30,
            ...
            [
                html.Div(
                    [
                        ...
                        html.P(
                            f"📅 Period: {processed_data.index[0].date()} to {processed_data.index[-1].date()}",
                            style={"fontSize": 14, "margin": 5},
                        ),
                        html.P(
                            f"📈 Records: {len(processed_data)} days",
                            style={"fontSize": 14, "margin": 5},
                        ),
                        html.P(
                            f"💰 Current Price: {processed_data['Close'].iloc[-1]:.2f} HKD",
                ...
                html.Div(
                    [
                        ...
                        html.P(
                            f"📊 RSI(14): {processed_data['RSI14'].iloc[-1]:.1f}",
                            style={
                                "fontSize": 14,
                                "margin": 5,
                                "color": (
                                    "purple"
                                    if 30 < processed_data["RSI14"].iloc[-1] < 70
                                    else "red"
                                ),
                            },
                        ),
                        html.P(
                            f"📈 MA20: {processed_data['MA20'].iloc[-1]:.2f} HKD",
                            style={"fontSize": 14, "margin": 5},
                        ),
                        html.P(
                            f"📉 MACD: {processed_data['MACD'].iloc[-1]:.3f}",
                            style={"fontSize": 14, "margin": 5},
                        ),
                        html.P(
                            f"🌊 Volatility: {processed_data['Volatility_20'].iloc[-1]:.2%}",
                            style={"fontSize": 14, "margin": 5},
                        ),
                    ],
                    className="four columns",
                    ...
                    [
                        ...
                        html.P(
                            ...
                            style={
                                ...
                                "color": (
                                    "green"
                                    if processed_data["Cumulative_Returns"].iloc[-1] > 0
                                    else "red"
                                ),
                            },
                        ),
                        html.P(
                            f"📊 Annual Volatility: {processed_data['Price_Change'].std() * np.sqrt(252):.2%}",
                            style={"fontSize": 14, "margin": 5},
                        ),
                        html.P(
                            f"🏆 Sharpe Ratio: {(processed_data['Price_Change'].mean() / processed_data['Price_Change'].std() * np.sqrt(252)):.3f}",
                            style={"fontSize": 14, "margin": 5},
                        ),
                        html.P(
        ...
        html.Div(
            [
                html.Hr(),
                html.P(
                    "🎉 Jupyter Notebook Analytics Dashboard - Simplified System Integration",
                    style={
                        "textAlign": "center",
                        "color": "#7f8c8d",
                        "margin": 20,
                ...
                html.P(
                    "Built with Professional Visualization Tools | Real - time 0700.HK Technical Analysis",
                    style={
                        "textAlign": "center",
                        "color": "#95a5a6",
                        "marginBottom": 20,
...
@app.callback(
    ...
    [Input("period - dropdown", "value"), Input("indicator - checklist", "value")],
)
def update_charts(period_days, indicators):
    ...
    colors = [
        (
            "green"
            if filtered_data["Close"].iloc[i] >= filtered_data["Close"].iloc[i - 1]
            else "red"
        )
    ...
    volume_fig.update_layout(
        title="Trading Volume", xaxis_title="Date", yaxis_title="Volume", height = 300
    )
    ...
    rsi_fig.update_layout(
        title="RSI(14) Indicator", xaxis_title="Date", yaxis_title="RSI", height = 300
    )
    ...
    macd_fig.update_layout(
        title="MACD Indicator", xaxis_title="Date", yaxis_title="MACD", height = 300
    )
