...
class VectorBTIntegrationTester:
    ...
    def run_all_tests(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.test_results["phases"][
                "core_engine"
            ] = self.test_core_vectorbt_engine()
    ...
    def test_core_vectorbt_engine(self) -> Dict[str, Any]:
        ...
        phase_results = {"status": "UNKNOWN", "tests": {}, "performance": {}}
        ...
        try:
            ...
            for strategy_name, params in strategies_to_test:
                try:
                    ...
                    logger.info(
                        f"  {strategy_name}: 回報 {result.total_return:.2%}, Sharpe {result.sharpe_ratio:.3f}"
                    )
            ...
            phase_results["tests"]["parameter_optimization"] = {
                ...
                "combinations_tested": len(param_ranges["period"])
            ...
            phase_results["performance"]["successful_strategies"] = sum(
                1 for r in strategy_results.values() if r.get("success", False)
            )

            # Check if all core tests passed
            core_tests_passed = (
                phase_results["tests"].get("engine_initialization", False)
                and phase_results["tests"]
                .get("parameter_optimization", {})
                .get("success", False)
                and phase_results["performance"]["successful_strategies"] > 0
            )
        ...
        try:
            ...
            phase_results["tests"]["multi_objective_optimization"] = {
                ...
                "pareto_frontier_size": len(
                    multi_obj_result.get("pareto_frontier", [])
                ),
                "best_compromise_score": multi_obj_result.get(
                    "best_compromise", {}
            ...
            phase_results["status"] = (
                "PASSED" if successful_algorithms >= 2 else "PARTIAL"
            )

            logger.info(
                f"  Bayesian 優化: 最佳分數 {bayesian_result.get('best_score', 0):.3f}"
            )
            logger.info(
                f"  遺傳算法: 最佳分數 {genetic_result.get('best_score', 0):.3f}"
            )
            logger.info(
                f"  多目標優化: Pareto前沿 {len(multi_obj_result.get('pareto_frontier', []))} 個解"
            )
    ...
    def test_distributed_computing(self) -> Dict[str, Any]:
        ...
        try:
            ...
            phase_results["performance"]["parallel_efficiency"] = sweep_result.get(
                "success_rate", 0
            )
            ...
            distributed_passed = (
                phase_results["tests"]
                .get("parallel_parameter_sweep", {})
                .get("success", False)
                and sweep_result.get("completed_tasks", 0) > 0
            )

            phase_results["status"] = "PASSED" if distributed_passed else "PARTIAL"

            logger.info(
                f"  並行優化: {sweep_result.get('completed_tasks', 0)}/{sweep_result.get('total_tasks', 0)} 任務成功"
            )
            logger.info(
                f"  可擴展性: {phase_results['performance'].get('speedup', 0):.2f}x 加速"
            )
        ...
        try:
            ...
            returns_matrix = pd.DataFrame(
                np.random.multivariate_normal(
                    [0.001, 0.0008], [[0.04, 0.02], [0.02, 0.03]], len(prices)
                ),
            ...
            phase_results["tests"]["portfolio_risk"] = {
                ...
                "risk_contribution_analysis": len(
                    portfolio_risk["risk_contribution_percentages"]
                )
                > 0,
            }
            ...
            ] = 8  # Number of risk metrics
            phase_results["performance"]["calculation_time_ms"] = risk_calc_time * 1000

            # Check if risk management tests passed
            risk_passed = (
                phase_results["tests"]
                .get("risk_metrics_calculation", {})
                .get("success", False)
                and not np.isnan(risk_metrics.sharpe_ratio)
                and phase_results["tests"]
                .get("var_calculations", {})
                .get("cvar_var_relationship", False)
            )

            phase_results["status"] = "PASSED" if risk_passed else "PARTIAL"

            logger.info(
                f"  風險指標: Sharpe {risk_metrics.sharpe_ratio:.3f}, 最大回撤 {risk_metrics.max_drawdown:.2%}"
            )
            logger.info(f"  VaR 95%: {var_95:.4f}, CVaR 95%: {cvar_95:.4f}")
            logger.info(
                f"  組合風險: 波動率 {portfolio_risk['portfolio_volatility']:.2%}"
            )
    ...
    def test_enhanced_indicators(self) -> Dict[str, Any]:
        ...
        try:
            ...
            phase_results["tests"]["vectorized_rsi"] = {
                ...
                "rsi_14_range": [
                    rsi_results["rsi_14"].min(),
                    rsi_results["rsi_14"].max(),
                ],
            }
            ...
            start_time = time.time()
            adaptive_period = enhanced_indicators.calculate_adaptive_parameters(
                data, "rsi", 14, 0.2
            )
            ...
            phase_results["performance"]["vectorization_efficiency"] = (
                rsi_time + macd_time + bb_time
            ) / 1000  # Total time in seconds

            # Check if enhanced indicators tests passed
            enhanced_passed = (
                phase_results["tests"].get("vectorized_rsi", {}).get("success", False)
                and phase_results["tests"]
                .get("vectorized_macd", {})
                .get("success", False)
                and phase_results["tests"]
                .get("adaptive_parameters", {})
                .get("success", False)
            )
            ...
            logger.info(
                f"  自適參數: {adaptive_period.min()}-{adaptive_period.max()} 範圍"
            )
    ...
    def test_system_integration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            phase_results["performance"]["components_compatible"] = len(
                compatibility_tests
            )
            phase_results["performance"]["data_integrity_score"] = (
                len([t for t in integrity_tests if "nan_count_0" in t])
                / len(integrity_tests)
                if integrity_tests
                else 0
            )
            ...
            integration_passed = phase_results["tests"].get(
                "end_to_end_workflow", {}
            ).get("success", False) and phase_results["tests"].get(
                "compatibility", {}
            ).get(
                "success", False
            )
            ...
            logger.info(
                f"  數據完整性: {phase_results['performance']['data_integrity_score']:.1%}"
            )
    ...
    def test_performance_benchmark(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for size in data_sizes:
                ...
                enhanced_indicators = VectorizedTechnicalIndicators()
                start_time = time.time()
                rsi_result = enhanced_indicators.calculate_rsi_vectorized(
                    test_data, [14]
                )
                ...
                benchmark_results[size] = {
                    ...
                    "records_per_second": (
                        size / (engine_time + indicators_time + risk_time)
                        if (engine_time + indicators_time + risk_time) > 0
                        else 0
                    ),
                }
            ...
            total_records = sum(
                benchmark_results[size]["data_points"] for size in data_sizes
            )
            total_time = sum(
                benchmark_results[size]["total_time"] for size in data_sizes
            )
            ...
            phase_results["performance"]["performance_target_met"] = (
                avg_records_per_second >= performance_target
            )
            phase_results["performance"]["performance_achievement_ratio"] = (
                avg_records_per_second / performance_target
            )
            ...
            benchmark_passed = (
                phase_results["tests"]
                .get("performance_scaling", {})
                .get("success", False)
                and avg_records_per_second > 100
            )
            ...
            logger.info(
                f"  性能目標 ({performance_target} 記錄 / 秒): {'達成' if phase_results['performance']['performance_target_met'] else '未達成'}"
            )
    ...
    def calculate_overall_status(self):
        ...
        self.test_results["success_rate"] = (
            passed_phases / total_phases if total_phases > 0 else 0
        )
