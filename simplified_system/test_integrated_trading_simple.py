...
try:
    ...
    if stock_data is not None and not stock_data.empty:
        ...
        if analysis_result.get("success"):
            ...
            if "composite_signals" in analysis_result:
                ...
                for column, signals in list(composite_signals.items())[
                    :2
                ]:  # 只顯示前2列
                    print(f"    {column}:")
                    if isinstance(signals, dict):
                        ...
                        if "weighted_scores" in signals:
                            print(
                                f"      Weighted Scores: {len(signals['weighted_scores'])} indicators"
                            )
            ...
            print(
                f"[FAIL] Advanced signals failed: {analysis_result.get('error', 'Unknown error')}"
            )
    ...
    for method in methods:
        ...
        if result.get("success"):
            ...
            print(
                f"  Quality metrics available: {len(result.get('metadata', {}).get('quality_metrics', {}))}"
            )
        else:
    ...
    for symbol in test_symbols:
        ...
        try:
            ...
            analysis_results[symbol] = {"success": False, "error": str(e)}
            print(f"  [ERROR] {symbol}: {e}")

    print(f"Multi - stock analysis completed: {len(analysis_results)} symbols")

    successful_analyses = sum(
        1 for r in analysis_results.values() if r.get("success", False)
    )
    ...
    for i in range(performance_iterations):
        ...
        if result.get("success"):
            print(
                f"  Iteration {i + 1}: {iter_time:.3f}s, {result.get('processed_records', 0)} records"
            )
        else:
    ...
    print(
        f"  Success rate: {sum(1 for t in analysis_times if t > 0)} / {performance_iterations}"
    )
    ...
    print(
        f"- Data Quality Score: {passed_checks}/{total_checks} ({passed_checks / total_checks * 100:.0f}%)"
    )
    print(
        f"- Analysis Throughput: {test_results['performance_metrics']['throughput']:.1f} records / second"
    )
    print(
        f"- Multi - Stock Success Rate: {test_results['performance_metrics']['success_rate']:.1%}"
    )
