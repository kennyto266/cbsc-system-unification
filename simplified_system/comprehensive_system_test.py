...
class ComprehensiveSystemTest:
    ...
    def test_module_imports(self):
        ...
        for module_name, class_name in modules_to_test:
            try:
                ...
                self.test_results["test_session"]["critical_issues"].append(
        ...
        try:
            ...
            try:
                ...
                if hibor and hibor.get("data"):
                    ...
                    latest_rate = (
                        hibor["data"][0].get("hibor_overnight", "N / A")
                        if hibor["data"]
                        else "N / A"
                    )
                    ...
                    print(
                        f"[OK] HIBOR API: {record_count} 條記錄, 最新利率: {latest_rate}"
                    )
                    self.test_results["test_session"]["passed_tests"] += 1
                else:
                    self.test_results["api_tests"]["hibor_api"] = {
            ...
            try:
                ...
                if stock_data is not None and len(stock_data) > 0:
                    ...
                    self.test_results["api_tests"]["stock_api"] = {
                        ...
                        "latest_price": (
                            float(latest_price) if latest_price != "N / A" else None
                        ),
                        "columns": list(stock_data.columns),
                    }
                    print(
                        f"[OK] 股票 API: {len(stock_data)} 條記錄, 最新價格: {latest_price}"
                    )
                    self.test_results["test_session"]["passed_tests"] += 1
                else:
                    self.test_results["api_tests"]["stock_api"] = {
        ...
        try:
            ...
            try:
                ...
                if standardized is not None and len(standardized) > 0:
                    ...
                    print(
                        f"[OK] 數據標準化: {len(test_data)} -> {len(standardized)} 條記錄"
                    )
    ...
    def test_performance(self):
        ...
        try:
            ...
            self.test_results["performance_tests"]["cpu"] = {"percent": cpu_percent}
            ...
            try:
                ...
                if response_time > 5000:  # 5 seconds
                    ...
                    self.test_results["test_session"]["critical_issues"].append(
                        f"Slow API response: {response_time:.0f}ms"
                    )
                else:
            ...
            self.test_results["test_session"]["critical_issues"].append(
                "psutil not installed for performance monitoring"
            )
    ...
    def test_dependencies(self):
        ...
        required_packages = ["pandas", "numpy", "requests", "pathlib"]

        optional_packages = ["psutil", "aiohttp", "vectorbt"]
    ...
    def test_local_files(self):
            ...
            if matching_files:
                ...
                try:
                    ...
                    if "data" in data and data["data"]:
                        ...
                        print(
                            f"         最新文件: {latest_file.name} ({record_count} 條記錄)"
                        )
                    else:
        ...
        if missing_files:
            self.test_results["test_session"]["critical_issues"].extend(
                [f"Missing data: {item}" for item in missing_files]
            )
    ...
    def test_integration(self):
        ...
        try:
            ...
            if alpha_result and alpha_result.get("quality_score", 0) > 0:
                ...
                print(
                    f"[OK] 系統集成測試: 質量評分 {quality_score:.2f}, 數據源 {data_sources} 個"
                )
                self.test_results["test_session"]["passed_tests"] += 1
            else:
                self.test_results["data_tests"]["integration"] = {
    ...
    def generate_report(self):
        ...
        for category, tests in categories.items():
            if tests:
                passed_count = sum(
                    1 for test in tests.values() if test.get("status") == "PASS"
                )
                total_count = len(tests)
                status = (
                    "[OK]"
                    if passed_count == total_count
                    else "[PARTIAL]" if passed_count > 0 else "[FAIL]"
                )
