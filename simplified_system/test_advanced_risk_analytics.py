...
def generate_test_data():
    ...
    returns.loc[extreme_events] *= np.random.uniform(
        -3, -1, 5
    ...
    for asset in portfolio_positions["asset"]:
        ...
        volumes = pd.Series(
            np.random.exponential(1000000, 504), index = dates, name="volume"
        )
        ...
        market_data[asset] = pd.DataFrame(
            {"close": prices, "volume": volumes, "bid": bid_prices, "ask": ask_prices}
        )
    ...
    print(
        f"✅ Generated funding data with {len(funding_data['funding_sources'])} sources"
    )

    return returns, portfolio_positions, market_data, funding_data


def test_advanced_risk_analyzer(
    returns, portfolio_positions, market_data, funding_data
):
    """測試高級風險分析器"""
    ...
    try:
        ...
        print(
            f"✅ Sharpe Ratio: {risk_results['basic_risk_metrics']['sharpe_variants']['sharpe_ratio']:.3f}"
        )
        print(
            f"✅ Max Drawdown: {risk_results['basic_risk_metrics']['drawdown_metrics']['max_drawdown']:.2%}"
        )
        print(
            f"✅ VaR 95%: {risk_results['basic_risk_metrics']['var_metrics']['var_95']:.2%}"
        )
        print(f"✅ Number of Risk Alerts: {len(risk_results['risk_alerts'])}")
        print(
            f"✅ Concentration Score: {risk_results['concentration_risk']['concentration_score']:.2f}"
        )
        print(
            f"✅ Liquidity Score: {risk_results['liquidity_risk']['liquidity_score']:.2f}"
        )
...
def test_monte_carlo_var(returns, portfolio_positions):
    ...
    try:
        from risk.monte_carlo_var import DistributionType, VaRConfig
...
def test_liquidity_risk_analyzer(portfolio_positions, market_data, funding_data):
    ...
    try:
        ...
        print(
            f"✅ Early Warning Indicators: {len(liquidity_results.early_warning_indicators)}"
        )
        ...
        for scenario in stress_results:
            print(
                f"  - {scenario.scenario_name}: Funding Gap {scenario.funding_gap:,.2f}"
            )
...
def test_quick_functions(returns, portfolio_positions, market_data):
    ...
    try:
        ...
        print(
            f"✅ Quick Stress Test: Worst case {quick_stress['worst_case_loss_percentage']:.2%}"
        )

        # Quick VaR calculation
        quick_var = quick_var_calculation(returns, portfolio_value)
        print(
            f"✅ Quick VaR 95%: {quick_var['var_value']:,.2f} ({quick_var['var_percentage']:.2%})"
        )
        ...
        quick_liquidity = quick_liquidity_assessment(
            portfolio_value, bid_ask_spreads, daily_volumes
        )
        print(
            f"✅ Quick Liquidity Risk: {quick_liquidity['overall_liquidity_risk']:.1f}/100"
        )
...
def test_comprehensive_analysis(
    ...
    try:
        ...
        print(
            f"✅ Analysis completed with {len(comprehensive_results['risk_modules_completed'])} modules"
        )
        print(
            f"✅ Overall Risk Assessment: {comprehensive_results['summary']['overall_risk_assessment']['risk_level']}"
        )
        print(
            f"✅ Overall Risk Score: {comprehensive_results['summary']['overall_risk_assessment']['overall_score']:.1f}/100"
        )
        print(
            f"✅ Confidence: {comprehensive_results['summary']['overall_risk_assessment']['confidence']:.1%}"
        )

        # Display module results summary
        if "basic_risk_metrics" in comprehensive_results:
            sharpe = comprehensive_results["basic_risk_metrics"]["sharpe_variants"][
                "sharpe_ratio"
            ]
            print(f"✅ Basic Risk Metrics: Sharpe = {sharpe:.3f}")

        if "advanced_risk_analysis" in comprehensive_results:
            risk_level = comprehensive_results["advanced_risk_analysis"][
                "overall_risk_level"
            ]
            print(f"✅ Advanced Risk Analysis: Risk Level = {risk_level}")

        if "stress_test_results" in comprehensive_results:
            worst_loss = comprehensive_results["stress_test_results"][
                "worst_case_loss_percentage"
            ]
...
def main():
    ...
    risk_results = test_advanced_risk_analyzer(
        returns, portfolio_positions, market_data, funding_data
    )
    stress_results = test_stress_test_engine(
        returns, portfolio_positions, market_data, funding_data
    )
    var_results = test_monte_carlo_var(returns, portfolio_positions)
    liquidity_results = test_liquidity_risk_analyzer(
        portfolio_positions, market_data, funding_data
    )
