...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("real_data_api.log"), logging.StreamHandler()],
)
...
class RealDataOnlyAPISystem:
        ...
        for attempt in range(self.retry_attempts):
            try:
                ...
                response = self.session.get(
                    primary_url, params = params, timeout = self.request_timeout
                )
                ...
                if parsed_data and self._is_real_data_quality_acceptable(parsed_data):
                    logger.info(
                        f"實時API成功: {len(parsed_data.get('data', []))} 條真實記錄"
                    )
    ...
    def _try_alternative_apis(
        self, data_type: str, days_back: int
    ) -> Optional[Dict[str, Any]]:
        """嘗試備選實時API端點"""
        config = self.real_data_sources[data_type]
        ...
        for i, alt_url in enumerate(alternatives):
            ...
            try:
                ...
                response = self.session.get(
                    alt_url, params = params, timeout = self.request_timeout
                )
                ...
                if parsed_data and self._is_real_data_quality_acceptable(parsed_data):
                    logger.info(
                        f"備選API {i + 1} 成功: {len(parsed_data.get('data', []))} 條真實記錄"
                    )
                    self._record_data_success(
                        data_type, f"備選API_{i + 1}", parsed_data
                    )
                    return parsed_data
        ...
        for file_path in local_files:
            ...
            try:
                ...
                if self._is_real_hkma_data(data):
                    ...
                    logger.info(
                        f"成功加載本地真實數據: {file_path} ({len(records)} 條記錄)"
                    )
        ...
        for record in records:
            try:
                hibor_record = {
                    ...
                    "hibor_overnight": self._safe_float(
                        record.get("hibor_overnight") or record.get("ir_1w")
                    ),
                    "hibor_1_week": self._safe_float(
                        record.get("hibor_1_week") or record.get("ir_1w")
                    ),
                    "hibor_1_month": self._safe_float(
                        record.get("hibor_1_month") or record.get("ir_1m")
                    ),
                    "hibor_3_months": self._safe_float(
                        record.get("hibor_3_months") or record.get("ir_3m")
                    ),
                    "hibor_6_months": self._safe_float(
                        record.get("hibor_6_months") or record.get("ir_6m")
                    ),
                    "hibor_12_months": self._safe_float(
                        record.get("hibor_12_months") or record.get("ir_12m")
                    ),
                ...
                # 驗證數據完整性
                if hibor_record["date"] and (
                    hibor_record["hibor_overnight"]
                    or hibor_record["hibor_1_week"]
                    or hibor_record["hibor_1_month"]
                ):
        ...
        for record in records:
            try:
                monetary_record = {
                    ...
                    or record.get("date")
                    or record.get("timestamp"),
                    "monetary_base_billion_hkd": self._safe_float(
                        record.get("monetary_base")
                    ),
                ...
                if (
                    monetary_record["date"]
                    and monetary_record["monetary_base_billion_hkd"]
                ):
    ...
    def _validate_and_return(
        self, data: Dict[str, Any], data_type: str, source: str
    ) -> Dict[str, Any]:
        """驗證並返回數據"""
        # 添加驗證信息
        ...
        logger.info(
            f"真實數據驗證成功: {data_type} 來源: {source}, 記錄: {len(data.get('data', []))}"
        )
        return data
    ...
    def get_real_data_status_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "real_only_policy": {
                ...
                "require_real_source": True,
                "minimum_real_data_age_days": self.real_only_config[
                    "minimum_real_data_age_days"
                ],
            },
            ...
            "recent_activity": {
                ...
                "successful_retrievals": sum(
                    1
                    for record in self.data_source_history
        ...
        if report["recent_activity"]["total_attempts"] > 0:
            success_rate = (
                report["recent_activity"]["successful_retrievals"]
                / report["recent_activity"]["total_attempts"]
            )
        ...
        report["recent_activity"]["last_24h_success"] = sum(
            1 for record in recent_records if record["status"] == "SUCCESS"
        )
        report["recent_activity"]["last_24h_success_rate"] = 0.0

        if report["recent_activity"]["last_24h_attempts"] > 0:
            recent_24h_success_rate = (
                report["recent_activity"]["last_24h_success"]
                / report["recent_activity"]["last_24h_attempts"]
            )
            report["recent_activity"]["last_24h_success_rate"] = round(
                recent_24h_success_rate, 3
            )
    ...
    try:
        full_report = {
            ...
            "system_config": {
                ...
                "file_age_limit_days": real_api.real_only_config[
                    "minimum_real_data_age_days"
                ],
            },
        }
