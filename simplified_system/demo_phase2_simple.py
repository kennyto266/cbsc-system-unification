...
def main():
    ...
    print(
        f"   Signal: {'Bullish' if price_data.iloc[-1] > dema.iloc[-1] else 'Bearish'}"
    )
    ...
    print(
        f"   Trend strength: {'Strong' if abs(price_data.iloc[-1] - tema.iloc[-1]) > 5 else 'Weak'}"
    )
    ...
    print(
        f"   Calculation time: {bb_result['adaptation_info']['calculation_time_ms']:.3f}ms"
    )
    ...
    if "term_spread" in hibor_result:
        ...
        print(
            f"   Calculation time: {hibor_result['adaptation_info']['calculation_time_ms']:.3f}ms"
        )
        print(f"   Latest Spread: {spread.iloc[-1]:.3f}%")
        print(f"   Z - Score: {zscore.iloc[-1]:.2f}")
        print(
            f"   Signal: {signal.iloc[-1]:.0f} ({'Bullish' if signal.iloc[-1] > 0 else 'Bearish' if signal.iloc[-1] < 0 else 'Neutral'})"
        )
    ...
    print(
        f"   Calculation time: {spread_result['adaptation_info']['calculation_time_ms']:.3f}ms"
    )
    ...
    print(
        f"Performance target (<1ms): {'PASS' if avg_time < 1.0 else 'NEEDS OPTIMIZATION'}"
    )
