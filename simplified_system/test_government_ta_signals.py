        ...
        if monetary_signals.get("success"):
            ...
            for signal_type, signal_data in signal_types.items():
                ...
                if "trend" in signal_data:
                    print(
                        f"  Trend: {signal_data['trend']} (strength: {signal_data.get('strength', 0):.3f})"
                    )
        else:
            ...
            if composite_signals.get("success"):
                ...
                if individual_scores:
                    ...
                    for source, score_data in individual_scores.items():
                        print(
                            f"  {source}: {score_data.get('score', 0):.3f} (weight: {score_data.get('weight', 0):.2f})"
                        )
                ...
                print(
                    f"Composite signals failed: {composite_signals.get('error', 'Unknown error')}"
                )
...
def main():
    ...
    print(
        "from indicators.government_ta_signals import generate_all_signals_from_real_data"
    )
    print()
