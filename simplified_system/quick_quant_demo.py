...
def analyze_signals(indicators):
    ...
    # Bollinger Bands
    bb_position = (indicators["current_price"] - indicators["lower_bb"]) / (
        indicators["upper_bb"] - indicators["lower_bb"]
    )
    if bb_position > 0.9:
...
def main():
    ...
    print(
        f"  Trend: {'UPTREND' if indicators['current_price'] > indicators['sma_20'] else 'DOWNTREND'}"
    )
    print(
        f"  Momentum: {'BULLISH' if indicators['macd'] > indicators['signal'] else 'BEARISH'}"
    )
