...
class HKGovDataCrawlerFixed:
    ...
    def save_data_to_files(self, results: List[Dict[str, Any]]) -> None:
        ...
        # 保存完整结果
        full_result_file = (
            self.cache_dir / f"hk_gov_financial_data_fixed_{timestamp}.json"
        )
        ...
        for result in results:
            if result.get("status") == "success":
                source_file = (
                    self.cache_dir / f"{result['source']}_fixed_{timestamp}.json"
                )
...
"""
    ...
    async def run_daily_collection(self) -> Dict[str, Any]:
        ...
        logger.info(
            "Starting daily HK government financial data collection - FIXED VERSION"
        )
