...
class GPUPerformanceBenchmark:
    ...
    def benchmark_rsi_calculation(self) -> Dict[str, Any]:
        ...
        for size in self.test_sizes:
            ...
            start_time = time.time()
            for period in self.rsi_periods:
                self.cpu_engine.calculate_rsi(test_data, period)
            ...
            logger.info(
                f"Size {size:,}: GPU {gpu_time:.3f}s, CPU {cpu_time:.3f}s, "
    ...
    def benchmark_macd_calculation(self) -> Dict[str, Any]:
        ...
        for size in self.test_sizes:
            ...
            for fast, slow, signal in self.macd_params:
                cpu_result = self.cpu_engine.calculate_macd(
                    test_data, fast, slow, signal
                )
            ...
            gpu_accuracy = self._measure_accuracy(
                gpu_results[key_param]["macd"], cpu_results[key_param]["macd"]
            )
    ...
    def benchmark_parameter_optimization(self) -> Dict[str, Any]:
        ...
        optimization_configs = [
            ...
            {
                "name": "Medium",
                "periods": list(range(5, 31, 2)),
                "oversold": [15, 20, 25, 30, 35],
                "overbought": [65, 70, 75, 80, 85],
            },
            {
                "name": "Large",
                "periods": list(range(3, 51, 2)),
        ...
        for config in optimization_configs:
            ...
            # 记录结果
            results["data_sizes"].append(
                f"{config['name']} ({total_combinations:,} combos)"
            )
            ...
            logger.info(
                f"{config['name']}: {total_combinations:,} combos in {gpu_time:.2f}s, "
    ...
    def benchmark_memory_usage(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for size in self.test_sizes:
                ...
                # 执行GPU计算
                test_data = self._generate_price_data(size)
                self.gpu_engine.calculate_rsi(test_data.values, 14)
    ...
    def benchmark_scalability(self) -> Dict[str, Any]:
        ...
        for thread_count in results["thread_counts"]:
            ...
            logger.info(
                f"Threads {thread_count}: {execution_time:.2f}s, Efficiency {efficiency:.2f}"
            )
    ...
    def _measure_accuracy(
        self, gpu_result: np.ndarray, cpu_result: np.ndarray
    ) -> float:
        """测量计算精度（RMSE）"""
        # 确保两个数组长度相同
    ...
    def run_comprehensive_benchmark(self) -> Dict[str, Any]:
        ...
        try:
            benchmark_results["parameter_optimization"] = (
    ...
    def _generate_comprehensive_report(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "rsi_calculation" in results:
            ...
            report["summary"]["rsi_calculation"] = {
                ...
                "best_performance_at_size": int(
                    rsi_results["data_sizes"][np.argmax(rsi_results["speedup_ratios"])]
                ),
            }
        ...
        if "macd_calculation" in results:
            ...
            report["summary"]["macd_calculation"] = {
                ...
                "best_performance_at_size": int(
                    macd_results["data_sizes"][
                        np.argmax(macd_results["speedup_ratios"])
                    ]
                ),
            }
            ...
            report["summary"]["parameter_optimization"] = {
                ...
                "total_strategies_tested": sum(
                    [
                        ...
                        for r in opt_results["combination_counts"]
                    ]
                ),  # 这个需要修正
            }
    ...
    def generate_visualization_report(
        self, results: Dict[str, Any], save_path: str = None
    ):
        """生成可视化报告"""
        try:
            import matplotlib.pyplot as plt
            ...
            fig.suptitle(
                "GPU Performance Benchmark Results", fontsize = 16, fontweight="bold"
            )
            ...
            if "rsi_calculation" in results:
                ...
                ax1 = axes[0, 0]
                ax1.plot(
                    rsi_results["data_sizes"],
                    rsi_results["cpu_times"],
                    "o-",
                    label="CPU",
                    linewidth = 2,
                )
            ...
            if "rsi_calculation" in results:
                ax2 = axes[0, 1]
                ax2.bar(
                    range(len(rsi_results["speedup_ratios"])),
                    rsi_results["speedup_ratios"],
                )
            ...
            if "memory_usage" in results:
                ...
                ax4 = axes[1, 1]
                ax4.plot(
                    mem_results["data_sizes"],
                    mem_results["memory_usage_mb"],
                    "o-",
                    color="red",
                    linewidth = 2,
                )
                ...
                ax4.text(
                    0.5,
                    0.5,
                    f"GPU Backend: {results['gpu_backend_info']['backend_type']}\n"
...
def run_gpu_performance_benchmark():
    ...
    print(
        f"GPU Available: {'Yes' if gpu_backend.get('cuda_available', False) else 'No'}"
    )
    ...
    if "summary" in results:
        ...
        if "overall_performance" in summary:
            ...
            print(
                f"   GPU Acceleration Effective: {'✅ Yes' if overall['gpu_acceleration_effective'] else '❌ No'}"
            )
        ...
        if "rsi_calculation" in summary:
            ...
            print(
                f"   Best Performance at {rsi['best_performance_at_size']:,} data points"
            )
        ...
        if "parameter_optimization" in summary:
            ...
            print(
                f"   Maximum Throughput: {opt['max_strategies_per_second']:.0f} strategies / second"
            )
