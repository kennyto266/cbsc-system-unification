    ...
    for period, oversold, overbought in rsi_params:
        ...
        if result:
            ...
            if result["sharpe_ratio"] > 0.5:
                print(
                    f"  Good RSI({period},{oversold},{overbought}): SR={result['sharpe_ratio']:.3f}"
                )

    # Test MACD strategies
    print("\nTesting MACD strategies...")
    macd_params = [(12, 26, 9), (8, 24, 8), (10, 30, 12), (15, 35, 10)]
        ...
        if fast < slow:
            ...
            if result:
                ...
                if result["sharpe_ratio"] > 0.3:
                    print(
                        f"  Good MACD({fast},{slow},{signal}): SR={result['sharpe_ratio']:.3f}"
                    )
...
def analyze_results(results):
    ...
    print(
        f"{'Rank':<4} {'Strategy':<8} {'Params':<15} {'Sharpe':<8} {'Max DD':<10} {'Return':<8}"
    )
    print("-" * 60)
    ...
    for i, result in enumerate(results[:10]):
        ...
        print(
            f"{i + 1:<4} {result['strategy']:<8} {params:<15} "
