...
class MultiObjectiveOptimizationDemo:
    ...
    def demonstrate_pareto_frontier(self, returns_df) -> List[Dict]:
        ...
        if pareto_portfolios:
            ...
            print(f"\nSpecial Portfolios:")
            print(
                f"Max Sharpe: Return={max_sharpe['return']:.2%}, Vol={max_sharpe['volatility']:.2%}, Sharpe={max_sharpe['sharpe']:.3f}"
            )
            print(
                f"Min Volatility: Return={min_vol['return']:.2%}, Vol={min_vol['volatility']:.2%}, Sharpe={min_vol['sharpe']:.3f}"
            )
            ...
            plt.scatter(
                all_vol, all_ret, c="lightblue", alpha = 0.6, s = 10, label="All Portfolios"
            )
    ...
    def demonstrate_multi_objective_optimization(self, returns_df) -> List[Dict]:
        ...
        def multi_objective_obj(weights, lambda_ret, lambda_vol, lambda_dd):
            ...
            objective = (
                ...
                + lambda_vol * portfolio_vol
                + lambda_dd * abs(max_dd_est)
            )
            return objective
        ...
        for scenario in scenarios:
            ...
            if result.success:
                ...
                print(
                    f"{scenario['name']:<15} {portfolio_return:<8.2%} {portfolio_vol:<10.2%} {sharpe:<8.3f}"
                )
    ...
    def run_complete_demo(self):
        ...
        try:
            ...
            self.demonstrate_objective_functions()
            returns_df = self.results["returns_data"]

            # 2. Pareto frontier
            self.demonstrate_pareto_frontier(returns_df)

            # 3. Multi - objective optimization
            optimization_results = self.demonstrate_multi_objective_optimization(
                returns_df
            )
