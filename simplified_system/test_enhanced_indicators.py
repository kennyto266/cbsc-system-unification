...
class EnhancedCoreIndicators:
    ...
    def calculate_sma(self, prices: pd.Series, period: int = 20) -> pd.Series:
        ...
        if len(prices) < period:
            logger.warning(
                f"Insufficient data for SMA{period}: need {period}, have {len(prices)}"
            )
    ...
    def calculate_ema(self, prices: pd.Series, period: int = 26) -> pd.Series:
        ...
        if len(prices) < period:
            logger.warning(
                f"Insufficient data for EMA{period}: need {period}, have {len(prices)}"
            )
            return pd.Series([np.nan] * len(prices), index = prices.index)
    ...
    def calculate_macd(
        ...
        if len(prices) < slow:
            logger.warning(
                f"Insufficient data for MACD: need {slow}, have {len(prices)}"
            )
            return {
    ...
    def calculate_rsi(self, prices: pd.Series, period: int = 14) -> pd.Series:
        ...
        if len(prices) < period + 1:
            logger.warning(
                f"Insufficient data for RSI{period}: need {period + 1}, have {len(prices)}"
            )
    ...
    def calculate_rate_spread(
        self, rate_data_short: pd.Series, rate_data_long: pd.Series
    ) -> Dict:
        """計算利率期限結構利差"""
        # 確保數據對齊
                ...
                if "error" not in data:
                    ...
                    if "indicators" in data:
                        ...
                        if "sma" in indicators and "info" in indicators["sma"]:
                            ...
                            print(
                                f"    SMA adaptation: {sma_info.get('adaptation_reason', 'Unknown')}"
                            )
                        ...
                        if "rsi" in indicators and "info" in indicators["rsi"]:
                            ...
                            print(
                                f"    RSI adaptation: {rsi_info.get('adaptation_reason', 'Unknown')}"
                            )
