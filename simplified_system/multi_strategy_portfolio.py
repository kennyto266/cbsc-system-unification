...
class MultiStrategyPortfolio:
    ...
    def calculate_portfolio_metrics(
        ...
        if strategy_vols:
            portfolio_vol = np.sqrt(
                sum(
                    ...
                    * s1.volatility
                    * s2.volatility
                    * self._get_correlation(s1, s2)
                    for i, (s1, w1) in enumerate(
                        zip(self.strategies, weights.strategy_weights.values())
                    )
                    for j, (s2, w2) in enumerate(
                        zip(self.strategies, weights.strategy_weights.values())
                    )
                )
            )
        ...
        portfolio_drawdown = sum(
            w * d for w, d in zip(weights.strategy_weights.values(), strategy_drawdowns)
        )

        # 勝率估算
        portfolio_win_rate = min(
            0.6, sum(w * 0.5 for w in weights.strategy_weights.values())
        )  # 簡化估算
        ...
        avg_correlation = correlation_df.values[
            np.triu_indices(n_strategies, k = 1)
        ].mean()
        diversification_ratio = n_strategies / (
    ...
    def _get_correlation(
        self, strategy1: StrategyDefinition, strategy2: StrategyDefinition
    ) -> float:
        """獲取策略間相關性"""
        ...
        if (
            strategy1.correlation_to_others
            and strategy2.name in strategy1.correlation_to_others
        ):
            return strategy1.correlation_to_others[strategy2.name]

        if (
            strategy2.correlation_to_others
            and strategy1.name in strategy2.correlation_to_others
        ):
    ...
    def _build_correlation_matrix(self) -> pd.DataFrame:
        ...
        for i in range(n_strategies):
            for j in range(n_strategies):
                correlation_matrix[i][j] = self._get_correlation(
                    self.strategies[i], self.strategies[j]
                )

        strategy_names = [s.name for s in self.strategies]
        return pd.DataFrame(
            correlation_matrix, index = strategy_names, columns = strategy_names
        )

    def optimize_portfolio(
        self, optimization_target: str = "sharpe"
    ) -> Tuple[PortfolioWeights, PortfolioMetrics]:
        """優化組合"""
    ...
    def generate_portfolio_report(
        self, weights: PortfolioWeights, metrics: PortfolioMetrics
    ) -> str:
        """生成組合報告"""
        report = "=" * 80 + "\n"
    ...
    def save_portfolio_results(
        self, weights: PortfolioWeights, metrics: PortfolioMetrics, filename: str = None
    ):
        """保存組合結果"""
        if filename is None:
