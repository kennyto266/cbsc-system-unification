...
class RiskParitySystemTester:
    ...
    def load_test_data(self):
        ...
        try:
            ...
            for symbol in self.test_assets:
                try:
                    ...
                    if data is not None and len(data) > 0:
                        ...
                        logger.info(
                            f"Loaded {len(returns)} return observations for {symbol}"
                        )
                    else:
            ...
            if asset_data:
                ...
                logger.info(
                    f"Data period: {self.test_data.index[0]} to {self.test_data.index[-1]}"
                )
                return True
            else:
    ...
    def generate_synthetic_data(self):
        ...
        try:
            ...
            returns = np.random.multivariate_normal(
                ...
                cov = np.eye(num_assets) * 0.02
                + np.random.rand(num_assets, num_assets) * 0.01,
                size = num_periods,
            )
            ...
            self.test_data = pd.DataFrame(
                correlation_adjusted_returns, index = dates, columns = asset_names
            )
    ...
    def test_risk_parity_optimizer(self):
        ...
        try:
            ...
            logger.info(
                f"✓ Equal Risk Contribution: Sharpe={result_erc.sharpe_ratio:.3f}, Parity={result_erc.risk_contributions.parity_satisfaction:.2%}"
            )
            ...
            logger.info(
                f"✓ Risk Budgeting: Sharpe={result_rb.sharpe_ratio:.3f}, Budget Error={result_rb.risk_contributions.budget_error:.6f}"
            )
            ...
            logger.info(
                f"✓ Hierarchical Risk Parity: Sharpe={result_hrp.sharpe_ratio:.3f}"
            )
            ...
            self.test_results["risk_parity_optimizer"] = {
                "success": False,
                "error": str(e),
            }
            return False
    ...
    def test_risk_budgeting_framework(self):
        ...
        try:
            ...
            equal_budget = config.create_equal_risk_budget(
                list(self.test_data.columns), "test_equal_budget"
            )

            # 创建自定义风险预算
            custom_allocations = {
                asset: 1.0 / len(self.test_data.columns)
                for asset in self.test_data.columns
            }
            # 给第一个资产更高权重
            first_asset = self.test_data.columns[0]
            custom_allocations[first_asset] = 0.5
            custom_allocations = {
                k: v for k, v in sorted(custom_allocations.items(), key = lambda x: x[0])
            }

            custom_budget = config.create_custom_risk_budget(
                list(self.test_data.columns), custom_allocations, "test_custom_budget"
            )
            ...
            logger.info(
                f"✓ Risk Budgeting Framework: Satisfaction={allocation.budget_satisfaction:.2%}, Sharpe={allocation.sharpe_ratio:.3f}"
            )
    ...
    def test_risk_contribution_calculator(self):
        ...
        try:
            ...
            logger.info(
                f"✓ Risk Contribution Calculator: Vol={analysis.portfolio_volatility:.4f}, DivRatio={analysis.diversification_ratio:.3f}"
            )
    ...
    def test_risk_parity_backtester(self):
        ...
        try:
            ...
            logger.info(
                f"✓ Risk Parity Backtester: ERC Sharpe={result_erc.sharpe_ratio:.3f}, HRP Sharpe={result_hrp.sharpe_ratio:.3f}"
            )
    ...
    def test_integration_with_existing_components(self):
        ...
        try:
            ...
            logger.info(
                f"✓ Integration Test: MPT Sharpe={mpt_result.sharpe_ratio:.3f}, Risk Metrics Sharpe={metrics.sharpe_ratio:.3f}"
            )
        ...
        try:
            ...
            strategies = [
                ...
                (
                    ...
                    {"linkage_method": "ward", "name": "Hierarchical RP (Ward)"},
                ),
                (
                    "risk_budgeting",
                    {
                        "risk_allocations": {
                            asset: (
                                0.6
                                if i == 0
                                else 0.4 / (len(self.test_data.columns) - 1)
                            )
            ...
            self.test_results["comprehensive_comparison"] = {
                ...
                "best_strategy": (
                    comparison_results.iloc[0]["strategy_name"]
                    if len(comparison_results) > 0
                    else None
                ),
                "best_sharpe": (
                    comparison_results.iloc[0]["sharpe_ratio"]
                    if len(comparison_results) > 0
                    else None
                ),
            }
            ...
            logger.info(
                f"✓ Comprehensive Comparison: {len(comparison_results)} strategies tested"
            )
            if len(comparison_results) > 0:
                logger.info(
                    f"Best strategy: {comparison_results.iloc[0]['strategy_name']} (Sharpe: {comparison_results.iloc[0]['sharpe_ratio']:.3f})"
                )
    ...
    def generate_test_report(self):
        ...
        try:
            ...
            print(
                f"Success Rate: {report['summary']['successful_tests'] / report['summary']['total_tests'] * 100:.1f}%"
            )
            print("=" * 60)
