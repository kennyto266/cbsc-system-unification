...
def demo_alpha_factor_engine(market_data):
    ...
    try:
        ...
        print(
            summary[
                ["factor_name", "factor_type", "lookback_period", "data_points"]
            ].head(10)
        )
...
def demo_technical_to_alpha_conversion(market_data):
    ...
    try:
        ...
        alpha_factors = converter.convert_technical_to_alpha(
            data, indicator_names = selected_indicators, lookback_periods=[14, 20, 30]
        )
...
def demo_factor_validation(market_data, factors):
    ...
    try:
        ...
        validation_results = validator.validate_multiple_factors(
            factor_metrics_dict, price_data, returns_data
        )
...
def demo_alpha_lens_analysis(market_data, factors):
    ...
    try:
        from alpha.factor_analyzer.alpha_lens_analyzer import (
            AlphaLensAnalyzer,
            AlphaLensConfig,
        )
        ...
        config = AlphaLensConfig(
            quantiles = 5, periods=[1, 5, 10, 20], factor_name="Technical Alpha Factor"
        )
        ...
        tear_sheet_results = analyzer.create_tear_sheet(
            factor_data_formatted, price_data_formatted
        )
...
def demo_multi_factor_model(market_data, factors):
    ...
    try:
        ...
        config = FactorModelConfig(
            model_type = ModelType.LINEAR_REGRESSION, max_factors = 5, lookback_window = 60
        )
        ...
        # 選擇因子
        available_factors = {
            col: factors[[col]] for col in factors.columns[:8]
        }  # 使用前8個因子
        selected_factors = portfolio.select_factors(
            available_factors, criteria="composite"
        )
...
def demo_factor_investment_portfolio(market_data, factors):
    ...
    try:
        ...
        selected_stocks = portfolio_manager.select_stocks(
            factor_scores, available_stocks
        )
        ...
        print(
            f"  總暴露度: {performance_report['current_positions']['total_exposure']:.2%}"
        )
...
def main():
    ...
    # 3. 因子有效性檢驗演示
    demo_factor_validation(market_data, combined_factors)

    # 4. AlphaLens分析演示
    demo_alpha_lens_analysis(market_data, combined_factors)

    # 5. 多因子模型構建演示
    demo_multi_factor_model(market_data, combined_factors)

    # 6. 因子投資組合管理演示
    demo_factor_investment_portfolio(market_data, combined_factors)
