...
def example_1_basic_allocation():
    ...
    print(
        f"當前制度: {allocation.current_regime.regime_name if allocation.current_regime else '未檢測'}"
    )
...
def example_2_regime_detection():
    ...
    regime_config = RegimeConfig(
        n_regimes = 3, volatility_window = 20, trend_window = 50, prediction_horizon = 5
    )
...
def example_3_tactical_overlay():
    ...
    # 戰略權重（等權重）
    strategic_weights = {
        asset.symbol: 1.0 / len(asset_configs) for asset in asset_configs
    }

    # 應用戰術覆蓋
    print("應用戰術覆蓋...")
    overlay_result = overlay_system.apply_tactical_overlay(
        strategic_weights, market_data
    )
    ...
    for symbol, summary in overlay_result.signal_summary.items():
        print(
            f"  {symbol}: {summary['signal_count']}個信號, 平均置信度: {summary['avg_confidence']:.3f}"
        )
...
def example_4_full_backtest():
    ...
    scenario = BacktestScenario(
        ...
        overlay_config = OverlayConfig(
            max_overlay_adjustment = 0.1, min_signal_confidence = 0.6
        ),
        benchmark_weights={
            asset.symbol: 1.0 / len(asset_configs) for asset in asset_configs
        },
    ...
    # 運行回測
    print(
        f"運行回測: {scenario.start_date.strftime('%Y-%m-%d')} 到 {scenario.end_date.strftime('%Y-%m-%d')}"
    )
