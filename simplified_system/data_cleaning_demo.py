...
class DataCleaningToolsDemo:
    ...
    def _check_tools_availability(self):
        ...
        try:
            ...
            print(
                "[ERROR] ydata - profiling not available (install with: pip install ydata - profiling)"
            )
        ...
        try:
            ...
            print(
                "[ERROR] missingno not available (install with: pip install missingno)"
            )
    ...
    def basic_data_analysis(self):
        ...
        print(
            f"[INFO] Price range: {data['Close'].min():.2f} - {data['Close'].max():.2f} HKD"
        )
        ...
        print(
            f"[INFO] Missing values: {total_missing} ({total_missing / data.size * 100:.2f}%)"
        )
    ...
    def demonstrate_sweetviz(self):
        ...
        report = sv.compare(
            [self.stock_data, "Original Data"], [processed_data, "Processed Data"]
        )
    ...
    def run_full_demo(self):
        ...
        print(
            f"[{'SUCCESS' if self.tools_available['missingno'] else 'ERROR'}] Missing data analysis: {'Completed' if self.tools_available['missingno'] else 'Skipped'}"
        )
        print(
            f"[{'SUCCESS' if self.tools_available['ydata_profiling'] else 'ERROR'}] Profiling report: {'Generated' if self.tools_available['ydata_profiling'] else 'Skipped'}"
        )
        print(
            f"[{'SUCCESS' if self.tools_available['sweetviz'] else 'ERROR'}] Comparison report: {'Generated' if self.tools_available['sweetviz'] else 'Skipped'}"
        )
