...
class RealDataAlphaIntegration:
    ...
    def test_government_data_integration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            results["integration_score"] = (
                success_count / total_sources if total_sources > 0 else 0
            )
        ...
        try:
            ...
            if hibor_data and hibor_data["data"]:
                ...
                standardized_hibor = self.data_standardizer.standardize_data(
                    hibor_data["data"], DataSourceType.GOVERNMENT_DATA
                )
                ...
                if standardized_hibor is not None:
                    ...
                    logger.info(
                        f"HIBOR數據標準化成功: {len(standardized_hibor)} 條記錄"
                    )
                else:
                    results["standardization_results"]["hibor"] = {
                        "status": "failed",
            ...
            try:
                ...
                if stock_data is not None and len(stock_data) > 0:
                    ...
                    standardized_stock = self.data_standardizer.standardize_data(
                        stock_data.to_dict("records"), DataSourceType.STOCK_API
                    )
                    ...
                    if standardized_stock is not None:
                        ...
                        logger.info(
                            f"股票數據標準化成功: {len(standardized_stock)} 條記錄"
                        )
                    else:
                        results["standardization_results"]["stock"] = {
                            "status": "failed",
                    ...
                    results["errors"].append(
                        "No stock data available for standardization"
                    )
            ...
            success_count = sum(
                1
                for result in results["standardization_results"].values()
                if result["status"] == "success"
            )
            total_count = len(results["standardization_results"])

            results["standardization_success_rate"] = (
        ...
        try:
            ...
            if alpha_signals:
                results["signal_results"] = {
                    ...
                    "signal_types": (
                        list(alpha_signals.keys())
                        if isinstance(alpha_signals, dict)
                        else ["composite"]
                    ),
                    "latest_signal": (
                        alpha_signals[-1] if isinstance(alpha_signals, list) else "N / A"
                    ),
                }

                # 計算信號質量指標
                signal_quality = self._evaluate_signal_quality(
                    alpha_signals, stock_data
                )
                results["performance_metrics"] = signal_quality

                logger.info(
                    f"Alpha信號生成成功: {results['signal_results']['total_signals']} 個信號"
                )
            else:
                results["signal_results"] = {
                    "status": "failed",
    ...
    def _generate_real_data_alpha_signals(
        self,
        stock_data: pd.DataFrame,
        hibor_data: Optional[Dict],
        monetary_data: Optional[Dict],
    ) -> List[Dict]:
        """使用真實數據生成Alpha信號"""
        signals = []
            ...
            signals.append(
                {
                    ...
                    "signal": (
                        "BULLISH"
                        if momentum_20 > 5
                        else "BEARISH" if momentum_20 < -5 else "NEUTRAL"
                    ),
                    "weight": 0.2,
                    "data_source": "stock_price",
                }
                    ...
                    signals.append(
                        {
                            ...
                            "signal": (
                                "RATE_CUTTING"
                                if hibor_change < -0.1
                                else "RATE_HIKING" if hibor_change > 0.1 else "STABLE"
                            ),
                            "weight": 0.25,
                            "data_source": "hibor",
                        }
                ...
                signals.append(
                    {
                        ...
                        "signal": (
                            "LOOSE"
                            if hibor_overnight < 3.0
                            else "TIGHT" if hibor_overnight > 5.0 else "NEUTRAL"
                        ),
                        "weight": 0.15,
                        "data_source": "hibor",
                    }
                    ...
                    signals.append(
                        {
                            ...
                            "signal": (
                                "EXPANSIONARY"
                                if monetary_base > 2000
                                else (
                                    "CONTRACTIONARY"
                                    if monetary_base < 1500
                                    else "NEUTRAL"
                                )
                            ),
                            "weight": 0.1,
    ...
    def _assess_hibor_data_quality(self, hibor_record: Dict) -> Dict[str, Any]:
        ...
        missing_fields = [
            field for field in required_fields if hibor_record.get(field) is None
        ]
        ...
        # 檢查數值合理性
        if (
            hibor_record.get("overnight", 0) < 0
            or hibor_record.get("overnight", 0) > 20
        ):
            quality_score -= 20
        ...
        if (
            three_month
            and overnight
            and (three_month < overnight * 0.8 or three_month > overnight * 3)
        ):
            quality_score -= 15
    ...
    def _evaluate_signal_quality(
        self, signals: List[Dict], stock_data: pd.DataFrame
    ) -> Dict[str, Any]:
        """評估信號質量"""
        quality_metrics = {
            "signal_diversity": 0,
        ...
        try:
            ...
            quality_metrics["weight_distribution"] = (
                max(weights) - min(weights) if weights else 0
            )

            # 信號時效性（基於最新數據）
            latest_timestamp = (
                stock_data.index[-1] if not stock_data.empty else datetime.now()
            )
    ...
    def run_comprehensive_test(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if "standardization_success_rate" in std_result:
                scores.append(
                    std_result["standardization_success_rate"] * 0.3
                )  # 30% weight
    ...
    def _generate_recommendations(self, results: Dict) -> List[str]:
        ...
        try:
            ...
            if gov_result.get("integration_score", 0) < 1.0:
                failed_sources = [
                    name
                    for name, data in gov_result.get("data_sources", {}).items()
