...
class GPUDetector:
    ...
    def _detect_gpu_environment(self) -> GPUEnvironment:
        ...
        try:
            ...
            logger.info(
                f"GPU Environment detected: {env.gpu_count} GPUs, CUDA {env.cuda_version}"
            )
    ...
    def _detect_cudnn(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if torch.backends.cudnn.is_available():
                return {"available": True, "version": torch.backends.cudnn.version()}
        except Exception:
            pass
...
class PyTorchGPUBackend:
    ...
    def compute_indicators(self, prices, indicators_config):
        ...
        if not isinstance(prices, torch.Tensor):
            prices_tensor = torch.tensor(
                prices, dtype = torch.float32, device = self.device
            )
        else:
    ...
    def _calculate_rsi_torch(self, prices, period = 14):
        ...
        rs = avg_gain / torch.where(
            avg_loss == 0, torch.tensor(float("nan"), device = self.device), avg_loss
        )
        rsi = 100 - (100 / (1 + rs))
