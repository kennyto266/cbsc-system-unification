...
class PerformanceMonitor:
    ...
    def _collect_parallel_stats(self):
        ...
        try:
            ...
            self.parallel_stats = {
                ...
                "tasks_per_second": parallel_stats["task_statistics"][
                    "tasks_per_second"
                ],
            }
    ...
    def _collect_gpu_stats(self):
        ...
        try:
            ...
            self.gpu_stats = {
                ...
                "gpu_available": gpu_stats.get("gpu_environment", {}).get(
                    "available", False
                ),
            }
    ...
    def record_metric(
        self, name: str, value: float, unit: str = "", category: str = "general"
    ):
        """记录性能指标"""
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        if self.system_resources:
            ...
            avg_cpu = sum(r.cpu_percent for r in recent_resources) / len(
                recent_resources
            )
            ...
            if stats["baseline_time"]:
                ...
                if stats["improvement_percent"] is not None:
                    if stats["improvement_percent"] > 0:
                        report_lines.append(
                            f"  性能提升: {stats['improvement_percent']:.1f}% ✅"
                        )
                    else:
                        report_lines.append(
                            f"  性能下降: {abs(stats['improvement_percent']):.1f}% ❌"
                        )
    ...
    def compare_with_baseline(
        self, operation_name: str, current_time: float
    ) -> Dict[str, Any]:
        """与基准时间比较"""
