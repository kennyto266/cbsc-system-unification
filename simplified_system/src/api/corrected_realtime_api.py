...
class CorrectedRealtimeAPI:
    ...
    def fetch_monetary_base(self, limit: int = 1000) -> Optional[Dict[str, Any]]:
        ...
        try:
                ...
                for i, record in enumerate(recent_records):
                    try:
                        monetary_record = {
                            ...
                            "monetary_base": self._safe_float(
            ...
            logger.info(
                f"Successfully fetched {len(records)} {endpoint['name']} records"
            )
            return result
    ...
    def fetch_all_real_data(self, limit: int = 1000) -> Dict[str, Any]:
        ...
        for source_name, fetch_function in fetch_functions:
            try:
                ...
                if data and data.get("success"):
                    ...
                    logger.info(
                        f"✅ {source_name}: {record_count} records fetched successfully"
                    )
        ...
        logger.info(
            f"Fetch complete: {successful_sources}/{len(fetch_functions)} sources, {total_records} total records"
        )
...
if __name__ == "__main__":
    ...
    print(
        f"Summary: {all_data['summary']['successful_sources']}/{all_data['summary']['total_sources']} sources successful"
    )
    ...
    for source_name, source_data in all_data["data_sources"].items():
        if source_data.get("success"):
            ...
            print(
                f"   Records: {metadata.get('records_returned', 0)}/{metadata.get('total_records_available', 0)}"
            )
            if source_data.get("data"):
                print(
                    f"   Date Range: {source_data['data'][0].get('date')} to {source_data['data'][-1].get('date')}"
                )
        else:
    ...
    print(
        "from corrected_realtime_api import get_real_monetary_base, get_real_hibor_rates"
    )
    print()
