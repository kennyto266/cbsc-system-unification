...
class StressTestEngine:
    ...
    def _initialize_scenarios(self) -> List[StressScenario]:
        ...
        scenarios = [
            ...
            StressScenario(
                ...
                liquidity_adjustments={"market_liquidity": -0.4, "bid_ask_spread": 1.5},
                duration_days = 45,
                probability = 0.05,
            ),
    ...
    def _run_single_stress_test(
        self, portfolio_data: Dict[str, Any], scenario: StressScenario
    ) -> StressTestResult:
        """運行單個壓力測試情景"""
        try:
            ...
            # 應用流動性調整
            shocked_returns = self._apply_liquidity_adjustments(
                shocked_returns, scenario
            )
            ...
            recovery_time = self._estimate_recovery_time(
                cumulative_returns, scenario.duration_days
            )
            ...
            volatility_increase = (
                stressed_returns.std() / historical_returns.std()
                if historical_returns.std() > 0
                else 1.0
            )
            correlation_breakdown = scenario.stress_type in [
                StressTestType.HISTORICAL,
                StressTestType.EXTREME,
            ]
            ...
            logger.error(
                f"Single stress test failed for scenario {scenario.scenario_id}: {e}"
            )
            raise
    ...
    def _apply_market_shocks(
        ...
        if scenario.stress_type == StressTestType.HISTORICAL:
            shocked_returns = self._apply_historical_shock_pattern(
                shocked_returns, scenario
            )
        elif scenario.stress_type == StressTestType.HYPOTHETICAL:
            shocked_returns = self._apply_hypothetical_shock_pattern(
                shocked_returns, scenario
            )
        elif scenario.stress_type == StressTestType.FACTOR_SHOCK:
            shocked_returns = self._apply_factor_shock_pattern(
                shocked_returns, scenario
            )
        else:
            shocked_returns = self._apply_generic_shock_pattern(
                shocked_returns, scenario
            )
    ...
    def _apply_historical_shock_pattern(
        ...
        for i in range(min(n_days, len(returns))):
            ...
            random_component = np.random.normal(
                0, returns.std() * volatility_factor * day_factor
            )

            shocked_returns.iloc[i] = (
                returns.iloc[i] + shock_component + random_component
            )
    ...
    def _apply_hypothetical_shock_pattern(
        ...
        for i in range(min(n_days, len(returns))):
            ...
            # 基礎市場沖擊
            base_shock = (
                scenario.market_shocks.get("equity_market", 0)
                * shock_intensity
                / n_days
            )
            ...
            if "interest_rates" in scenario.market_shocks:
                rate_shock = (
                    scenario.market_shocks["interest_rates"]
                    * shock_intensity
                    * 0.5
                    / n_days
                )
            ...
            random_shock = np.random.normal(
                0, returns.std() * volatility_factor * shock_intensity
            )
    ...
    def _apply_factor_shock_pattern(
        ...
        if "volatility" in scenario.market_shocks:
            ...
            for i in range(min(n_days, len(returns))):
                ...
                # 添加均值回歸傾向
                shocked_returns.iloc[i] += np.random.normal(
    ...
    def _calculate_stressed_portfolio_returns(
        ...
        for _, position in positions.iterrows():
            ...
            if asset_type.lower() in ["equity", "stock"]:
                ...
                asset_returns = (
                    shocked_returns * 0.3 + bond_shock / scenario.duration_days
                )
            elif asset_type.lower() in ["commodity"]:
                # 商品通常下跌
                commodity_shock = scenario.market_shocks.get("commodities", -0.25)
                asset_returns = (
                    shocked_returns * 0.8 + commodity_shock / scenario.duration_days
                )
            else:
                asset_returns = shocked_returns
    ...
    def _estimate_recovery_time(
        self, cumulative_returns: pd.Series, stress_duration_days: int
    ) -> int:
        """估算恢復時間"""
        # 找到最大回撤點
        ...
        # 計算恢復時間
        recovery_mask = (
            cumulative_returns.loc[max_drawdown_idx:]
            >= cumulative_returns.loc[max_drawdown_idx]
        )
    ...
    def _calculate_liquidity_impact(
        self, positions: pd.DataFrame, scenario: StressScenario
    ) -> float:
        """計算流動性影響"""
        ...
        for _, position in positions.iterrows():
            ...
            if asset_type in ["equity", "stock"]:
                liquidity_adjustment = scenario.liquidity_adjustments.get(
                    "equity_liquidity", -0.4
                )
    ...
    def _calculate_sector_impacts(
        ...
        for sector, sector_positions in positions.groupby("sector"):
            ...
            sector_shock = sector_shocks.get(
                sector, sector_shocks.get("equity_market", -0.15)
            )
    ...
    def _generate_stress_test_report(
        self, portfolio_data: Dict[str, Any], detailed_results: List[StressTestResult]
    ) -> StressTestReport:
        """生成壓力測試報告"""
        ...
        loss_percentages = [
            result.portfolio_loss_percentage for result in detailed_results
        ]

        # 找到最壞情況
        worst_case_result = max(
            detailed_results, key = lambda x: x.portfolio_loss_percentage
        )
        ...
        stress_expected_shortfall = np.mean(
            [l for l in loss_percentages if l <= stress_var]
        )
    ...
    def _calculate_portfolio_resilience(
        self, detailed_results: List[StressTestResult]
    ) -> float:
        """計算投資組合韌性評分"""
        ...
        avg_loss_percentage = np.mean(
            [r.portfolio_loss_percentage for r in detailed_results]
        )
        ...
        avg_liquidity_impact = np.mean(
            [abs(r.liquidity_impact) for r in detailed_results]
        )
        liquidity_resilience = max(
            0, 1 - avg_liquidity_impact / 0.5
        )  # 假設50%流動性影響為0分
        resilience_factors.append(liquidity_resilience * 0.15)

        # 5. 波動率增加控制 (權重10%) - 波動率增加越少韌性越高
        avg_volatility_increase = np.mean(
            [r.volatility_increase for r in detailed_results]
        )
        volatility_resilience = max(
            0, 1 - (avg_volatility_increase - 1) / 3
        )  # 假設4倍波動率為0分
        resilience_factors.append(volatility_resilience * 0.1)

        return sum(resilience_factors) * 100  # 轉換為100分制

    def _calculate_stress_risk_concentration(
        self, detailed_results: List[StressTestResult]
    ) -> Dict[str, float]:
        """計算壓力下的風險集中度"""
        if not detailed_results:
    ...
    def run_monte_carlo_stress_test(
        ...
        try:
            ...
            logger.info(
                f"Starting Monte Carlo stress test with {num_simulations} simulations"
            )

            portfolio_data.get("current_value", 1000000)
            portfolio_data.get("returns", pd.Series())
            ...
            mc_results = {
                "monte_carlo_var": np.percentile(
                    simulated_losses, (1 - confidence_level) * 100
                ),
                "monte_carlo_expected_shortfall": np.mean(
                    simulated_losses[
                        simulated_losses
                        <= np.percentile(simulated_losses, (1 - confidence_level) * 100)
                    ]
                ),
                ...
                "tail_risk_measures": {
                    "conditional_var_95": np.mean(
                        simulated_losses[
                            simulated_losses <= np.percentile(simulated_losses, 5)
                        ]
                    ),
                    "conditional_var_99": np.mean(
                        simulated_losses[
                            simulated_losses <= np.percentile(simulated_losses, 1)
                        ]
                    ),
    ...
    def _generate_random_stress_scenario(self, seed: int) -> StressScenario:
        ...
        if primary_shock == "equity_market":
            market_shocks["equity_market"] = np.random.uniform(
                -0.15, -0.45
            )  # 15% to 45% drop
            market_shocks["volatility"] = np.random.uniform(1.5, 3.0)
        elif primary_shock == "interest_rate":
            market_shocks["interest_rates"] = np.random.uniform(
                0.02, 0.04
            )  # 200 - 400 bps
    ...
    def _generate_monte_carlo_recommendations(
        self, simulated_losses: np.ndarray
    ) -> List[str]:
        """生成蒙特卡羅壓力測試建議"""
        recommendations = []
...
def quick_stress_test(
    portfolio_value: float, returns: pd.Series, scenario_ids: Optional[List[str]] = None
) -> Dict[str, Any]:
    """便利函數：快速壓力測試"""
