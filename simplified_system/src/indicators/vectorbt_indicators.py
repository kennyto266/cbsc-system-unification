...
class VectorBTIndicators:
    ...
    def batch_calculate_rsi(
        self, prices: pd.DataFrame, periods: List[int] = None
    ) -> pd.DataFrame:
        """
        批量计算RSI指标
        ...
        logger.info(
            f"Batch calculating RSI for {len(periods)} periods across {prices.shape[1]} assets"
        )
        ...
        if VECTORBT_AVAILABLE:
            ...
            for period in periods:
                ...
                for asset in prices.columns:
                    asset_rsi = self.core_indicators.calculate_rsi(
                        prices[asset], period
                    )
    ...
    def batch_calculate_macd(
        ...
        logger.info(
            f"Batch calculating MACD with {len(fast_periods)}x{len(slow_periods)}x{len(signal_periods)} combinations"
        )
            ...
            for fast in fast_periods:
                for slow in slow_periods:
                    for signal in signal_periods:
                        ...
                        macd_results["histogram"].append(
                            pd.concat(histogram_values, axis = 1)
                        )

            return macd_results

    def vectorized_cross_indicator_analysis(
        self, prices: pd.DataFrame, indicator_configs: Dict[str, Dict]
    ) -> Dict[str, Any]:
        """
        向量化交叉指标分析
        ...
        """
        logger.info(
            f"Performing cross - indicator analysis with {len(indicator_configs)} indicators"
        )
        ...
        for name, config in indicator_configs.items():
            if config["type"] == "rsi":
                ...
                indicators = self.batch_calculate_bollinger_bands(
                    prices, config.get("periods", [20]), config.get("std_devs", [2.0])
                )
            else:
                continue
        ...
        # 性能归因分析
        attribution = self._analyze_indicator_attribution(
            all_indicators, signals, prices
        )
    ...
    def batch_calculate_bollinger_bands(
        self,
        prices: pd.DataFrame,
        ...
        logger.info(
            f"Batch calculating Bollinger Bands with {len(periods)}x{len(std_devs)} combinations"
        )

        if VECTORBT_AVAILABLE:
            # 使用VectorBT批量布林带计算
            bb_indicator = vbt.BBANDS.run(prices, window = periods, std = std_devs)
    ...
    def adaptive_parameter_optimization(
        ...
        for start_idx in range(
            0, len(prices) - optimization_window, rebalance_frequency
        ):
    ...
    def calculate_indicator_performance_attribution(
        self,
        prices: pd.DataFrame,
    ...
    def _calculate_indicator_correlations(
        ...
        if len(indicator_series) > 1:
            ...
            correlations["summary"] = {
                "avg_correlation": corr_df.corr()
                .values[np.triu_indices_from(corr_df.corr().values, k = 1)]
                .mean(),
                "max_correlation": corr_df.corr()
                .values[np.triu_indices_from(corr_df.corr().values, k = 1)]
                .max(),
                "min_correlation": corr_df.corr()
                .values[np.triu_indices_from(corr_df.corr().values, k = 1)]
                .min(),
            }
    ...
    def _generate_cross_indicator_signals(
        self, indicators: Dict[str, Any], prices: pd.DataFrame
    ) -> Dict[str, pd.Series]:
        """生成交叉指标信号"""
        signals = {}
        ...
        if "MACD" in indicators:
            ...
            if isinstance(macd_data, dict) and "macd" in macd_data:
                ...
                if isinstance(macd_line, pd.DataFrame) and isinstance(
                    signal_line, pd.DataFrame
                ):
                    for col in macd_line.columns:
                        if col in signal_line.columns:
                            # MACD金叉死叉信号
                            golden_cross = (macd_line[col] > signal_line[col]).astype(
                                int
                            )
                            death_cross = (macd_line[col] < signal_line[col]).astype(
                                int
                            )
    ...
    def _analyze_indicator_attribution(
        self,
        ...
        for indicator_name in indicators.keys():
            indicator_signals = {
                k: v for k, v in signals.items() if indicator_name in k
            }
            ...
            if indicator_signals:
                ...
                avg_signal_strength = np.mean(
                    [signal.mean() for signal in indicator_signals.values()]
                )
                    ...
                    if len(signal) == len(price_changes):
                        ...
                        for asset in price_changes.columns:
                            correlation = np.corrcoef(
                                signal.fillna(0), price_changes[asset]
                            )[0, 1]
                            if not np.isnan(correlation):
                                signal_correlations[f"{signal_name}_{asset}"] = (
                                    correlation
                                )
                ...
                attribution[indicator_name] = {
                    ...
                    "max_correlation": (
                        max(signal_correlations.values()) if signal_correlations else 0
                    ),
                    "avg_correlation": (
                        np.mean(list(signal_correlations.values()))
    ...
    def _evaluate_parameters(
        ...
        """评估参数性能"""
        if indicator_type == "rsi":
            rsi = self.core_indicators.calculate_rsi(
                prices.iloc[:, 0], params["period"]
            )
            signals = pd.Series(0, index = prices.index)
            signals[rsi < 30] = 1
            signals[rsi > 70] = -1
    ...
    def _summarize_optimization_results(
        ...
        for key in optimal_params[0].keys():
            ...
            param_stats[key] = {
                ...
                "max": max(values),
                "stability": (
                    1 - (np.std(values) / np.mean(values)) if np.mean(values) > 0 else 0
                ),
            }
...
def test_vectorbt_indicators():
    ...
    cross_analysis = vbt_indicators.vectorized_cross_indicator_analysis(
        prices_df, indicator_configs
    )
    ...
    logger.info(
        f"Adaptive optimization completed: {adaptive_results['average_performance']:.3f}"
    )
