...
class AdvancedTechnicalSignals:
    ...
    def generate_enhanced_signals(
        ...
        try:
            logger.info(
                f"Generating enhanced signals for {data_type} data with {len(data)} records"
            )
            ...
            # 4. 生成多時間框架信號
            timeframes_result = self._generate_multi_timeframe_signals(
                df, indicators_result["indicators"]
            )

            # 5. 模式識別
            patterns_result = self._identify_trading_patterns(
                df, indicators_result["indicators"]
            )
            ...
            result = {
                ...
                "recommendations": self._generate_trading_recommendations(
                    composite_result, risk_assessment
                ),
            }

            logger.info(
                f"Enhanced signals generated successfully: {result['signal_quality_score']:.2f}/10"
            )
            return result
    ...
    def _preprocess_data(
        self, data: List[Dict[str, Any]], extend_to_1000: bool
    ) -> List[Dict[str, Any]]:
        """數據預處理和擴展"""
        try:
            if not data:
            ...
            if extend_to_1000 and len(data) < 1000:
                ...
                if extension_result.get("success"):
                    logger.info(
                        f"Data extension successful: {extension_result['original_count']} -> {extension_result['final_count']}"
                    )
                    return extension_result["data"]
                else:
                    logger.warning(
                        f"Data extension failed, using original data: {extension_result.get('error')}"
                    )
    ...
    def _calculate_advanced_indicators(
        ...
        try:
            ...
            # 計算跨指標信號
            cross_indicator_signals = self._calculate_cross_indicator_signals(
                df, indicators
            )
            ...
            return {
                ...
                "total_indicators": len(
                    [indicators[col] for col in indicators if col in numeric_columns]
                ),
            }
    ...
    def _calculate_cross_indicator_signals(
        ...
        try:
            ...
            for column in indicators:
                ...
                # RSI與MACD結合信號
                if any("rsi" in key for key in column_indicators.keys()) and any(
                    "macd" in key for key in column_indicators.keys()
                ):
                ...
                # 布林帶與RSI結合信號
                if any("bb_" in key for key in column_indicators.keys()) and any(
                    "rsi" in key for key in column_indicators.keys()
                ):

                    bb_keys = [
                        k
                        for k in column_indicators.keys()
                        if "bb_" in k and "middle" in k
                    ]
                    rsi_keys = [k for k in column_indicators.keys() if "rsi" in k]
                    ...
                    if bb_keys and rsi_keys:
                        ...
                        if (
                            price_series.iloc[-1] < bb_middle.iloc[-1]
                            and latest_rsi < 30
                        ):
                ...
                # 隨機指標與威廉氏%R結合
                if any("stoch" in key for key in column_indicators.keys()) and any(
                    "williams" in key for key in column_indicators.keys()
                ):

                    stoch_keys = [
                        k
                        for k in column_indicators.keys()
                        if "stoch_" in k and "k" in k
                    ]
                    williams_keys = [
                        k for k in column_indicators.keys() if "williams" in k
                    ]
                    ...
                    if stoch_keys and williams_keys:
                        ...
                        latest_stoch = (
                            stoch_series.iloc[-1]
                            if hasattr(stoch_series, "iloc") and len(stoch_series) > 0
                            else 50
                        )
                        williams_series = column_indicators[williams_keys[0]]
                        latest_williams = (
                            williams_series.iloc[-1]
                            if hasattr(williams_series, "iloc")
                            and len(williams_series) > 0
                            else -50
                        )
            ...
            return {
                ...
                "total_cross_signals": sum(
                    len(signals) for signals in cross_signals.values()
                ),
            }
    ...
    def _generate_multi_timeframe_signals(
        ...
        try:
            ...
            for column in indicators:
                ...
                # 短期信號 (1 - 2週)
                short_signals = self._generate_short_term_signals(
                    df[column], indicators[column]
                )
                timeframe_signals[column]["short_term"] = short_signals

                # 中期信號 (1個月)
                medium_signals = self._generate_medium_term_signals(
                    df[column], indicators[column]
                )
                timeframe_signals[column]["medium_term"] = medium_signals

                # 長期信號 (3個月+)
                long_signals = self._generate_long_term_signals(
                    df[column], indicators[column]
                )
                timeframe_signals[column]["long_term"] = long_signals

                # 時間框架一致性檢查
                consistency_score = self._calculate_timeframe_consistency(
    ...
    def _identify_trading_patterns(
        ...
        try:
            ...
            for column in indicators:
                ...
                # 支撐阻力位
                patterns[column]["support_resistance"] = (
                    self._identify_support_resistance_levels(
                        pd.DataFrame({column: series}), indicators[column]
                    )
                )

                # 趨勢線模式
                patterns[column]["tline_patterns"] = self._identify_trendline_patterns(
                    series
                )

                # 圖形模式
                patterns[column]["chart_patterns"] = self._identify_chart_patterns(
                    series
                )

                # 價格模式
                patterns[column]["price_patterns"] = self._identify_price_patterns(
                    series
                )

                # 波動率模式
                patterns[column]["volatility_patterns"] = (
                    self._identify_volatility_patterns(series)
                )
    ...
    def _generate_composite_signals(
        ...
        try:
            ...
            for column in indicators_result["indicators"]:
                ...
                # 收集所有信號
                all_signals = {
                    "indicators": self._extract_indicator_signals(
                        indicators_result["indicators"][column]
                    ),
                    "timeframes": self._extract_timeframe_signals(
                        timeframes_result.get(column, {})
                    ),
                    "patterns": self._extract_pattern_signals(
                        patterns_result.get(column, {})
                    ),
                }
                ...
                if "weighted_scores" in composite_signals[column]:
                    composite_signals[column]["smoothed_signals"] = (
                        self._smooth_signals(
                            composite_signals[column]["weighted_scores"]
                        )
                    )
                else:
                    composite_signals[column]["smoothed_signals"] = []
                ...
                # 最終信號
                composite_signals[column]["final_signal"] = self._generate_final_signal(
                    composite_signals[column]
                )
    ...
    def _assess_signal_risk(
        self, composite_signals: Dict, df: pd.DataFrame
    ) -> Dict[str, Any]:
        """評估信號風險"""
        try:
            risk_assessment = {}
        ...
        try:
            ...
            if "risk_assessment" in composite_result:
                risk_scores = [
                    ...
                    for risk in composite_result["risk_assessment"].values()
                ]
                if risk_scores:
            ...
            if "multi_timeframe_signals" in composite_result:
                consistency_scores = [
                    tf.get("consistency_score", 0)
                    for tf in composite_result["multi_timeframe_signals"].values()
                ]
    ...
    def _generate_trading_recommendations(
        self, composite_result: Dict, risk_assessment: Dict
    ) -> List[str]:
        """生成交易建議"""
        try:
            recommendations = []
            ...
            if risk_assessment:
                avg_risk = np.mean(
                    [
                        risk.get("overall_risk_score", 0.5)
                        for risk in risk_assessment.values()
                    ]
                )
                if avg_risk < 0.3:
    ...
    def _calculate_stochastic(
        self, series: pd.Series, k_period: int, d_period: int
    ) -> Dict[str, pd.Series]:
        """計算隨機指標"""
        # 使用指數移動平均作為簡化實現
    ...
    def _calculate_cci(self, series: pd.Series, period: int) -> pd.Series:
        """計算CCI"""
        tp = (
            series.rolling(window = period, min_periods = 1).mean()
            + series.rolling(window = period, min_periods = 1).max()
            + series.rolling(window = period, min_periods = 1).min()
        ) / 3
        mad = series.rolling(window = period, min_periods = 1).apply(
    ...
    def _extract_indicator_signals(self, indicators: Dict) -> Dict[str, float]:
        ...
        for indicator_name, indicator_data in indicators.items():
            if indicator_name.startswith("rsi_"):
                rsi_values = [
                    indicator_data.iloc[-1] if len(indicator_data) > 0 else 50
                ]
    ...
    def _calculate_weighted_signal_scores(
        ...
        for signal_name, signal_value in signals.items():
            ...
            if not isinstance(signal_value, (int, float)):
                try:
                    signal_value = (
                        float(signal_value) if signal_value is not None else 0.0
                    )
        ...
        # 計算加權平均值
        if total_weight > 0:
            weighted_scores["weighted_average"] = (
                sum(weighted_scores.values()) / total_weight
            )
    ...
    def _extract_signals_from_indicators(self, indicators: Dict) -> Dict[str, float]:
        ...
        for indicator_name, indicator_data in indicators.items():
            if isinstance(indicator_data, dict):
                if "macd" in indicator_data and isinstance(
                    indicator_data["macd"], pd.Series
                ):
    ...
    # 缺失的方法 - 添加完整功能
    def _generate_short_term_signals(
        self, series: pd.Series, indicators: Dict
    ) -> Dict[str, float]:
        """生成短期信號 (1 - 2週)"""
        try:
            signals = {}
    ...
    def _generate_medium_term_signals(
        self, series: pd.Series, indicators: Dict
    ) -> Dict[str, float]:
        """生成中期信號 (1個月)"""
        try:
            ...
            if any("bb_" in key for key in indicators.keys()):
                ...
                if bb_upper_keys and bb_lower_keys:
                    ...
                    if (
                        hasattr(upper_series, "iloc")
                        and hasattr(lower_series, "iloc")
                        and len(upper_series) > 0
                        and len(lower_series) > 0
                    ):
    ...
    def _generate_long_term_signals(
        self, series: pd.Series, indicators: Dict
    ) -> Dict[str, float]:
        """生成長期信號 (3個月+)"""
        try:
                ...
                if pd.api.types.is_numeric_dtype(df[column]):
                    ...
                    if len(series) >= 20:
                        ...
                        for i in range(2, len(lows) - 2):
                            if (
                                lows.iloc[i] == series.iloc[i - 2 : i + 3].min()
                                and lows.iloc[i]
                                < series.iloc[i - 2 : i + 3].mean() * 0.98
                            ):
                        ...
                        for i in range(2, len(highs) - 2):
                            if (
                                highs.iloc[i] == series.iloc[i - 2 : i + 3].max()
                                and highs.iloc[i]
                                > series.iloc[i - 2 : i + 3].mean() * 1.02
                            ):
                                resistance_candidates.append(highs.iloc[i])

                        # 選擇最重要的支撐阻力位
                        if support_candidates:
                            support_levels = sorted(support_candidates)[
                                :3
                            ]  # 最低的3個支撐位
                            result["support_levels"].extend(
                                [(column, level) for level in support_levels]
                            )

                        if resistance_candidates:
                            resistance_levels = sorted(
                                resistance_candidates, reverse = True
                            )[
                                :3
                            ]  # 最高的3個阻力位
                            result["resistance_levels"].extend(
                                [(column, level) for level in resistance_levels]
                            )
                        ...
                        if result["support_levels"] and result["resistance_levels"]:
                            avg_support = np.mean(
                                [level[1] for level in result["support_levels"][-2:]]
                            )  # 最近2個支撐位
                            avg_resistance = np.mean(
                                [level[1] for level in result["resistance_levels"][-2:]]
                            )  # 最近2個阻力位
    ...
    def _calculate_timeframe_consistency(
        self, short_signals: Dict, medium_signals: Dict, long_signals: Dict
    ) -> float:
        """計算時間框架信號一致性"""
        try:
            all_signals = []
...
def generate_advanced_signals(
    data: List[Dict[str, Any]], data_type: str = "government"
) -> Dict[str, Any]:
    """生成高級技術分析信號"""
