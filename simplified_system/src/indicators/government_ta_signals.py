...
try:
    from complete_realtime_api import complete_api, get_all_government_data
    from enhanced_realtime_api_fixed import fetch_enhanced_data

    api_available = True
except ImportError:
    print(
        "Warning: Could not import API modules. Please ensure the API modules are available."
    )
...
class GovernmentTASignals:
    ...
    def generate_monetary_base_signals(
        self, monetary_data: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """基於貨幣基礎數據生成技術分析信號"""
        ...
        try:
            ...
            if "m1" in df.columns and "m2" in df.columns:
                ...
                df["m1_m2_ma_20"] = (
                    df["m1_m2_ratio"].rolling(window = 20, min_periods = 1).mean()
                )
                df["m1_m2_ma_60"] = (
                    df["m1_m2_ratio"].rolling(window = 60, min_periods = 1).mean()
                )

                # M1 / M2比率突破信號
                signals["signal_types"]["m1_m2_breakout"] = (
                ...
                # 貨幣供應變化率信號
                signals["signal_types"]["money_supply_change"] = (
                    self._generate_change_signals(df["monetary_base"], "貨幣基礎變化率")
                )

            # 2. 貨幣基礎水平信號
            if "monetary_base" in df.columns:
                df["monetary_base_ma_20"] = (
                    df["monetary_base"].rolling(window = 20, min_periods = 1).mean()
                )
                df["monetary_base_ma_60"] = (
                    df["monetary_base"].rolling(window = 60, min_periods = 1).mean()
                )
                df["monetary_base_zscore"] = self._calculate_zscore(
                    df["monetary_base"], 60
                )

                # 貨幣基礎異常信號
                signals["signal_types"]["monetary_base_anomaly"] = (
                    self._generate_anomaly_signals(
                        df["monetary_base_zscore"], "貨幣基礎異常"
                    )
                )

                # 貨幣基礎趨勢信號
                signals["signal_types"]["monetary_base_trend"] = (
            ...
            if "m3" in df.columns:
                ...
                signals["signal_types"]["m3_expansion"] = (
                    self._generate_expansion_signals(
    ...
    def generate_liquidity_signals(
        ...
        try:
            ...
            if "liquidity_ratio" in df.columns:
                df["liquidity_ma_10"] = (
                    df["liquidity_ratio"].rolling(window = 10, min_periods = 1).mean()
                )
                df["liquidity_ma_20"] = (
                    df["liquidity_ratio"].rolling(window = 20, min_periods = 1).mean()
                )
                df["liquidity_std"] = (
                    df["liquidity_ratio"].rolling(window = 20, min_periods = 1).std()
                )

                # 計算流動性緊張指標
                df["liquidity_tightness"] = (
                    df["liquidity_ratio"] - df["liquidity_ma_20"]
                ) / df["liquidity_std"]

                # 流動性緊張信號
                signals["signal_types"]["liquidity_tightness"] = (
                    self._generate_extreme_signals(
            ...
            if "interbank_offer_rate" in df.columns:
                df["rate_ma_5"] = (
                    df["interbank_offer_rate"].rolling(window = 5, min_periods = 1).mean()
                )
                df["rate_ma_20"] = (
                    df["interbank_offer_rate"].rolling(window = 20, min_periods = 1).mean()
                )

                # 利率變化信號
                df["rate_volatility"] = (
                    df["interbank_offer_rate"].rolling(window = 20, min_periods = 1).std()
                )

                signals["signal_types"]["rate_volatility"] = (
    ...
    def generate_rmb_liquidity_signals(
        self, rmb_data: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """基於人民幣流動資金數據生成技術分析信號"""
        if not rmb_data:
        ...
        try:
            ...
            if "rmb_liquidity_facility" in df.columns:
                ...
                signals["signal_types"]["rmb_activity"] = (
                    self._generate_activity_signals(
    ...
    def generate_economic_signals(
        self, economic_data: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """基於經濟統計數據生成技術分析信號"""
        ...
        try:
            ...
            if "composite_cpi" in df.columns:
                df["cpi_ma_12"] = (
                    df["composite_cpi"].rolling(window = 12, min_periods = 1).mean()
                )
                df["cpi_change"] = df["composite_cpi"].pct_change(12)  # 年同比

                # 通脹趨勢信號
                signals["signal_types"]["inflation_trend"] = (
                    self._generate_inflation_signals(df["cpi_change"], "CPI通脹趨勢")
                )

                # 通脹壓力信號
                signals["signal_types"]["inflation_pressure"] = (
                    self._generate_pressure_signals(df["cpi_change"], "通脹壓力")
                )

            # 2. 失業率趨勢信號
            if "unemploy_rate" in df.columns:
                df["unemployment_ma_6"] = (
                    df["unemploy_rate"].rolling(window = 6, min_periods = 1).mean()
                )
        ...
        try:
            ...
            for source_name, signal_data in all_signals.items():
                if signal_data.get("success"):
                    ...
                    weighted_score += quality_score * source_weights.get(
                        source_name, 0.1
                    )
            ...
            composite_signals["recommendations"] = self._generate_recommendations(
                composite_signals["individual_scores"],
                composite_signals["composite_score"],
            )
    ...
    def _generate_breakout_signals(
        self, data: pd.Series, ma_data: pd.Series, name: str
    ) -> Dict[str, Any]:
        """生成突破信號"""
        buy_signals = []
        sell_signals = []
                    ...
                    buy_signals.append(
                        {
                            ...
                            "strength": (
                                "STRONG"
                                if data.iloc[i] > ma_data.iloc[i] * 1.05
                                else "MODERATE"
                            ),
                            "value": data.iloc[i],
                    ...
                    sell_signals.append(
                        {
                            ...
                            "strength": (
                                "STRONG"
                                if data.iloc[i] < ma_data.iloc[i] * 0.95
                                else "MODERATE"
                            ),
                            "value": data.iloc[i],
                            "ma_value": ma_data.iloc[i],
    ...
    def _generate_trend_signals(self, ma_data: pd.Series, name: str) -> Dict[str, Any]:
        ...
        trend_direction = (
            "UP" if slope > 0.001 else "DOWN" if slope < -0.001 else "SIDEWAYS"
        )
    ...
    def _generate_anomaly_signals(
        self, zscore_data: pd.Series, name: str
    ) -> Dict[str, Any]:
        """生成異常信號"""
        signals = []
        threshold = 2.0  # 2個標準差
    ...
    def _generate_expansion_signals(
        self, change_rate: pd.Series, name: str
    ) -> Dict[str, Any]:
        """生成擴張信號"""
        expansion_periods = []
    ...
    def _generate_volatility_signals(
        self, volatility_data: pd.Series, name: str
    ) -> Dict[str, Any]:
        """生成波動性信號"""
        high_vol_periods = []
        threshold = (
            volatility_data.quantile(0.8)
            if len(volatility_data) > 10
            else volatility_data.std()
        )
        ...
        return {
            ...
            "volatility_regime": (
                "HIGH"
                if len(high_vol_periods) / len(volatility_data) > 0.3
                else "NORMAL"
            ),
        }

    def _generate_activity_signals(
        self, change_data: pd.Series, name: str
    ) -> Dict[str, Any]:
        """生成活動性信號"""
        activity_levels = []
                ...
                activity_levels.append(
                    {
                        ...
                        "period": (
                            "HIGH_ACTIVITY" if abs(change) > threshold * 2 else "NORMAL"
                        ),
                    }
                )
        ...
        return {
            ...
            "activity_regime": (
                "ACTIVE" if len(activity_levels) / len(change_data) > 0.3 else "NORMAL"
            ),
        }

    def _generate_inflation_signals(
        self, cpi_change: pd.Series, name: str
    ) -> Dict[str, Any]:
        """生成通脹趨勢信號"""
        ...
        return {
            ...
            "inflation_regime": (
                "HIGH"
                if avg_inflation > 0.04
                else "MODERATE" if avg_inflation > 0.02 else "LOW"
            ),
        }

    def _generate_pressure_signals(
        self, cpi_change: pd.Series, name: str
    ) -> Dict[str, Any]:
        """生成通脹壓力信號"""
        pressure_level = (
            "HIGH"
            if cpi_change.mean() > 0.05
            else "MODERATE" if cpi_change.mean() > 0.02 else "LOW"
        )
        ...
        for i, change in enumerate(cpi_change):
            if change > warning_threshold:
                warning_periods.append(
                    {"index": i, "inflation_rate": change, "warning_level": "HIGH"}
                )
    ...
    def _generate_labor_signals(
        self, trend_data: pd.Series, name: str
    ) -> Dict[str, Any]:
        """生成就業市場信號"""
        if len(trend_data) < 5:
        ...
        recent_trend = (
            trend_data.iloc[-5:].mean() if len(trend_data) >= 5 else avg_trend
        )

        market_condition = (
            "TIGHT" if avg_trend < -0.5 else "RELAXED" if avg_trend < 0 else "NORMAL"
        )
    ...
    def _calculate_r_squared(
        self, y_data: pd.Series, x_data: np.ndarray, slope: float, intercept: float
    ) -> float:
        """計算R²"""
    ...
    def _generate_recommendations(
        self, individual_scores: Dict[str, Any], composite_score: float
    ) -> List[str]:
        """生成交易建議"""
        recommendations = []
            ...
            if signal_data.get("success"):
                matrix[source_name] = {
                    ...
                    "has_buy_signals": bool(
                        signal_data.get("signal_types", {}).get("buy_signals")
                    ),
                    "has_sell_signals": bool(
                        signal_data.get("signal_types", {}).get("sell_signals")
                    ),
                    "has_trend_signals": bool(
                        signal_data.get("signal_types", {}).get("m1_m2_trend")
                    ),
                    "quality_score": signal_data.get("metadata", {}).get(
                        "record_quality", "unknown"
                    ),
                }
    ...
    def get_real_government_data(
        self, source_name: Optional[str] = None, limit: int = 1000
    ) -> Dict[str, Any]:
        """獲取真實政府數據"""
        try:
            # 檢查緩存
    ...
    def generate_signals_from_real_data(
        ...
        try:
            ...
            if not real_data_response or not real_data_response.get("success"):
                return {
                    "success": False,
                    "error": f"Failed to fetch real data for {source_name}",
                }
            ...
            if not real_data:
                return {
                    "success": False,
                    "error": f"No real data available for {source_name}",
                }
            ...
            logger.error(
                f"Error generating signals from real data for {source_name}: {e}"
            )
            return {"success": False, "error": str(e)}
    ...
    def generate_all_signals_from_real_data(self, limit: int = 1000) -> Dict[str, Any]:
        ...
        try:
            ...
            if not all_real_data.get("success"):
                return {
                    "success": False,
                    "error": "Failed to fetch real government data",
                }
            ...
            # 處理每個數據源
            for source_name, source_data in all_real_data.get(
                "data_sources", {}
            ).items():
                if source_data.get("success"):
                    ...
                    if real_records:
                        # 生成該數據源的技術分析信號
                        signals = self.generate_signals_from_real_data(
                            source_name, limit
                        )
                        if signals.get("success"):
                            ...
                            logger.info(
                                f"✅ Generated signals for {source_name}: {len(real_records)} records"
                            )
                        else:
                            logger.warning(
                                f"❌ Failed to generate signals for {source_name}"
                            )
                    else:
            ...
            # 生成綜合信號
            composite_signals = (
                self.generate_composite_signal(all_signals) if all_signals else {}
            )
            ...
            return {
                ...
                "metadata": {
                    "records_per_source": {
                        ...
                        for name, data in all_real_data.get("data_sources", {}).items()
                        if data.get("success")
                    },
...
# 便捷函數
def generate_government_ta_signals(
    source_name: str, data: List[Dict[str, Any]]
) -> Dict[str, Any]:
    """生成指定數據源的技術分析信號"""
    if source_name == "monetary_base":
...
def generate_composite_government_signals(
...
if __name__ == "__main__":
    ...
    try:
        ...
        if monetary_signals.get("success"):
            ...
            print(
                f"📈 Signal types: {list(monetary_signals.get('signal_types', {}).keys())}"
            )
        else:
        ...
        if all_signals_result.get("success"):
            ...
            print(
                f"📡 Data sources processed: {all_signals_result['data_sources_processed']}"
            )
            print(
                f"📊 Total signals generated: {all_signals_result['total_signals_generated']}"
            )
            ...
            print(
                f"❌ Failed to generate all signals: {all_signals_result.get('error', 'Unknown error')}"
            )
