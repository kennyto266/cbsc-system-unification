...
class GovernmentDataConverter:
    ...
    def __init__(self):
        ...
        self.data_sources = {
            ...
            "institutional_bond": {
                ...
                "indicators": [
                    "bond_yield_trend",
                    "bond_volume_momentum",
                    "bond_market_strength",
                    "bond_signal",
                ],
            },
            "forward_exchange": {
    ...
    def convert_hibor_to_indicators(self, hibor_data: pd.Series) -> Dict[str, float]:
        ...
        try:
            ...
            indicators["hibor_trend"] = (
                (ma_short.iloc[-1] - ma_long.iloc[-1]) / ma_long.iloc[-1] * 100
            )

            # 3. HIBOR波動率 (利率變化程度)
            indicators["hibor_volatility"] = (
                hibor_data.rolling(window = 20).std().iloc[-1]
            )

            # 4. HIBOR動量 (利率變化速度)
            indicators["hibor_momentum"] = (
                (hibor_data.iloc[-1] - hibor_data.iloc[-5]) / hibor_data.iloc[-5] * 100
            )

            # 5. HIBOR交叉信號 (利率與歷史均線交叉)
            ma_200 = hibor_data.rolling(window = 50).mean()
            current_rate = hibor_data.iloc[-1]
            indicators["hibor_cross_ma"] = (
                (current_rate - ma_200.iloc[-1]) / ma_200.iloc[-1] * 100
            )
    ...
    def convert_monetary_base_to_indicators(
        self, monetary_data: pd.Series
    ) -> Dict[str, float]:
        """貨幣基礎數據轉換為技術指標"""
        try:
            ...
            indicators["monetary_growth"] = (
                growth_rate.iloc[-1] if not growth_rate.empty else 0
            )

            # 2. 貨幣動量 (增長趨勢強度)
            indicators["monetary_momentum"] = self.calculate_momentum(
                monetary_data, period = 10
            )

            # 3. 均線交叉信號
            ma_short = monetary_data.rolling(window = 10).mean()
            ma_long = monetary_data.rolling(window = 30).mean()
            indicators["monetary_ma_crossover"] = (
                (ma_short.iloc[-1] - ma_long.iloc[-1]) / ma_long.iloc[-1] * 100
            )

            # 4. 貨幣加速 (二階導數)
            indicators["monetary_acceleration"] = self.calculate_acceleration(
                monetary_data
            )
    ...
    def convert_exchange_rates_to_indicators(
        self, exchange_data: pd.Series
    ) -> Dict[str, float]:
        """匯率數據轉換為技術指標"""
        try:
            ...
            # 2. 匯率趨勢
            indicators["exchange_trend"] = self.calculate_trend_strength(
                exchange_data, period = 20
            )

            # 3. 匯率波動率
            indicators["exchange_volatility"] = (
                exchange_data.rolling(window = 20).std().iloc[-1]
            )

            # 4. 匯率強度 (相對強弱指標)
            indicators["exchange_strength"] = self.calculate_relative_strength(
                exchange_data
            )
    ...
    def convert_efbn_yield_to_indicators(
        self, efbn_data: pd.Series
    ) -> Dict[str, float]:
        """外匯基金票據收益率轉換為技術指標"""
        try:
            if efbn_data.empty:
            ...
            # 1. 收益率趨勢
            indicators["efbn_yield_trend"] = self.calculate_trend_strength(
                efbn_data, period = 15
            )

            # 2. 收益率動量
            indicators["efbn_yield_momentum"] = self.calculate_momentum(
                efbn_data, period = 7
            )
            ...
            indicators["efbn_yield_spread"] = (
                short_yield.iloc[-1] - long_yield.iloc[-1]
            ) * 100
    ...
    def generate_all_government_indicators(
        self, gov_data: Dict[str, pd.Series]
        ...
        try:
            ...
            if "hibor_rates" in gov_data:
                all_indicators["hibor"] = self.convert_hibor_to_indicators(
                    gov_data["hibor_rates"]
                )

            if "monetary_base" in gov_data:
                all_indicators["monetary"] = self.convert_monetary_base_to_indicators(
                    gov_data["monetary_base"]
                )

            if "exchange_rates" in gov_data:
                all_indicators["exchange"] = self.convert_exchange_rates_to_indicators(
                    gov_data["exchange_rates"]
                )

            if "efbn_yield" in gov_data:
                all_indicators["efbn"] = self.convert_efbn_yield_to_indicators(
                    gov_data["efbn_yield"]
                )

            # 統計總指標數量
            total_indicators = sum(
                len(indicators) for indicators in all_indicators.values()
            )
            logger.info(
                f"成功生成 {len(all_indicators)} 個數據源的 {total_indicators} 個技術指標"
            )
    ...
    def create_trading_signals_from_gov_data(
        ...
        try:
            ...
            if "monetary" in indicators:
                ...
                # 貨幣增長 = 看漲股市
                if (
                    "monetary_growth" in monetary_ind
                    and monetary_ind["monetary_growth"] > 0
                ):
                    monetary_score += 1
            ...
            logger.info(
                f"政府數據信號生成: {final_signal}, 置信度: {signals['confidence']:.2f}"
            )
            return signals
    ...
    def calculate_momentum(self, data: pd.Series, period: int = 10) -> float:
        ...
        try:
            if len(data) < period + 1:
                return 0
            return (
                (data.iloc[-1] - data.iloc[-period - 1]) / data.iloc[-period - 1] * 100
            )
        except Exception:
            return 0
