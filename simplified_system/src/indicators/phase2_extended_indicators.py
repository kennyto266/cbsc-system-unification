...
class Phase2ExtendedIndicators:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Registered {len(self.indicator_metadata)} indicators across 4 categories"
        )
    ...
    def calculate_dema(
        self, data: pd.Series, period: int = 21
    ...
    def calculate_tema(
        self, data: pd.Series, period: int = 15
    ) -> Tuple[pd.Series, Dict]:
        """
        計算三指數移動平均線 (TEMA)
    ...
    def calculate_trima(
        self, data: pd.Series, period: int = 18
    ) -> Tuple[pd.Series, Dict]:
        """
        計算三角移動平均線 (TRIMA)
    ...
    def calculate_macd_extended(
        ...
        data_type = self.detect_data_type(data)
        adapted_params = self.adapt_parameters_for_data_type(
            "MACD_EXTENDED", data_type, {"fast": fast, "slow": slow, "signal": signal}
        )
    ...
    def calculate_williams_r(
        ...
        data_type = self.detect_data_type(data)
        adapted_params = self.adapt_parameters_for_data_type(
            "WILLIAMS_R", data_type, {"period": period}
        )
        period = int(adapted_params["period"])
    ...
    def calculate_cci(
        ...
        data_type = self.detect_data_type(data)
        adapted_params = self.adapt_parameters_for_data_type(
            "CCI", data_type, {"period": period, "constant": constant}
        )
        period = int(adapted_params["period"])
    ...
    def calculate_rsi_extended(
        self, data: pd.Series, period: int = 14, adaptive_range: List[int] = [5, 100]
    ) -> Tuple[pd.Series, Dict]:
        """
        ...
        gain = (
            ...
            .rolling(window = optimal_period, min_periods = 1)
            .mean()
        )
        loss = (
            (-delta.where(delta < 0, 0))
    ...
    def calculate_bollinger_bands(
        ...
        data_type = self.detect_data_type(data)
        adapted_params = self.adapt_parameters_for_data_type(
            "BOLLINGER_BANDS", data_type, {"period": period, "std_dev": std_dev}
        )
        period = int(adapted_params["period"])
    ...
    def calculate_atr(
        ...
        data_type = self.detect_data_type(close_data)
        adapted_params = self.adapt_parameters_for_data_type(
            "ATR", data_type, {"period": period, "method": method}
        )
        period = int(adapted_params["period"])
    ...
    def calculate_keltner_channels(
        ...
        data_type = self.detect_data_type(data)
        adapted_params = self.adapt_parameters_for_data_type(
            "KELTNER_CHANNELS", data_type, {"period": period, "multiplier": multiplier}
        )
        period = int(adapted_params["period"])
    ...
    def calculate_hibor_term_structure(
        self,
        hibor_data: Dict[str, pd.Series],
        short_term: str = "ON",
        long_term: str = "1M",
    ) -> Dict:
        """
    ...
    def calculate_rate_spread_analysis(
        self,
        rate_data1: pd.Series,
        rate_data2: pd.Series,
        period: int = 20,
        ...
        # 計算歷史百分位數
        spread_percentile = (
            spread.rolling(window = zscore_window, min_periods = 1).rank(pct = True) * 100
        )
        ...
        # 波動率分析
        spread_volatility = (
            spread.pct_change().rolling(window = period, min_periods = 1).std() * 100
        )
    ...
    def adapt_parameters_for_data_type(
        self, indicator_name: str, data_type: str, base_params: Dict = None
    ) -> Dict:
        """
        根據數據類型適配參數
        """
        ...
        if data_type == "rate_data":
            ...
            adapted_params["adaptation_reason"] = (
                "利率數據適配：使用較短週期捕捉利率變化"
            )
    ...
    def validate_indicator_performance(
        self, indicator_name: str, calculation_time_ms: float
    ) -> Dict:
        """
        驗證指標性能是否符合目標
        """
    ...
    def run_comprehensive_test(
        self, test_data: pd.Series, indicator_list: List[str] = None
    ) -> Dict:
        """
        運行全面的指標測試
        """
        ...
        print(
            f"Testing {len(indicator_list)} indicators on {len(test_data)} data points"
        )
        print("=" * 70)
        ...
        for indicator_name in indicator_list:
            ...
            try:
                ...
                # 驗證性能
                performance_validation = self.validate_indicator_performance(
                    indicator_name, calculation_time
                )
                ...
                print(
                    f"  {status} {calculation_time:.2f}ms (target: {metadata.performance_target:.1f}ms)"
                )
        ...
        successful_tests = [
            r
            for r in test_results["individual_results"].values()
            if r.get("success", False)
        ]
        if successful_tests:
            avg_calc_time = np.mean(
                [r["calculation_time_ms"] for r in successful_tests]
            )
            ...
            test_results["performance_analysis"] = {
                ...
                "performance_targets_met_percent": (
                    test_results["test_summary"]["performance_targets_met"]
                ...
                * 100,
                "system_performance_rating": (
                    ...
                    else "Good" if avg_calc_time < 1.0 else "Needs Optimization"
                ),
            }
        ...
        print(
            f"Performance Targets Met: {test_results['test_summary']['performance_targets_met']}/{test_results['test_summary']['total_tests']}"
        )
        print(
            f"Average Calculation Time: {test_results['performance_analysis'].get('average_calculation_time_ms', 0):.2f}ms"
        )
        print(
            f"System Performance Rating: {test_results['performance_analysis'].get('system_performance_rating', 'Unknown')}"
        )

        return test_results

    def _calculate_indicator_by_name(
        self, indicator_name: str, data: pd.Series
    ) -> Union[Tuple[pd.Series, Dict], Dict]:
        """
        根據指標名稱調用相應的計算方法
        """
        # 趨勢類擴展指標
