...
from .rule import ActionType, Rule, RulePriority
...
class RulesEngine:
        ...
        try:
            with open(rule_path, "r", encoding="utf - 8") as f:
                if rule_path.suffix.lower() in [".yaml", ".yml"]:
                    ...
                    raise ValueError(
                        f"Unsupported rule file format: {rule_path.suffix}"
                    )
    ...
    async def _execute_rule_with_timeout(
        ...
        try:
            ...
            return await asyncio.wait_for(
                asyncio.to_thread(rule.execute, context), timeout = self.execution_timeout
            )
    ...
    def _update_stats(self, results: List[Dict[str, Any]]):
        ...
        self.execution_stats["rules_matched"] += sum(
            1 for r in results if r.get("matched", False)
        )
        ...
        try:
            ...
            for rule_data in import_data.get("rules", []):
                try:
                    ...
                    logger.error(
                        f"Failed to import rule {rule_data.get('id', 'unknown')}: {e}"
                    )
