...
class TLSCertificateValidator(IVerifier):
    ...
    def __init__(
        self,
        name: str = "TLS Certificate Validator",
        ...
        if not CRYPTO_AVAILABLE:
            raise ImportError(
                "Required cryptography library not installed. Install with: pip install cryptography"
            )

        # Certificate pinning configuration
        self.pinned_certificates = self.config.get("pinned_certificates", {})
        self.certificate_store_path = self.config.get(
            "certificate_store_path", "config / certificates/"
        )

        # Critical endpoints that require certificate pinning
        self.critical_endpoints = self.config.get(
            "critical_endpoints",
            ["api.hkma.gov.hk", "18.180.162.113:9191", "data.gov.hk"],
        )
        ...
        logger.info(
            f"TLS Certificate Validator initialized with {len(self.pinned_certificates)} pinned certificates"
        )
        ...
        try:
            ...
            for cert_file in cert_store_dir.glob("*.pem"):
                try:
                    ...
                    self.pinned_certificates[cert_file.stem] = base64.b64encode(
                        fingerprint
                    ).decode("ascii")
    ...
    async def verify(
        ...
        try:
            ...
            if requires_pinning and result.overall_verdict == Verdict.AUTHENTIC:
                pinning_result = await self._verify_certificate_pinning(
                    hostname, validation_result["certificate"]
                )
            ...
            logger.info(
                f"TLS certificate validation completed for {hostname}: {result.overall_verdict.value}"
            )

        except asyncio.TimeoutError:
            result.overall_verdict = Verdict.ERROR
            result.error_message = (
                f"TLS validation timeout after {self.validation_timeout}s"
            )
    ...
    async def _validate_tls_certificate(
        self,
        hostname: str,
        port: int,
        protocol: str,
        ...
        try:
            if protocol == "https":
                ...
                with socket.create_connection(
                    (hostname, port), timeout = self.validation_timeout
                ) as sock:
                ...
                validation_result = await self._validate_certificate_details(
                    cert, hostname
                )
    ...
    async def _validate_certificate_details(
        ...
        try:
            ...
            try:
                ...
                return {
                    ...
                    "confidence": 0.7,
                    "details": {
                        "error": f"Certificate hostname does not match {hostname}"
                    },
                }
            ...
            final_confidence = max(
                0.0, min(1.0, base_confidence + confidence_adjustment)
            )
    ...
    async def _check_certificate_revocation(
    ...
    async def _verify_certificate_pinning(
        self, hostname: str, certificate: x509.Certificate
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"valid": False, "error": f"Certificate pinning error: {str(e)}"}
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_result = await self._validate_tls_certificate(
                "google.com", 443, "https", False, {}
            )

            if test_result["verdict"] == Verdict.AUTHENTIC:
                health_status["certificate_validation_test"] = "passed"
            else:
                health_status["certificate_validation_test"] = (
                    f'failed: {test_result["metadata"].get("error", "unknown")}'
                )
