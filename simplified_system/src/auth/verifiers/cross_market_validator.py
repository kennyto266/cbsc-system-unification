...
class CrossMarketValidator(IVerifier):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.exchange_rate_tolerance = self.config.get(
    ...
    async def verify(
        ...
        try:
            ...
            # 3. 汇率一致性验证
            exchange_rate_results = await self._validate_exchange_rates(
                market_data_list
            )
            ...
            # 5. 跨资产关系验证
            cross_asset_results = await self._validate_cross_asset_relationships(
                market_data_list
            )
            ...
            result = AuthResult(
                ...
                metadata={
                    ...
                    "validation_types_performed": list(
                        set(r.validation_type.value for r in validation_results)
                    ),
                },
            )

            logger.info(
                f"Cross - market validation completed for {data_id}: {verdict.value} "
            ...
            for item in data:
                if isinstance(item, dict) and "data" in item:
                    ...
                    market_name = item.get(
                        "market_name", f"market_{len(market_data_list)}"
                    )
    ...
    async def _validate_correlations(
        ...
        if len(commodity_markets) >= 2:
            commodity_correlations = await self._analyze_market_correlations(
                commodity_markets
            )
        ...
        for i, market1 in enumerate(markets):
            for j, market2 in enumerate(markets[i + 1 :], i + 1):
                ...
                try:
                    ...
                    # 对齐时间序列
                    aligned_prices1, aligned_prices2 = self._align_time_series(
                        prices1, prices2
                    )
                    ...
                    if len(aligned_prices1) < self.min_data_points:
                        results.append(
                            ValidationResult(
                                ...
                                details={
                                    "message": f"Insufficient aligned data points: {len(aligned_prices1)} < {self.min_data_points}"
                                },
                            )
                        )
                        continue
                    ...
                    # 验证相关性是否在合理范围内
                    # 对于同类型市场，相关性通常应该为正
                    expected_correlation_sign = (
                        1 if market1.data_type == market2.data_type else 0
                    )
                    ...
                    confidence = (
                        ...
                        if passed
                            ...
                            0.3,
                            1.0 - abs(actual_correlation - expected_correlation_sign),
                        )
                    )
    ...
    async def _detect_arbitrage_opportunities(
        ...
        for symbol, symbol_markets in asset_groups.items():
            if len(symbol_markets) >= 2:
                arbitrage_results = await self._analyze_arbitrage_for_symbol(
                    symbol, symbol_markets
                )
                results.extend(arbitrage_results)

        return results

    async def _analyze_arbitrage_for_symbol(
        self, symbol: str, markets: List[MarketData]
    ) -> List[ValidationResult]:
        """分析特定资产的套利机会"""
        results = []
        ...
        for market in markets:
            symbol_data = (
                market.data[market.data["symbol"] == symbol]
                if "symbol" in market.data.columns
                else market.data
            )
        ...
        try:
            ...
            if len(common_timestamps) < 10:
                results.append(
                    ValidationResult(
                        ...
                        execution_time_ms=(time.time() - start_time) * 1000,
                        details={
                            "message": f"Insufficient common timestamps: {len(common_timestamps)}"
                        },
                    )
                )
                return results
            ...
            for i in range(1, len(common_timestamps)):
                ...
                prices_at_timestamp = {
                    name: aligned_prices[name].iloc[i] for name in market_names
                }
                ...
                # 计算价格差异百分比
                price_diff_pct = (
                    (max_price - min_price) / min_price if min_price > 0 else 0
                )
            ...
            for opportunity in arbitrage_opportunities:
                anomaly = AnomalyDetection(
                    ...
                    severity=(
                        ...
                        if opportunity["price_difference_pct"]
                        > self.arbitrage_threshold * 2
            ...
            arbitrage_frequency = (
                len(arbitrage_opportunities) / len(price_diffs) if price_diffs else 0
            )
        ...
        for i, market1 in enumerate(forex_markets):
            for j, market2 in enumerate(forex_markets[i + 1 :], i + 1):
                ...
                try:
                    ...
                    for pair in common_pairs:
                        ...
                        if max_diff > self.exchange_rate_tolerance:
                            ...
                            anomaly = AnomalyDetection(
                                ...
                                severity=(
                                    ValidationSeverity.HIGH
                                    if max_diff > self.exchange_rate_tolerance * 2
                                    else ValidationSeverity.MEDIUM
                                ),
                                confidence = 0.9,
                                context={
                                    "currency_pair": pair,
                                    "market1_rate": float(
                                        aligned_data1.loc[max_diff_idx]
                                    ),
                                    "market2_rate": float(
                                        aligned_data2.loc[max_diff_idx]
                                    ),
                                    "difference_percentage": float(
                                        max_diff.loc[max_diff_idx]
                                    ),
                    ...
                    confidence = (
                        consistent_pairs / validated_pairs
    ...
    async def _test_market_efficiency(
        self, markets: List[MarketData]
    ) -> List[ValidationResult]:
        """测试市场效率"""
        results = []
        ...
        for market in stock_markets:
            ...
            try:
                ...
                if len(price_data) < self.min_data_points:
                    results.append(
                        ValidationResult(
                            ...
                            execution_time_ms=(time.time() - start_time) * 1000,
                            details={
                                "message": f"Insufficient data points: {len(price_data)} < {self.min_data_points}"
                            },
                        )
                    )
                    continue
                ...
                efficiency_ratio = (
                    efficiency_score / total_tests if total_tests > 0 else 0
                )
    ...
    async def _validate_cross_asset_relationships(
        self, markets: List[MarketData]
    ) -> List[ValidationResult]:
        """验证跨资产关系"""
        results = []
        ...
        if "stock" in asset_groups and "commodity" in asset_groups:
            stock_commodity_results = (
                await self._validate_stock_commodity_relationships(
    ...
    async def _validate_stock_forex_relationships(
        self, stock_markets: List[MarketData], forex_markets: List[MarketData]
    ) -> List[ValidationResult]:
        """验证股票 - 外汇关系"""
        results = []
            ...
            for forex_market in forex_markets:
                if forex_market.currency in ["EUR", "JPY", "CNY"]:  # 非美元货币
                    ...
                    try:
                        ...
                        # 对齐时间序列
                        aligned_stock, aligned_forex = self._align_time_series(
                            stock_prices, forex_series
                        )
                        ...
                        passed = (
                            expected_correlation_range[0]
                            <= correlation
                            <= expected_correlation_range[1]
                        )
    ...
    async def _validate_stock_commodity_relationships(
        self, stock_markets: List[MarketData], commodity_markets: List[MarketData]
    ) -> List[ValidationResult]:
        """验证股票 - 商品关系"""
        results = []
        ...
        for stock_market in stock_markets:
            for commodity_market in commodity_markets:
                ...
                try:
                    ...
                    # 对齐时间序列
                    aligned_stock, aligned_commodity = self._align_time_series(
                        stock_prices, commodity_prices
                    )
                    ...
                    confidence = (
                        1.0 - abs(correlation) / max_reasonable_correlation
                        if passed
                        else 0.3
                    )
        ...
        if "pair" in market.data.columns and "rate" in market.data.columns:
            for pair in market.data["pair"].unique():
                ...
                if market.timestamp_col in pair_data.columns:
                    pair_data["timestamp"] = pd.to_datetime(
                        pair_data[market.timestamp_col]
                    )
...
# Factory function
def create_cross_market_validator(
