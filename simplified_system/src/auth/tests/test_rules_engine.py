...
class TestRulesEngine(unittest.TestCase):
    ...
    def test_rule_conditions(self):
        ...
        # 测试匹配条件
        matching_context = {"data": {"source": "hkma.gov.hk"}}
        self.assertTrue(rule.evaluate(matching_context))

        # 测试不匹配条件
        non_matching_context = {"data": {"source": "untrusted.source"}}
    ...
    def test_rule_actions(self):
        ...
        self.assertEqual(
            action_result["action_type"], ActionType.REQUIRE_VERIFIER.value
        )
    ...
    def test_multiple_conditions(self):
        ...
        complex_rule = Rule(
            ...
            actions=[
                RuleAction(
                    action_type = ActionType.APPROVE_DATA, parameters={"confidence": 0.9}
                )
            ],
        )
        ...
        # 测试所有条件匹配
        matching_context = {
            "data": {"source": "hkma.gov.hk", "type": "hibor"},
        ...
        non_matching_context = {
            "data": {"source": "hkma.gov.hk", "type": "unauthorized_type"},
            "verification": {"confidence": 0.85},
        }
...
class TestRuleOperators(unittest.TestCase):
    ...
    def test_string_operators(self):
        ...
        condition = RuleCondition(
            field="data.domain", operator = RuleOperator.CONTAINS, value="hkma"
        )
        self.assertTrue(condition.evaluate(context))

        # 测试开始操作符
        condition = RuleCondition(
            field="data.domain", operator = RuleOperator.STARTS_WITH, value="api"
        )
        self.assertTrue(condition.evaluate(context))

        # 测试结束操作符
        condition = RuleCondition(
            field="data.domain", operator = RuleOperator.ENDS_WITH, value="gov.hk"
        )
        self.assertTrue(condition.evaluate(context))
