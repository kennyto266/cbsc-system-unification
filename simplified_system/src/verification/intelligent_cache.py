...
class AdaptiveTTLManager:
    ...
    def calculate_adaptive_ttl(
        self, key: str, data_type: str, access_history: List[float]
    ) -> float:
        """計算自適應TTL"""
        ...
        recent_accesses = [
            t for t in access_history if current_time - t < 3600
        ]  # 最近1小時
...
class IntelligentCache:
    ...
    def __init__(
        ...
        logger.info(
            f"Intelligent Cache initialized - Memory: {max_memory_mb}MB, Entries: {max_entries}"
        )
    ...
    def set(
        ...
        try:
            # 計算自適應TTL
            if ttl is None:
                access_history = self.ttl_manager.access_patterns.get(key, {}).get(
                    "access_times", []
                )
                ttl = self.ttl_manager.calculate_adaptive_ttl(
                    key, data_type, access_history
                )
            ...
            logger.debug(
                f"Cache set: {key} (TTL: {ttl}s, Size: {entry.size_bytes} bytes)"
            )
    ...
    def clear(self, pattern: Optional[str] = None):
        ...
        if pattern:
            ...
            for key in keys_to_remove:
                self._remove_entry(key)
            logger.info(
                f"Cleared {len(keys_to_remove)} entries matching pattern: {pattern}"
            )
        else:
            # 清空所有緩存
    ...
    def get_ttl_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "hit_ratios": {
                k: self.ttl_manager.get_hit_ratio(v)
