...
# 導入驗證系統組件
from .monitoring_dashboard import AlertManager, AlertRule
...
""",
    ...
    def format_message(self, alert: AlertMessage, template: Dict[str, Any]) -> str:
        """格式化警報消息"""
        # 獲取建議操作
        suggestions = self.action_suggestions.get(
            alert.metric_name, ["Investigate the issue", "Check system logs"]
        )
        ...
        formatted_message = template["template"].format(
            ...
            resolved_time=(
                datetime.fromtimestamp(alert.resolved_at).strftime("%Y-%m-%d %H:%M:%S")
...
class AlertAggregator:
    ...
    def _create_aggregation_key(self, alert: AlertMessage) -> str:
        """創建聚合鍵"""
        key_components = [alert.source, alert.metric_name, alert.severity]
...
class TelegramAlertBot:
    ...
    async def send_alert(self, alert: AlertMessage) -> bool:
        ...
        try:
                ...
                keyboard = [
                    [
                        InlineKeyboardButton(
                            "✅ Acknowledge", callback_data = f"ack_{send_alert.alert_id}"
                        )
                    ],
                    [
                        InlineKeyboardButton(
                            "📊 Details", callback_data = f"details_{send_alert.alert_id}"
                        )
                    ],
                    [
                        InlineKeyboardButton(
                            "🔕 Mute", callback_data = f"mute_{send_alert.alert_id}"
                        )
                    ],
                ]
            ...
            logger.info(
                f"Alert sent to {success_count}/{len(self.config.chat_ids)} chats: {send_alert.alert_id}"
            )
    ...
    async def send_resolution(self, alert: AlertMessage):
        ...
        try:
            ...
            for chat_id in self.config.chat_ids:
                try:
                    await self.bot.send_message(
                        chat_id = chat_id, text = resolved_message, parse_mode="Markdown"
                    )
                except Exception as e:
    ...
    def _check_rate_limit(self) -> bool:
        ...
        for chat_id in list(self.rate_limits.keys()):
            self.rate_limits[chat_id] = [
                timestamp
                for timestamp in self.rate_limits[chat_id]
    ...
    async def cmd_alerts(self, update: Update, context):
        ...
        try:
            ...
            if not recent_alerts:
                await update.message.reply_text(
                    "🎉 *No recent alerts!*", parse_mode="Markdown"
                )
                return
            ...
            for alert in recent_alerts:
                ...
                message += (
                    f"{severity_emoji} *{alert['severity'].title()}* - {time_str}\n"
                )
    ...
    async def cmd_acknowledge(self, update: Update, context):
        ...
        if len(context.args) == 0:
            await update.message.reply_text(
                "❌ Please provide an alert ID: /ack_<alert_id>"
            )
            return
...
async def initialize_telegram_alerts(
    bot_token: str, chat_ids: List[str], alert_levels: List[str] = None
) -> TelegramAlertBot:
    """初始化Telegram警報系統"""
...
if __name__ == "__main__":
    ...
    async def test_telegram_alerts():
        ...
        try:
            # 初始化警報系統
            await initialize_telegram_alerts(bot_token, chat_ids)
            ...
            success = await send_alert_resolution(
                "test_critical_001", "Test Critical Alert"
            )
