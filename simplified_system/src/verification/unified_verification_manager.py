            ...
            with socket.create_connection((hostname, port), timeout = 10) as sock:
                with context.wrap_socket(sock, server_hostname = hostname) as secure_sock:
                    ...
                    issuer_score = (
                        1.0
                        if any(
                            trusted in issuer.get("organizationName", "")
    ...
    def _validate_business_rules(self, data: Any, data_type: str) -> float:
        ...
        try:
            ...
            if isinstance(data, dict) and "data" in data and data["data"]:
                ...
                field_score = (
                    1.0 - (len(missing_fields) / len(required_fields))
                    if required_fields
                    else 1.0
                )
...
class BehavioralAnalyzer:
    ...
    async def analyze_behavior(
        self, data: Any, data_type: str, historical_context: Optional[Dict] = None
    ) -> Tuple[float, Dict[str, Any]]:
        """分析數據行為模式"""
        start_time = time.time()
        details = {}
        ...
        try:
            ...
            historical_score = self._compare_with_historical(
                data, data_type, historical_context
            )
    ...
    def _analyze_patterns(self, data: Any, data_type: str) -> float:
        ...
        try:
            ...
            if len(values_array) >= 12:  # 至少需要12個數據點
                ...
                seasonal_score = (
                    min(autocorr[center + 1] / autocorr[center], 1.0)
                    if autocorr[center] > 0
                    else 0.5
                )
            else:
                seasonal_score = 0.7
            ...
            if len(values_array) > 1:
                volatility = (
                    ...
                    else np.std(values_array)
                )
                volatility_score = (
                    1.0 - min(volatility, 2.0) / 2.0
                )  # 波動性越小分數越高
            else:
    ...
    async def _ml_anomaly_detection(self, data: Any, data_type: str) -> float:
        ...
        try:
            ...
            # 1. 統計異常檢測 (Z - score)
            z_scores = np.abs(
                (values_array - np.mean(values_array)) / np.std(values_array)
            )
    ...
    def _compare_with_historical(
        ...
        try:
            ...
            # 比較均值和標準差
            mean_diff_ratio = (
                abs(current_mean - historical_mean) / abs(historical_mean)
                if historical_mean != 0
                else 0
            )
            std_diff_ratio = (
                abs(current_std - historical_std) / historical_std
                if historical_std != 0
                else 0
            )
...
class UnifiedVerificationManager:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        # 異步處理配置
        self.max_concurrent_verifications = self.config.get("performance", {}).get(
        ...
        try:
                ...
                if source_url:
                    ...
                    if cached_source:
                        ...
                        source_score, source_details = (
                            await self.source_authenticator.verify_source(
                                source_url, data
                            )
                        )
                        self.cache.set(
                            "source", source_cache_key, (source_score, source_details)
                        )
                else:
            ...
            if layers_config["content_validation"]["enabled"]:
                ...
                if cached_content:
                    ...
                    content_score, content_details = (
                        await self.content_validator.validate_content(data, data_type)
                    )
                    self.cache.set(
                        "content", content_cache_key, (content_score, content_details)
                    )
            ...
            if layers_config["behavioral_analysis"]["enabled"]:
                ...
                if cached_behavior:
                    ...
                    behavior_score, behavior_details = (
                        await self.behavioral_analyzer.analyze_behavior(
                            data, data_type, historical_context
                        )
                    )
                    self.cache.set(
            ...
            alerts = self._generate_alerts(
                composite_score, source_score, content_score, behavior_score, data_type
            )
    ...
    def _generate_cache_key(
        self, data: Any, data_type: str, source_url: Optional[str] = None
    ) -> str:
        """生成緩存鍵"""
        # 使用數據哈希、類型和URL生成唯一鍵
    ...
    def _generate_alerts(
        ...
        if composite_score < thresholds["low"]:
            alerts.append(
                f"LOW: Low verification confidence ({composite_score:.3f}) for {data_type}"
            )
        elif composite_score < thresholds["medium"]:
            alerts.append(
                f"MEDIUM: Moderate verification confidence ({composite_score:.3f}) for {data_type}"
            )
        elif composite_score < thresholds["high"]:
            alerts.append(
                f"HIGH: High verification confidence ({composite_score:.3f}) for {data_type}"
            )

        # 各層分數警報
        if source_score < thresholds["low"]:
            alerts.append(
                f"CRITICAL: Source authentication failed ({source_score:.3f})"
            )
    ...
    def _update_metrics(self, result: VerificationResult):
        ...
        # 更新平均響應時間
        total_time = (
            self.metrics.avg_response_time_ms * (self.metrics.total_verifications - 1)
        ...
        self.metrics.error_rate = (
            self.metrics.failed_verifications / self.metrics.total_verifications
        )
        ...
        if len(self.performance_history) > 0:
            ...
            if time_diff > 0:
                self.metrics.throughput_per_second = (
                    len(self.performance_history) / time_diff
                )
            ...
            async def verify_single(data_item):
                async with semaphore:
                    ...
                    return await self.verify_data(
                        data, data_type, source_url, historical_context
                    )
    ...
    async def verify_government_data(
        ...
        try:
            ...
            if data_type == "hibor_rates":
                ...
                source_url = self.government_api.data_sources["exchange_rates"][
                    "base_url"
                ]
            elif data_type == "monetary_base":
                data = self.government_api.get_monetary_base(days_back)
                source_url = self.government_api.data_sources["monetary_base"][
                    "base_url"
                ]
            else:
            ...
            logger.info(
                f"Government data verification completed: {data_type}, score: {result.composite_score:.3f}"
            )
            return result
    ...
    async def verify_stock_data(
        ...
        try:
            ...
            logger.info(
                f"Stock data verification completed: {symbol}, score: {result.composite_score:.3f}"
            )
            return result
...
async def verify_government_data(
    data_type: str, days_back: int = 30
) -> VerificationResult:
    """便捷函數：驗證政府數據"""
    return await unified_verification_manager.verify_government_data(
        data_type, days_back
    )
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        print(
            f"Success rate: {metrics.successful_verifications}/{metrics.total_verifications}"
        )
