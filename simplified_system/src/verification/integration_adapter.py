...
from src.api.government_data import (
    GovernmentDataAPI,
    get_hibor_data,
    get_hkma_data,
    get_latest_hibor,
...
class VerificationEnabledGovernmentAPI(GovernmentDataAPI):
    ...
    def __init__(self, enable_verification: bool = True, auto_verify: bool = False):
        ...
        self.verification_manager = (
            UnifiedVerificationManager() if enable_verification else None
        )

        logger.info(
            f"VerificationEnabledGovernmentAPI initialized - Verification: {enable_verification}, Auto - verify: {auto_verify}"
        )

    async def get_hibor_data_with_verification(
        ...
        if self.auto_verify:
            ...
            try:
                verification_result = (
                    await self.verification_manager.verify_government_data(
                        "hibor_rates", days_back
                    )
                )
                ...
                if verification_result.composite_score < 0.7:
                    logger.warning(
                        f"HIBOR data verification score low: {verification_result.composite_score:.3f}"
                    )

                # 如果有嚴重警報，記錄錯誤
                critical_alerts = [
                    alert for alert in verification_result.alerts if "CRITICAL" in alert
                ]
                if critical_alerts:
                    logger.error(
                        f"HIBOR data critical verification alerts: {critical_alerts}"
                    )
    ...
    async def get_exchange_rates_with_verification(
        self, days_back: int = 30
    ) -> Dict[str, Any]:
        """獲取匯率數據並進行驗證"""
        if not self.enable_verification:
        ...
        if self.auto_verify:
            ...
            try:
                ...
                if verification_result.composite_score < 0.7:
                    logger.warning(
                        f"Exchange rates data verification score low: {verification_result.composite_score:.3f}"
                    )
    ...
    async def get_monetary_base_with_verification(
        ...
        if self.auto_verify:
            ...
            try:
                ...
                if verification_result.composite_score < 0.7:
                    logger.warning(
                        f"Monetary base data verification score low: {verification_result.composite_score:.3f}"
                    )
...
class VerificationEnabledStockAPI(StockDataAPI):
    ...
    def __init__(self, enable_verification: bool = True, auto_verify: bool = False):
        ...
        logger.info(
            f"VerificationEnabledStockAPI initialized - Verification: {enable_verification}, Auto - verify: {auto_verify}"
        )

    async def get_stock_data_with_verification(
        ...
        if self.auto_verify:
            ...
            try:
                ...
                if verification_result.composite_score < 0.7:
                    logger.warning(
                        f"Stock data verification score low: {symbol} - {verification_result.composite_score:.3f}"
                    )
    ...
    async def verify_multiple_stocks(
...
class BackwardCompatibilityLayer:
    ...
    def enable_verification(self, auto_verify: bool = False):
        ...
        self.enhanced_government_api = VerificationEnabledGovernmentAPI(
            enable_verification = True, auto_verify = auto_verify
        )
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                ...
                verification_manager = (
                    backward_compatibility.enhanced_government_api.verification_manager
                )

                # 執行驗證
                source_url = kwargs.get("source_url")  # 可以通過參數傳入
                verification_result = await verification_manager.verify_data(
                    result, data_type, source_url
                )
...
async def _sync_verify_result(
    result: Any, data_type: str
...
async def get_stock_data_verified(
    ...
    def load_config(config_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        default_config = {
            ...
            "alerts": {
                ...
                "alert_thresholds": {"min_score": 0.7, "error_rate": 0.1},
            },
        }
...
# 全局初始化
def initialize_verification_system(
    config_path: Optional[str] = None, auto_verify: bool = False
):
    """初始化驗證系統"""
    try:
        # 加載配置
    ...
    async def main():
        ...
        if hasattr(gov_api, "get_hibor_data_with_verification"):
            ...
            if hibor_data and "verification" in hibor_data:
                print(
                    f"HIBOR data verification score: {hibor_data['verification']['composite_score']:.3f}"
                )
            else:
        ...
        if hasattr(stock_api, "get_stock_data_with_verification"):
            ...
            if stock_data and "verification" in stock_data:
                print(
                    f"Stock data verification score: {stock_data['verification']['composite_score']:.3f}"
                )
            else:
