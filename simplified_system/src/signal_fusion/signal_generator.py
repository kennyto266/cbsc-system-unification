...
class SignalGenerator:
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "volume_thresholds": {"spike_multiplier": 1.5, "dry_up_threshold": 0.5},
            ...
            for indicator_name, indicator_data in indicators.items():
                ...
                if base_indicator in self.signal_rules:
                    try:
                        ...
                        logger.warning(
                            f"Error generating signals for {indicator_name}: {e}"
                        )
                        continue
    ...
    def _generate_rsi_signals(
        ...
        if isinstance(rsi_series, pd.Series):
            ...
            prev_rsi = (
                float(rsi_series.iloc[-2]) if len(rsi_series) > 1 else current_rsi
            )
        else:
    ...
    def _generate_macd_signals(
        ...
        # 檢測MACD零軸突破
            ...
            prev_macd <= 0 < current_macd
            and current_hist > thresholds["zero_cross_strength"]
        ):
            signal_type = SignalType.BUY
            strength = SignalStrength.MODERATE.value
            reason = "MACD突破零軸"
            ...
            reason = "MACD無明顯信號"
            explanation = (
                f"MACD線({current_macd:.4f})與信號線({current_signal:.4f})無明顯交叉"
            )
    ...
    def _generate_bollinger_signals(
        ...
        if isinstance(upper_series, pd.Series):
            ...
            current_width = (
                float(width_series.iloc[-1]) if width_series is not None else 0
            )
        else:
            return signals
        ...
        # 檢測突破中軌
            ...
            reason = "價格突破布林帶中軌"
            explanation = (
                f"價格({current_price:.2f})站上中軌({current_middle:.2f})，勢頭轉強"
            )
            ...
            reason = "價格跌破布林帶中軌"
            explanation = (
                f"價格({current_price:.2f})跌破中軌({current_middle:.2f})，勢頭轉弱"
            )
    ...
    def _generate_ma_signals(
        ...
        if prev_short <= prev_long and current_short > current_long:
            ...
            explanation = (
                f"短期{ma_type}({ma_periods[0]})向上穿越長期{ma_type}({ma_periods[-1]})"
            )
        ...
        # 檢測死亡交叉
            ...
            explanation = (
                f"短期{ma_type}({ma_periods[0]})向下穿越長期{ma_type}({ma_periods[-1]})"
            )
    ...
    def _filter_and_validate_signals(
    ...
    def _get_indicator_performance(
        self, indicator: str, signal_type: SignalType
    ) -> Dict[str, float]:
        """獲取指標歷史表現"""
    ...
    def update_signal_performance(self, signal: TradingSignal, outcome: float):
        ...
        if outcome > 0:
            perf["profit_rate"] = (
                perf["profit_rate"] * (perf["signal_count"] - 1) + 1
            ) / perf["signal_count"]
        else:
            perf["profit_rate"] = (
                perf["profit_rate"] * (perf["signal_count"] - 1)
            ) / perf["signal_count"]
...
class ConfidenceCalculator:
    ...
    def calculate_rsi_confidence(
        self, current_rsi: float, prev_rsi: float, data_points: int
    ) -> float:
        """計算RSI信號置信度"""
        # 基於RSI值的極值程度
    ...
    def calculate_bollinger_confidence(
        self, band_position: float, band_width: float, data_points: int
    ) -> float:
        """計算布林帶信號置信度"""
        # 基於價格位置
    ...
    def calculate_ma_confidence(
        self, price: float, short_ma: float, long_ma: float, data_points: int
    ) -> float:
        """計算移動平均線信號置信度"""
        # 基於均線間距
