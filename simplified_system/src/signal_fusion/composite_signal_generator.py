...
class CompositeSignalGenerator:
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "quality_thresholds": {"excellent": 90, "good": 80, "fair": 70, "poor": 60},
            "risk_parameters": {
                "risk_free_rate": 0.03,
    ...
    def generate_composite_signal(
        ...
        if data is not None and symbol is None and indicators is None:
            ...
            try:
                ...
                indicators["RSI"] = {
                    "signal": (
                        "HOLD"
                        if 40 <= rsi.iloc[-1] <= 60
                        else ("BUY" if rsi.iloc[-1] < 40 else "SELL")
                    ),
                    "strength": 5.0,
                    "confidence": 0.7,
                }
            ...
            # 計算MACD
            try:
                core_indicators.calculate_macd(data["close"], 12, 26, 9)
                indicators["MACD"] = {
                    "signal": "HOLD",
                    "strength": 5.0,
                    "confidence": 0.6,
        ...
        try:
            ...
            if (
                len(base_signals)
                < self.config["fusion_parameters"]["min_component_signals"]
            ):
                logger.warning(
                    f"Insufficient signals for {symbol}: {len(base_signals)} < {self.config['fusion_parameters']['min_component_signals']}"
                )
            ...
            if conflicts:
                resolutions = self.conflict_resolver.resolve_conflicts(
                    conflicts, weights = weights
                )
                base_signals = self._apply_conflict_resolutions(
                    base_signals, resolutions
                )
            ...
            quality_score = self.quality_assessor.assess_quality(
                weighted_signal, base_signals, data
            )

            # 6. 評估風險
            risk_score, risk_level = self.risk_assessor.assess_risk(
                weighted_signal, data
            )

            # 7. 生成決策解釋
            explanation = self._generate_explanation(
                weighted_signal, base_signals, weights, data
            )
            ...
            logger.info(
                f"Generated composite signal for {symbol}: {weighted_signal['signal_type'].value}"
            )
    ...
    def _create_default_signal(
        ...
        try:
            return CompositeSignal(
                ...
                data_time = data.index[-1],
                current_price=(
                    float(data["close"].iloc[-1]) if "close" in data.columns else 0.0
                ),
                risk_level = RiskLevel.MODERATE,
                risk_score = 5.0,
    ...
    def _calculate_weighted_signal(
        ...
        """計算加權信號"""
        if not signals:
            return {"signal_type": SignalType.HOLD, "strength": 5.0, "confidence": 0.5}
    ...
    def _apply_conflict_resolutions(
        self, signals: List[TradingSignal], resolutions: List[ConflictResolution]
    ) -> List[TradingSignal]:
        """應用衝突解決結果"""
        if not resolutions:
            return signals
    ...
    def _calculate_target_price(
        self, weighted_signal: Dict[str, Any], data: pd.DataFrame
    ) -> Optional[float]:
        """計算目標價格"""
        current_price = float(data["close"].iloc[-1])
        ...
        if weighted_signal["signal_type"] == SignalType.BUY:
            ...
            atr = (
                data.get("ATR", pd.Series([current_price * 0.02])).iloc[-1]
                if "ATR" in data
                else current_price * 0.02
            )

            target_multiplier = 1 + (
                strength_factor * confidence_factor * 2 + 1
            )  # 1 - 3倍ATR
    ...
    def _generate_explanation(
        ...
        if supporting_signals:
            ...
            top_signals = sorted(
                supporting_signals,
                key = lambda s: weights.get(s.indicator_name, 0)
                * s.strength
                * s.confidence,
                reverse = True,
            )[:3]
    ...
    def _find_similar_signals(
        self, weighted_signal: Dict[str, Any]
    ) -> Dict[str, float]:
        """尋找類似信號的歷史表現"""
        ...
        similar_signals = [
            signal
            for signal in self.composite_signal_history
            if signal.signal_type == signal_type
            and abs(signal.strength - strength) < 2.0  # 強度差異小於2
        ]
    ...
    def generate_signal_report(
        ...
        risk_analysis = {
            ...
            "potential_loss": (
                abs(composite_signal.stop_loss - composite_signal.current_price)
                if composite_signal.stop_loss
                else 0
            ),
        ...
        recommendations = self._generate_recommendations(
            composite_signal, market_context
        )
    ...
    def _calculate_position_size(self, signal: CompositeSignal) -> float:
        ...
        # 簡化凱利公式
        kelly_fraction = (
            win_prob * avg_return - (1 - win_prob) * risk_loss
        ) / risk_loss

        # 限制倉位大小
        max_position = self.config["risk_parameters"]["max_position_size"]
        position_size = max(
            0.01, min(max_position, kelly_fraction * 0.5)
        )  # 保守因子0.5
    ...
    def _generate_market_context(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        if len(data) >= 50:
            ...
            trend = (
                ...
                else "DOWN" if current_price < ma_20 < ma_50 else "SIDEWAYS"
            )
        else:
            trend = "UNKNOWN"
    ...
    def _generate_recommendations(
        self, signal: CompositeSignal, market_context: Dict[str, Any]
    ) -> List[str]:
        """生成操作建議"""
        recommendations = []
        ...
        if signal.signal_type == SignalType.BUY:
            recommendations.append(
                f"建議買入 {signal.symbol}，目標價格 {signal.target_price:.2f}"
            )
            if signal.stop_loss:
            ...
            recommendations.append(
                f"建議賣出 {signal.symbol}，目標價格 {signal.target_price:.2f}"
            )
            if signal.stop_loss:
                recommendations.append(f"設置止損價格 {signal.stop_loss:.2f}")
        else:
...
class SignalQualityAssessor:
    ...
    def assess_quality(
        ...
        # 信號強度 (+15)
        avg_strength = (
            np.mean([s.strength for s in base_signals]) if base_signals else 5
        )
        ...
        # 置信度 (+15)
        avg_confidence = (
            np.mean([s.confidence for s in base_signals]) if base_signals else 0.5
        )
        ...
        # 指標多樣性 (+10)
        indicator_types = (
            len(set(s.indicator_name for s in base_signals)) if base_signals else 1
        )
...
# 便利函數
def generate_composite_signal(
    symbol: str, data: pd.DataFrame, indicators: Dict[str, Any]
) -> CompositeSignal:
    """便利函數：生成綜合信號"""
    return composite_signal_generator.generate_composite_signal(
        symbol, data, indicators
    )
