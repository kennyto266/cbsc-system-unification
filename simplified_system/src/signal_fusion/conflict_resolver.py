...
class ConflictResolver:
    ...
    def detect_conflicts(self, signals: List[TradingSignal]) -> List[SignalConflict]:
        ...
        if (
            len(signals)
            < self.config["detection_thresholds"]["min_conflicting_signals"]
        ):
            return []
    ...
    def _detect_symbol_conflicts(
        ...
        # 檢測買賣衝突
        if (
            signal_distribution.get(SignalType.BUY, 0) > 0
            and signal_distribution.get(SignalType.SELL, 0) > 0
        ):
        ...
        if (
            strength_variance
            > self.config["detection_thresholds"]["strength_variance_threshold"]
        ):
            conflict = SignalConflict(
        ...
        if (
            confidence_variance
            > self.config["detection_thresholds"]["confidence_variance_threshold"]
        ):
            conflict = SignalConflict(
    ...
    def resolve_conflicts(
        ...
        resolutions = []
        default_strategy = (
            strategy or self.config["resolution_preferences"]["default_strategy"]
        )

        for conflict in conflicts:
            try:
                resolution = self._resolve_single_conflict(
                    conflict, default_strategy, weights
                )
                if resolution:
                ...
                # 使用備用策略
                try:
                    fallback_strategy = self.config["resolution_preferences"][
                        "fallback_strategy"
                    ]
                    resolution = self._resolve_single_conflict(
                        conflict, fallback_strategy, weights
                    )
                    if resolution:
                        resolutions.append(resolution)
                        self.resolution_history.append(resolution)
        ...
        if (
            ...
            % self.config["learning_parameters"]["update_frequency"]
            == 0
        ):
    ...
    def _majority_vote_resolution(
        self, conflict: SignalConflict, weights: Optional[Dict[str, float]]
    ) -> ConflictResolution:
        """多數投票解決策略"""
        ...
        if (
            consensus_level
            < self.config["resolution_preferences"]["min_consensus_threshold"]
        ):
        ...
        selected_signals = [
            s
            for s in conflict.conflicting_signals
            if s.signal_type == final_signal_type
        ]
        if selected_signals:
        ...
        resolution = ConflictResolution(
            ...
            resolution_quality = self._calculate_resolution_quality(
                conflict, final_signal_type, consensus_level
            ),
        )
    ...
    def _weighted_vote_resolution(
        ...
        if selected_signals:
            final_strength = np.mean([s.strength for s in selected_signals])
            final_confidence = type_confidence[final_signal_type] / sum(
                type_confidence.values()
            )
        else:
            final_strength = 5.0
            final_confidence = 0.5
    ...
    def _highest_confidence_resolution(
        ...
        # 計算共識程度
        confidence_ranks = sorted(
            [s.confidence for s in conflict.conflicting_signals], reverse = True
        )
        if len(confidence_ranks) > 1:
            consensus_level = (
                confidence_ranks[0] - confidence_ranks[1]
            ) / confidence_ranks[0]
        else:
            consensus_level = 1.0
        ...
        resolution = ConflictResolution(
            ...
            conflict_intensity = 1.0 - consensus_level,
            resolution_quality = self._calculate_resolution_quality(
                conflict, best_signal.signal_type, consensus_level
            ),
        )
    ...
    def _machine_learning_resolution(
        ...
        if (
            ...
            or len(self.ml_labels)
            < self.config["learning_parameters"]["min_samples_for_ml"]
        ):
            return self._weighted_vote_resolution(conflict, weights)
        ...
        try:
            ...
            matching_signals = [
                s
                for s in conflict.conflicting_signals
                if s.signal_type == predicted_type
            ]
            ...
            if matching_signals:
                ...
                # 沒有匹配的信號，選擇置信度最高的
                selected_signal = max(
                    conflict.conflicting_signals, key = lambda s: s.confidence
                )
            ...
            resolution = ConflictResolution(
                ...
                resolution_quality = self._calculate_resolution_quality(
                    conflict, selected_signal.signal_type, abs(prediction - 0.5) * 2
                ),
            )
    ...
    def _risk_averse_resolution(
        ...
        # 優先考慮HOLD信號
        hold_signals = [
            s for s in conflict.conflicting_signals if s.signal_type == SignalType.HOLD
        ]
        ...
        resolution = ConflictResolution(
            ...
            resolution_quality = self._calculate_resolution_quality(
                conflict, selected_signal.signal_type, 0.7
            ),
        )
    ...
    def _aggressive_resolution(
        ...
        if non_hold_signals:
            # 選擇強度和置信度組合最高的信號
            selected_signal = max(
                non_hold_signals, key = lambda s: s.strength * s.confidence
            )
        else:
            # 只有HOLD信號，選擇最強烈的
            selected_signal = max(
                conflict.conflicting_signals, key = lambda s: s.strength
            )
        ...
        resolution = ConflictResolution(
            ...
            resolution_quality = self._calculate_resolution_quality(
                conflict, selected_signal.signal_type, 0.6
            ),
        )
    ...
    def _update_strategy_performance(self, resolution: ConflictResolution):
        ...
        perf.resolution_time += (
            datetime.now() - resolution.resolution_time
        ).total_seconds() * 1000  # 轉換為毫秒
    ...
    def update_outcome(
        self, resolution: ConflictResolution, outcome: float, profit_loss: float
    ):
        """更新解決結果"""
        strategy = resolution.resolution_strategy
        perf = self.strategy_performance[strategy]
        ...
        if profit_loss > 0:
            perf.avg_profit = (
                perf.avg_profit * (perf.total_resolutions - 1) + profit_loss
            ) / perf.total_resolutions
            perf.win_rate = (
                perf.win_rate * (perf.total_resolutions - 1) + 1
            ) / perf.total_resolutions
        else:
            perf.avg_loss = (
                perf.avg_loss * (perf.total_resolutions - 1) + profit_loss
            ) / perf.total_resolutions
            perf.win_rate = (
                perf.win_rate * (perf.total_resolutions - 1)
            ) / perf.total_resolutions
        ...
        features = self._extract_features(
            next(
                (
                    ...
                    if c.conflict_id == resolution.conflict_id
                ),
                None,
            )
        )
    ...
    def get_strategy_recommendations(self) -> Dict[str, Any]:
        ...
        sorted_by_speed = sorted(
            self.strategy_performance.items(), key = lambda x: x[1].resolution_time
        )
        ...
        if strategy_scores:
            ...
            recommendations["strategy_rankings"] = [
                ...
                for (strategy, perf), score in zip(
                    self.strategy_performance.items(), [s for _, s in rankings]
                )
            ]
...
def resolve_signal_conflicts(
    signals: List[TradingSignal], strategy: str = "weighted_vote"
) -> List[ConflictResolution]:
    """便利函數：解決信號衝突"""
