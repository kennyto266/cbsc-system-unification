...
class ExpandedStrategies:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Expanded strategies initialized: {len(self.strategies)} strategies"
        )
    ...
    def _macd_crossover(
        ...
        macd = vbt.MACD.run(
            data["close"], fast_window = fast, slow_window = slow, signal_window = signal
        )
    ...
    def _triple_moving_average(
        ...
        entries = (
            ...
            & (
                ...
                | ~(medium_ma.shift(1) > long_ma.shift(1))
            )
        )
    ...
    def _parabolic_sar(
        ...
        af = pd.Series(
            [acceleration] * len(data), index = data.index
        )  # Acceleration factor
    ...
    def _bollinger_bands(
        ...
        entries = (data["close"] <= bb.lower) & (
            ~(data["close"].shift(1) <= bb.lower.shift(1))
        )
    ...
    def _average_true_range(
        ...
        entries = (close_change > volatility_threshold) & (
            ~(close_change.shift(1) > volatility_threshold.shift(1))
        )
        exits = (close_change < -volatility_threshold) & (
            ~(close_change.shift(1) < -volatility_threshold.shift(1))
        )
    ...
    def _donchian_channels(
        ...
        entries = (data["close"] > upper_channel) & (
            ~(data["close"].shift(1) > upper_channel.shift(1))
        )
        exits = (data["close"] < lower_channel) & (
            ~(data["close"].shift(1) < lower_channel.shift(1))
        )
    ...
    def _volume_weighted_average_price(
        self, data: pd.DataFrame, params: Dict[str, Any]
    ) -> pd.DataFrame:
        """成交量加權平均價策略"""
        period = params.get("period", 20)
    ...
    def _multi_timeframe_rsi(
        ...
        exits = ((rsi_short.rsi > overbought) & (rsi_long.rsi > overbought)) & (
            ~(rsi_short.rsi.shift(1) > overbought)
            | ~(rsi_long.rsi.shift(1) > overbought)
        )

        return pd.DataFrame({"entries": entries, "exits": exits}, index = data.index)

    def _volatility_adjusted_rsi(
        self, data: pd.DataFrame, params: Dict[str, Any]
    ) -> pd.DataFrame:
        """波動率調整RSI策略"""
        rsi_period = params.get("rsi_period", 14)
        ...
        entries = (rsi.rsi < adjusted_oversold) & (
            ~(rsi.rsi.shift(1) < adjusted_oversold.shift(1))
        )
        exits = (rsi.rsi > adjusted_overbought) & (
            ~(rsi.rsi.shift(1) > adjusted_overbought.shift(1))
        )
    ...
    def _ichimoku_cloud(
        ...
        entries = (
            ...
            & (
                ~(close.shift(1) > cloud_top.shift(1))
