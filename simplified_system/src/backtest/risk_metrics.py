    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "tail_risk": {
                ...
                "conditional_value_at_risk": round(
                    self.conditional_value_at_risk * 100, 2
                ),
            },
            "metadata": {
...
class AdvancedRiskMetrics:
    ...
    def calculate_risk_metrics(
        ...
        try:
            ...
            drawdown_metrics = self._calculate_drawdown_metrics(
                portfolio_value if portfolio_value is not None else returns
            )
    ...
    def _calculate_sharpe_variants(
        ...
        if len(downside_returns) > 0 and downside_returns.std() > 0:
            sortino_ratio = excess_returns.mean() / (
                downside_returns.std() * np.sqrt(self.config.trading_days_per_year)
            )
        else:
            sortino_ratio = 0.0
        ...
        if benchmark_returns is not None:
            ...
            if beta != 0:
                treynor_ratio = (
                    excess_returns.mean() * self.config.trading_days_per_year / beta
                )
            else:
                treynor_ratio = 0.0
        else:
    ...
    def _calculate_drawdown_metrics(self, values: pd.Series) -> Dict[str, Any]:
        ...
        if values is None or len(values) == 0:
            return {"max_dd": 0.0, "avg_dd": 0.0, "duration": 0, "recovery": 0}

        # 計算回撤
        name_str = str(values.name) if values.name is not None else ""
        if (
            "close" in name_str.lower()
            or "price" in name_str.lower()
    ...
    def _calculate_market_risk(
        ...
        alpha = (
            aligned_returns.mean() * self.config.trading_days_per_year
            - self.config.risk_free_rate
            - beta
            * (
                aligned_benchmark.mean() * self.config.trading_days_per_year
                - self.config.risk_free_rate
            )
        )
        ...
        if up_periods.sum() > 0:
            up_capture = (
                ...
                if aligned_benchmark[up_periods].mean() != 0
                else 0
            )
        else:
            up_capture = 0.0
        ...
        if down_periods.sum() > 0:
            down_capture = (
                ...
                if aligned_benchmark[down_periods].mean() != 0
                else 0
            )
        else:
            down_capture = 0.0
    ...
    def _calculate_other_metrics(
        self, returns: pd.Series, portfolio_value: Optional[pd.Series]
    ) -> Dict[str, float]:
        """計算其他專業指標"""
        ...
        # Burke比率
        sqrt_drawdown_sums = self._calculate_burke_ratio(
            portfolio_value if portfolio_value is not None else returns
        )
    ...
    def _calculate_pain_index(self, values: pd.Series) -> float:
        ...
        return (
            drawdown[drawdown < 0].sum() / len(drawdown)
    ...
    def _calculate_tail_risk(self, returns: pd.Series) -> Dict[str, float]:
        ...
        return {"tail_ratio": tail_ratio, "cvar": cvar}
...
def calculate_risk_metrics(
    ...
    return calculator.calculate_risk_metrics(
        returns, benchmark_returns, portfolio_value
    )


def calculate_portfolio_risk(
    portfolio_returns: pd.Series, market_returns: Optional[pd.Series] = None
) -> Dict[str, float]:
    """便利函數：計算投資組合風險"""
