...
class ThreadedOptimizer:
    ...
    def _execute_vectorbt_optimization(self, task: OptimizationTask) -> Dict[str, Any]:
        ...
        return engine.optimize_parameters(
            data = task.data, strategy = task.strategy, param_ranges = task.param_ranges
        )
    ...
    def run_optimization(self, timeout: Optional[float] = None) -> Dict[str, Any]:
        ...
        try:
            with ThreadPoolExecutor(max_workers = self.max_workers) as executor:
                ...
                while True:
                    ...
                    while (
                        len(futures) < self.max_workers and not self.task_queue.empty()
                    ):
                        try:
                            task = self.task_queue.get(timeout = 0.1)
                            future = executor.submit(
                                self.execute_task, task, len(futures)
                            )
        ...
        logger.info(
            f"Threaded optimization completed: {completed_tasks}/{total_tasks} tasks successful"
        )
