            ...
            "execution_stats": {
                ...
                "success_rate": round(
                    (
                        ...
                        / (self.successful_optimizations + self.failed_optimizations)
                        if (self.successful_optimizations + self.failed_optimizations)
                        > 0
                        else 0
                    ),
                    3,
...
class WalkForwardAnalyzer:
    ...
    def run_walk_forward_analysis(
        ...
        try:
            ...
            if len(data) < self.config.window_size + self.config.test_size:
                raise ValueError(
                    f"Insufficient data: {len(data)} < {self.config.window_size + self.config.test_size}"
                )
            ...
            for i, window in enumerate(windows):
                try:
                    ...
                    optimal_params, optimization_results = self._optimize_parameters(
                        window["train_data"], strategy_name, param_dict
                    )

                    # 測試階段
                    test_result = self._test_strategy(
                        window["test_data"], strategy_name, optimal_params
                    )
            ...
            # 參數穩定性分析
            param_stability, most_frequent_params = self._analyze_parameter_stability(
                period_results
            )
            ...
            logger.info(
                f"Walk - forward analysis completed in {result.total_execution_time:.3f}s"
            )
            return result
    ...
    def _optimize_parameters(
        self, train_data: pd.DataFrame, strategy: str, param_ranges: Dict[str, Any]
    ...
    def _test_strategy(
        self, test_data: pd.DataFrame, strategy: str, params: Dict[str, Any]
    ) -> Dict[str, float]:
        """在測試數據上測試策略"""
    ...
    def _backtest_strategy(
        self, data: pd.DataFrame, strategy: str, params: Dict[str, Any]
    ) -> Dict[str, float]:
        """回測策略的通用方法"""
        try:
            # 根據策略生成信號
            ...
            total_trades = (
                len(portfolio.trades.records_readable)
