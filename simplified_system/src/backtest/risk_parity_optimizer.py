        ...
        return {
            ...
            "risk_contributions": {
                "marginal_contributions": {
                    ...
                    for asset, marginal in zip(
                        self.assets, self.risk_contributions.marginal_contributions
                    )
                },
                "risk_contributions": {
                    asset: round(risk, 4)
                    for asset, risk in zip(
                        self.assets, self.risk_contributions.risk_contributions
                    )
                },
                "percentage_contributions": {
                    asset: round(pct * 100, 2)
                    for asset, pct in zip(
                        self.assets, self.risk_contributions.percentage_contributions
                    )
                },
                "systematic_risk": round(self.risk_contributions.systematic_risk, 4),
                "idiosyncratic_risk": round(
                    self.risk_contributions.idiosyncratic_risk, 4
                ),
                "diversification_ratio": round(
                    self.risk_contributions.diversification_ratio, 3
                ),
                "budget_error": round(self.risk_contributions.budget_error, 6),
                "parity_satisfaction": round(
                    self.risk_contributions.parity_satisfaction * 100, 2
                ),
            },
            "optimization_info": {
            ...
            "hierarchical_structure": {
                "clusters": (
                    self.clusters.tolist() if self.clusters is not None else None
                ),
                "cluster_weights": (
                    self.cluster_weights.tolist()
...
class RiskParityOptimizer:
    ...
    def optimize_equal_risk_contribution(
        ...
        try:
            logger.info(
                f"Starting equal risk contribution optimization for {returns.shape[1]} assets"
            )
            ...
            if risk_budget is None:
                if self.config.equal_risk_budget:
                    ...
                    raise ValueError(
                        "Risk budget must be specified when equal_risk_budget is False"
                    )
            ...
            risk_contributions = self._calculate_risk_contributions(
                result.x, cov_matrix, risk_budget
            )
            ...
            logger.info(
                f"Equal risk contribution optimization completed in {optimization_result.calculation_time:.3f}s"
            )
    ...
    def optimize_risk_budgeting(
        ...
        try:
            logger.info(
                f"Starting risk budgeting optimization for {returns.shape[1]} assets"
            )
            ...
            logger.info(
                f"Risk budgeting optimization completed in {optimization_result.calculation_time:.3f}s"
            )
            return optimization_result
    ...
    def optimize_hierarchical_risk_parity(
        ...
        try:
            logger.info(
                f"Starting hierarchical risk parity optimization for {returns.shape[1]} assets"
            )
            ...
            clusters, cluster_order = self._hierarchical_clustering(
                cov_matrix, linkage_method
            )
            ...
            risk_contributions = self._calculate_risk_contributions(
                weights, cov_matrix, risk_budget
            )
            ...
            logger.info(
                f"Hierarchical risk parity optimization completed in {optimization_result.calculation_time:.3f}s"
            )
            return optimization_result
    ...
    def _build_constraints(
        ...
        if self.config.leverage_limit < float("inf"):
            constraints_list.append(
                {
                    "type": "ineq",
                    "fun": lambda weights: self.config.leverage_limit
    ...
    def _optimize_risk_budgeting(
        ...
        def risk_budgeting_objective(weights: np.ndarray) -> float:
            ...
            # 添加交易成本惩罚
            trading_cost_penalty = self.config.trading_cost * np.sum(
                np.abs(weights - initial_weights)
            )
    ...
    def _hierarchical_clustering(
        self, cov_matrix: np.ndarray, linkage_method: str
        ...
        try:
            ...
            correlation = np.corrcoef(
                np.sqrt(np.diag(cov_matrix)) * cov_matrix / np.sqrt(np.diag(cov_matrix))
            )
            ...
            logger.warning(
                "SciPy hierarchical clustering not available, using simple ordering"
            )
            # 简单的替代方案：基于相关性排序
            correlation = np.corrcoef(
                np.sqrt(np.diag(cov_matrix)) * cov_matrix / np.sqrt(np.diag(cov_matrix))
    ...
    def _recursive_bisection(
        ...
        left_weights = self._recursive_bisection(
            left_cov, list(range(len(left_cluster)))
        )
        right_weights = self._recursive_bisection(
            right_cov, list(range(len(right_cluster)))
        )
        ...
        inverse_volatility_split = (
            1 / np.sqrt(left_var) / (1 / np.sqrt(left_var) + 1 / np.sqrt(right_var))
        )
    ...
    def _calculate_risk_contributions(
        self, weights: np.ndarray, cov_matrix: np.ndarray, risk_budget: np.ndarray
    ) -> RiskContribution:
        """计算风险贡献"""
        portfolio_variance = np.dot(weights.T, np.dot(cov_matrix, weights))
        ...
        diversification_ratio = np.sum(
            weights * np.sqrt(np.diag(cov_matrix))
    ...
    def _calculate_portfolio_metrics(
        ...
        sharpe_ratio = (
            excess_return / volatility if volatility > self.config.epsilon else 0.0
        )
...
def optimize_hierarchical_risk_parity(
    ...
    """便利函数：层次风险平价优化"""
    optimizer = RiskParityOptimizer()
    return optimizer.optimize_hierarchical_risk_parity(
        returns, linkage_method, constraints
    )
