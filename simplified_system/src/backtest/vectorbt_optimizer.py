...
class OptimizationConfig:
    ...
    objective: str = (
        "sharpe_ratio"  # 'sharpe_ratio', 'total_return', 'calmar_ratio', 'max_drawdown'
    )
    maximize: bool = True
...
class VectorBTOptimizer:
    ...
    def optimize_strategy(
        self, data: pd.DataFrame, strategy: str, param_ranges: Dict[str, List]
    ) -> Dict:
        """
        單策略參數優化
        ...
        """
        logger.info(
            f"Starting optimization for {strategy} with {len(param_ranges)} parameters"
        )
    ...
    def optimize_multi_objective(
        ...
        results = self._evaluate_parameter_combinations(
            data, strategy, param_combinations, objectives
        )
    ...
    def optimize_walk_forward(
        ...
        for start_idx in range(0, total_points - window_size, step_size):
            ...
            optimal_params = self._find_optimal_params(
                train_data, strategy, param_ranges
            )

            # 測試期評估
            test_data = data.iloc[test_start:test_end]
            test_result = self.engine.backtest_strategy(
                test_data, strategy, optimal_params
            )
        ...
        overall_performance = self._calculate_portfolio_metrics(
            np.array(out_of_sample_returns)
        )
    ...
    def _optimize_rsi_strategy(
        self, data: pd.DataFrame, param_combinations: List[Dict]
    ) -> Dict:
        """RSI策略優化"""
        close = data["close"]
        ...
        portfolios = vbt.Portfolio.from_signals(
            close = close, entries = buy_signals, exits = sell_signals, freq="D"
        )
    ...
    def _optimize_bollinger_strategy(
        self, data: pd.DataFrame, param_combinations: List[Dict]
    ) -> Dict:
        """布林帶策略優化"""
        close = data["close"]
    ...
    def _optimize_generic_strategy(
        self, data: pd.DataFrame, strategy: str, param_combinations: List[Dict]
    ) -> Dict:
        """通用策略優化"""
        best_result = None
    ...
    def _evaluate_parameter_combinations(
        self,
        data: pd.DataFrame,
        strategy: str,
