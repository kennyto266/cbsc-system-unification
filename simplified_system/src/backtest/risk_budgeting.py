...
class RiskBudgetConfig:
    ...
    risk_factors: List[str] = field(
        default_factory = lambda: ["market", "size", "value", "momentum", "quality"]
    )
...
class RiskBudgetingFramework:
    ...
    def create_equal_risk_budget(
        self, assets: List[str], name: str = "equal_risk_budget"
    ) -> RiskBudget:
        """
        创建等风险预算
    ...
    def allocate_portfolio(
        ...
        result = self.rp_optimizer.optimize_risk_budgeting(
            aligned_returns, budget.budget_allocation, constraints
        )
    ...
    def create_target_volatility_budget(
        ...
        self.risk_budgets[name] = budget
        logger.info(
            f"Created target volatility budget '{name}' with vol={target_vol:.2%}"
        )
    ...
    def monitor_budget_drift(
        ...
        # 判断是否需要再平衡
        needs_rebalance = (
            drift_magnitude > self.config.rebalance_threshold
            or np.max(np.abs(drift)) > self.config.drift_tolerance
        )

        # 计算跟踪误差 (如果有基准)
        tracking_error = 0.0
        if self.config.benchmark_weights is not None:
            active_returns = recent_returns @ (
                current_allocation - self.config.benchmark_weights
            )
            tracking_error = active_returns.std() * np.sqrt(
        ...
        logger.info(
            f"Budget drift monitoring completed for '{budget_name}': drift={drift_magnitude:.4f}"
        )
    ...
    def rebalance_portfolio(
        ...
        # 决策：是否执行再平衡
        should_rebalance = (
            expected_improvement > 0 and trade_costs < expected_improvement * 0.1
        )
    ...
    def stress_test_budget(
        ...
        for scenario_name, stressed_returns in stress_scenarios.items():
            try:
                # 在压力情景下重新分配
                stressed_allocation = self.allocate_portfolio(
                    stressed_returns, budget_name
                )
                ...
                stress_impact = {
                    ...
                    "sharpe_decline": baseline_allocation.sharpe_ratio
                    - stressed_allocation.sharpe_ratio,
                    "baseline_volatility": baseline_allocation.volatility,
                    "stressed_volatility": stressed_allocation.volatility,
                    "volatility_increase": stressed_allocation.volatility
                    - baseline_allocation.volatility,
                    "weight_changes": np.abs(
                        stressed_allocation.weights - baseline_allocation.weights
                    ).sum(),
                ...
                logger.warning(
                    f"Stress test failed for scenario '{scenario_name}': {e}"
                )
        ...
        logger.info(
            f"Stress testing completed for budget '{budget_name}' with {len(stress_scenarios)} scenarios"
        )

        return stress_summary

    def compare_budgets(
        self, returns: pd.DataFrame, budget_names: List[str]
    ) -> pd.DataFrame:
        """
        比较多个风险预算
        ...
        for budget_name in budget_names:
            try:
                ...
                comparison_results.append({"budget_name": budget_name, "error": str(e)})
            ...
            summary_data.append(
                {
                    ...
                    "description": (
                        budget.description[:50] + "..."
                        if len(budget.description) > 50
                        else budget.description
                    ),
                }
            )
...
# 便利函数
def create_risk_budgeting_framework(
