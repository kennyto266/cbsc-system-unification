...
from .dynamic_allocator import AllocationResult, AssetConfig, DynamicAssetAllocator
from .market_regime import MarketRegimeDetector, RegimePrediction, RegimeState
...
class OverlayConfig:
    ...
    signal_aggregation_method: str = (
        "weighted_average"  # 'weighted_average', 'max_strength', 'consensus'
    )
...
class TacticalOverlaySystem:
    ...
    def generate_signals(
        ...
        logger.info(
            f"Generated {len(ranked_signals)} new signals. Total active: {len(self.active_signals)}"
        )
    ...
    def _generate_seasonal_signals(
        self, market_data: Dict[str, pd.DataFrame]
    ) -> List[OverlaySignal]:
        """生成季節性信號"""
        signals = []
    ...
    def _generate_risk_on_risk_off_signals(
        self, market_data: Dict[str, pd.DataFrame]
    ) -> List[OverlaySignal]:
    ...
    def apply_tactical_overlay(
        ...
        # 風險分析
        risk_analysis = self._analyze_overlay_risk(tactical_adjustments, market_data)
        ...
        result = OverlayResult(
            ...
            cost_efficiency = self._calculate_cost_efficiency(
                overlay_costs, tactical_adjustments
            ),
        ...
        logger.info(
            f"Tactical overlay applied. Total cost: {result.total_overlay_cost:.4f}"
        )
            ...
            if symbol in signal_adjustments:
                ...
                if total_weight > 0:
                    ...
                    limited_adjustment = np.clip(
                        average_adjustment, -max_adjustment, max_adjustment
                    )
    ...
    def _calculate_cost_efficiency(
        self, costs: Dict[str, float], adjustments: Dict[str, float]
    ) -> float:
        """計算成本效率"""
...
def apply_tactical_overlay_to_allocation(
    ...
    """便利函數：應用戰術覆蓋到配置"""
    # 創建基礎配置器
    from .dynamic_allocator import AllocationConfig, DynamicAssetAllocator
