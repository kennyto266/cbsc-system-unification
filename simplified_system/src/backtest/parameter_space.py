...
class ParameterRange:
    ...
    def generate_values(self, max_values: int = 100) -> List[Union[int, float]]:
        ...
        if self.dtype == int:
            values = list(
                range(int(self.min_value), int(self.max_value) + 1, int(step))
            )
        else:
    ...
    def _apply_constraints(
        ...
        for constraint_type, constraint_value in self.constraints.items():
            if constraint_type == "exclude":
                filtered_values = [
                    v for v in filtered_values if v not in constraint_value
                ]
                ...
                filtered_values = [
                    v for v in filtered_values if v % constraint_value == 0
                ]
...
class StrategyParameterSpace:
    ...
    def generate_parameter_combinations(
        self, max_combinations: Optional[int] = None, smart_filter: bool = True
    ) -> List[Dict[str, Any]]:
        """生成參數組合"""
        ...
        # 轉換為字典格式
        combinations = [dict(zip(param_names, combo)) for combo in all_combinations]
        ...
        if len(combinations) > max_combinations:
            combinations = self._select_diverse_combinations(
                combinations, max_combinations
            )

        logger.info(
            f"Generated {len(combinations)} parameter combinations for {self.strategy_name}"
        )
        return combinations

    def _smart_filter_combinations(
        ...
        for combo in combinations:
            ...
            for param_range in self.parameter_ranges:
                ...
                if param_value is not None:
                    # 根據參數的重要性和常見值給分
                    score += param_range.importance * self._parameter_score(
    ...
    def _parameter_score(
        self, param_range: ParameterRange, value: Union[int, float]
    ) -> float:
        """計算參數值分數"""
        # 偏向中間範圍的值
    ...
    def _select_diverse_combinations(
        ...
        try:
            ...
            X = np.array(
                [[combo[name] for name in param_names] for combo in combinations]
            )
...
class ParameterSpaceManager:
    ...
    def get_strategy_space(
        self, strategy_name: str
    ) -> Optional[StrategyParameterSpace]:
        """獲取策略參數空間"""
    ...
    def add_strategy_space(self, strategy_space: StrategyParameterSpace):
        ...
        logger.info(
            f"Added parameter space for strategy: {strategy_space.strategy_name}"
        )
        ...
        for strategy_name in strategy_names:
            if strategy_name in self.strategy_spaces:
                ...
                try:
                    ...
                    logger.error(
                        f"Error generating combinations for {strategy_name}: {e}"
                    )
                    all_combinations[strategy_name] = []
            else:
                logger.warning(
                    f"Strategy {strategy_name} not found in parameter spaces"
                )
                all_combinations[strategy_name] = []
    ...
    def get_space_statistics(self) -> Dict[str, Any]:
        """獲取參數空間統計信息"""
        stats = {"total_strategies": len(self.strategy_spaces), "strategy_details": {}}
    ...
    def visualize_parameter_space(
        self, strategy_name: str, save_path: Optional[str] = None
    ):
        """可視化參數空間"""
        ...
        try:
            ...
            filtered_combinations = space.generate_parameter_combinations(
                smart_filter = True
            )
...
def get_strategy_parameters(
    strategy_name: str, max_combinations: int = 1000
) -> List[Dict[str, Any]]:
    """獲取策略參數組合"""
...
def get_all_strategy_parameters(
    max_combinations_per_strategy: int = 1000,
) -> Dict[str, List[Dict[str, Any]]]:
    """獲取所有策略參數組合"""
