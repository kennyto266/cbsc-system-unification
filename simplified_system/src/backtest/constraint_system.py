...
class PortfolioConstraint(ABC):
    ...
    @abstractmethod
    def check(
        self, weights: np.ndarray, data: Optional[Dict[str, Any]] = None
    ) -> ConstraintResult:
        """檢查約束是否滿足"""
...
class WeightBoundConstraint(PortfolioConstraint):
    ...
    def check(
        ...
        for i, (weight, asset) in enumerate(zip(weights, asset_names)):
            asset_min = self.asset_bounds.get(
                asset, (self.min_weight, self.max_weight)
            )[0]
            asset_max = self.asset_bounds.get(
                asset, (self.min_weight, self.max_weight)
            )[1]
        ...
        limit_value = (
            self.max_weight if current_max > self.max_weight else self.min_weight
        )
...
class PositionLimitConstraint(PortfolioConstraint):
    ...
    def check(
        ...
        violation_percentage = (
            (violation_amount / self.max_positions * 100)
...
class SectorConstraint(PortfolioConstraint):
    ...
    def check(
        ...
        is_satisfied = len(violations) == 0
        max_sector_limit = (
            max([max for _, max in self.sector_limits.values()])
        ...
        for sector, (min_limit, max_limit) in self.sector_limits.items():
            ...
            if sector_indices:
                ...
                constraints.append(
                    {
                        "type": "ineq",
                        "fun": lambda w, idx = sector_indices, max_w = max_limit: max_w
                        - np.sum(w[idx]),
                    }
                )
...
class TurnoverConstraint(PortfolioConstraint):
    ...
    def check(
        ...
        violation_percentage = (
            (violation_amount / self.max_turnover * 100)
        ...
        affected_assets = [
            asset
            for i, (asset, change) in enumerate(zip(asset_names, changes))
            if change >= threshold
        ]
...
class RiskBudgetConstraint(PortfolioConstraint):
    ...
    def check(
        ...
        return ConstraintResult(
            ...
            violation_amount = max_violation,
            violation_percentage=(
                (max_violation / limit_value * 100) if limit_value > 0 else float("inf")
            ),
            affected_assets = affected_assets,
            timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
...
class ConstraintSystem:
    ...
    def add_constraint(self, constraint: PortfolioConstraint) -> None:
        ...
        logger.info(
            f"Added constraint: {constraint.name} ({constraint.constraint_type})"
        )
    ...
    def generate_scipy_constraints(
        ...
        # 如果沒有邊界，使用默認邊界
        if not bounds_list:
            bounds_list = [(self.config.min_weight, self.config.max_weight)] * len(
                asset_names
            )
    ...
    def generate_constraint_report(
        ...
        try:
            ...
            for result in results:
                ...
                report_lines.append(
                    f"Constraint: {result.constraint_name} ({result.constraint_type})"
                )
                ...
                if not result.is_satisfied:
                    report_lines.append(
                        f"  Violation Amount: {result.violation_amount:.6f}"
                    )
                    report_lines.append(
                        f"  Violation Percentage: {result.violation_percentage:.2f}%"
                    )
                    if result.affected_assets:
                        report_lines.append(
                            f"  Affected Assets: {', '.join(result.affected_assets[:5])}"
                        )
                        if len(result.affected_assets) > 5:
                            report_lines.append(
                                f"    ... and {len(result.affected_assets) - 5} more"
                            )
            ...
            for i, (asset, weight) in enumerate(weight_data[:10]):
                report_lines.append(
                    f"  {i + 1:2d}. {asset}: {weight:.4f} ({weight * 100:.2f}%)"
                )

            zero_weights = [
                (asset, weight) for asset, weight in weight_data if weight < 1e - 6
            ]
            if zero_weights:
                ...
                if len(zero_weights) <= 5:
                    ...
                    report_lines.append(
                        f"  - {zero_weights[0][0]}, {zero_weights[1][0]}, {zero_weights[2][0]}, ... ({len(zero_weights)-3} more)"
                    )
...
def create_basic_constraint_system(
    ...
    system.add_constraint(
        WeightBoundConstraint(min_weight = min_weight, max_weight = max_weight)
    )
...
def create_turnover_constraint_system(
    previous_weights: np.ndarray, max_turnover: float = 1.0
) -> ConstraintSystem:
    """創建周轉率約束系統"""
    system = create_basic_constraint_system()
    system.add_constraint(
        TurnoverConstraint(max_turnover = max_turnover, previous_weights = previous_weights)
    )
