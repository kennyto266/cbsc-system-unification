...
from .strategy_attribution import AttributionConfig, StrategyAttributionAnalyzer
from .strategy_correlation import CorrelationConfig, StrategyCorrelationAnalyzer
...
class StrategyCombinationConfig:
    ...
    optimization_method: str = (
        "sharpe_ratio"  # sharpe_ratio, risk_adjusted_return, utility
    )
...
class StrategyCombinationOptimizer:
    ...
    def optimize_strategy_combinations(
        ...
        logger.info(
            f"Starting strategy combination optimization with {len(available_strategies)} strategies"
        )

        try:
            # 预处理策略
            strategy_performances = self._preprocess_strategies(
                price_data, available_strategies
            )

            # 策略相关性分析
            correlation_matrix = self._analyze_strategy_correlations(
            ...
            for i, combination in enumerate(candidate_combinations):
                logger.info(
                    f"Optimizing combination {i + 1}/{len(candidate_combinations)}"
                )
            ...
            logger.info(
                f"Strategy combination optimization completed in {result.optimization_time:.3f}s"
            )
            return result
    ...
    def _preprocess_strategies(
        ...
        for strategy_name, parameters in available_strategies:
            try:
                ...
                signals = self.expanded_strategies.generate_signals(
                    price_data, strategy_name, parameters
                )
        ...
        logger.info(
            f"Filtered {len(filtered_strategies)}/{len(strategy_performances)} strategies"
        )
        return filtered_strategies

    def _analyze_strategy_correlations(
        self, strategy_performances: List[StrategyInfo]
    ) -> pd.DataFrame:
        """分析策略相关性"""
    ...
    def _generate_candidate_combinations(
        ...
        sorted_strategies = sorted(
            strategy_performances, key = lambda x: x.sharpe_ratio, reverse = True
        )

        # 生成不同大小的组合
        for size in range(
            2, min(self.config.max_strategies_per_combination + 1, n_strategies + 1)
        ):
            # 基于性能和相关性生成组合
        ...
        try:
            ...
            if optimization_objective == "sharpe_ratio":
                optimal_weights = self._optimize_for_sharpe_ratio(
                    returns_matrix, strategies
                )
            elif optimization_objective == "risk_adjusted_return":
                optimal_weights = self._optimize_for_risk_adjusted_return(
                    returns_matrix, strategies
                )
            ...
            correlation_metrics = self._calculate_correlation_metrics(
                optimal_weights, returns_matrix
            )
            cost_metrics = self._calculate_cost_metrics(optimal_weights, returns_matrix)
            attribution_metrics = self._calculate_attribution_metrics(
                optimal_weights, strategies
            )
    ...
    def _optimize_for_sharpe_ratio(
        self, returns_matrix: np.ndarray, strategies: List[StrategyInfo]
    ) -> np.ndarray:
        """为最大化Sharpe比率优化权重"""
    ...
    def _generate_strategy_returns_matrix(
        self, price_data: pd.DataFrame, strategies: List[StrategyInfo]
    ) -> Optional[np.ndarray]:
        """生成策略收益矩阵"""
        try:
            returns_matrix = []
    ...
    def _calculate_correlation_metrics(
        ...
        if returns_matrix.shape[1] > 1:
            ...
            avg_correlation = (np.sum(corr_matrix) - n_strategies) / (
        ...
        weighted_vol = np.sqrt(
            np.dot(weights.T, np.dot(np.cov(returns_matrix.T), weights))
        )
    ...
    def _calculate_cost_metrics(
        ...
        if len(returns_matrix) > 1:
            ...
            expected_turnover = (
                np.mean(strategy_volatilities) * np.sqrt(252) * 0.5
            )  # 经验系数
        else:
    ...
    def _calculate_attribution_metrics(
        self, weights: np.ndarray, strategies: List[StrategyInfo]
    ) -> Dict[str, float]:
        """计算归因指标"""
    ...
    def _calculate_stability_metrics(
        ...
        if rolling_returns:
            performance_consistency = 1.0 - (
                np.std(rolling_returns) / (np.mean(rolling_returns) + 1e - 8)
            )
    ...
    def _select_best_combination(
        self, combinations: List[StrategyCombination], optimization_objective: str
    ) -> StrategyCombination:
        """选择最佳策略组合"""
        if not combinations:
        ...
        if optimization_objective == "sharpe_ratio":
            ...
            best = max(
                combinations, key = lambda c: c.expected_return / (c.volatility + 1e - 8)
            )
        elif optimization_objective == "utility":
            # 均值 - 方差效用
            risk_aversion = 3.0
    ...
    def _stability_test(
        self, price_data: pd.DataFrame, combinations: List[StrategyCombination]
        ...
        for combination in combinations:
            ...
            for i in range(n_periods):
                ...
                end_idx = (
                    (i + 1) * period_length if i < n_periods - 1 else len(price_data)
                )
        ...
        logger.info(
            f"Stability testing completed: {len(stable_combinations)}/{len(combinations)} stable combinations"
        )
        return stable_combinations

    def _generate_performance_summary(
        self, combinations: List[StrategyCombination]
    ) -> Dict[str, Any]:
        """生成性能总结"""
        if not combinations:
            return {}
    ...
    def _generate_attribution_summary(
        ...
        for combination in combinations:
            for i, strategy in enumerate(combination.strategies):
                ...
                # 频率统计
                strategy_frequency[strategy_name] = (
                    strategy_frequency.get(strategy_name, 0) + 1
                )

                # 贡献统计
                contribution = weight * combination.sharpe_ratio
                strategy_contribution[strategy_name] = (
                    strategy_contribution.get(strategy_name, 0) + contribution
                )
        ...
        return {
            ...
            "top_strategies": sorted(
                strategy_contribution.items(), key = lambda x: x[1], reverse = True
            )[:10],
        }
    ...
    def _generate_cluster_based_combinations(
        ...
        try:
            ...
            if SKLEARN_AVAILABLE:
                ...
                squareform(correlation_distances)
                ...
                for cluster_id in range(n_clusters):
                    ...
                    if cluster_strategies:
                        # 选择每个聚类中Sharpe最高的策略
                        best_in_cluster = max(
                            cluster_strategies, key = lambda s: s.sharpe_ratio
                        )
            ...
            for i in range(0, len(strategy_performances), 3):
                for j in range(i + 1, min(i + 4, len(strategy_performances))):
                    for k in range(j + 1, min(i + 5, len(strategy_performances))):
                        if (
                            i < len(strategy_performances)
                            and j < len(strategy_performances)
                            and k < len(strategy_performances)
                        ):
                            combination = [
                                strategy_performances[i],
