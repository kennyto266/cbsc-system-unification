...
class ComprehensiveCombinationBacktester:
    ...
    def calculate_portfolio_signals(
        ...
        try:
            ...
            for i, (date, idx) in enumerate(
                zip(stock_data.index, range(len(stock_data)))
            ):
    ...
    def calculate_government_combination_score(
        self,
        combination: Set[str],
        gov_indicators: Dict[str, Dict[str, float]],
        date: datetime,
    ...
    def backtest_combination(
        ...
        try:
            # 計算信號
            signals, scores = self.calculate_portfolio_signals(
                combination, stock_data, gov_indicators
            )
    ...
    def run_comprehensive_backtest(
        self, stock_data: pd.DataFrame, gov_indicators: Dict[str, Dict[str, float]]
    ) -> Dict[str, Any]:
        """運行全組合回測"""
        try:
            logger.info("開始全組合回測...")
