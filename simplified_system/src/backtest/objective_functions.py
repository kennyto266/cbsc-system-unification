...
class PortfolioObjective:
    ...
    def gradient(
        self, weights: np.ndarray, returns: pd.DataFrame, **kwargs
    ) -> np.ndarray:
        """计算目标函数梯度"""
        # 默认数值梯度
...
class SharpeRatioObjective(PortfolioObjective):
    ...
    def evaluate(self, weights: np.ndarray, returns: pd.DataFrame, **kwargs) -> float:
        ...
        volatility = portfolio_returns.std() * np.sqrt(
...
class TradingCostObjective(PortfolioObjective):
    ...
    def evaluate(self, weights: np.ndarray, returns: pd.DataFrame, **kwargs) -> float:
        """计算交易成本"""
        # 如果没有提供前一个权重，使用等权重
        previous_weights = kwargs.get(
            "previous_weights", np.ones(len(weights)) / len(weights)
        )
...
class ObjectiveFactory:
    ...
    def create_objective(
        name: str, config: Optional[ObjectiveConfig] = None, **kwargs
    ) -> PortfolioObjective:
        """创建目标函数实例"""
        objectives = {
...
def create_risk_objective(
    risk_type: str = "var", config: Optional[ObjectiveConfig] = None
) -> PortfolioObjective:
    """创建风险目标函数"""
