...
class StandardizedSharpeCalculator:
    ...
    def _preprocess_returns(
        self, returns: Union[np.ndarray, List[float], pd.Series]
    ) -> np.ndarray:
        """預處理收益率數據"""
        # 轉換為numpy數組
    ...
    def _calculate_statistics(self, returns: np.ndarray) -> Dict[str, float]:
        ...
        if len(downside_returns) > 0:
            downside_volatility = (
                np.std(downside_returns, ddof = 1) * self.annualization_factor
            )
    ...
    def validate_sharpe_calculation(
        self, returns: Union[np.ndarray, List[float]]
    ) -> Dict[str, Any]:
        """
        驗證Sharpe計算的合理性
        ...
        if results["data_points"] < 50:
            validation["warnings"].append(
                f"Low data points: {results['data_points']} (< 50)"
            )
        ...
        if results["annual_volatility"] < 0.05:  # 5%以下
            validation["warnings"].append(
                f"Very low volatility: {results['annual_volatility']:.3f}"
            )
        elif results["annual_volatility"] > 1.0:  # 100%以上
            validation["warnings"].append(
                f"Very high volatility: {results['annual_volatility']:.3f}"
            )

        return validation

    def get_recommended_sharpe(
        self, returns: Union[np.ndarray, List[float]]
    ) -> Dict[str, float]:
        """
        獲取推薦的Sharpe比率計算結果
...
def get_sharpe_calculator(
    risk_free_rate: float = 0.03, trading_days: int = 252
    ...
    if (
        _global_calculator is None
        or _global_calculator.risk_free_rate != risk_free_rate
        or _global_calculator.trading_days != trading_days
    ):
