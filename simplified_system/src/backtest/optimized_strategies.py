...
class OptimizedStrategySignals:
    ...
    def rsi_strategy_signals_optimized(
    ...
    def macd_strategy_signals_optimized(
        self, data: pd.DataFrame, params: Dict[str, Any]
    ) -> pd.DataFrame:
        """优化的MACD策略信号生成"""
        ...
        signal_line = pd.Series(
            macd_data["MACD_Signal"], index = data.index, name="Signal"
        )
        ...
        return pd.DataFrame(
            {"entries": golden_cross, "exits": death_cross}, index = data.index
        )

    def bollinger_strategy_signals_optimized(
        self, data: pd.DataFrame, params: Dict[str, Any]
    ) -> pd.DataFrame:
        """优化的布林带策略信号生成"""
        ...
        # 生成交易信号 - 向量化操作
        price_cross_below_lower = (close < bb_lower) & (
            close.shift(1) >= bb_lower.shift(1)
        )
        price_cross_above_upper = (close > bb_upper) & (
            close.shift(1) <= bb_upper.shift(1)
        )

        return pd.DataFrame(
            {"entries": price_cross_below_lower, "exits": price_cross_above_upper},
            index = data.index,
        )

    def dual_ma_strategy_signals_optimized(
        self, data: pd.DataFrame, params: Dict[str, Any]
    ) -> pd.DataFrame:
        """优化的双移动平均策略信号生成"""
    ...
    def momentum_strategy_signals_optimized(
        self, data: pd.DataFrame, params: Dict[str, Any]
    ) -> pd.DataFrame:
        """优化的动量突破策略信号生成"""
        ...
        return pd.DataFrame(
            {"entries": upward_cross, "exits": downward_cross}, index = data.index
        )

    def volatility_strategy_signals_optimized(
        self, data: pd.DataFrame, params: Dict[str, Any]
    ) -> pd.DataFrame:
        """优化的波动率突破策略信号生成"""
        ...
        downward_breakout = (close < lower_band) & (
            close.shift(1) >= lower_band.shift(1)
        )

        return pd.DataFrame(
            {"entries": upward_breakout, "exits": downward_breakout}, index = data.index
        )
    ...
    def _calculate_macd_cpu(self, prices, fast = 12, slow = 26, signal = 9):
        ...
        return {"MACD": macd_line.values, "MACD_Signal": signal_line.values}
    ...
    def _calculate_bollinger_cpu(self, prices, period = 20, std_dev = 2.0):
        ...
        return {"upper": upper.values, "middle": middle.values, "lower": lower.values}
    ...
    def _calculate_ma_cpu(self, prices, short_period = 20, long_period = 50):
        ...
        return {"short_ma": short_ma.values, "long_ma": long_ma.values}
