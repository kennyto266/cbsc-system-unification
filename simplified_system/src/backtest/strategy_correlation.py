...
try:
    ...
    logging.warning(
        "Statsmodels not available. Advanced time series analysis not available"
    )
...
class StrategyCorrelationAnalyzer:
    ...
    def analyze_correlations(
        self, returns_data: pd.DataFrame, methods: Optional[List[str]] = None
        ...
        logger.info(
            f"Starting correlation analysis for {returns_data.shape[1]} strategies"
        )
        ...
        try:
            ...
            logger.info(
                f"Correlation analysis completed in {(datetime.now() - start_time).total_seconds():.3f}s"
            )
            return results
    ...
    def rolling_correlation_analysis(
        ...
        logger.info(
            f"Starting rolling correlation analysis with window={window}, step={step}"
        )

        try:
            returns_data.shape[1]
            strategy_names = returns_data.columns

            # 初始化结果存储
            rolling_correlations = {}
            correlation_volatility = pd.DataFrame(
                index = strategy_names, columns = strategy_names, dtype = float
            )
                    ...
                    if i <= j:  # 只计算上三角矩阵
                        ...
                        if rolling_corr:
                            rolling_correlations[pair_key] = pd.Series(
                                rolling_corr,
                                index = dates,
                                name = f"{strategy1}_vs_{strategy2}",
                            )

                            # 计算波动率和趋势
                            correlation_volatility.loc[strategy1, strategy2] = np.std(
                                rolling_corr
                            )
                            correlation_volatility.loc[strategy2, strategy1] = (
                                correlation_volatility.loc[strategy1, strategy2]
                            )
                            ...
                            if len(rolling_corr) > 1:
                                ...
                                correlation_trends.loc[strategy1, strategy2] = (
                                    trend_slope
                                )
                                correlation_trends.loc[strategy2, strategy1] = (
                                    trend_slope
                                )

            # 计算稳定性指标
            stability_metrics = self._calculate_correlation_stability(
                rolling_correlations
            )
            ...
            logger.info(
                f"Rolling correlation analysis completed in {(datetime.now() - start_time).total_seconds():.3f}s"
            )
            return result
    ...
    def cointegration_analysis(
        self, price_data: pd.DataFrame, method: Optional[str] = None
    ) -> CointegrationResult:
        """
        协整分析
        ...
        try:
            ...
            cointegration_matrix = pd.DataFrame(
                index = asset_names, columns = asset_names, dtype = float
            )
            test_statistics = pd.DataFrame(
                index = asset_names, columns = asset_names, dtype = float
            )
            p_values = pd.DataFrame(index = asset_names, columns = asset_names, dtype = float)
                    ...
                    if i < j:  # 只计算上三角矩阵
                        try:
                            ...
                            if method == "engle_granger":
                                ...
                                cointegration_matrix.loc[asset1, asset2] = (
                                    1 if pvalue < self.config.significance_level else 0
                                )
                                cointegration_matrix.loc[asset2, asset1] = (
                                    cointegration_matrix.loc[asset1, asset2]
                                )
                                ...
                                if pvalue < self.config.significance_level:
                                    ...
                                    coint_vector = np.array(
                                        [-model.params[1], 1.0]
                                    )  # [beta, 1]
                                    cointegration_vectors[f"{asset1}_{asset2}"] = (
                                        coint_vector
                                    )

                                    # 误差修正项
                                    error_term = series1 - model.params[1] * series2
                                    error_correction_terms[f"{asset1}_{asset2}"] = (
                                        error_term
                                    )

                        except Exception as e:
                            logger.warning(
                                f"Cointegration test failed for {asset1}-{asset2}: {e}"
                            )
                            continue
            ...
            logger.info(
                f"Cointegration analysis completed in {(datetime.now() - start_time).total_seconds():.3f}s"
            )
            return result
    ...
    def lead_lag_analysis(
        self, returns_data: pd.DataFrame, max_lag: Optional[int] = None
    ) -> LeadLagResult:
        """
        领先滞后关系分析
        ...
        try:
            ...
            optimal_lags = pd.DataFrame(
                index = strategy_names, columns = strategy_names, dtype = int
            )
                ...
                for j, strategy2 in enumerate(strategy_names):
                    if i != j:
                        try:
                            ...
                            best_lag = 0
                            best_direction = (
                                0  # 1: strategy1 leads, -1: strategy2 leads
                            )
                            ...
                            lead_lag_matrix.loc[strategy1, strategy2] = (
                                best_corr * best_direction
                            )
                            optimal_lags.loc[strategy1, strategy2] = (
                                best_lag * best_direction
                            )
                            ...
                            if len(series1) > max_lag and len(series2) > max_lag:
                                try:
                                    ...
                                    gc_test_21[max_lag][0]["ssr_ftest"][0]
                                    ...
                                    logger.warning(
                                        f"Granger causality test failed for {strategy1}-{strategy2}: {e}"
                                    )

                        except Exception as e:
                            logger.warning(
                                f"Lead - lag analysis failed for {strategy1}-{strategy2}: {e}"
                            )
            ...
            logger.info(
                f"Lead - lag analysis completed in {(datetime.now() - start_time).total_seconds():.3f}s"
            )
            return result
    ...
    def pca_analysis(
        self, returns_data: pd.DataFrame, n_components: Optional[int] = None
    ) -> PCAResult:
        """
        主成分分析
        ...
        try:
            ...
            if n_components is None:
                ...
                n_components = (
                    np.argmax(cumsum_variance >= self.config.variance_threshold) + 1
                )
            ...
            logger.info(
                f"PCA analysis completed in {(datetime.now() - start_time).total_seconds():.3f}s"
            )
            return result
    ...
    def clustering_analysis(
        self, correlation_matrix: pd.DataFrame, n_clusters: Optional[int] = None
    ) -> ClusteringResult:
        """
        聚类分析
        ...
        try:
            ...
            for i in range(optimal_k):
                ...
                if len(cluster_indices) > 0:
                    cluster_corr = correlation_matrix.iloc[
            ...
            logger.info(
                f"Clustering analysis completed in {(datetime.now() - start_time).total_seconds():.3f}s"
            )
            return result
    ...
    def regime_detection(
        self, returns_data: pd.DataFrame, window: Optional[int] = None
        ...
        try:
            ...
            regimes[
                (correlations >= percentiles[0]) & (correlations < percentiles[1])
            ] = 1  # 中等相关性趋势
                ...
                if regimes[i] != regimes[i - 1]:
                    regime_transitions.append(
                        (
                            dates[i - 1],
                            dates[i],
                            f"Regime {int(regimes[i - 1])} -> Regime {int(regimes[i])}",
                        )
                    )
            ...
            logger.info(
                f"Regime detection completed in {(datetime.now() - start_time).total_seconds():.3f}s"
            )
            return result
    ...
    def _calculate_correlation(
        self, returns_data: pd.DataFrame, method: str
    ) -> CorrelationResult:
        """计算相关性"""
        # 移除NaN值
        ...
        if len(clean_data) < self.config.min_periods:
            logger.warning(
                f"Insufficient data for correlation calculation: {len(clean_data)} < {self.config.min_periods}"
            )
        ...
        # 识别显著相关性
        significant_mask = (p_values < self.config.significance_level) & (
            correlation_matrix.abs() > 0.3
        )
    ...
    def _calculate_correlation_stability(
        self, rolling_correlations: Dict[str, pd.Series]
    ) -> Dict[str, float]:
        """计算相关性稳定性指标"""
        stability_metrics = {}
...
def analyze_strategy_correlations(
    returns_data: pd.DataFrame, methods: Optional[List[str]] = None
) -> Dict[str, CorrelationResult]:
    """便利函数：分析策略相关性"""
