...
class PortfolioEngine:
    ...
    def backtest_portfolio(
        ...
        try:
            ...
            # 生成信號
            signals_dict = self._generate_portfolio_signals(
                data_dict, strategy, strategy_params or {}
            )
            ...
            # 執行回測
            portfolio_metrics = self._execute_portfolio_backtest(
                price_matrix, signals_dict, final_weights
            )

            # 計算個別資產表現
            individual_metrics = self._calculate_individual_metrics(
                data_dict, signals_dict
            )
    ...
    def optimize_portfolio(
        ...
        try:
            ...
            result = self.backtest_portfolio(
                data_dict = data_dict, strategy="OPTIMIZED_WEIGHTS", weights = weights_dict
            )

            logger.info(
                f"Portfolio optimization completed in {time.time() - start_time:.3f}s"
            )
            return result
    ...
    def _validate_data(self, data_dict: Dict[str, pd.DataFrame]) -> None:
        ...
        if len(data_dict) > self.config.max_positions:
            raise ValueError(
                f"Too many assets: {len(data_dict)} > {self.config.max_positions}"
            )
    ...
    def _generate_portfolio_signals(
        self, data_dict: Dict[str, pd.DataFrame], strategy: str, params: Dict[str, Any]
        ...
        for symbol, data in data_dict.items():
            ...
            signals_dict[symbol] = pd.DataFrame({"entries": entries, "exits": exits})
    ...
    def _execute_portfolio_backtest(
        ...
        for asset in assets:
            ...
            # 確保索引與price_matrix對齊
            entries_series = entries_series.reindex(
                price_matrix.index, fill_value = False
            )
    ...
    def _calculate_individual_metrics(
        self, data_dict: Dict[str, pd.DataFrame], signals_dict: Dict[str, pd.DataFrame]
    ...
    def _calculate_risk_metrics(self, returns: pd.Series) -> Dict[str, float]:
        ...
        alpha = (returns.mean() - risk_free_rate) - beta * (
            market_returns.mean() - risk_free_rate
        )

        # Information Ratio
        tracking_error = (returns - market_returns).std()
        information_ratio = (
            (returns.mean() - market_returns.mean()) / tracking_error
            if tracking_error != 0
            else 0
        )
