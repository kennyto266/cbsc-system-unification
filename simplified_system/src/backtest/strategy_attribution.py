...
try:
    ...
    logging.warning(
        "Statsmodels not available. Advanced regression analysis not available"
    )
...
class AttributionConfig:
    ...
    attribution_methods: List[str] = field(
        default_factory = lambda: ["return", "risk", "timing"]
    )
...
class StrategyAttributionAnalyzer:
    ...
    def analyze_performance_attribution(
        ...
        try:
            ...
            # 选股和配置效应
            selection_return, allocation_return = (
                self._calculate_selection_allocation_effects(
            ...
            # 风险归因
            total_risk, systematic_risk, idiosyncratic_risk, marginal_risk = (
                self._risk_attribution(
                    portfolio_returns, strategy_returns, benchmark_returns
                )
            )
            ...
            result = AttributionResult(
                ...
                total_return = float(
                    portfolio_returns.mean() * self.config.trading_days_per_year
                ),
                active_return = float(
            ...
            logger.info(
                f"Performance attribution analysis completed in {(datetime.now() - start_time).total_seconds():.3f}s"
            )
            return result
    ...
    def analyze_factor_attribution(
        self, portfolio_returns: pd.Series, factor_data: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            factor_var_contribution = (
                portfolio_factor_exposure @ factor_cov @ portfolio_factor_exposure
            )
            ...
            logger.info(
                f"Factor attribution analysis completed in {(datetime.now() - start_time).total_seconds():.3f}s"
            )
            return result
    ...
    def analyze_sector_attribution(
        ...
        try:
            ...
            sector_selection_effect = (
                portfolio_weights
                * (sector_returns.subtract(benchmark_return, axis = 0)).mean()
            )
            ...
            logger.info(
                f"Sector attribution analysis completed in {(datetime.now() - start_time).total_seconds():.3f}s"
            )
            return result
    ...
    def analyze_time_series_attribution(
        self, portfolio_returns: pd.Series, attribution_frequency: Optional[str] = None
        ...
        attribution_frequency = (
            attribution_frequency or self.config.attribution_frequency
        )

        logger.info(
            f"Starting time series attribution analysis with {attribution_frequency} frequency"
        )

        try:
            # 重采样数据
            if attribution_frequency == "monthly":
                resampled_returns = portfolio_returns.resample("M").apply(
                    lambda x: (1 + x).prod() - 1
                )
            elif attribution_frequency == "weekly":
                resampled_returns = portfolio_returns.resample("W").apply(
                    lambda x: (1 + x).prod() - 1
                )
            elif attribution_frequency == "quarterly":
                resampled_returns = portfolio_returns.resample("Q").apply(
                    lambda x: (1 + x).prod() - 1
                )
            else:
            ...
            if len(resampled_returns) > 24:  # 需要足够的数据点
                ...
                # 简化的分解
                trend_component = resampled_returns.rolling(
                    window = min(12, len(resampled_returns) // 2)
                ).mean()
            ...
            if len(resampled_returns) > window_size:
                ...
                # 归因趋势
                attribution_trends = rolling_attributions.rolling(
            ...
            logger.info(
                f"Time series attribution analysis completed in {(datetime.now() - start_time).total_seconds():.3f}s"
            )
            return result
    ...
    def analyze_style_attribution(
        ...
        try:
            ...
            style_scores = self._calculate_style_scores(
                portfolio_returns, style_factor_data
            )
            ...
            # 风格一致性分析
            rolling_exposures = style_factor_data.rolling(
            ...
            for style in style_factor_data.columns:
                style_rotation[style] = (
                    style_factor_data[style]
                    .rolling(window = 30)
                    .apply(
                        lambda x: (
                            x.tail(1).iloc[0] - x.head(1).iloc[0] if len(x) > 1 else 0
                        )
                    )
                )
            ...
            logger.info(
                f"Style attribution analysis completed in {(datetime.now() - start_time).total_seconds():.3f}s"
            )
            return result
    ...
    def comprehensive_attribution_analysis(
        ...
        try:
            ...
            # 因子归因
            try:
                factor_result = self.analyze_factor_attribution(
            ...
            # 时间序列归因
            try:
                time_series_result = self.analyze_time_series_attribution(
                    portfolio_returns
                )
            ...
            if sector_data is not None:
                try:
                    sector_result = self.analyze_sector_attribution(
                        portfolio_returns, sector_data
                    )
            ...
            logger.info(
                f"Comprehensive attribution analysis completed in {(datetime.now() - start_time).total_seconds():.3f}s"
            )
            return results
    ...
    def _align_sector_data(
        self, portfolio_returns: pd.Series, sector_returns: pd.DataFrame
    ) -> Dict[str, Any]:
        """对齐行业数据"""
    ...
    def _calculate_strategy_weights(
        self, portfolio_returns: pd.Series, strategy_returns: pd.DataFrame
    ) -> Dict[str, float]:
        """计算策略权重"""
        # 使用回归方法计算策略对组合的贡献权重
        weights = {}
        ...
        if total_weight > 0:
            ...
            # 如果没有正相关，使用等权重
            weights = {
                strategy: 1.0 / n_strategies for strategy in strategy_returns.columns
            }

        return weights

    def _calculate_strategy_contributions(
        self, strategy_weights: Dict[str, float], strategy_returns: pd.DataFrame
    ) -> Dict[str, float]:
        """计算策略贡献"""
        contributions = {}
        ...
        for strategy, weight in strategy_weights.items():
            strategy_return = (
                strategy_returns[strategy].mean() * self.config.trading_days_per_year
            )
    ...
    def _calculate_selection_allocation_effects(
        ...
        return (
            selection_return * self.config.trading_days_per_year,
            allocation_return * self.config.trading_days_per_year,
        )
    ...
    def _factor_attribution(
        ...
        for strategy in strategy_returns.columns:
            ...
            if len(strategy_return) > 1 and len(market_proxy) > 1:
                ...
                factor_contributions[f"{strategy}_market"] = (
                    market_exposure * market_proxy.mean()
                )

                # 择时能力（基于超额收益的波动性）
                excess_returns = strategy_return - market_proxy * market_exposure
                timing_skill = np.std(excess_returns) * np.sqrt(
                    self.config.trading_days_per_year
                )
    ...
    def _calculate_interaction_effects(
        self, strategy_returns: pd.DataFrame, factor_contributions: Dict[str, float]
    ) -> Dict[str, float]:
        """计算交互效应"""
        interaction_effects = {}
        ...
        for i, strategy1 in enumerate(strategies):
            for j, strategy2 in enumerate(strategies):
                if i < j:
                    # 计算两个策略之间的交互效应
                    correlation = strategy_returns[strategy1].corr(
                        strategy_returns[strategy2]
                    )
                    interaction_effects[f"{strategy1}_{strategy2}"] = (
                        correlation * 0.1
                    )  # 缩放因子
    ...
    def _risk_attribution(
        ...
        """风险归因分析"""
        total_risk = portfolio_returns.std() * np.sqrt(
            self.config.trading_days_per_year
        )
        ...
        for strategy in strategy_returns.columns:
            # 使用相关性作为边际风险的近似
            correlation = portfolio_returns.corr(strategy_returns[strategy])
            marginal_risk[strategy] = (
                abs(correlation) * total_risk / len(strategy_returns.columns)
            )
    ...
    def _calculate_attribution_statistics(
        ...
        # 跟踪误差
        if benchmark_returns is not None:
            tracking_error = (portfolio_returns - benchmark_returns).std() * np.sqrt(
                self.config.trading_days_per_year
            )
            information_ratio = (
    ...
    def _generate_style_factor_data(self, portfolio_returns: pd.Series) -> pd.DataFrame:
        ...
        style_data["value"] = (
            style_data["value"] - style_data["value"].mean()
        ) / style_data["value"].std()

        # Momentum因子（基于过去收益）
        style_data["momentum"] = portfolio_returns.rolling(window = 20).mean()
        style_data["momentum"] = (
            style_data["momentum"] - style_data["momentum"].mean()
        ) / (style_data["momentum"].std() + 1e - 8)
        ...
        style_data["quality"] = (
            style_data["quality"] - style_data["quality"].mean()
        ) / (style_data["quality"].std() + 1e - 8)

        # Volatility因子
        style_data["volatility"] = portfolio_returns.rolling(window = 20).std()
        style_data["volatility"] = (
            style_data["volatility"] - style_data["volatility"].mean()
        ) / (style_data["volatility"].std() + 1e - 8)

        # Growth因子（基于收益趋势）
        style_data["growth"] = portfolio_returns.rolling(window = 60).apply(
            lambda x: np.polyfit(range(len(x)), x, 1)[0] if len(x) > 1 else 0
        )
        style_data["growth"] = (style_data["growth"] - style_data["growth"].mean()) / (
            style_data["growth"].std() + 1e - 8
        )

        return style_data.fillna(0)

    def _calculate_style_scores(
        self, portfolio_returns: pd.Series, style_factor_data: pd.DataFrame
    ) -> pd.DataFrame:
        """计算风格评分"""
        # 基于因子暴露计算风格评分
        ...
        for factor in style_factor_data.columns:
            ...
            if factor_values.std() > 1e - 8:
                scores.loc[factor, "z_score"] = (
                    factor_values.iloc[-1] - factor_values.mean()
                ) / factor_values.std()
            else:
...
def analyze_strategy_attribution(
    ...
    return analyzer.analyze_performance_attribution(
        portfolio_returns, strategy_returns, benchmark_returns
    )
