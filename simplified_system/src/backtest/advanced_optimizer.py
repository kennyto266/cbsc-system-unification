...
from .vectorbt_engine import BacktestConfig, VectorBTEngine
...
class BayesianOptimizer:
    ...
    def optimize(
        ...
        for _ in range(n_initial):
            ...
            try:
                score = self._evaluate_parameters(
                    data, strategy, params, optimization_metric
                )
        ...
        for iteration in range(n_initial, n_calls):
            ...
            # 轉換回參數字典
            candidate_params = {
                param_names[i]: x_next[i] for i in range(len(param_names))
            }

            # 評評新點
            score = self._evaluate_parameters(
                data, strategy, candidate_params, optimization_metric
            )
            ...
            if iteration % 10 == 0:
                logger.info(
                    f"Bayesian optimization iteration {iteration}: best_score={best_score:.4f}"
                )
    ...
    def _evaluate_parameters(self, data, strategy, params, optimization_metric):
        ...
        try:
            ...
            result = self.engine.backtest_strategy(
                data, strategy, params, "OPTIMIZATION"
            )
...
class GeneticOptimizer:
    ...
    def optimize(
        ...
        population = self._initialize_population(
            bounds_array, population_size, param_names
        )
        fitness_scores = self._evaluate_population(
            data, strategy, population, param_names, optimization_metric
        )
        ...
        for generation in range(n_generations):
            ...
            offspring_fitness = self._evaluate_population(
                data, strategy, offspring, param_names, optimization_metric
            )
            ...
            elite_indices = np.argsort(combined_fitness)[
                -int(len(population) * (1 - self.elitism_rate)) :
            ]
        ...
        # 轉換最佳個體回參數字典
        best_params = {
            param_names[i]: best_individual[i] for i in range(len(param_names))
        }
    ...
    def _evaluate_population(
        self, data, strategy, population, param_names, optimization_metric
    ):
        """評估種群適應度"""
        fitness_scores = []
...
class AdvancedOptimizer:
    ...
    def __init__(self, engine: VectorBTEngine, config: OptimizationConfig = None):
        ...
        logger.info(
            f"Advanced Optimizer initialized with {self.config.parallel_cores} cores"
        )
    ...
    def _differential_evolution_optimization(
        ...
        def objective_function(params):
            ...
            try:
                result = self.engine.backtest_strategy(
                    data, strategy, params_dict, "OPTIMIZATION"
                )
    ...
    def optimize_parallel(
        ...
        """並行優化"""
        logger.info(
            f"Starting parallel optimization with {self.config.parallel_cores} cores"
        )
        ...
        def parallel_objective(params_subset):
            ...
            return optimizer.optimize(
                data, strategy, subset_bounds, optimization_metric, "bayesian"
            )
        ...
        with ProcessPoolExecutor(max_workers = self.config.parallel_cores) as executor:
            futures = [
                executor.submit(parallel_objective, chunk) for chunk in param_chunks
            ]
