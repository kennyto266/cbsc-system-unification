...
class MarketRegimeDetector:
    ...
    # 預定義制度類型
    REGIME_TYPES = {0: "Bull Market", 1: "Bear Market", 2: "Sideways / Range - Bound"}
    ...
    def _extract_single_asset_features(self, data: pd.DataFrame) -> np.ndarray:
        ...
        # 4. 價格特徵
        price_position = (close - low.rolling(20).min()) / (
        ...
        # 處理NaN值
        feature_matrix = (
            pd.DataFrame(feature_matrix).fillna(method="ffill").fillna(0).values
        )

        return feature_matrix

    def _extract_multi_asset_features(
        ...
        features.extend(
            [market_vol, market_vol.rolling(5).mean(), market_vol.rolling(20).mean()]
        )
        ...
        for date in rolling_corr.index.levels[0]:
            ...
            avg_corr = date_corr.values[
                np.triu_indices_from(date_corr.values, k = 1)
            ].mean()
            avg_correlation.append(avg_corr)

        avg_correlation = pd.Series(avg_correlation, index = market_vol.index)
        features.extend(
            [avg_correlation, avg_correlation.rolling(5).mean(), avg_correlation.diff()]
        )
        ...
        # 5. 市場廣度特徵
        positive_returns = (returns_matrix > 0).sum(axis = 1) / len(
        ...
        feature_matrix = np.array(
            [f.values if hasattr(f, "values") else f for f in features]
        ).T
    ...
    def _smooth_transition_matrix(self) -> None:
        ...
        smoothed = (
            transmat * (1 - self.config.transition_smoothing)
            + np.ones_like(transmat)
    ...
    def _predict_simplified(self, features: np.ndarray) -> RegimePrediction:
        ...
        current_regime = self._create_regime_state(
            current_state, current_prob, features[-1:]
        )
    ...
    def _create_regime_state(
        self, regime_id: int, probability: float, features: np.ndarray
    ) -> RegimeState:
        """創建制度狀態對象"""
        ...
        # 分類特徵
        volatility_level = self._classify_level(
            characteristics["volatility"], [-0.02, 0.02]
        )
        trend_strength = self._classify_level(characteristics["trend"], [-0.01, 0.01])
        momentum_direction = self._classify_level(
            characteristics["momentum"], [-0.01, 0.01]
        )
    ...
    def analyze_regime_history(
        ...
        if HMM_AVAILABLE and self.hmm_model is not None:
            ...
            state_probabilities = np.zeros(
                (len(features_scaled), self.config.n_regimes)
            )
        ...
        for i, (date, state) in enumerate(zip(index, hidden_states)):
            regime = self._create_regime_state(
                state, float(state_probabilities[i, state]), features_scaled[i : i + 1]
            )
    ...
    def get_regime_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "regime_names": [
                self.REGIME_TYPES.get(i, f"Regime_{i}")
...
def detect_market_regimes(
    data: Union[pd.DataFrame, Dict[str, pd.DataFrame]], n_regimes: int = 3
) -> RegimePrediction:
    """便利函數：檢測市場制度"""
...
def analyze_regime_transitions(
    data: Union[pd.DataFrame, Dict[str, pd.DataFrame]], model_path: Optional[str] = None
) -> pd.DataFrame:
    """便利函數：分析制度轉換歷史"""
