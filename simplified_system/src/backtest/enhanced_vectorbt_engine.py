...
try:
    ...
    logging.warning(
        "VectorBT not available. Install with: pip install vectorbt[yfinance]"
    )
...
class EnhancedVectorBTEngine(VectorBTEngine):
    ...
    def __init__(self, config: Optional[EnhancedBacktestConfig] = None):
        ...
        if not VECTORBT_AVAILABLE:
            raise ImportError(
                "VectorBT is required. Install with: pip install vectorbt[yfinance]"
            )
    ...
    def backtest_portfolio(
        ...
        try:
            ...
            for symbol, data in data_dict.items():
                signals_dict[symbol] = self._generate_signals(
    ...
    def optimize_parameters_parallel(
            ...
            with concurrent.futures.ThreadPoolExecutor(
                ...
                for i, future in enumerate(concurrent.futures.as_completed(futures)):
                    try:
                        ...
                        if (i + 1) % 100 == 0:
                            logger.info(
                                f"Completed {i + 1}/{len(futures)} combinations"
                            )
            ...
            for params in param_combinations:
                try:
                    ...
                    if len(results) % 100 == 0:
                        logger.info(
                            f"Completed {len(results)}/{len(param_combinations)} combinations"
                        )
        ...
        logger.info(
            f"Parallel optimization completed. Best {optimization_metric}: {getattr(best_result, optimization_metric, 0):.4f}"
        )
    ...
    def walk_forward_analysis(
                ...
                results.append(
                    {
                        ...
                        "optimization_performance": optimization_result[
                            "best_performance"
                        ],
                    }
                )
        ...
        if results:
            test_returns = [
                r["test_performance"]["performance"]["total_return"] for r in results
            ]
            test_sharpes = [
                r["test_performance"]["performance"]["sharpe_ratio"] for r in results
            ]
            ...
            walk_forward_summary = {
                ...
                "out_of_sample_performance": {
                    ...
                    "positive_periods_ratio": sum(1 for r in test_returns if r > 0)
                    / len(test_returns),
                },
            ...
            logger.info(
                f"Walk - forward analysis completed. Out - of - sample mean Sharpe: {np.mean(test_sharpes):.4f}"
            )
    ...
    # 進階策略實現
    def _advanced_mean_reversion_signals(
        ...
        buy_conditions = (
            (close <= bb["lower"]) & (rsi < 30) & (volatility < volatility_threshold)
        )

        # 出場信號：價格觸及上布林帶 + RSI超買
        sell_conditions = (close >= bb["upper"]) & (rsi > 70)
        ...
        return pd.DataFrame(
            {"entries": buy_signals, "exits": sell_signals}, index = data.index
        )

    def _dual_rsi_confluence_signals(
        self, data: pd.DataFrame, params: Dict[str, Any]
    ) -> pd.DataFrame:
        """雙RSI匯流策略"""
        ...
        return pd.DataFrame(
            {"entries": buy_conditions, "exits": sell_conditions}, index = data.index
        )

    def _ichimoku_cloud_signals(
        self, data: pd.DataFrame, params: Dict[str, Any]
    ) -> pd.DataFrame:
        """一目均衡表策略"""
        # 簡化版一目均衡表計算
    ...
    def _atr_volatility_breakout_signals(
        self, data: pd.DataFrame, params: Dict[str, Any]
    ) -> pd.DataFrame:
        """ATR波動率突破策略"""
        atr_period = params.get("atr_period", 14)
    ...
    def _multi_timeframe_rsi_signals(
        self, data: pd.DataFrame, params: Dict[str, Any]
    ) -> pd.DataFrame:
        """多時間框架RSI策略"""
    ...
    def _vix_reversion_signals(
        ...
        # 進場信號：高波動率後的均值回歸
        buy_conditions = (proxy_vix > high_vix_threshold) & (
            proxy_vix < proxy_vix.shift(1)
        )

        # 出場信號：低波動率後的趨勢跟隨
        sell_conditions = (proxy_vix < low_vix_threshold) & (
    ...
    def _validate_portfolio_data(self, data_dict: Dict[str, pd.DataFrame]) -> None:
        ...
        for symbol, data in data_dict.items():
            ...
            if len(data) < 20:
                raise ValueError(
                    f"Insufficient data for {symbol}: {len(data)} (minimum 20 required)"
                )
    ...
    def _execute_portfolio_backtest(
        ...
        for symbol, data in data_dict.items():
            ...
            if weight > 0:
                ...
                # 執行單個資產回測
                result = self._backtest_with_config(
                    data, signals, symbol, portfolio_config
                )
    ...
    def _calculate_enhanced_metrics(
        ...
        sharpe_ratio = (
            float(excess_returns.mean() / returns.std() * np.sqrt(252))
            if returns.std() > 0
            else 0.0
        )
        ...
        calmar_ratio = (
            float(total_return / abs(max_drawdown)) if max_drawdown != 0 else 0.0
        )
    ...
    def _calculate_advanced_risk_metrics(
        self, returns: pd.Series, config: EnhancedBacktestConfig
    ) -> AdvancedRiskMetrics:
        """計算進階風險指標"""
        # Value at Risk
        if config.var_method == "historical":
            ...
            std_ret = returns.std()
            var_95 = float(
                mean_ret + stats.norm.ppf(1 - config.var_confidence) * std_ret
            )
    ...
    def _generate_param_combinations(
        self, param_ranges: Dict[str, Union[List, range]], max_combinations: int
    ) -> List[Dict[str, Any]]:
        """生成參數組合"""
        import itertools
    ...
    def _backtest_single_safe(
        self, data: pd.DataFrame, strategy: str, params: Dict[str, Any], symbol: str
    ...
    def _generate_signals(
        self, data: pd.DataFrame, strategy: str, parameters: Dict[str, Any]
    ) -> pd.DataFrame:
        """生成交易信號"""
...
def run_walk_forward_analysis(
    ...
    return engine.walk_forward_analysis(
        data, strategy, param_ranges, window_size, step_size, test_size, symbol
    )
