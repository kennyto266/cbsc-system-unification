...
from scipy.signal import find_cwt, find_peaks
...
class TradingSessionAnalyzer:
        ...
        try:
            ...
            for session, (start_time, end_time) in self.trading_hours.items():
                ...
                if not session_data.empty:
                    results[session.value] = self._analyze_session(
            ...
            logger.info(
                f"Trading session analysis completed for {len(data)} data points"
            )
            return results
    ...
    def _extract_session_data(
        self, data: pd.Series, start_time: time, end_time: time
    ) -> pd.Series:
        """提取特定时段的数据"""
        try:
            # 筛选时间范围内的数据
    ...
    def _analyze_session(
        self, session_data: pd.Series, session_name: str
    ) -> Dict[str, Any]:
        """分析单个交易时段"""
        try:
            analysis = {
            ...
            if len(session_data) > 1:
                ...
                if len(returns) > 0:
                    analysis["returns"] = {
                        ...
                        "total_return": float(
                            (session_data.iloc[-1] / session_data.iloc[0]) - 1
                        ),
            ...
            if session_name == TradingSession.MORNING_SESSION.value:
                analysis["morning_patterns"] = self._analyze_morning_patterns(
                    session_data
                )
            elif session_name == TradingSession.AFTERNOON_SESSION.value:
                analysis["afternoon_patterns"] = self._analyze_afternoon_patterns(
                    session_data
                )
        ...
        try:
            ...
            if len(data) >= 20:
                ...
                patterns["volatility_pattern"] = {
                    ...
                    "volatility_trend": (
                        "increasing"
                        if rolling_vol.iloc[-10:].mean() > rolling_vol.iloc[:10].mean()
                        else "decreasing"
                    ),
                }
        ...
        try:
            ...
            if len(data) >= 10:
                ...
                patterns["post_lunch_impact"] = {
                    ...
                    "first_10min_avg": float(first_10min_avg),
                    "post_lunch_movement": float(
                        (first_10min_avg / lunch_end_price) - 1
                    ),
                }
        ...
        try:
            ...
            if len(data) >= 5:
                ...
                patterns["price_deviation"] = {
                    ...
                    "relative_volatility": (
                        float((data.max() - data.min()) / lunch_avg)
                        if lunch_avg != 0
                        else 0
                    ),
                }
        ...
        try:
            ...
            if len(session_metrics) >= 2:
                ...
                comparison["price_level_comparison"] = {
                    "highest_avg_session": max(
                        session_metrics.keys(),
                        key = lambda k: session_metrics[k]["avg_price"],
                    ),
                    "lowest_avg_session": min(
                        session_metrics.keys(),
                        key = lambda k: session_metrics[k]["avg_price"],
                    ),
                ...
                if volatilities:
                    comparison["volatility_comparison"] = {
                        "most_volatile_session": max(
                            volatilities.keys(), key = lambda k: volatilities[k]
                        ),
                        "least_volatile_session": min(
                            volatilities.keys(), key = lambda k: volatilities[k]
                        ),
                        "volatility_ratio": float(
                            max(volatilities.values()) / min(volatilities.values())
                        ),
                    }
            ...
            for transition_time, transition_name in transition_times:
                ...
                if len(before_data) > 0 and len(after_data) > 0:
                    ...
                    transitions[transition_name] = {
                        ...
                        "gap_significance": abs((after_price / before_price) - 1)
                        > 0.005,  # 0.5%以上认为显著
                    }
...
class VolatilityPatternRecognizer:
    ...
    def recognize_volatility_patterns(self, data: pd.Series) -> Dict[str, Any]:
        ...
        try:
            ...
            # 多时间窗口波动率分析
            results["multi_timeframe_volatility"] = (
                self._analyze_multi_timeframe_volatility(returns)
            )

            # 波动率聚集检测
            results["volatility_clustering"] = self._detect_volatility_clustering(
                returns
            )
            ...
            logger.info(
                f"Volatility pattern recognition completed for {len(data)} data points"
            )
            return results
    ...
    def _analyze_multi_timeframe_volatility(self, returns: pd.Series) -> Dict[str, Any]:
        ...
        try:
                    ...
                    if len(rolling_vol) > 0:
                        volatility_analysis[label] = {
                            ...
                            "volatility_range": {
                                ...
                                "range_ratio": (
                                    float(rolling_vol.max() / rolling_vol.min())
                                    if rolling_vol.min() > 0
                                    else float("inf")
                                ),
                            },
                        }
            ...
            if len(volatility_analysis) >= 2:
                short_vol = volatility_analysis.get("short_term", {}).get(
                    "current_volatility", 0
                )
                long_vol = volatility_analysis.get("long_term", {}).get(
                    "current_volatility", 0
                )
                ...
                if long_vol > 0:
                    volatility_analysis["term_structure"] = {
                        ...
                        "structure_type": (
                            "contango" if short_vol > long_vol else "backwardation"
                        ),
    ...
    def _identify_volatility_patterns(self, returns: pd.Series) -> Dict[str, Any]:
        ...
        try:
            ...
            peaks, peak_properties = find_peaks(
                rolling_vol, height = rolling_vol.quantile(0.9), distance = 5
            )
            if len(peaks) > 0:
                patterns["volatility_spikes"] = {
                    ...
                    "avg_spike_height": float(
                        np.mean([rolling_vol.iloc[i] for i in peaks])
                    ),
                }
            ...
            troughs, trough_properties = find_peaks(
                inverted_vol, height = -rolling_vol.quantile(0.1), distance = 5
            )
            if len(troughs) > 0:
                patterns["volatility_troughs"] = {
                    ...
                    "avg_trough_depth": float(
                        np.mean([rolling_vol.iloc[i] for i in troughs])
                    ),
                }
            ...
            if len(rolling_vol) >= 40:
                ...
                if np.any(positive_freq_idx):
                    ...
                    dominant_period = (
                        int(1 / freq[positive_freq_idx][dominant_freq_idx])
                        if freq[positive_freq_idx][dominant_freq_idx] > 0
                        else None
                    )
                    ...
                    if dominant_period and 1 < dominant_period < len(rolling_vol) // 2:
                        patterns["volatility_cyclicity"] = {
                            ...
                            "cycle_strength": float(
                                psd[positive_freq_idx][dominant_freq_idx]
                            ),
                        }
    ...
    def _detect_extreme_volatility_events(self, returns: pd.Series) -> Dict[str, Any]:
        ...
        try:
            ...
            returns.quantile(0.99) - returns.quantile(0.01)
            ...
            if len(returns) >= 10:
                ...
                if len(extreme_vol_changes) > 0:
                    extreme_events["volatility_jumps"] = {
                        ...
                        "events": [
                            {"date": str(idx), "vol_change": float(change)}
            ...
            if extreme_events:
                ...
                extreme_events["summary"] = {
                    ...
                    "is_high_frequency": total_events / len(returns)
                    > 0.05,  # 5%以上认为高频
                }
...
class IntradayPatternRecognizer:
    ...
    def recognize_intraday_patterns(
        self, data: pd.Series, symbol: str = "UNKNOWN"
    ) -> Dict[str, Any]:
        """
        综合盘中模式识别
        ...
        try:
            logger.info(
                f"Starting comprehensive intraday pattern recognition for {symbol}"
            )
            ...
            analysis_results = {
                ...
                "time_range": {"start": str(data.index[0]), "end": str(data.index[-1])},
            }

            # 交易时段分析
            analysis_results["trading_sessions"] = (
                self.session_analyzer.analyze_trading_sessions(data)
            )

            # 波动率模式识别
            analysis_results["volatility_patterns"] = (
                self.volatility_analyzer.recognize_volatility_patterns(data)
            )

            # 价格行为模式
            analysis_results["price_behavior_patterns"] = (
                self._analyze_price_behavior_patterns(data)
            )

            # 香港市场特定模式
            analysis_results["hk_specific_patterns"] = (
                self._detect_hk_specific_patterns(data)
            )

            # 模式综合分析
            analysis_results["pattern_summary"] = self._generate_pattern_summary(
                analysis_results
            )

            # 预测性信号
            analysis_results["predictive_signals"] = self._generate_predictive_signals(
                analysis_results
            )
        ...
        try:
            ...
            if len(short_ma.dropna()) > 0 and len(long_ma.dropna()) > 0:
                momentum_analysis["short_term_momentum"] = {
                    ...
                    "momentum_signal": (
                        "bullish" if short_ma.iloc[-1] > long_ma.iloc[-1] else "bearish"
                    ),
                }
                    ...
                    if peak_values and trough_values:
                        ...
                        reversal_analysis["reversal_strength"] = {
                            ...
                            "avg_volatility": float(
                                (peak_volatility + trough_volatility) / 2
                            ),
                        }
        ...
        try:
            ...
            if len(data) >= 50:
                ...
                breakout_analysis["support_resistance"] = {
                    ...
                    "price_position": (
                        "near_resistance"
                        if current_price > current_resistance * 0.98
                        else (
                            "near_support"
                    ...
                    breakout_analysis["breakout_signals"] = {
                        ...
                        "breakout_strength": (
                            ...
                            if current_price > recent_high
                                ...
                                float((recent_low - current_price) / recent_low)
            ...
            if len(data) >= 30:
                ...
                breakout_analysis["volatility_breakout"] = {
                    ...
                    "expansion_factor": (
                        float(current_volatility / avg_volatility)
                        if avg_volatility > 0
                        else 1
                    ),
                }
        ...
        try:
            ...
            if len(data) >= 50:
                ...
                range_analysis["price_range"] = {
                    ...
                    "range_width_pct": (
                        float(price_range / mean_price) if mean_price != 0 else 0
                    ),
                ...
                range_analysis["boundary_strength"] = {
                    ...
                    "is_strong_range": (upper_touches + lower_touches) / total_periods
                    > 0.1,
                }
        ...
        try:
            ...
            if isinstance(data.index, pd.DatetimeIndex):
                ...
                non_lunch_data = data[
                    ...
                    & ~(
                        (data.index.time >= time(12, 0))
                        & (data.index.time < time(13, 0))
                    )
                ]
                ...
                if len(lunch_data) > 0 and len(non_lunch_data) > 0:
                    ...
                    hk_patterns["lunch_time_activity"] = {
                        "lunch_volatility": (
                            float(lunch_volatility)
                            if not np.isnan(lunch_volatility)
                            else 0
                        ),
                        "normal_volatility": (
                            float(normal_volatility)
                            if not np.isnan(normal_volatility)
                            else 0
                        ),
                        "activity_ratio": (
                            float(lunch_volatility / normal_volatility)
                            if normal_volatility > 0
                            else 1
                        ),
                        "has_abnormal_lunch_activity": lunch_volatility
                        > normal_volatility * 1.5,
                    }

                # 收盘前集合竞价影响
                close_data = data[
                    (data.index.time >= time(15, 30)) & (data.index.time < time(16, 0))
                ]
                if len(close_data) > 0:
                    pre_close_data = data[
                        (data.index.time >= time(15, 0))
                    ...
                    if len(pre_close_data) > 0:
                        ...
                        hk_patterns["closing_auction_effect"] = {
                            "close_period_return": (
                                float(close_return) if not np.isnan(close_return) else 0
                            ),
                            "pre_close_return": (
                                float(pre_close_return)
                                if not np.isnan(pre_close_return)
                                else 0
                            ),
                            "auction_impact": (
                                float(abs(close_return - pre_close_return))
                                if not np.isnan(close_return)
                                and not np.isnan(pre_close_return)
                                else 0
                            ),
                        }
    ...
    def _generate_pattern_summary(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 交易时段摘要
            if (
                "trading_sessions" in results
                and "session_comparison" in results["trading_sessions"]
            ):
                comparison = results["trading_sessions"]["session_comparison"]
                summary["most_active_session"] = comparison.get(
                    "volatility_comparison", {}
                ).get("most_volatile_session", "unknown")

            # 波动率模式摘要
            if (
                "volatility_patterns" in results
                and "volatility_clustering" in results["volatility_patterns"]
            ):
            ...
            if "price_behavior_patterns" in results:
                ...
                if "momentum_patterns" in price_patterns:
                    ...
                    summary["momentum_signal"] = momentum.get(
                        "short_term_momentum", {}
                    ).get("momentum_signal", "neutral")

                # 反转信号
                if "reversal_patterns" in price_patterns:
                    reversal = price_patterns["reversal_patterns"]
                    rsi_signal = reversal.get("rsi_signals", {}).get(
                ...
                if "breakout_patterns" in price_patterns:
                    ...
                    summary["price_position"] = support_resistance.get(
            ...
            if "hk_specific_patterns" in results:
                ...
                summary["hk_market_features"] = {
                    "abnormal_lunch_activity": lunch_activity.get(
                        "has_abnormal_lunch_activity", False
                    )
                }
    ...
    def _generate_predictive_signals(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                volatility_regime.get("has_clustering", False)
                and volatility_regime.get("clustering_strength", 0) > 0.3
            ):
                signals["risk_warnings"].append(
                    {
                        ...
                        "severity": (
                            "high"
                            if volatility_regime.get("clustering_strength", 0) > 0.5
                            else "medium"
                        ),
...
if __name__ == "__main__":
    ...
    if (
        ...
        comparison = results["trading_sessions"]["session_comparison"]
        print(
            f"Volatility Comparison: {'available' if 'volatility_comparison' in comparison else 'not available'}"
        )
        print(
            f"Price Level Comparison: {'available' if 'price_level_comparison' in comparison else 'not available'}"
        )
    ...
    if hk_patterns:
        print(
            f"Lunch Time Activity: {'detected' if 'lunch_time_activity' in hk_patterns else 'not detected'}"
        )
        print(
            f"Closing Auction Effect: {'detected' if 'closing_auction_effect' in hk_patterns else 'not detected'}"
        )
