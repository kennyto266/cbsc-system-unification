...
from ..core.behavioral_config import TimeSeriesConfig, get_behavioral_config
...
class SeasonalPatternDetector:
    ...
    def _stl_decomposition(self, data: pd.Series) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(data) < 2 * self.config.seasonal_period:
                logger.warning(
                    f"Insufficient data for STL decomposition: {len(data)} < {2 * self.config.seasonal_period}"
                )
    ...
    def _classical_decomposition(self, data: pd.Series) -> Dict[str, Any]:
        ...
        try:
            ...
            seasonal_var = (
                decomposition.seasonal.var() / total_var if total_var > 0 else 0
            )
    ...
    def _calculate_seasonal_strength(self, data: pd.Series) -> Dict[str, float]:
        ...
        try:
            ...
            # 方法1: 基于方差比例
            decomposition = seasonal_decompose(
                data, period = self.config.seasonal_period, extrapolate_trend="freq"
            )
            if len(decomposition.seasonal.dropna()) > 0:
                seasonal_var = decomposition.seasonal.var()
                total_var = data.var()
                strength_methods["variance_ratio"] = float(
            ...
            if len(data) >= self.config.seasonal_period:
                autocorr = [
                    data.autocorr(lag = i)
                    for i in range(1, min(25, self.config.seasonal_period // 2))
                ]
                if autocorr:
            ...
            if len(data) >= 2 * self.config.seasonal_period:
                ...
                if len(seasonal_pattern) >= self.config.seasonal_period:
                    ...
                    for i in range(
                        0,
                        len(seasonal_pattern) - self.config.seasonal_period + 1,
                        self.config.seasonal_period,
                    ):
                        pattern = seasonal_pattern.iloc[
                            i : i + self.config.seasonal_period
                        ]
                    ...
                    if len(patterns) >= 2:
                        ...
                        if correlations:
                            strength_methods["pattern_consistency"] = float(
    ...
    def _detect_periodicity(self, data: pd.Series) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(data) >= 20:
                ...
                detrended = (
                    data
                    - data.rolling(window = min(20, len(data) // 4), center = True).mean()
                )
                ...
                if np.any(positive_freq_idx):
                    ...
                    if dominant_period and 1 < dominant_period < len(data) // 2:
                        ...
                        periodicity_info["psd_peak"] = float(
            ...
            if len(data) >= self.config.seasonal_period:
                autocorr_values = []
                for lag in range(
                    1, min(self.config.seasonal_period * 2, len(data) // 2)
                ):
    ...
    def _classify_seasonal_pattern(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            seasonal_strength = 0
            if (
                "seasonal_strength" in results
                and "variance_ratio" in results["seasonal_strength"]
            ):
            ...
            classification["has_seasonality"] = (
                seasonal_strength > 0.1 and has_periodicity
            )

            # 基于分解质量的分类
            decomposition_quality = 0
            if (
                "stl_decomposition" in results
                and "decomposition_quality" in results["stl_decomposition"]
            ):
                decomposition_quality = results["stl_decomposition"][
    ...
    def _calculate_trend_strength(self, data: pd.Series) -> Dict[str, float]:
        ...
        try:
            ...
            if len(data) >= 50:
                ...
                strength_metrics["ma_relationship_strength"] = float(
                    abs(ma_relationship - 0.5) * 2
                )  # 归一化到0 - 1
    ...
    def _detect_trend_changes(self, data: pd.Series) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(data) >= self.config.min_segment_length * 2:
                ...
                model = "l2"  # L2成本函数
                algo = rpt.Pelt(
                    model = model, min_size = self.config.min_segment_length
                ).fit(data.values)
            ...
            if change_points:
                ...
                for i, cp in enumerate(change_points):
                    ...
                    if len(prev_segment) >= 5 and len(next_segment) >= 5:
                        # 计算前后段的斜率
                        prev_slope, _, _, _, _ = stats.linregress(
                            np.arange(len(prev_segment)), prev_segment.values
                        )
                        next_slope, _, _, _, _ = stats.linregress(
                            np.arange(len(next_segment)), next_segment.values
                        )
    ...
    def _classify_trend(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if "linear_trend" in results and "direction" in results["linear_trend"]:
                ...
                classification["is_significant"] = results["linear_trend"].get(
                    "is_significant", False
                )

            # 基于趋势强度的分类
            if (
                "trend_strength" in results
                and "overall_strength" in results["trend_strength"]
            ):
            ...
            classification["trend_type"] = (
                "_".join(trend_type) if trend_type else "undefined"
            )
...
class TimeSeriesPatternAnalyzer:
    ...
    def analyze_patterns(
        ...
        try:
            ...
            analysis_results = {
                ...
                "date_range": {"start": str(data.index[0]), "end": str(data.index[-1])},
            }

            # 季节性分析
            analysis_results["seasonal_analysis"] = (
                self.seasonal_detector.detect_seasonality(data)
            )
            ...
            # 香港市场特定模式
            analysis_results["hk_market_patterns"] = self._detect_hk_market_patterns(
                data
            )

            # 预测性分析
            analysis_results["predictive_patterns"] = (
                self._identify_predictive_patterns(data)
            )
    ...
    def _generate_pattern_summary(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if "seasonal_pattern" in seasonal_analysis:
                ...
                summary["has_seasonality"] = seasonal_summary.get(
                    "has_seasonality", False
                )
                summary["seasonal_strength"] = seasonal_summary.get(
                    "strength", "unknown"
                )
            ...
            summary["analysis_confidence"] = self._calculate_analysis_confidence(
                results
            )
    ...
    def _classify_overall_pattern(self, summary: Dict[str, Any]) -> str:
        ...
        try:
            ...
            # 趋势部分
            if trend_direction != "unknown" and trend_strength in [
                "strong",
                "moderate",
            ]:
    ...
    def _calculate_analysis_confidence(self, results: Dict[str, Any]) -> float:
        ...
        try:
            ...
            seasonal_analysis = results.get("seasonal_analysis", {})
            if (
                "stl_decomposition" in seasonal_analysis
                and "decomposition_quality" in seasonal_analysis["stl_decomposition"]
            ):
                quality = seasonal_analysis["stl_decomposition"][
                    "decomposition_quality"
                ]
            ...
            trend_analysis = results.get("trend_analysis", {})
            if (
                "linear_trend" in trend_analysis
                and "r_squared" in trend_analysis["linear_trend"]
            ):
    ...
    def _detect_hk_market_patterns(self, data: pd.Series) -> Dict[str, Any]:
        ...
        try:
            ...
            if isinstance(data.index, pd.DatetimeIndex):
                ...
                morning_avg = (
                    data_hourly.loc[10:11].mean()
                    if 10 in data_hourly.index and 11 in data_hourly.index
                    else None
                )
                afternoon_avg = (
                    data_hourly.loc[14:15].mean()
                    if 14 in data_hourly.index and 15 in data_hourly.index
                    else None
                )
                ...
                if morning_avg is not None and afternoon_avg is not None:
                    hk_patterns["session_pattern"] = {
                        ...
                        "session_ratio": (
                            float(afternoon_avg / morning_avg)
                            if morning_avg != 0
                            else 1.0
                        ),
                    }
                ...
                if lunch_hour.any():
                    ...
                    if len(lunch_data) > 0 and len(non_lunch_data) > 0:
                        ...
                        hk_patterns["lunch_time_analysis"] = {
                            ...
                            "is_abnormal_lunch_time": lunch_volatility
                            > 2 * normal_volatility,
                        }
            ...
            if isinstance(data.index, pd.DatetimeIndex):
                ...
                if month_end.any() and month_start.any():
                    ...
                    hk_patterns["month_end_effect"] = {
                        "month_end_return": (
                            float(month_end_returns)
                            if not np.isnan(month_end_returns)
                            else 0.0
                        ),
                        "month_start_return": (
                            float(month_start_returns)
                            if not np.isnan(month_start_returns)
                            else 0.0
                        ),
                        "effect_strength": float(
                            abs(month_end_returns - month_start_returns)
                        ),
                    }
        ...
        try:
            ...
            if autocorr_values:
                predictive_patterns["autocorr_predictability"] = {
                    ...
                    "predictable_lags": [
                        i + 1 for i, ac in enumerate(autocorr_values) if ac > 0.3
                    ],
                }
            ...
            if len(returns) > 20:
                ...
                if len(volatility) > 0:
                    ...
                    predictive_patterns["volatility_clustering"] = {
                        "volatility_autocorr": (
                            float(vol_autocorr) if not np.isnan(vol_autocorr) else 0.0
                        ),
                        "has_clustering": (
                        ...
                        "volatility_regime": (
                            "high" if volatility.mean() > returns.std() else "low"
                        ),
                    }
                ...
                for i in range(window, len(deviations)):
                    if abs(deviations.iloc[i]) > 0:
                        ...
                        if len(future_deviation) > 0:
                            ...
                            reversion = np.sign(deviations.iloc[i]) * np.sign(
                                future_deviation.mean()
                            )
                ...
                if mean_reversion_strength:
                    predictive_patterns["mean_reversion"] = {
                        "reversion_probability": float(
                            np.mean(mean_reversion_strength)
                        ),
...
if __name__ == "__main__":
    ...
    print("\n=== Seasonal Analysis ===")
    if (
        "seasonal_analysis" in results
        and "seasonal_pattern" in results["seasonal_analysis"]
    ):
        seasonal = results["seasonal_analysis"]["seasonal_pattern"]
        print(f"Seasonal Strength: {seasonal.get('strength', 'unknown')}")
        print(
            f"Decomposition Quality: {seasonal.get('decomposition_quality', 'unknown')}"
        )

    print("\n=== Trend Analysis ===")
    if (
        "trend_analysis" in results
        and "trend_classification" in results["trend_analysis"]
    ):
    ...
    if hk_patterns:
        print(
            f"Session Pattern: {'detected' if 'session_pattern' in hk_patterns else 'not detected'}"
        )
        print(
            f"Lunch Time Analysis: {'available' if 'lunch_time_analysis' in hk_patterns else 'not available'}"
        )
        print(
            f"Month End Effect: {'detected' if 'month_end_effect' in hk_patterns else 'not detected'}"
        )
