...
from ..ml_anomaly.ml_anomaly_detector import EnsembleAnomalyDetector, MLAnomalyDetector
        ...
        try:
            ...
            return {
                ...
                "volatility_percentile": float(
                    stats.percentileofscore([volatility], volatility)
                ),
            }
...
class AdaptiveThresholdManager:
    ...
    def update_thresholds(
        self, metric_name: str, new_value: float, is_anomaly: bool
    ) -> None:
        """更新自适应阈值"""
...
class RealTimeBehaviorMonitor:
        ...
        while self.is_running:
            try:
                ...
                if processing_time > self.config.max_processing_time_ms:
                    logger.warning(
                        f"Processing time exceeded limit: {processing_time:.2f}ms"
                    )
    ...
    def add_data_point(
        self, value: float, timestamp: Optional[datetime] = None, **metadata
    ) -> None:
        """添加新的数据点"""
        if timestamp is None:
            timestamp = datetime.now()

        data_point = {"value": value, "timestamp": timestamp, "metadata": metadata}
    ...
    def _process_data_point(self, data_point: Dict[str, Any]) -> None:
        ...
        for metric_name, metric_value in metrics.items():
            if metric_name in self.threshold_manager.thresholds:
                threshold_result = self.threshold_manager.check_thresholds(
    ...
    def _calculate_realtime_metrics(self) -> Dict[str, float]:
        ...
        for window_type in ["short", "medium"]:
            ...
            metrics[f"{window_type}_has_change_point"] = float(
                change_point.get("has_change_point", False)
            )
            if change_point.get("has_change_point", False):
                metrics[f"{window_type}_change_magnitude"] = float(
                    change_point.get("deviation_magnitude", 0)
                )

        # 波动率状态
        for window_type in ["short", "medium", "long"]:
            volatility_regime = self.window_processor.calculate_volatility_regime(
                window_type
            )
            metrics[f"{window_type}_volatility"] = float(
                volatility_regime.get("volatility", 0)
            )

            regime = volatility_regime.get("regime", "unknown")
            metrics[f"{window_type}_is_high_volatility"] = float(
                regime == "high_volatility"
            )
            metrics[f"{window_type}_is_low_volatility"] = float(
                regime == "low_volatility"
            )
        ...
        metrics["is_lunch_time"] = float(
            self.hk_trading_hours["morning_end"]
            <= current_time
            < self.hk_trading_hours["afternoon_start"]
        )

        return metrics

    def _detect_anomaly(
        self, data_point: Dict[str, Any], metrics: Dict[str, float]
    ) -> Dict[str, Any]:
        """检测异常"""
        anomaly_result = {
            "is_anomaly": False,
        ...
        try:
            ...
            if self.anomaly_detector and self.anomaly_detector.ensemble.is_fitted:
                ...
                feature_vector = np.array(
                    [metrics.get(key, 0) for key in sorted(metrics.keys())]
                ).reshape(1, -1)

                # 预测异常
                prediction = self.anomaly_detector.ensemble.predict(feature_vector)[0]
                probability = self.anomaly_detector.ensemble.predict_proba(
                    feature_vector
                )[0, 1]
                decision_score = self.anomaly_detector.ensemble.decision_function(
                    feature_vector
                )[0]
                ...
                # 使用简单的统计方法
                anomaly_result = self._statistical_anomaly_detection(
                    data_point, metrics
                )
        ...
        try:
            ...
            if metrics.get("short_has_change_point", 0):
                ...
                anomaly_result["confidence"] = (
                    metrics.get("short_change_magnitude", 0) / 10
                )
    ...
    def _generate_alerts(
        ...
        if anomaly_result.get("is_anomaly", False):
            alert = {
                "type": "anomaly",
                "severity": (
                    "high" if anomaly_result.get("confidence", 0) > 0.8 else "medium"
                ),
        ...
        for alert in alerts:
            ...
            if (
                ...
                or (current_time - last_alert_time).total_seconds()
        ...
        for alert in filtered_alerts:
            ...
            logger.warning(
                f"Alert generated: {alert['type']} - {alert.get('severity', 'unknown')}"
            )
    ...
    def _update_online_models(
        ...
        try:
            ...
            1 if anomaly_result.get("is_anomaly", False) else 0
        ...
        try:
            ...
            for i in range(len(self.metrics["short_mean"])):
                if i < len(self.metrics["short_mean"]) - 1:
                    ...
                    current_trend = (
                        1
                        if self.metrics["short_mean"][i + 1]
                    ...
                    feature_vectors.append(
                        [
                            ...
                            (
                                self.metrics["medium_mean"][i]
                                if i < len(self.metrics["medium_mean"])
                                else 0
                            ),
                        ]
                    )
    ...
    def _is_hk_trading_hours(self, current_time: time) -> bool:
        """检查是否在香港交易时间"""
        return (
            self.hk_trading_hours["morning_start"]
            <= current_time
            < self.hk_trading_hours["morning_end"]
        ) or (
            self.hk_trading_hours["afternoon_start"]
            <= current_time
            < self.hk_trading_hours["afternoon_end"]
        )
    ...
    def _update_performance_stats(self, processing_time: float) -> None:
        ...
        self.performance_stats["average_processing_time"] = (
            current_avg * (total_samples - 1) + processing_time
        ) / total_samples
    ...
    def initialize_from_baseline(self, baseline_data: pd.Series) -> None:
        ...
        try:
            ...
            # 滑动窗口统计
            for window_size in [
                self.config.short_window_size,
                self.config.medium_window_size,
                self.config.long_window_size,
            ]:
                if len(baseline_data) >= window_size:
                    window_data = (
                        baseline_data.rolling(window = window_size).mean().dropna()
                    )
            ...
            logger.info(
                f"Monitor initialized with {len(baseline_data)} baseline data points"
            )
    ...
    def get_monitoring_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "performance_summary": {
                ...
                "average_processing_time_ms": self.performance_stats[
                    "average_processing_time"
                ],
                "samples_per_second": (
                    self.performance_stats["processed_samples"]
    ...
    def _summarize_alerts(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "alerts_per_hour": (
                ...
                / (self.performance_stats["processed_samples"] / 60)
                if self.performance_stats["processed_samples"] > 0
                else 0
            ),
        }
...
if __name__ == "__main__":
    ...
    for idx in anomaly_indices:
        baseline_data[idx] = baseline_data[idx] + np.random.choice(
            [-20, 20]
        )  # 显著的异常
    ...
    def alert_callback(alert):
        print(
            f"🚨 ALERT: {alert['type']} - {alert['severity']} at {alert['timestamp']}"
        )
        if "anomaly_details" in alert:
            print(
                f"   Anomaly score: {alert['anomaly_details'].get('anomaly_score', 0):.3f}"
            )
        if "threshold_details" in alert:
            print(
                f"   Threshold breach: {alert['metric_name']} = {alert['threshold_details'].get('value', 0):.3f}"
            )
    ...
    for i, value in enumerate(baseline_data):
        ...
        if i % 100 == 0:
            ...
            print(
                f"  Samples processed: {status['performance_stats']['processed_samples']}"
            )
            print(
                f"  Alerts generated: {status['performance_stats']['alerts_generated']}"
            )
