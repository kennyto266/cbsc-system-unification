...
class IsolationForestDetector(BaseAnomalyDetector):
    ...
    def fit(
        ...
        try:
            ...
            logger.info(
                f"Isolation Forest trained with {X.shape[0]} samples, {X.shape[1]} features"
            )
            return self
...
class OneClassSVMDetector(BaseAnomalyDetector):
    ...
    def fit(
        ...
        try:
            ...
            logger.info(
                f"One - Class SVM trained with {X.shape[0]} samples, {X.shape[1]} features"
            )
            return self
...
class RandomForestAnomalyDetector(BaseAnomalyDetector):
    ...
    def fit(
        ...
        try:
            ...
            logger.info(
                f"Random Forest trained with {X.shape[0]} samples, {X.shape[1]} features"
            )
            return self
...
class GradientBoostingAnomalyDetector(BaseAnomalyDetector):
    ...
    def fit(
        ...
        try:
            ...
            logger.info(
                f"Gradient Boosting trained with {X.shape[0]} samples, {X.shape[1]} features"
            )
            return self
...
class NeuralNetworkAnomalyDetector(BaseAnomalyDetector):
    ...
    def __init__(self, config: Optional[MLModelConfig] = None):
        ...
        if not TENSORFLOW_AVAILABLE:
            logger.warning(
                "TensorFlow not available, Neural Network detector will not work"
            )
    ...
    def _build_model(self, input_shape: int) -> None:
        ...
        self.model = Sequential(
            [
                Dense(
                    self.config.nn_hidden_layers[0],
                ...
                Dense(
                    self.config.nn_hidden_layers[1],
                    activation = self.config.nn_activation,
                ),
                BatchNormalization(),
                Dropout(self.config.nn_dropout),
                Dense(
                    self.config.nn_hidden_layers[2],
                    activation = self.config.nn_activation,
                ),
                BatchNormalization(),
                Dropout(self.config.nn_dropout),
    ...
    def fit(
        ...
        try:
            ...
            self.is_fitted = True
            logger.info(
                f"Neural Network trained with {X.shape[0]} samples, {X.shape[1]} features"
            )
            return self
...
class EnsembleAnomalyDetector:
    ...
    def _initialize_detectors(self) -> None:
        ...
        try:
            ...
            # 监督检测器（这些需要标签数据）
            self.detectors["random_forest"] = RandomForestAnomalyDetector(
                self.ml_config
            )
            self.detectors["gradient_boosting"] = GradientBoostingAnomalyDetector(
                self.ml_config
            )

            # 神经网络检测器（如果TensorFlow可用）
            if TENSORFLOW_AVAILABLE:
                self.detectors["neural_network"] = NeuralNetworkAnomalyDetector(
                    self.ml_config
                )
    ...
    def fit(
        ...
        try:
            logger.info(
                f"Training ensemble with {X.shape[0]} samples, {X.shape[1]} features"
            )
            ...
            if total_weight > 0:
                ...
                if n_detectors > 0:
                    for name in self.weights:
                        self.weights[name] = (
                            1.0 / n_detectors if self.weights[name] > 0 else 0
                        )
...
class MLAnomalyDetector:
    ...
    def detect_anomalies(
        ...
        try:
            logger.info(
                f"Starting ML anomaly detection with {X.shape[0]} samples, {X.shape[1]} features"
            )
            ...
            # 异常类型分析
            results["anomaly_analysis"] = self._analyze_anomalies(
                X_selected, predictions, probabilities
            )

            # 模型性能评估（如果有标签）
            if y is not None:
                results["performance_metrics"] = self._evaluate_performance(
                    y, predictions, probabilities
                )

            logger.info(
                f"Anomaly detection completed, found {results['predictions']['num_anomalies']} anomalies"
            )
            return results
        ...
        try:
            if y is not None and len(np.unique(y)) > 1:
                ...
                selector = SelectKBest(
                    f_classif, k = min(self.config.top_features_count, X.shape[1])
                )
                ...
                top_indices = np.argsort(feature_variances)[
                    -self.config.top_features_count :
                ]
    ...
    def _analyze_anomalies(
        self, X: np.ndarray, predictions: np.ndarray, probabilities: np.ndarray
    ) -> Dict[str, Any]:
        """分析异常特征"""
        try:
            anomaly_analysis = {}
            ...
            if np.sum(anomaly_mask) > 0:
                ...
                for i in range(X.shape[1]):
                    ...
                    normal_values = (
                        normal_data[:, i] if normal_data is not None else None
                    )
                    ...
                    if normal_values is not None:
                        ...
                        diff_stats = {
                            "mean_difference": float(
                                np.mean(anomaly_values) - np.mean(normal_values)
                            ),
                            "std_difference": float(
                                np.std(anomaly_values) - np.std(normal_values)
                            ),
                            "effect_size": float(
                                (np.mean(anomaly_values) - np.mean(normal_values))
                                / np.sqrt(
                                    (np.var(anomaly_values) + np.var(normal_values)) / 2
                                )
                            ),
                        }
            ...
            if len(np.unique(y_true)) > 1 and len(y_true) > 10:
                try:
                    cv_scores = cross_val_score(
                        self.ensemble.detectors.get(
                            "random_forest",
                            self.ensemble.detectors.get("gradient_boosting"),
                        ),
                        self.ensemble.scaler.transform(
                            self.ensemble.feature_selector.transform(X)
                            if hasattr(self.ensemble, "feature_selector")
                            else X
                        ),
                        y_true,
                        cv = 5,
...
if __name__ == "__main__":
    ...
    if "performance_metrics" in results:
        ...
        if "cross_validation" in perf:
            ...
            print(
                f"Cross - validation F1 Score: {cv['mean_f1_score']:.4f} ± {cv['std_f1_score']:.4f}"
            )
