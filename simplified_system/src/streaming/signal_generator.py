...
class SignalGenerator:
    ...
    def __init__(self, signal_timeout_minutes: int = 30):
        ...
        # 技术指标参数配置
        self.rsi_config = {"oversold": 30, "overbought": 70, "period": 14}

        self.macd_config = {"fast_period": 12, "slow_period": 26, "signal_period": 9}

        self.bollinger_config = {"period": 20, "std_dev": 2}

        self.ma_config = {"short_period": 20, "long_period": 50}
    ...
    def _generate_rsi_signal(
        self, symbol: str, tick: StockTick, indicators: TechnicalIndicators
        ...
        if rsi < self.rsi_config["oversold"]:
            ...
            signal_strength = (
                SignalStrength.STRONG if rsi < 20 else SignalStrength.MODERATE
            )
            ...
            signal_strength = (
                SignalStrength.STRONG if rsi > 80 else SignalStrength.MODERATE
            )
    ...
    def _generate_combined_signal(
        self, symbol: str, tick: StockTick, signals: List[TradingSignal]
    ) -> Optional[TradingSignal]:
        """生成综合信号"""
        if len(signals) < 2:
            return None
        ...
        if buy_count > sell_count:
            ...
            signal_strength = (
                SignalStrength.STRONG if buy_count >= 3 else SignalStrength.MODERATE
            )
        elif sell_count > buy_count:
            signal_type = SignalType.STRONG_SELL if sell_count >= 3 else SignalType.SELL
            signal_strength = (
                SignalStrength.STRONG if sell_count >= 3 else SignalStrength.MODERATE
            )
        else:
        ...
        return TradingSignal(
            ...
            target_price = self._calculate_target_price(
                signal_type, tick.price, avg_confidence
            ),
    ...
    def _calculate_target_price(
        self, signal_type: SignalType, current_price: float, confidence: float
    ) -> Optional[float]:
        """计算目标价格"""
    ...
    async def _process_signal(self, signal: TradingSignal) -> None:
        ...
        try:
            ...
            logger.info(
                f"Generated signal: {signal.symbol} - {signal.signal_type.value} ({signal.confidence:.2f})"
            )
