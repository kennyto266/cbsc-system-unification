...
class RealTimeStreamingServer:
    ...
    async def _register_event_handlers(self) -> None:
        ...
        self.event_processor.register_handler(
            "price_updates", handle_price_update, async_handler = True
        )
        self.event_processor.register_handler(
            "technical_signals", handle_technical_signal, async_handler = True
        )
    ...
    async def _on_price_update(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            await self.event_processor.publish_price_update(
                symbol = symbol, price = price, volume = volume, source="data_streamer"
            )
    ...
    async def start(self, symbols: List[str] = None) -> None:
        ...
        try:
            ...
            await self.data_streamer.start_streaming(
                symbols or ["0700.HK", "0941.HK", "1398.HK"]
            )
            ...
            logger.info(
                f"WebSocket server available at ws://{self.websocket_server.host}:{self.websocket_server.port}"
            )
    ...
    async def get_system_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "components": {
                "data_streamer": (
                    self.data_streamer.get_performance_stats()
                ...
                "event_processor": (
                    self.event_processor.get_stats() if self.event_processor else None
                ),
                "signal_generator": (
                    self.signal_generator.get_statistics()
                    if self.signal_generator
                    else None
                ),
                "redis_manager": (
                    self.redis_manager.get_stats() if self.redis_manager else None
                ),
                "kafka_processor": (
                    self.kafka_processor.get_stats() if self.kafka_processor else None
                ),
            },
        }
        ...
        try:
            ...
            report = {
                ...
                "configuration": {
                    ...
                    "websocket_port": (
                        self.websocket_server.port if self.websocket_server else None
                    ),
                },
            }
    ...
    async def main():
        ...
        try:
            ...
            while server._running:
                ...
                logger.info(
                    f"Server status: {status['server_status']}, "
