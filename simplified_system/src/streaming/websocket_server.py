...
class WebSocketServer:
    ...
    async def handle_client(
        ...
        logger.info(
            f"New WebSocket connection: {connection_id} from {websocket.remote_address}"
        )
    ...
    async def handle_message(
        ...
        try:
            ...
            if command == "ping":
                await connection.send_message(
                    "pong", {"timestamp": datetime.now().isoformat()}
                )
    ...
    async def handle_subscribe(
        ...
        for symbol in symbols:
            ...
            self.streamer.subscribe(
                symbol,
                lambda data, conn = connection, sym = symbol: self.broadcast_to_connection(
                    conn, "price_update", data
                ),
            )
        ...
        logger.info(
            f"Connection {connection.connection_id} subscribed to: {subscribed_symbols}"
        )
    ...
    async def handle_unsubscribe(
        ...
        logger.info(
            f"Connection {connection.connection_id} unsubscribed from: {unsubscribed_symbols}"
        )
    ...
    async def handle_get_data(
        ...
        await connection.send_message(
            "data_response",
            {"symbols": all_data, "request_time": datetime.now().isoformat()},
        )
    ...
    async def broadcast_to_connection(
        self, connection: WebSocketConnection, message_type: str, data: Any
    ) -> None:
        """向特定连接广播消息"""
        try:
    ...
    async def broadcast_stats(self) -> None:
        ...
        while self._running:
            try:
                ...
                stats_message = {"server": server_stats, "streamer": streamer_stats}
