...
class KafkaTopicManager:
    ...
    def get_topic_for_message_type(
        self, message_type: KafkaMessageType
    ) -> Optional[str]:
        """根据消息类型获取主题"""
...
class KafkaStreamProcessor:
    ...
    def __init__(
        ...
        logger.info(
            f"Kafka Stream Processor initialized with group_id: {self.group_id}"
        )
    ...
    async def _process_message(self, message) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Received message: {kafka_message.message_type.value} from {kafka_message.source}"
            )
    ...
    async def send_message(self, message: KafkaMessage) -> bool:
        ...
        try:
            # 确定目标主题
            topic_name = self.topic_manager.get_topic_for_message_type(
                message.message_type
            )
            if not topic_name:
                logger.error(
                    f"No topic configured for message type: {message.message_type}"
                )
                return False

            # 确定分区键
            partition_key = (
                message.partition_key or message.symbol or message.correlation_id
            )

            # 发送消息
            await self.producer.send_and_wait(
                topic = topic_name, value = message.to_dict(), key = partition_key
            )
    ...
    def register_handler(
        self, message_type: KafkaMessageType, handler: Callable[[KafkaMessage], None]
    ) -> None:
        """
        注册消息处理器
...
def get_kafka_processor(
    bootstrap_servers: List[str], group_id: str
...
if __name__ == "__main__":
    ...
    async def test_kafka_integration():
        ...
        try:
            ...
            await processor.send_risk_alert(
                "0700.HK", "VOLATILITY", "HIGH", "High volatility detected"
            )
