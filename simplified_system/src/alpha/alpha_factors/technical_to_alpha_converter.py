...
class TechnicalIndicatorConverter:
    ...
    def _initialize_factor_mappings(self) -> Dict[str, List[FactorMapping]]:
        ...
        mappings = {
            "momentum": [
                ...
                FactorMapping(
                    "MACD", "momentum", "macd_signal", "MACD signal factor", "direct"
                ),
                FactorMapping(
                    "CCI", "momentum", "cci_momentum", "CCI momentum factor", "zscore"
                ),
                FactorMapping(
                    "Stochastic",
                    "momentum",
            ...
            "reversal": [
                FactorMapping(
                    "RSI", "reversal", "rsi_reversal", "RSI reversal factor", "inverse"
                ),
                FactorMapping(
                    "Stochastic",
                    "reversal",
                    "stoch_reversal",
                    "Stochastic reversal factor",
                    "inverse",
                ),
                FactorMapping(
                    "Williams_R",
            ...
            "trend": [
                ...
                FactorMapping(
                    "Aroon", "trend", "aroon_trend", "Aroon trend factor", "normalized"
                ),
                FactorMapping(
                    "DMI", "trend", "dmi_trend", "DMI trend factor", "zscore"
                ),
            ],
            "quality": [
                FactorMapping(
                    ...
                    "quality",
                    "cmf_quality",
                    "Chaikin Money Flow quality factor",
                    "normalized",
                ),
                FactorMapping(
                    "Force_Index",
    ...
    def convert_technical_to_alpha(
        self,
        data: pd.DataFrame,
        ...
        for indicator_name in indicator_names:
            try:
                ...
                indicator_data = self._calculate_technical_indicator(
                    data, indicator_name, lookback_periods
                )
                    ...
                    if period in indicator_data:
                        ...
                        for mapping in self._get_mappings_for_indicator(indicator_name):
                            ...
                            if mapping.transformation_method == "direct":
                                ...
                                alpha_factors[factor_name] = self._normalize_factor(
                                    factor_data
                                )

                            elif mapping.transformation_method == "zscore":
                                alpha_factors[factor_name] = self._zscore_factor(
                                    factor_data
                                )
                ...
                logger.warning(
                    f"Failed to convert {indicator_name} to alpha factor: {e}"
                )
                continue

        logger.info(
            f"Converted {len(alpha_factors.columns)} technical indicators to alpha factors"
        )
        return alpha_factors

    def _calculate_technical_indicator(
        self, data: pd.DataFrame, indicator_name: str, lookback_periods: List[int]
    ) -> Dict[int, pd.Series]:
        """
        計算技術指標數據
        ...
        for period in lookback_periods:
            try:
                if indicator_name == "RSI":
                    indicator_data[period] = self.core_indicators.calculate_rsi(
                        data["Close"], period
                    )
                    ...
                    macd_line, signal_line, histogram = (
                        self.core_indicators.calculate_macd(
                    ...
                    upper, middle, lower = (
                        self.core_indicators.calculate_bollinger_bands(
                            data["Close"], period, 2
                        )
                    )
                    # 布林帶位置因子
                    ...
                    indicator_data[period] = self.core_indicators.calculate_atr(
                        data, period
                    )

                elif indicator_name == "ADX":
                    indicator_data[period] = self.core_indicators.calculate_adx(
                        data, period
                    )

                elif indicator_name == "Stochastic":
                    k_percent, d_percent = self.core_indicators.calculate_stochastic(
                        data, period, 3
                    )
                    indicator_data[period] = k_percent

                elif indicator_name == "Williams_R":
                    indicator_data[period] = self.core_indicators.calculate_williams_r(
                        data, period
                    )

                elif indicator_name == "CCI":
                    indicator_data[period] = self.core_indicators.calculate_cci(
                        data, period
                    )
                    ...
                    if "Volume" in data.columns:
                        indicator_data[period] = (
                            data["Volume"]
                            / data["Volume"].rolling(window = period).mean()
                        )
                    else:
                        logger.warning(
                            f"Volume data not available for {indicator_name}"
                        )
                    ...
                    indicator_data[period] = self.core_indicators.calculate_mfi(
                        data, period
                    )
                    ...
                    aroon_up, aroon_down = self.core_indicators.calculate_aroon(
                        data, period
                    )
                    indicator_data[period] = aroon_up - aroon_down

                elif indicator_name == "DMI":
                    di_plus, di_minus, adx = self.core_indicators.calculate_dmi(
                        data, period
                    )
                    indicator_data[period] = di_plus - di_minus

                elif indicator_name == "Chaikin_Money_Flow":
                    indicator_data[period] = self.core_indicators.calculate_cmf(
                        data, period
                    )

                elif indicator_name == "Force_Index":
                    indicator_data[period] = self.core_indicators.calculate_force_index(
                        data, period
                    )
                ...
                logger.warning(
                    f"Failed to calculate {indicator_name} with period {period}: {e}"
                )
                continue
    ...
    def generate_factor_universe(
        self, data: pd.DataFrame, factor_types: List[str] = None
    ) -> pd.DataFrame:
        """
        生成完整的Alpha因子宇宙
        ...
        for factor_type in factor_types:
            if factor_type in self.factor_mappings:
                ...
                indicator_names = list(
                    set(mapping.indicator_name for mapping in mappings)
                )
        ...
        logger.info(
            f"Generated factor universe with {len(all_factors.columns)} factors"
        )
        return all_factors
...
class BulkTechnicalConverter:
    ...
    def convert_all_available_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(
            f"Converting {len(available_indicators)} technical indicators to alpha factors"
        )
