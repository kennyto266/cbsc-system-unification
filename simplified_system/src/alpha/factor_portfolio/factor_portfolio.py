...
class LinearRegressionModel(BaseFactorModel):
    ...
    def _fit_simple_ols(self, factor_data: pd.DataFrame, returns: pd.Series):
        ...
        try:
            ...
            self.model_metrics = {
                "r_squared": 1
                - np.sum((y - y_pred) ** 2) / np.sum((y - y.mean()) ** 2),
                "mse": np.mean((y - y_pred) ** 2),
                "rmse": np.sqrt(np.mean((y - y_pred) ** 2)),
            }
...
class FactorPortfolio:
    ...
    def __init__(self, config: Optional[FactorModelConfig] = None):
        ...
        logger.info(
            f"Factor Portfolio initialized with model type: {self.config.model_type.value}"
        )

    def select_factors(
        self, factor_dict: Dict[str, pd.DataFrame], criteria: str = "ic_ir"
    ) -> List[str]:
        """
        因子篩選
        ...
        if len(selected_factors) > self.config.max_factors:
            if criteria in factor_stats:
                ...
                selected_factors = sorted(
                    selected_factors,
                    key = lambda x: self._calculate_factor_statistics(factor_dict[x]).get(
                        criteria, 0
                    ),
                    reverse = True,
        ...
        logger.info(
            f"Selected {len(selected_factors)} factors out of {len(factor_dict)}"
        )
    ...
    def _calculate_factor_statistics(
        ...
        stats = {
            "mean": (
                factor_data.mean().mean()
                if len(factor_data.columns) > 1
                else factor_data.iloc[:, 0].mean()
            ),
            "std": (
                factor_data.std().mean()
                if len(factor_data.columns) > 1
                else factor_data.iloc[:, 0].std()
            ),
            ...
            stats.update(
                {
                    ...
                    "ic_ir": (
                        abs(auto_corr) / 0.1 if auto_corr != 0 else 0
                    ),  # 假設標準差為0.1
    ...
    def _passes_selection_criteria(
        self, factor_stats: Dict[str, float], criteria: str
    ) -> bool:
        """檢查因子是否通過篩選標準"""
        if not factor_stats:
            return False
    ...
    def build_model(
        self, factors: Dict[str, pd.DataFrame], returns: pd.Series, method: str = None
    ) -> BaseFactorModel:
        """
        構建多因子模型
        ...
        if method == "linear_regression":
            ...
            logger.warning(
                f"Model method {method} not implemented, using linear regression"
            )
            model = LinearRegressionModel(self.config)
    ...
    def _remove_highly_correlated_factors(
    ...
    def optimize_weights(
        self, factor_data: pd.DataFrame, returns: pd.Series, method: str = "ic_weighted"
    ) -> Dict[str, float]:
        """
        優化因子權重
        ...
        if method == "equal_weight":
            ...
            logger.warning(
                f"Weight optimization method {method} not implemented, using equal weights"
            )
            return self._equal_weight_optimization(factor_data)
    ...
    def _mean_variance_optimization(
        ...
        try:
            ...
            for factor_col in factor_data.columns:
                # 使用因子值作為收益的代理
                factor_returns[factor_col] = (
                    factor_data[factor_col].pct_change().fillna(0)
                )
            ...
            logger.warning(
                f"Mean - variance optimization failed: {e}, falling back to equal weights"
            )
            return self._equal_weight_optimization(factor_data)
