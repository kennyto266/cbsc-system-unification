...
class HistoricalDataExtender:
    ...
    def extend_historical_data(
        ...
        if len(original_data) >= target_records:
            logger.info(
                f"Original data already has {len(original_data)} records, no extension needed"
            )
            return {
                "success": True,
        ...
        try:
            logger.info(
                f"Extending {len(original_data)} records to {target_records} using {method} method"
            )

            if method not in self.extension_methods:
                return {
                    "success": False,
                    "error": f"Unknown extension method: {method}",
                }

            # 使用指定的擴展方法
            extended_data = self.extension_methods[method](
                original_data, target_records
            )
            ...
            result = {
                ...
                "metadata": self._generate_metadata(
                    extended_data, original_data, method
                ),
            }

            logger.info(
                f"Successfully extended data: {result['original_count']} -> {result['final_count']} records"
            )
            return result
    ...
    def _extend_with_trend_preservation(
        self, data: List[Dict[str, Any]], target: int
    ) -> List[Dict[str, Any]]:
        """基於趨勢保持擴展數據"""
            ...
            for col in numeric_columns:
                if col in new_record and new_record[col] is not None:
                    ...
                    if col in df.iloc[base_idx] and col in df.iloc[next_idx]:
                        ...
                        trend = (
                            (next_value - base_value) / base_value
                            if base_value != 0
                            else 0
                        )
    ...
    def _extend_with_seasonal_adjustment(
        self, data: List[Dict[str, Any]], target: int
    ) -> List[Dict[str, Any]]:
        """基於季節性調整擴展數據"""
        df = pd.DataFrame(data)
        ...
        for i in range(len(data), target):
            ...
            for col in numeric_columns:
                if (
                    col in new_record
                    and new_record[col] is not None
                    and col in seasonal_patterns
                ):
                    seasonal_avg = seasonal_patterns[col].get(
                        day_of_year, new_record[col]
                    )
                    ...
                    if overall_avg != 0:
                        ...
                        new_record[col] = (
                            new_record[col] * seasonal_factor * random_variation
                        )
    ...
    def _extend_with_statistical_simulation(
        self, data: List[Dict[str, Any]], target: int
    ) -> List[Dict[str, Any]]:
        """基於統計模擬擴展數據"""
        df = pd.DataFrame(data)
        ...
        extended_data = data.copy()
        last_values = {
            col: df[col].iloc[-1] if df[col].notna().any() else 0
            for col in numeric_columns
        }
    ...
    def _extend_with_hybrid_approach(
        self, data: List[Dict[str, Any]], target: int
    ) -> List[Dict[str, Any]]:
        """混合方法擴展數據"""
        df = pd.DataFrame(data)
        ...
        if len(data) < 30:
            ...
            if len(trend_extended) < target:
                target - len(trend_extended)
                seasonal_extended = self._extend_with_seasonal_adjustment(
                    trend_extended, target
                )
    ...
    def _generate_metadata(
        ...
        numeric_columns = df_extended.select_dtypes(
            include=[np.number]
        ).columns.tolist()
        ...
        for col in numeric_columns:
            if col in df_original.columns:
                ...
                quality_metrics[col] = {
                    "mean_preservation": (
                        abs(ext_mean - orig_mean) / orig_mean if orig_mean != 0 else 0
                    ),
                    "std_preservation": (
                        abs(ext_std - orig_std) / orig_std if orig_std != 0 else 0
                    ),
        ...
        return {
            ...
            "time_span": {
                "start_date": (
                    df_extended["date"].min() if "date" in df_extended.columns else None
                ),
                "end_date": (
                    df_extended["date"].max() if "date" in df_extended.columns else None
                ),
    ...
    def batch_extend_multiple_sources(
        ...
        """批量擴展多個數據源"""
        logger.info(
            f"Starting batch extension of {len(data_sources)} sources to {target_records} records each"
        )
        ...
        for source_name, source_data in data_sources.items():
            try:
                ...
                if extension_result.get("success"):
                    ...
                    logger.info(
                        f"[OK] {source_name}: {extension_result['original_count']} -> {extension_result['final_count']} records"
                    )
                else:
                    logger.error(
                        f"[FAIL] {source_name}: {extension_result.get('error', 'Unknown error')}"
                    )
                    results["extended_data"][source_name] = extension_result
        ...
        # 更新總結
        results["session_info"]["end_time"] = datetime.now().strftime(
            "%Y-%m-%d %H:%M:%S"
        )
        results["summary"] = {
            ...
            "average_extension_ratio": (
                (total_original_records + total_extended_records)
        ...
        logger.info(
            f"Batch extension complete: {successful_extensions}/{len(data_sources)} sources successful"
        )
        logger.info(
            f"Total records: {total_original_records} -> {results['summary']['total_final_records']}"
        )
...
# 便捷函數
def extend_data_records(
    data: List[Dict[str, Any]], target: int = 1000, method: str = "hybrid_approach"
) -> Dict[str, Any]:
    """擴展數據記錄"""
    return historical_extender.extend_historical_data(data, target, method)


def batch_extend_sources(
    data_sources: Dict[str, List[Dict[str, Any]]], target: int = 1000
) -> Dict[str, Any]:
    """批量擴展多個數據源"""
...
if __name__ == "__main__":
    ...
    for method in methods:
        ...
        if result.get("success"):
            print(
                f"[OK] {method}: {result['original_count']} -> {result['final_count']} records"
            )
            print(f"  Extension ratio: {result['extension_ratio']:.2f}x")
            print(
                f"  Quality score: {len(result.get('metadata', {}).get('quality_metrics', {}))}"
            )
        else:
    ...
    if batch_result.get("success"):
        ...
        print(
            f"Batch extension: {summary['successful_sources']}/{summary['total_sources']} successful"
        )
        print(
            f"Total records: {summary['total_original_records']} -> {summary['total_final_records']}"
        )
    ...
    print("\n=== USAGE EXAMPLES ===")
    print(
        "from historical_data_extender import extend_data_records, batch_extend_sources"
    )
    print()
    print("# Extend single data source")
    print("extended_data = extend_data_records(original_data, 1000, 'hybrid_approach')")
    print(
        'print(f\'Extended from {extended_data["original_count"]} to {extended_data["final_count"]} records\')'
    )
    print()
    print("# Batch extend multiple sources")
    print(
        "batch_result = batch_extend_sources({'source1': data1, 'source2': data2}, 1000)"
    )
