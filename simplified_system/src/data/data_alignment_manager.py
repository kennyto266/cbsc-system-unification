...
class TemporalAligner:
    ...
    def find_common_timeframe(
        ...
        self.logger.info(
            f"Common timeframe: {common_start.date()} to {common_end.date()}"
        )

        # 記錄每個數據源的時間範圍信息
        for source_name, info in date_ranges.items():
            self.logger.info(
                f"{source_name}: {info['start'].date()} to {info['end'].date()} ({info['count']} records)"
            )
...
class MissingDataHandler:
    ...
    def detect_missing_patterns(self, df: pd.DataFrame, date_col: str) -> Dict:
        ...
        missing_ratio = (
            len(missing_dates) / len(expected_dates) if len(expected_dates) > 0 else 0
        )
        ...
        self.logger.info(
            f"Missing data analysis: {missing_ratio:.2%} missing, {len(patterns)} gaps"
        )
    ...
    def fill_missing_data(
        ...
        if value_columns is None:
            value_columns = df_reindexed.select_dtypes(
                include=[np.number]
            ).columns.tolist()

        # 根據方法填充數據
        if method == "forward_fill":
            df_reindexed[value_columns] = df_reindexed[value_columns].fillna(
                method="ffill"
            )
        elif method == "linear":
        ...
        self.logger.info(
            f"Filled {filled_records} missing records using {method} method"
        )
...
class DataQualityValidator:
    ...
    def validate_data_quality(self, df: pd.DataFrame, data_source: str = None) -> Dict:
        ...
        for col, ratio in null_ratios.items():
            if ratio > 0.5:
                ...
                quality_report["recommendations"].append(
                    f"Consider imputing or removing '{col}' column"
                )
        ...
        for col in numeric_cols:
            ...
            if not values.empty:
                ...
                if outlier_ratio > 0.1:  # 超過10%異常值
                    ...
                    quality_report["issues"].append(
                        f"Column '{col}' has {outlier_ratio:.1%} outliers"
                    )

        consistency_score = (
            max(0, 1 - consistency_issues / total_checks) if total_checks > 0 else 1
        )
            ...
            if any(keyword in col.lower() for keyword in ["date", "time"]):
                ...
                try:
                    ...
                    if dates.isnull().all():
                        validity_issues += 1
                        quality_report["issues"].append(
                            f"Date column '{col}' contains no valid dates"
                        )
                except Exception:
                    validity_issues += 1
                    quality_report["issues"].append(
        ...
        for col in numeric_cols:
            ...
            if (values < 0).any() and "rate" in col.lower():
                ...
                if negative_ratio > 0:
                    validity_issues += 1
                    quality_report["issues"].append(
                        f"Rate column '{col}' has {negative_ratio:.1%} negative values"
                    )

        validity_score = (
            max(0, 1 - validity_issues / total_validity_checks)
        ...
        if quality_report["overall_score"] < 0.7:
            quality_report["recommendations"].append(
                "Overall data quality is below threshold"
            )
        ...
        self.logger.info(
            f"Data quality validation for {data_source}: overall score {quality_report['overall_score']:.3f}"
        )
...
class DataAlignmentManager:
    ...
    def align_multiple_sources(
        ...
        try:
            ...
            # Step 1: 找到共同時間範圍
            common_start, common_end = self.temporal_aligner.find_common_timeframe(
                data_dict
            )
            ...
            for source_name, df in data_dict.items():
                if df.empty:
                    alignment_result["errors"].append(
                        f"Data source '{source_name}' is empty"
                    )
                    continue
                ...
                try:
                    ...
                    if date_col is None:
                        alignment_result["errors"].append(
                            f"No date column found in '{source_name}'"
                        )
                        continue
                    ...
                    # 檢測缺失數據模式
                    missing_report = self.missing_data_handler.detect_missing_patterns(
                        aligned_df, date_col
                    )
                    alignment_result["missing_data_reports"][
                        source_name
                    ] = missing_report

                    # 填充缺失數據
                    if fill_missing and missing_report["missing_ratio"] > 0:
                        value_columns = aligned_df.select_dtypes(
                            include=[np.number]
                        ).columns.tolist()
                    ...
                    # 數據質量驗證
                    if validate_quality:
                        quality_report = self.quality_validator.validate_data_quality(
                            aligned_df, source_name
                        )
                        alignment_result["quality_reports"][
                            source_name
                        ] = quality_report
            ...
            if alignment_result["quality_reports"]:
                avg_quality = np.mean(
                    [
                        ...
                        for report in alignment_result["quality_reports"].values()
                    ]
                )
            else:
                avg_quality = 0
            ...
            total_records_after = sum(
                len(df) for df in alignment_result["aligned_data"].values()
            )
            ...
            alignment_result["summary"] = {
                ...
                "record_change_ratio": (
                    total_records_after / total_records_before
            ...
            self.logger.info(
                f"Alignment completed: {successful_sources}/{total_sources} sources successful"
            )
    ...
    def save_alignment_result(self, result: Dict, output_dir: str = None):
        ...
        serializable_result = {
            ...
            "saved_files": [
                f"{name}_aligned_{timestamp}.csv"
                for name in result["aligned_data"].keys()
            ],
        }
...
def main():
    ...
    alignment_result = manager.align_multiple_sources(
        data_dict, fill_missing = True, fill_method="forward_fill", validate_quality = True
    )
    ...
    if alignment_result["success"]:
        ...
        for source_name, df in alignment_result["aligned_data"].items():
            ...
            print(
                f"  {source_name}: {len(df)} records, quality={quality_score:.3f}, missing={missing_ratio:.2%}"
            )
