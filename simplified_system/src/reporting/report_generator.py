...
class ReportGenerator:
    ...
    def generate_report(
        self, data: ReportData, config: ReportConfig, output_path: Optional[str] = None
    ) -> Dict[str, str]:
        """
        生成完整報告
        ...
        try:
            ...
            html_file = (
                output_dir / f"{config.report_type.value}_report_{timestamp}.html"
            )
            ...
            if config.export_pdf:
                try:
                    ...
                    pdf_file = (
                        output_dir
                        / f"{config.report_type.value}_report_{timestamp}.pdf"
                    )
            ...
            if config.export_excel:
                excel_file = (
                    output_dir / f"{config.report_type.value}_report_{timestamp}.xlsx"
                )
    ...
    def _generate_charts(self, data: ReportData) -> Dict[str, Any]:
        ...
        try:
            ...
            if data.raw_data is not None:
                ...
                charts["performance"] = pio.to_html(
                    fig, include_plotlyjs="cdn", div_id="performance_chart"
                )
            ...
            fig_radar.add_trace(
                go.Scatterpolar(
                    r = risk_metrics, theta = risk_labels, fill="toself", name="風險指標"
                )
            )

            fig_radar.update_layout(
                polar = dict(
                    radialaxis = dict(visible = True, range=[0, max(risk_metrics) * 1.1])
                ),
                title="風險指標雷達圖",
                height = 600,
            )

            charts["risk_radar"] = pio.to_html(
                fig_radar, include_plotlyjs="cdn", div_id="risk_radar"
            )

            # Monthly returns heatmap
            if data.raw_data is not None and "returns" in data.raw_data.columns:
                monthly_returns = (
                    data.raw_data["returns"]
                    .groupby([data.raw_data.index.year, data.raw_data.index.month])
                ...
                charts["monthly_heatmap"] = pio.to_html(
                    fig_heatmap, include_plotlyjs="cdn", div_id="monthly_heatmap"
                )
        ...
        try:
            with pd.ExcelWriter(str(output_file), engine="openpyxl") as writer:
                ...
                params_df = pd.DataFrame(
                    list(data.strategy_parameters.items()), columns=["參數", "數值"]
                )
    ...
    def generate_batch_reports(
        ...
        """批量生成報告"""
        self.logger.info(
            f"Starting batch report generation for {len(reports_data)} reports"
        )
        ...
        for i, (data, config) in enumerate(reports_data):
            future = self.executor.submit(
                self.generate_report, data, config, str(output_dir / f"batch_{i + 1}")
            )
        ...
        self.logger.info(
            f"Batch report generation completed: {len(generated_files)} reports generated"
        )
        return generated_files
    ...
    def create_report_from_backtest(
        ...
        report_data = ReportData(
            ...
            expected_shortfall_95 = getattr(
                backtest_result, "expected_shortfall_95", -0.07
            ),
            expected_shortfall_99 = getattr(
                backtest_result, "expected_shortfall_99", -0.12
            ),
...
def create_performance_report(
    ...
    report_data = generator.create_report_from_backtest(
        backtest_result, config, raw_data
    )
