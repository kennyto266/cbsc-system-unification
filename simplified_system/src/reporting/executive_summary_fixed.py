#!/usr / bin / env python3
"""
Executive Summary Generator - Fixed Version
執行摘要生成器 - 修復版本
...
class ExecutiveSummaryGenerator:
    ...
    def generate(
        ...
        try:
            ...
            recommendations = self._generate_recommendations(
                data, performance_analysis, risk_assessment
            )

            # Create summary structure
            summary = ExecutiveSummary(
                title=(
                    f"{data.strategy_name} 策略執行摘要"
                ...
                conclusion = self._generate_conclusion(
                    data, performance_analysis, language
                ),
    ...
    def _analyze_performance(self, data: ReportData) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "overall_rating": self._calculate_overall_rating(
                sharpe_rating, return_rating, drawdown_rating
            ),
        }
    ...
    def _generate_key_insights(
        self, data: ReportData, performance_analysis: Dict[str, Any]
    ) -> List[KeyInsight]:
        """生成關鍵洞察"""
        insights = []
    ...
    def _generate_overview_chinese(
        ...
        overview = f"""
        本報告分析了{data.strategy_name}策略在{data.period}期間的表現。
    ...
    def _calculate_overall_rating(
        self, sharpe_rating: str, return_rating: str, drawdown_rating: str
    ) -> str:
        """計算綜合評級"""
    ...
    def _generate_next_steps(
        self, data: ReportData, language: ReportLanguage
    ) -> List[str]:
        """生成下一步行動"""
        if language == ReportLanguage.ENGLISH:
            return [
    ...
    def _format_summary(
        self, summary: ExecutiveSummary, language: ReportLanguage, tone: SummaryTone
    ) -> str:
        """格式化摘要輸出"""
    ...
    def _format_chinese_summary(
        self, summary: ExecutiveSummary, tone: SummaryTone
    ) -> str:
        """格式化中文摘要"""
        formatted = f"""
# {summary.title}
