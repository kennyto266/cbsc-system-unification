...
class JSONExporter:
    ...
    def _convert_to_json_compatible(self, data: Any) -> Any:
        ...
        if data is None:
            ...
            return {
                key: self._convert_to_json_compatible(value)
    ...
    def _dataframe_to_dict(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "index": (
                    df.index.tolist() if len(df) <= 10000 else df.index[:10000].tolist()
                ),
                "data": records,
                "shape": df.shape,
        ...
        try:
            ...
            return {
                ...
                "index": (
                    series.index.tolist()
                    if len(series) <= 10000
                    else series.index[:10000].tolist()
                ),
                "data": data_dict,
    ...
    def export_schema(self, data: Any, output_path: Path) -> bool:
        ...
        try:
            ...
            with open(output_path, "w", encoding="utf - 8") as f:
                json.dump(schema, f, indent = self.indent, ensure_ascii = self.ensure_ascii)
    ...
    def _generate_schema(self, data: Any) -> Dict:
        """生成數據模式"""
        schema = {"schema_version": "1.0", "generated_at": datetime.now().isoformat()}
