...
class CSVExporter:
    ...
    def export(self, data: Any, output_path: Path, options: Dict = None) -> bool:
        ...
        try:
            ...
            df.to_csv(
                ...
                float_format = lambda x: f"{x:.{self._get_decimal_places(x)}f}".replace(
            ...
            logger.info(
                f"✅ CSV文件導出成功: {output_path} ({file_size} bytes, {len(df)} 行)"
            )
            return True
        ...
        try:
            ...
            for col in formatted_df.columns:
                if formatted_df[col].dtype == "datetime64[ns]":
                    if self.date_format:
                        formatted_df[col] = formatted_df[col].dt.strftime(
                            self.date_format
                        )
                ...
                # 處理數值列
                    ...
                    if self.decimal_separator != ".":
                        formatted_df[col] = formatted_df[col].apply(
                            lambda x: (
                                f"{x:.{self._get_decimal_places(x)}f}".replace(
                                    ".", self.decimal_separator
                                )
                                if pd.notna(x)
                                else ""
    ...
    def export_multiple_sheets(
        self, data_dict: Dict[str, Any], base_path: Path
    ) -> bool:
        """
        導出多個數據表到多個CSV文件
        ...
        try:
            ...
            for sheet_name, data in data_dict.items():
                ...
                sheet_path = (
                    base_path.parent
                    / f"{base_path.stem}_{sheet_name}{base_path.suffix}"
                )
            ...
            if success:
                ...
                logger.warning(
                    f"⚠️ 部分CSV導出失敗: {success_count}/{total_count} 個文件"
                )
    ...
    def export_with_summary(
        self, data: Any, output_path: Path, summary_data: Dict = None
    ) -> bool:
        """
        導出數據並包含摘要信息
        ...
        try:
            ...
            if summary_data:
                ...
                combined_df = pd.concat(
                    [summary_df, pd.DataFrame([["", ""]])], ignore_index = True
                )
    ...
    def export_validation_report(
        self, validation_results: Dict, output_path: Path
    ) -> bool:
        """
        導出數據驗證報告
        ...
        try:
            ...
            report_data.append(
                ["驗證時間", validation_results.get("validation_time", "")]
            )
            report_data.append(["", ""])
    ...
    def create_csv_template(
        self, columns: List[str], output_path: Path, sample_data: List = None
    ) -> bool:
        """
        創建CSV模板文件
        ...
        try:
            ...
            # 導出模板
            template_df.to_csv(
                output_path, index = False, encoding = self.encoding, sep = self.delimiter
            )
    ...
    def merge_csv_files(
        self, file_paths: List[Path], output_path: Path, add_source_column: bool = True
    ) -> bool:
        """
        合併多個CSV文件
        ...
        try:
            ...
            for file_path in file_paths:
                try:
                    df = pd.read_csv(
                        file_path, encoding = self.encoding, sep = self.delimiter
                    )
            ...
            file_info.update(
                {
                    ...
                    "estimated_rows": (
                        ...
                        // (
                            len(df.to_csv(index = False, header = False).encode()) / len(df)
                        )
                        if len(df) > 0
                        else 0
                    ),
