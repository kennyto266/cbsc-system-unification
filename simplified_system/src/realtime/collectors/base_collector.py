...
class BaseCollector(ABC):
    ...
    async def _process_data_point(self, data_point: DataPoint) -> None:
        ...
        try:
            ...
            if not self._validate_data_point(data_point):
                logger.warning(
                    f"Invalid data point from '{self.name}': {data_point.symbol}"
                )
                return
    ...
    async def _reconnect(self) -> None:
        ...
        while self._running and reconnect_count < max_reconnects:
            reconnect_count += 1
            logger.info(
                f"Reconnect attempt {reconnect_count}/{max_reconnects} for '{self.name}'"
            )
            ...
            try:
                ...
                if await self.connect():
                    ...
                    logger.warning(
                        f"Reconnect failed for '{self.name}', attempt {reconnect_count}"
                    )
    ...
    def _process_remaining_data(self) -> None:
        ...
        if processed > 0:
            logger.info(
                f"Processed {processed} remaining data points from '{self.name}'"
            )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "metrics": {
                ...
                "last_data_time": (
                    self.metrics.last_data_time.isoformat()
    ...
    async def health_check(self) -> bool:
        ...
        if self.metrics.last_data_time:
            time_since_last_data = (
                datetime.now() - self.metrics.last_data_time
            ).total_seconds()
