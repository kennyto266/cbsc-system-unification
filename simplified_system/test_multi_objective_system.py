...
class MultiObjectiveSystemTester:
        ...
        for test_case in test_cases:
            ...
            try:
                ...
                is_valid = (
                    ...
                    and objective.direction == test_case["expected_direction"]
                )

                if is_valid:
                    self.passed_tests += 1
                    logger.info(
                        f"  ✓ {test_name} - Value: {value:.6f}, Direction: {objective.direction}, Time: {calc_time:.4f}s"
                    )
    ...
    def _test_objective_factory(self, returns: pd.DataFrame):
        ...
        try:
            ...
            self.passed_tests += 1
            logger.info(
                f"  ✓ {test_name} - {len(objectives_to_test)} objectives created successfully"
            )
            self.test_results[test_name] = {
                "status": "PASS",
                "objectives_count": len(objectives_to_test),
            }
    ...
    def _test_pareto_frontier_calculation(self, returns: pd.DataFrame):
        ...
        try:
            # 准备目标函数
            objectives = [SharpeRatioObjective(), VarianceObjective()]

            # 计算帕累托边界
            calculator = ParetoFrontierCalculator()
            start_time = time.time()
            frontier = calculator.calculate_pareto_frontier(
                returns = returns, objectives = objectives, n_solutions = 50
            )
            calc_time = time.time() - start_time
            ...
            is_valid = (
                ...
                and len(frontier.points) > 0
                and frontier.n_objectives == len(objectives)
                and frontier.n_assets == returns.shape[1]
            )
            ...
            if is_valid:
                ...
                if has_pareto_optimal:
                    self.passed_tests += 1
                    logger.info(
                        f"  ✓ {test_name} - {len(frontier.points)} solutions, {len(first_front)} Pareto optimal, Time: {calc_time:.4f}s"
                    )
                    self.test_results[test_name] = {
                        "status": "PASS",
                    ...
                    self.test_results[test_name] = {
                        "status": "FAIL",
                        "error": "No Pareto optimal solutions",
                    }
            else:
        ...
        for algorithm in algorithms_to_test:
            ...
            try:
                ...
                config = MultiObjectiveConfig(
                    algorithm = algorithm, population_size = 50, n_generations = 50
                )
                ...
                is_valid = (
                    frontier is not None
                    and len(frontier.points) > 0
                    and frontier.algorithm == algorithm
                )

                if is_valid:
                    self.passed_tests += 1
                    logger.info(
                        f"  ✓ {test_name} - {len(frontier.points)} solutions, Time: {calc_time:.4f}s"
                    )
                    self.test_results[test_name] = {
                        "status": "PASS",
    ...
    def _test_preference_optimization(self, returns: pd.DataFrame):
        ...
        for method in preference_methods:
            ...
            try:
                ...
                is_valid = (
                    result is not None
                    and result.preferred_solution is not None
                    and result.preference_method == method
                )

                if is_valid:
                    self.passed_tests += 1
                    logger.info(
                        f"  ✓ {test_name} - Satisfaction: {result.satisfaction_level:.3f}, Time: {calc_time:.4f}s"
                    )
                    self.test_results[test_name] = {
                        "status": "PASS",
    ...
    def _test_robust_optimization(self, returns: pd.DataFrame):
        ...
        try:
            ...
            # 验证结果
            is_valid = (
                robust_frontier is not None
                and len(robust_frontier.points) > 0
                and robust_frontier.algorithm == "robust"
            )

            if is_valid:
                self.passed_tests += 1
                logger.info(
                    f"  ✓ {test_name} - {len(robust_frontier.points)} robust solutions, Time: {calc_time:.4f}s"
                )
                self.test_results[test_name] = {
                    "status": "PASS",
    ...
    def _test_system_integration(self, returns: pd.DataFrame):
        ...
        integration_tests = [
            {"name": "MPT Integration", "test_func": self._test_mpt_integration},
            {
        ...
        for test in integration_tests:
            ...
            try:
                ...
                if result:
                    self.passed_tests += 1
                    logger.info(
                        f"  ✓ {test_name} - Integration successful, Time: {calc_time:.4f}s"
                    )
    ...
    def _test_mpt_integration(self, returns: pd.DataFrame) -> bool:
        ...
        try:
            ...
            if frontier.points:
                ...
                # 比较结果（都是夏普比率最大化）
                return (
                    len(weights) == len(mpt_result.weights)
                    and np.allclose(np.sum(weights), 1.0)
                    and np.allclose(np.sum(mpt_result.weights), 1.0)
                )
    ...
    def _test_risk_parity_integration(self, returns: pd.DataFrame) -> bool:
        ...
        try:
            ...
            if frontier.points:
                ...
                # 比较结果
                return (
                    len(weights) == len(rp_result.weights)
                    and np.allclose(np.sum(weights), 1.0)
                    and np.allclose(np.sum(rp_result.weights), 1.0)
                )
    ...
    def _test_real_data_optimization(self):
        ...
        try:
            ...
            optimizer = MultiObjectiveOptimizer(
                config = MultiObjectiveConfig(
                    algorithm="weighted_sum", population_size = 30, n_generations = 30
                )
            )

            start_time = time.time()
            frontier = optimizer.optimize_portfolio(
                returns = real_returns, objectives=["sharpe", "variance", "return"]
            )
            calc_time = time.time() - start_time
            ...
            is_valid = (
                frontier is not None
                and len(frontier.points) > 0
                and frontier.asset_names == list(real_returns.columns)
            )
            ...
            if is_valid:
                ...
                weights_dict = {
                    ...
                    for asset, weight in zip(
                        best_solution.asset_names, best_solution.weights
                    )
                }
                ...
                logger.info(
                    f"    Best weights: {dict(sorted(weights_dict.items(), key = lambda x: x[1], reverse = True))}"
                )
    ...
    def _output_test_results(self):
        ...
        if self.passed_tests == self.total_tests:
            logger.info(
                "🎉 ALL TESTS PASSED! Multi - Objective Optimization System is ready for production."
            )
