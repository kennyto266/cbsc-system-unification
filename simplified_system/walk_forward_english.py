...
class WalkForwardAnalyzer:
    ...
    def load_0700_data(self) -> pd.DataFrame:
        ...
        try:
            ...
            if len(data) < 1000:
                print(
                    f"Insufficient data points ({len(data)}), generating extended data..."
                )
    ...
    def run_walk_forward_analysis(
        ...
        for i, config in enumerate(strategy_configs):
            ...
            while (
                ...
                print(
                    f"  Window {window_count + 1}: Training {train_data.index[0].date()} to {train_data.index[-1].date()}, "
                ...
                if result.trades_count >= self.config.min_trades:
                    ...
                    print(
                        f"    OK Return: {result.total_return:.2%}, Sharpe: {result.sharpe_ratio:.2f}, Trades: {result.trades_count}"
                    )
                else:
                    print(
                        f"    FAIL Insufficient trades ({result.trades_count} < {self.config.min_trades})"
                    )
            ...
            if strategy_results:
                ...
                print(
                    f"{strategy_name} Overall Performance: Avg Sharpe {all_results[strategy_name]['summary']['mean_sharpe']:.2f}, "
    ...
    def generate_report(self, results: Dict) -> str:
        ...
        if high_risk_strategies:
            report += (
                f"High Drawdown Risk Strategies: {', '.join(high_risk_strategies)}\n"
            )
        ...
        if low_trade_strategies:
            report += (
                f"Low Trading Frequency Strategies: {', '.join(low_trade_strategies)}\n"
            )
        ...
        if results:
            ...
            report += (
                f"Expected Sharpe: {best_strategy[1]['summary']['mean_sharpe']:.2f}\n"
            )
