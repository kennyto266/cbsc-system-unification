    ...
    def professional_rsi_backtest(prices, period = 14, oversold = 30, overbought = 70):
        ...
        for i in range(1, len(prices)):
            if positions[i] == 1 and positions[i - 1] == 0:  # Entry
                ...
                positions[i] == 0 and positions[i - 1] == 1 and entry_price > 0
            ):  # Exit
    ...
    def run_professional_optimization():
            ...
            for i in range(0, len(test_params), chunk_size):
                ...
                for strategy_type, params in chunk:
                    if strategy_type == "RSI":
                        result = professional_rsi_backtest(
                            prices, params[0], params[1], params[2]
                        )
        ...
        if results:
            ...
            for i, result in enumerate(sorted_results[:10], 1):
                ...
                print(
                    f"    Sharpe: {result['sharpe']:.3f}, Return: {result['total_return']:.1%}"
                )
                print(
                    f"    Trades: {result['trades']}, Win Rate: {result['win_rate']:.1%}"
                )
