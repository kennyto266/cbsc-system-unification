...
class UnifiedDataArchitectureStandard:
    ...
    def standardize_data(
        self, data: Union[pd.DataFrame, Dict, List], source_type: DataSourceType
        ...
        try:
            ...
            if is_valid:
                print(
                    f"SUCCESS Data standardization: {len(df_standardized)} records, quality: {self.standards[source_type].quality_level.value}"
                )
            ...
            self._log_error(
                f"Data standardization error: {str(e)}", "standardization_error"
            )
            return None
    ...
    def _convert_to_dataframe(
        self, data: Union[pd.DataFrame, Dict, List], input_format: str
    ) -> Optional[pd.DataFrame]:
        """将各种格式转换为DataFrame"""
        try:
            ...
            if standard_col in df_standardized.columns:
                try:
                    if mapping.data_type == "datetime":
                        df_standardized[standard_col] = pd.to_datetime(
                            df_standardized[standard_col]
                        )
                        ...
                        df_standardized[standard_col] = pd.to_numeric(
                            df_standardized[standard_col], errors="coerce"
                        )
                        ...
                        df_standardized[standard_col] = df_standardized[
                            standard_col
                        ].astype(str)
                except Exception as e:
                    print(
                        f"WARNING Data type conversion failed {standard_col}: {str(e)}"
                    )

        # 排序数据
        if (
            df_standardized.index.name == "timestamp"
            or "timestamp" in df_standardized.columns
        ):
    ...
    def _validate_data(
        ...
        # 检查必需列
        missing_cols = [
            col for col in standard.required_columns if col not in df.columns
        ]
        if missing_cols:
    ...
    def get_quality_report(self) -> Dict:
        ...
        return {
            ...
            "error_types": pd.Series([e["error_type"] for e in self.validation_errors])
            .value_counts()
            .to_dict(),
...
def main():
    ...
    test_cases = [
        ...
        {
            "data": {
                "close": {"2025 - 01 - 01": 400.0, "2025 - 01 - 02": 410.0},
                "volume": {"2025 - 01 - 01": 1000000, "2025 - 01 - 02": 1200000},
            }
        },
        # 平面字典格式
        {"2025 - 01 - 01": 400.0, "2025 - 01 - 02": 410.0},
        # 列表格式
        [
    ...
    for i, test_data in enumerate(test_cases):
        ...
        if result is not None:
            print(
                f"SUCCESS Standardization: {len(result)} rows, {len(result.columns)} columns"
            )
