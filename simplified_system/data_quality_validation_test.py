...
class DataQualityValidator:
    ...
    def validate_data_file(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"File validation completed: {overall_score:.3f} score, status: {result['status']}"
            )
            return result
    ...
    def _validate_structure(self, data: Dict) -> Dict[str, Any]:
        ...
        if "record_count" in data and "data" in data:
            checks["record_count_match"] = data["record_count"] == len(
                data.get("data", [])
            )
    ...
    def _validate_data_quality(self, data: Dict) -> Dict[str, Any]:
        ...
        quality_metrics["record_count_adequate"] = (
            record_count >= self.quality_standards["min_record_count"]
        )
        ...
        if records:
            ...
            for field in records[0].keys():
                missing_count = sum(
                    1 for record in records if record.get(field) is None
                )
            ...
            quality_metrics["max_missing_rate"] = (
                max(field_missing_rates.values()) if field_missing_rates else 0
            )
            quality_metrics["missing_rate_acceptable"] = (
                quality_metrics["max_missing_rate"]
                <= self.quality_standards["max_missing_rate"]
            )

            # 數據完整性
            total_fields = len(records[0])
            total_values = sum(
                sum(1 for field in record.values() if field is not None)
                for record in records
            )
            completeness = total_values / (record_count * total_fields)
            quality_metrics["data_completeness"] = completeness
            quality_metrics["completeness_acceptable"] = (
                completeness >= self.quality_standards["min_data_completeness"]
            )
        ...
        quality_score = (
            sum(
                ...
                if key.endswith("_adequate") or key.endswith("_acceptable")
            )
            / 4
        )
    ...
    def _validate_authenticity(self, data: Dict, file_path: str) -> Dict[str, Any]:
        ...
        if "source" in data:
            ...
            authenticity_checks["source_recognized"] = source in [
                "HKMA",
                "HK Government",
                "Official Data",
            ]
        ...
        if "collection_time" in data:
            try:
                collection_time = datetime.fromisoformat(
                    data["collection_time"].replace("Z", "+00:00")
                )
        ...
        if "data" in data and data["data"]:
            content_authenticity = self._validate_content_authenticity(
                data["data"], data.get("source", "")
            )
        ...
        authenticity_score = (
            sum(
                1
                for key, value in authenticity_checks.items()
                if key.endswith("_valid")
                or key.endswith("_recognized")
                or key.endswith("_fresh")
            )
            / 4
        )
    ...
    def _validate_content_authenticity(
        ...
        if source == "HKMA":
            ...
            # 檢查邏輯一致性（利率期限結構）
            if all(
                key in sample_record for key in ["hibor_overnight", "ir_1m", "ir_3m"]
            ):
    ...
    def _validate_temporal_consistency(self, data: Dict) -> Dict[str, Any]:
        ...
        if len(dates) > 1:
            # 檢查日期是否按時間順序排列
            temporal_checks["date_sequence_ordered"] = dates == sorted(
                dates, reverse = True
            )
            ...
            if intervals:
                ...
                temporal_checks["interval_reasonable"] = (
                    0 <= avg_interval <= 365
                )  # 合理間隔
        ...
        temporal_score = (
            sum(
                ...
                or key.endswith("_reasonable")
                or key.endswith("_adequate")
            )
            / 3
        )
    ...
    def run_comprehensive_validation(self, data_directory: str) -> Dict[str, Any]:
        ...
        if not json_files:
            return {"error": "No JSON files found", "directory": data_directory}
        ...
        # 計算平均分
        valid_scores = [
            r["overall_score"] for r in validation_results if "overall_score" in r
        ]
        if valid_scores:
        ...
        overall_status = (
            "EXCELLENT"
            if summary_stats["average_score"] >= 0.9
            else (
                "GOOD"
                if summary_stats["average_score"] >= 0.8
                else "ACCEPTABLE" if summary_stats["average_score"] >= 0.7 else "POOR"
            )
        )
        ...
        return {
            ...
            "recommendations": self._generate_recommendations(
                summary_stats, validation_results
            ),
        }

    def _generate_recommendations(
        self, summary_stats: Dict, results: List[Dict]
    ) -> List[str]:
        """生成改進建議"""
        recommendations = []
        ...
        if summary_stats["error_files"] > 0:
            recommendations.append(
                f"有{summary_stats['error_files']}個文件出現錯誤，需要檢查文件格式"
            )

        if summary_stats["failed_files"] > 0:
            recommendations.append(
                f"有{summary_stats['failed_files']}個文件未通過驗證，建議檢查數據完整性"
            )
        ...
        structure_issues = sum(
            1
            for r in results
            if "structure_validation" in r and not r["structure_validation"]["passed"]
        )
        if structure_issues > 0:
            recommendations.append(
                f"有{structure_issues}個文件存在結構問題，需要檢查必需字段"
            )

        quality_issues = sum(
            1
            for r in results
            if "quality_validation" in r and not r["quality_validation"]["passed"]
        )
        if quality_issues > 0:
            recommendations.append(
                f"有{quality_issues}個文件存在質量問題，需要處理缺失值和異常值"
            )

        authenticity_issues = sum(
            1
            for r in results
            if "authenticity_validation" in r
            and not r["authenticity_validation"]["passed"]
        )
        if authenticity_issues > 0:
            recommendations.append(
                f"有{authenticity_issues}個文件存在真實性問題，需要驗證數據來源"
            )
...
def main():
    ...
    for i, result in enumerate(government_result["detailed_results"][:5], 1):
        ...
        print(
            f"   {i}. {file_name}: {result['status']} (Score: {result.get('overall_score', 0):.3f})"
        )
