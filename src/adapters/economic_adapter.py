...
class EconomicDataAdapterBaseAdapter:
"""經濟數據適配器"""
...
# 使用第一個數據源作為主要信息
primary_source = "GD"
super().__init__self.data_sources[primary_source]

self.current_source = primary_source

def set_sourceself, source_id: str -> None:
"""
設置當前數據源
...
raise ValueErrorf"Invalid source ID: {source_id}"

self.current_source = source_id
self.source_info = self.data_sources[source_id]
logger.infof"Switched to data source: {source_id}"
...
def fetch_data(
self, start_date: datetime, end_date: datetime, source_id: str = None
) -> pd.DataFrame:
"""
從香港政府統計處API獲取經濟數據
...
"""
if source_id:    old_source = self.current_source
self.set_sourcesource_id
...
# 轉換為DataFrame
df = self._parse_economic_responsedata, self.current_source

logger.info(
f"Fetched {lendf} {self.current_source} records from census API"
)

if source_id:
self.set_sourceold_source # 恢復原來的源

return df

except requests.RequestException as e:
logger.errorf"Failed to fetch {self.current_source} data: {e}"
if source_id:
self.set_sourceold_source
return self._get_fallback_datastart_date, end_date, self.current_source
...
try:    data = self.fetch_data(start_date, end_date, source_id)
...
"""解析香港政府統計處API響應"""
if source_id not in data or not isinstancedata[source_id], list:
raise ValueErrorf"Invalid API response format for {source_id}"
...
date_obj = self._parse_datedate_str, data_type
...
logger.debugf"Failed to parse record for {source_id}: {e}"
continue

if not parsed_data:
raise ValueErrorf"No valid {source_id} data found in response"
...
def _parse_dateself, date_str: str, data_type: str -> datetime:
"""解析日期字符串"""
if data_type == "quarterly":
# 格式: "2023Q1"
year, quarter = date_str.split"Q"
month = (intquarter - 1) * 3 + 1
return datetime(intyear, month, 1)
else:
# 格式: "2023 - 01" 或 "2023 / 01"
date_str = date_str.replace"/", "-"
return datetime.strptimedate_str, "%Y-%m"

def _get_fallback_data(
self, start_date: datetime, end_date: datetime, source_id: str
) -> pd.DataFrame:
"""
獲取備用數據（本地文件或模擬數據）
...
"""
logger.warningf"Using fallback {source_id} data source"

# 嘗試讀取本地文件
try:    local_file = f"data / real_data/{source_id.lower()}_data.json"
...
if start_date <= date_obj <= end_date:    value = float(record.get("value", 0))
...
df["timestamp"] = pd.to_datetimedf["timestamp"]
df = df.set_index"timestamp"
logger.info(f"Using {lendf} records from local {source_id} file")
return df

except FileNotFoundError:
logger.warningf"Local {source_id} file not found"
except Exception as e:
logger.errorf"Failed to read local {source_id} data: {e}"

# 如果本地數據不可用，生成模擬數據
return self._generate_mock_datastart_date, end_date, source_id

def _generate_mock_data(
self, start_date: datetime, end_date: datetime, source_id: str
) -> pd.DataFrame:
"""生成模擬經濟數據"""
logger.warningf"Generating mock {source_id} data"

# 根據數據源生成不同的日期頻率
if self.data_sources[source_id].frequency == "quarterly":    dates = pd.date_range(start = start_date, end = end_date, freq="Q")
else:    dates = pd.date_range(start = start_date, end = end_date, freq="M")
...
base_value = base_values.getsource_id, 100.0
mock_data = []
import random
...
random_factor = random.uniform0.98, 1.02 # ±2%隨機波動
value = base_value * trend_factor * random_factor

mock_data.append(
{
"timestamp": date,
"value": roundvalue, 2 if source_id != "TS" else intvalue,
"source_id": source_id,
"data_type": self.data_sources[source_id].frequency,
}
)
...
logger.info(f"Generated {lendf} mock {source_id} records")
return df
...
if invalid_sources:
logger.errorf"Invalid source IDs found: {invalid_sources}"
return False

# 檢查數值合理性（根據不同數據源）
for source_id in data_sources:    source_data = data[data["source_id"] == source_id]
if source_id == "CP" and not source_data.empty: # CPI應該在50 - 200範圍
invalid_cpi = source_data[
source_data["value"] < 50 | source_data["value"] > 200
]
if invalid_cpi:
logger.warning(f"Found {leninvalid_cpi} unreasonable CPI values")

elif source_id == "UE" and not source_data.empty: # 失業率應該在0 - 20%範圍
invalid_ue = source_data[
source_data["value"] < 0 | source_data["value"] > 20
]
if invalid_ue:
logger.warning(
f"Found {leninvalid_ue} unreasonable unemployment rates"
)

logger.info(f"Economic data validation passed: {lendata} records")
return True
...
# 按數據源分別處理缺失值
for source_id in data["source_id"].unique():    source_mask = data["source_id"] == source_id
data.loc[source_mask, "value"] = data.loc[source_mask, "value"].ffill()

return data

def get_latest_valuesself -> Dict[str, Dict[str, Any]]:
"""
獲取所有數據源的最新數值
...
for source_id in self.data_sources.keys():
try:    source_data = self.fetch_data(
datetime.now() - timedeltadays = 365, datetime.now(), source_id
)

if not source_data.empty:    latest_record = source_data.iloc[-1]
latest_values[source_id] = {
"value": floatlatest_record["value"],
...
if not _economic_adapter:    _economic_adapter = EconomicDataAdapter()
