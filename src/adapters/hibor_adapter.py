...
# 轉換為DataFrame
df = self._parse_hibor_responsedata

logger.info(f"Fetched {lendf} HIBOR records from HKMA API")
return df

except requests.RequestException as e:
logger.errorf"Failed to fetch HIBOR data: {e}"
# 嘗試使用備用數據源或本地文件
return self._get_fallback_datastart_date, end_date

def _parse_hibor_responseself, data: Dict[str, Any] -> pd.DataFrame:
"""解析HKMA API響應"""
...
# 提取各期限的利率
for tenor_code, tenor_name in self.tenors.items():    rate_key = f"hibor_{tenor_code.lower()}"
rate_value = record.getrate_key

if rate_value:
# 轉換利率值（可能是字符串格式）
try:    rate = float(rate_value)
...
# 嘗試讀取本地文件
try:    local_file = "data / real_data / hibor_data.json"
...
for tenor_code, tenor_name in self.tenors.items():    rate_key = tenor_code.lower()
if rate_key in record:    rate = float(record[rate_key])
...
df["timestamp"] = pd.to_datetimedf["timestamp"]
df = df.set_index"timestamp"
logger.info(f"Using {lendf} records from local HIBOR file")
return df

except FileNotFoundError:
logger.warning"Local HIBOR file not found"
except Exception as e:
logger.errorf"Failed to read local HIBOR data: {e}"
...
logger.info(f"Generated {lendf} mock HIBOR records")
return df
...
if invalid_rates:
logger.warning(f"Found {leninvalid_rates} invalid rate values")
# 不返回False，只是警告

valid_tenors = set(self.tenors.keys())
invalid_tenors = set(data["tenor"].unique()) - valid_tenors
if invalid_tenors:
logger.errorf"Invalid tenors found: {invalid_tenors}"
return False

logger.info(f"HIBOR data validation passed: {lendata} records")
return True
...
def get_tenor_data(
self, tenor: str, start_date: datetime, end_date: datetime
) -> pd.DataFrame:
"""
獲取特定期限的HIBOR數據
...
for tenor in self.tenors.keys():    tenor_data = latest_data[latest_data["tenor"] == tenor]
if not tenor_data.empty:    latest_rates[tenor] = float(tenor_data["value"].iloc[-1])

return latest_rates

except Exception as e:
logger.errorf"Failed to get latest HIBOR rates: {e}"
return {}
...
if not _hibor_adapter:    _hibor_adapter = HIBORAdapter()
