"""
Tests for Strategy Category Service
測試策略分類管理服務
"""
...
class TestStrategyCategoryService:
    """測試策略分類服務"""
    ...
    async def test_get_category_tree(self, strategy_service, mock_db):
        ...
        # 執行測試
        with pytest.raises(NotImplementedError):
            await strategy_service.get_category_tree()
    ...
    async def test_update_category_success(self, strategy_service, mock_db, mock_user):
        ...
        # 執行測試
        with pytest.raises(NotImplementedError):
            await strategy_service.update_category(category_id, update_data, mock_user.id)
    ...
    async def test_delete_category_success(self, strategy_service, mock_db, mock_user):
        ...
        mock_db.get.return_value = existing_category
        mock_db.execute.return_value.scalar.return_value = 0  # 沒有策略使用此分類
        mock_db.execute.return_value.scalar.side_effect = [0, 0]  # 策略數和子分類數都為0

        # 執行測試
        with pytest.raises(NotImplementedError):
            await strategy_service.delete_category(category_id, mock_user.id)
    ...
    async def test_get_categories_with_pagination(self, strategy_service, mock_db):
        ...
        # 執行測試
        with pytest.raises(NotImplementedError):
            await strategy_service.get_categories(page=1, page_size=10)
    ...
    async def test_batch_update_categories(self, strategy_service, mock_db, mock_user):
        ...
        # 設置mock返回值
        categories = [
            StrategyCategory(id=category_ids[0], name="分類1"),
            StrategyCategory(id=category_ids[1], name="分類2"),
            StrategyCategory(id=category_ids[2], name="分類3")
        ]
        ...
        # 執行測試
        with pytest.raises(NotImplementedError):
            await strategy_service.batch_update_categories(category_ids, updates, mock_user.id)
    ...
    async def test_move_category(self, strategy_service, mock_db, mock_user):
        ...
        # 執行測試
        with pytest.raises(NotImplementedError):
            await strategy_service.move_category(category_id, new_parent_id, mock_user.id)
    ...
    async def test_search_categories(self, strategy_service, mock_db):
        ...
        # 執行測試
        with pytest.raises(NotImplementedError):
            await strategy_service.search_categories(query)
    ...
    async def test_get_category_statistics(self, strategy_service, mock_db):
        ...
        # 執行測試
        with pytest.raises(NotImplementedError):
            await strategy_service.get_category_statistics(category_id)
    ...
    async def test_export_categories(self, strategy_service, mock_db):
        ...
        # 執行測試
        with pytest.raises(NotImplementedError):
            await strategy_service.export_categories(format="json")
    ...
    async def test_import_categories(self, strategy_service, mock_db, mock_user):
        ...
        # 執行測試
        with pytest.raises(NotImplementedError):
            await strategy_service.import_categories(import_data, mock_user.id)