...
class BaseMomentumStrategy(BaseStrategy, ABC):
    """
    ...
    # 支持的時間週期
    SUPPORTED_TIMEFRAMES: List[str] = ["5m", "15m", "30m", "1h", "4h", "1d"]
        ...
        if "close" in df.columns:
            ...
            for period in [5, 10, 20]:
                if period <= len(df):
                    ...
                    df[f"rate_of_change_{period}"] = ((df["close"] - df["close"].shift(period)) / df["close"].shift(period)) * 100
    ...
    def _calculate_trend_strength(self, df: pd.DataFrame) -> Dict[str, float]:
        ...
        try:
            ...
            trend_slope = slope / close_prices[0] * 100  # 正規化
            ...
            # 3. 價格動量
            momentum_20 = (close_prices[-1] - close_prices[-20]) / close_prices[-20] * 100 if len(close_prices) >= 20 else 0

            # 綜合趨勢強度
            strength = (abs(trend_slope) + adx_value / 50 + abs(momentum_20) / 10) / 3
            strength = min(strength, 1.0)  # 限制在0-1之間

            # 趨勢方向
            direction = "bullish" if trend_slope > 0.1 else "bearish" if trend_slope < -0.1 else "neutral"
            ...
            return {
                ...
                "adx": adx_value if "adx" in df.columns else 25
            }
    ...
    def _apply_risk_management(self, signals: pd.DataFrame) -> pd.DataFrame:
        ...
        # 應用趨勢強度過濾
        if "trend_strength" in df.columns and risk_rules["min_trend_strength"]:
            df = df[df["trend_strength"] >= risk_rules["min_trend_strength"]]
        ...
        if "signal" in df.columns and risk_rules["max_positions"]:
            ...
            if position_count > risk_rules["max_positions"]:
                # 保留強度最高的信號
                df = df.nlargest(risk_rules["max_positions"], "trend_strength")
    ...
    def _apply_stop_loss_take_profit(self, df: pd.DataFrame, risk_rules: Dict[str, Any]) -> pd.DataFrame:
        ...
        if "signal" in df.columns and risk_rules["use_take_profit"]:
            profit_target = risk_rules.get("profit_target", 0.15)
    ...
    def _update_strategy_state(self, results: Dict[str, Any]) -> None:
        ...
        # 計算平均趨勢強度
        trend_strengths = [r.get("trend_strength", {}).get("strength", 0) for r in results.values()]
    ...
    def _calculate_performance_metrics(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        trend_strengths = [r.get("trend_strength", {}).get("strength", 0) for r in results.values()]
        if trend_strengths:
            metrics["avg_trend_strength"] = np.mean(trend_strengths)
        ...
        for symbol, df in data.items():
            ...
            try:
                ...
                backtest_results[symbol] = {
                    ...
                    "trend_analysis": result.get("results", {}).get(symbol, {}).get("trend_strength", {})
                }
            except Exception as e:
